...
- Scaling (Cloud): Parallelize backend dashboard data fetching, compute token burn rate dynamically, upgrade built-in agent harness, and ensure PostgreSQL transaction safety.
- Privacy/Offline (Standalone): Implement PII redaction for MCP local tools, fix login routing, and link hybrid FS MCP architecture walkthrough.
