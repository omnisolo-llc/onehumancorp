1. **Create New Walkthrough Doc**:
   - Use `run_in_bash_session` to write out the full file content including glassmorphism div and Mermaid graph:
     ```bash
     cat << 'EOF2' > docs/walkthroughs/agent_harness_os_sandbox.md
...
2. **Update Help Portal Index**:
   - Use `replace_with_git_merge_diff` to add a link to `[Agent Harness OS Sandboxing Walkthrough](agent_harness_os_sandbox.md)` in `docs/walkthroughs/help_portal.md` under the "Deep Dive Walkthroughs" section.
     ```diff
     <<<<<<< SEARCH
     - **[Sandbox Isolation Harness](sandbox_isolation_harness.md)**: Visual guide to the OS-level sandbox isolation.
     =======
     - **[Sandbox Isolation Harness](sandbox_isolation_harness.md)**: Visual guide to the OS-level sandbox isolation.
     - **[Agent Harness OS Sandboxing Walkthrough](agent_harness_os_sandbox.md)**: Visual guide to the Hybrid Agent Harness process isolation, MCP, and memory directories.
     >>>>>>> REPLACE
     ```

3. **Verify Markdown File Creation**:
   - Use `run_in_bash_session` to run `cat docs/walkthroughs/agent_harness_os_sandbox.md` and `cat docs/walkthroughs/help_portal.md` to ensure the markdown is properly formatted.

4. **Test Documentation Build & Tests**:
   - Run `bazelisk run //:docs_build` to ensure the MkDocs site builds properly.
...
6. **Submit the PR**:
   - Submit the PR with the title `✍️ Scribe: [Agent Harness OS-Level Sandboxing Walkthrough]`.
   - Provide a complete PR description detailing What, Why, and Impact. Include `issue_id: <ID>` if available.
