...
4. **Implement Interfaces, Telemetry, and DB Implementation**:
...
FILE_EOF
   ```
   Modify `srcs/server/telemetry/telemetry.go` to add OpenTelemetry counters for `rag_records_synced_total` and `rag_sync_errors_total` exactly as specified in the instructions. Verify with `cat srcs/server/hub/rag_sync.go`.
