...
This document details the complete end-to-end user journey for each of the core personas in the OmniSolo (OmniSolo) platform. It covers their acquisition, onboarding, activation, retention, revenue upgrade triggers, and referral mechanisms, aiming to identify and mitigate friction points.
...
- **Onboarding:** Maya opens the OmniSolo mobile app. The wizard asks: "What do you sell?" (Cakes). "What's your Instagram?" (@mayascakes). OmniSolo imports 5 recent cake photos, creates a Glassmorphism-style catalog, and generates her site.
...
sequenceDiagram
    ...
    participant OmniSolo Backend
    participant Instagram

    Maya->>Instagram: Clicks OmniSolo Ad
    Instagram-->>Maya: Opens OmniSolo Web App
    Maya->>OmniSolo Mobile App: Enters "Cakes" & IG handle
    OmniSolo Mobile App->>AI Agent (Promoter): Extract photos & design site
    AI Agent (Promoter)-->>OmniSolo Mobile App: Show preview (30s)
    Maya->>OmniSolo Mobile App: Approve & Publish
    ...
    Maya->>OmniSolo Mobile App: 1-Tap Approve
    AI Agent (Customer Success)->>Instagram: Sends reply
    Instagram->>OmniSolo Backend: Customer places order
    OmniSolo Backend-->>Maya: Push Notification "New Custom Order!"
```
...
- **Acquisition:** Carlos hears about OmniSolo from another tradesperson at Home Depot. He searches Google for "easy booking app for handymen" and finds OmniSolo.
- **Onboarding:** Carlos enters "Handyman Services". The wizard asks for his base hourly rate and 3 common jobs (Plumbing, Painting, Repairs). OmniSolo generates a service menu and calendar view.
...
- **Retention:** Carlos uses the OmniSolo calendar as his primary daily schedule. The AI "Salesperson" agent drafts quotes based on customer problem descriptions, waiting in his inbox for approval.
...
- **Referral:** Carlos recommends OmniSolo to his plumber friend when discussing how he eliminated no-shows.

#### Carlos's Customer Journey
```mermaid
sequenceDiagram
    actor Carlos
    participant OmniSolo Android App
    participant AI Agent (Salesperson)
    participant Client
    participant Stripe

    Carlos->>OmniSolo Android App: Setup services & prices
    OmniSolo Android App-->>Carlos: Generates booking link
    Carlos->>Client: SMS link to book
    Client->>OmniSolo Android App: Selects date, describes issue
    Client->>Stripe: Pays deposit
    Stripe-->>OmniSolo Android App: Payment confirmed
    OmniSolo Android App-->>Carlos: Notification "New Booking"
    OmniSolo Android App->>AI Agent (Salesperson): Analyze issue description
    AI Agent (Salesperson)-->>Carlos: Drafts formal quote
    Carlos->>OmniSolo Android App: Approves quote
    OmniSolo Android App->>Client: Sends final quote
```
...
- **Acquisition:** Priya is frustrated with Shopify's POS pricing. She reads a blog comparing Shopify vs OmniSolo.
...
sequenceDiagram
    actor Priya
    participant OmniSolo Dashboard (Web/Mobile)
    participant In-Store Customer
    participant Online Store
    participant AI Agent (Advisor)

    Priya->>OmniSolo Dashboard (Web/Mobile): Bulk import inventory
    In-Store Customer->>Priya: Buys item in person
    Priya->>OmniSolo Dashboard (Web/Mobile): Tap-to-Pay checkout
    OmniSolo Dashboard (Web/Mobile)-->>Online Store: Sync inventory (-1)
    Online Store->>OmniSolo Dashboard (Web/Mobile): Customer buys online
    OmniSolo Dashboard (Web/Mobile)-->>Priya: Notification "Ship order"
...
- **Acquisition:** Leo searches for "how to sell guitar lessons online" and finds an OmniSolo landing page targeted at educators.
...
- **Activation:** Leo posts a guitar cover on TikTok with his OmniSolo link. A student signs up for a trial lesson.
...
sequenceDiagram
    actor Leo
    participant OmniSolo App
    participant TikTok
    participant Student
    participant AI Agent (Salesperson)
    participant Zoom

    Leo->>OmniSolo App: Create subscription package & connect Zoom
    OmniSolo App-->>Leo: Link-in-bio generated
    Leo->>TikTok: Posts video + Link
    Student->>TikTok: Clicks link
    Student->>OmniSolo App: Buys subscription
    OmniSolo App->>Zoom: Generate recurring meeting link
    OmniSolo App-->>Student: Email with Zoom link
    Student->>OmniSolo App: Cancels a lesson
    OmniSolo App->>AI Agent (Salesperson): Student inactive
    AI Agent (Salesperson)-->>Leo: Drafts makeup class email
    Leo->>OmniSolo App: Approve email
```
**Friction Point:** Connecting external apps (Zoom, Google Calendar) involves OAuth flows that can drop users.
**Mitigation:** Native integration where OmniSolo just handles the video link directly, or providing clear, step-by-step guidance within the app without kicking them out to a browser.
...
sequenceDiagram
    ...
    Fatima->>OmniSolo Mobile App: Set language to Arabic, take dish photos
    OmniSolo Mobile App->>AI Agent (Promoter): Enhance photos, draft EN descriptions
    AI Agent (Promoter)-->>Fatima: Show dual-language menu
    Fatima->>OmniSolo Mobile App: Print QR code
    Customer->>Customer: Scans QR code
    Customer->>OmniSolo Mobile App (Web): Orders and pays
    OmniSolo Mobile App (Web)-->>Fatima: LOUD notification "New Order"
    Fatima->>OmniSolo Mobile App: 1-Tap toggle "Sold Out"
```
