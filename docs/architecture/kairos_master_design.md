...
3. **AutoDream (The Memory):** The long-term persistence layer. Ephemeral session logs and intermediate artifacts are compressed via existing LLM clients (`srcs/server/agents/local/llm.go`) and embedded into a `pgvector` index (specifically `swarm_memory` and `swarm_memory_embeddings`), granting the swarm exact semantic search capabilities.
...
graph TD
    ...
    subgraph KAIROS Orchestrator
        ...
        V[(pgvector swarm_memory_embeddings)]
    end
