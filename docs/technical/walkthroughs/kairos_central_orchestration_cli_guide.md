...
The KAIROS Orchestrator CLI (`ohc-cli`) is the central tool for orchestrating your swarm of agents in the OmniSolo (OHC) ecosystem. It provides a powerful command-line interface for interacting with the OHC Hybrid Architecture, allowing direct command-line interactions for initializing the hybrid architecture, delegating tasks, and observing the real-time teammate mesh.
...
*This command automatically negotiates with the [Central Orchestration Hub](../architecture/kairos/master-design-doc.md) and establishes local fallback capabilities (SQLite) via the [Hybrid OS architecture](../architecture/kairos/hybrid-os-design.md).*
