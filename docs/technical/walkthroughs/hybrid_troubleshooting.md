...
graph TD
    ...
    B -- Yes (SQLite) --> C[Check Local .omnisolo/runtime/status]
...
- **Database Lock Contention (SQLite)**: Ensure multiple background processes are not writing to `.omnisolo/runtime` directories simultaneously without respecting the file lock protocol.
