...
4. **Implement Factory:**
   ...
   func NewBlobProvider() BlobProvider {
       if os.Getenv("OMNISOLO_STANDALONE") == "true" {
...
6. **Write tests:**
   ...
   func TestFactory(t *testing.T) {
       os.Setenv("OMNISOLO_STANDALONE", "true")
       provider := NewBlobProvider()
       if _, ok := provider.(*LocalBlobProvider); !ok {
           t.Errorf("expected LocalBlobProvider")
       }

       os.Unsetenv("OMNISOLO_STANDALONE")
       os.Setenv("OMNISOLO_MULTITENANT", "true")
...
8. **Update mission status:**
   Command:
   ```bash
   sed -i '2s/status: PENDING/status: DONE/' .agent-task/missions/2026-04-05T17-03-50Z_hybrid_mcp_rag_market_audit.md
...
9. **Cleanup:**
   Command:
   ```bash
   rm -f test_plan.txt test_mcp_blob.go plan.md plan_v2.md plan_v3.md patch_build.py patch_test_build.py
   ```
...
12. Submit changes using the submit tool.
    Branch: `palette/hybrid-blob-mcp`
    Title: `🎨 Palette: [Hybrid MCP Blob Storage Proxy]`
