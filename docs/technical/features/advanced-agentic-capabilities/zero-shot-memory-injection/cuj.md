...
# CUJ: Zero Shot Memory Injection

**Persona:** Autonomous Agent / Human Manager
**Context:** Leveraging Zero Shot Memory Injection during standard operational workflows or cross-team collaboration.
...
When an agent is dynamically scheduled, the system provisions its memory space instantly by pulling pre-computed contextual embeddings from the semantic vector store, injecting them via LangChain interfaces before the first prompt token is processed.
...
| 3 | Core Processing | The workflow integrates Zero Shot Memory Injection securely | Operation completed | DB Check |
