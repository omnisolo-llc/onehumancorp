...
- **Setup:** A local Kubernetes cluster (via kind/minikube) with the `omnisolo-operator` running.
...
- **Assertion:** Verify the `omnisolo-operator` detects the pod crash, emits a `TaskFailed` event, and the Manager Agent receives the failure notification and updates its state accordingly.

## 3. Automation & CI/CD
- All unit and integration tests must be integrated into the Bazel `//...` test suite.
- Coverage MUST exceed 95% for `src/server/orchestration/delegation.go` and the associated `omnisolo-operator` reconciliation loops.
