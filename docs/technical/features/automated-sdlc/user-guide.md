...
- **Workflow Orchestration**: CI/CD pipelines are triggered autonomously by the Rust `Hub` observing LangGraph execution states. Test logs are fed directly back to SWE agents for automated debugging via their standard interaction context.

## Edge Cases
- **Bazel Sandboxing Issues**: If a test requires network access (e.g., DNS resolution for `dl.google.com`), strict Bazel sandboxing may cause timeouts. The pipeline automatically falls back to standard `cargo test` for diagnostic runs, alerting the DevOps agent.
