...
3. **Visual Excellence**: All panels must use OmniSolo premium CSS tokens. For text panels, inject global `<style>` blocks with:
...
Crucially, inject a Text panel with HTML mode enabled that adds the global OmniSolo Glassmorphism CSS styling to the dashboard body. Do not replace it with localized inline styles. Use this `<style>` block:
```html
<style>
  .grafana-app {
