...
# 🔬 OmniSolo Market Research Report: Claude Code Harness

## 1. Executive Summary
This report analyzes the leaked Claude Code execution environment to extract architectural insights for OmniSolo's Agentic OS. The analysis focused on their "Agent Harness" implementation, exploring how they execute tasks, manage memory, communicate with remote cloud services, and handle process isolation.
...
**Key takeaway for OmniSolo:** Strict separation between local execution and remote control flow. When a tool runs, the local harness emits a `PermissionRequest` to the cloud, putting user authorization physically outside the local agent's direct control.
...
## 3. Comparative Matrix: OmniSolo vs Claude Code

| Feature Area | Claude Code Harness | OmniSolo Hybrid Architecture | Gap Assessment |
|--------------|---------------------|--------------------------|----------------|
| **Execution** | Async Fork Subagents | Parallel Agent Pools | Enhance OmniSolo sub-agent context inheritance |
| **Control** | Bridge API (WS/HTTP) | OmniSolo-SIP (Redis/Postgres) | OmniSolo is more durable, Claude is more streaming-focused |
| **Memory** | Auto/Team file dirs | pgvector / Pinecone | Consider adding local `memdir` failovers for Standalone mode |
| **Isolation** | ID-tracked child processes | Docker / K8s pods | OmniSolo is safer cloud-side, but Claude's local tracker is excellent |

## 4. OmniSolo Actionable Missions

Based on this research, we need to introduce the following missions for the OmniSolo swarm. (Note: these have been created as corresponding GitHub Issues for the Implementer agents).

### 4.1 [backend] Implement Async Fork Subagents with Context Inheritance
*   **Problem Statement**: Currently, when OmniSolo agents spawn sub-agents, they don't efficiently inherit the parent's full system prompt and conversational context, leading to repetitive context-loading overhead.
*   **Design Doc**:
    *   Create a new `ForkAgent` service in the OmniSolo backend.
...
*   **Implementation Prompt**:
    1. Modify `deploy/scripts/omnisolo-mode.sh` or the local startup logic to ensure `.omnisolo/memory/` directories are created.
