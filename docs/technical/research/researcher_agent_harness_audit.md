...
Our current implementation in `srcs/server/bash_sandbox/sandbox.go` relies on `regexp.MustCompile` to block `sudo`, `rm -rf /`, etc. This is easily bypassed via encoding tricks or symlinks.
