...
## Research Report
Audit shows the metric exists in `srcs/server/telemetry/telemetry.go` but no dashboard panels currently query it. This prevents tracking whether task claims fail due to PostgreSQL row locks or SQLite database locks.
