...
# 🔬 OmniSolo Market Research Report: Claude Code Harness Architecture

## 1. Executive Summary
This report analyzes the leaked Claude Code (v2.1.88) execution environment to extract architectural insights for OmniSolo's Agentic OS. The analysis focused on their "Agent Harness" implementation, exploring how they execute tasks, manage memory, communicate with remote cloud services, handle process isolation, and abstract tool execution via MCP.
...
|--------------|---------------------|--------------------------|----------------|
| **Execution** | Async Fork Subagents (Shared Context) | Parallel Agent Pools | Enhance OmniSolo sub-agent context inheritance for cache hits |
| **Control** | Bridge API (WS/HTTP) & SdkTransport | OmniSolo-SIP (Redis/Postgres) | OmniSolo is more durable, Claude's in-process transport is elegant |
| **Memory** | Auto/Team file dirs (`memdir`) | pgvector / Pinecone | Consider adding local `memdir` failovers for Standalone mode |
| **Isolation** | SessionSpawner & AST Parsing | Host Default / Basic Pods | Critical gap: OmniSolo needs robust AST Bash parsing & session tracking |

## 4. OmniSolo Actionable Missions

Based on this research, we need to introduce the following missions for the OmniSolo swarm.

### 4.1 [backend] Implement Async Fork Subagents with Cache-Safe Context Inheritance
*   **Problem Statement**: Currently, when OmniSolo agents spawn sub-agents, they don't efficiently inherit the parent's full system prompt and conversational context, leading to repetitive context-loading overhead and prompt cache misses.
*   **Design Doc**:
