# 🔬 Research Brief: Claude Code Agent Harness Architecture & OmniSolo Integration
...
| Feature | Current OmniSolo | Market (Claude Code/Gstack) | Required OmniSolo Enhancement |
| :--- | :--- | :--- | :--- |
...
## Design Doc
To implement these protections in OmniSolo Hybrid Architecture (OmniSolo-HA), we will build the **OmniSolo Zero-Trust Harness**.

### Architecture Overview

```mermaid
flowchart TD
    A[Autonomous Agent] -->|Shell Command Request| B[OmniSolo Orchestrator API]
    B --> C[AST Security Validator]

    C -->|Parse Failure / Danger| D[OpenTelemetry Violation Log]
    D --> E[Grafana Dashboard]

    C -->|Valid AST| F[OmniSolo Sandbox Manager]
...
1. **OmniSolo Sandbox Runtime (Linux focus)**:
...
2. **OmniSolo Command AST Validator**:
...
3. **Telemetry Integration**:
    - Any command rejected by the AST Validator or blocked by Bwrap will emit a Prometheus metric `ohc_agent_harness_violation_total` and an OpenTelemetry trace with the raw command snippet, strictly following OmniSolo's Visual Excellence Mandate for Grafana dashboards.
...
**Task**: Implement the OmniSolo Zero-Trust Harness using Bubblewrap and AST Validation.
