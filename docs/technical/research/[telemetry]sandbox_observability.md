...
1. Enhance `srcs/server/telemetry/telemetry.go` with new OTel instruments for sandbox resource tracking.
2. Update `srcs/server/bash_sandbox/sandbox.go` to collect metrics during execution (using `syscall.Rusage`).
