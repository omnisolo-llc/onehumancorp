...
# Problem Statement
Competitors like Claude Code employ robust Agent Harness environments using native OS-level sandboxing (`bwrap`), explicit `allowRead` and `denyWrite` capabilities, and native MCP integration via `@anthropic-ai/sandbox-runtime`. OmniSolo currently lacks this granular isolation, exposing vulnerabilities, especially in our Standalone Mode, and relies on generic OS boundaries rather than strict, scoped policies.
...
### Competitive Analysis: OmniSolo vs Market

| Feature | OmniSolo (Current State) | Market Standard (Claude Code) | Gap Resolution |
| :--- | :--- | :--- | :--- |
...
| **State Sharing** | Centralized OmniSolo-SIP | File-based memory directory (`memdir.ts`) | Add local memory directory fallback for Standalone Mode |
