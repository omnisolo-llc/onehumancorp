...
## Research Report
- **Context**: The `srcs/server/telemetry/minimax_metrics.go` and `srcs/server/telemetry/telemetry.go` track token usage, but there is no dedicated dashboard for provider health.
