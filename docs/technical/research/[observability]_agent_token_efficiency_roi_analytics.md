...
## Research Report
Current telemetry in `srcs/server/telemetry/telemetry.go` tracks `ohc_token_usage_total` with labels for `agent_id`, `role`, `model`, and `type`. However, this data is decoupled from the **outcome** of the tasks.
...
3. **Code Changes**:
   ...
   - Update `srcs/server/orchestration/statemachine/machine.go`:
...
1. In `srcs/server/telemetry/telemetry.go`:
...
2. In `srcs/server/orchestration/statemachine/machine.go` (or wherever tasks are finalized):
...
4. Verify by running `bazelisk test //srcs/server/telemetry/...`.
