...
## Research Report
Market research indicates that typical agentic frameworks (like CrewAI, AutoGen) assume a monolithic backend for vector storage (like Pinecone or Qdrant) or rely entirely on local embeddings, making them inflexible for hybrid deployments. OmniSolo's "Unfair Advantage" requires an MCP Tool that abstracts the vector DB interaction, dynamically routing queries and embeddings based on the `OMNISOLO_MULTITENANT` configuration, without the agent needing to implement the underlying DB connections.
...
2. Implement driver-agnostic logic. To determine if the connection is Cloud, use: `os.Getenv(\"OMNISOLO_MULTITENANT\") == \"true\"`. For Cloud, implement the logic using `pgvector`. For Standalone, use `sqlite-vss`.
