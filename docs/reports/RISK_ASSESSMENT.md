...
* **Audit Finding:** `AcquireTask` logic in `srcs/server/db/postgres_provider.go` and `srcs/server/db/sqlite_provider.go` properly checks for lock contention logic ensuring 100% parity.
