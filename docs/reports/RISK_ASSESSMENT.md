...
|----------------------|------------|-----------|
| **Local SQLite SIPDB vs PostgreSQL Parity** | **High** | Standalone mode relies on `${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/` storage conventions and `sync_daemon.go`. Any discrepancy in database locking or task execution directly compromises the local-to-cloud resilience. |
| **Team Mesh (Pub/Sub) Migration** | **Medium** | Redis is optional for Standalone, but fallback runtime directories such as `${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/mailbox/` could introduce deadlocks if not gracefully handled during file lock contention (`.agent-lock/`). |
| **Telemetry & Observability Heartbeats** | **Low** | Agents write to `${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/status/`. If this directory is read-only, it fails safe without crashing core logic, but breaks observability. |
