...
- Before this evidence section was added, `rg -l -i 'chatwoot' --glob '!Cargo.lock' --glob '!MODULE.bazel.lock' . | sort` returned six classified paths: the residue guard, the native removal plan and omnichannel design, and exactly three marked historical research reports. No other active reference was present. `git diff --check` exited 0, and the only worktree entry was the unrelated untracked `docs/superpowers/plans/2026-07-13-authentication-hardening.md`.
- After adding this section, `git diff --check` and the residue guard again exited 0 with no output. The same global scan returned exactly seven allowed paths: this evidence report, the residue guard, the native removal plan and omnichannel design, and the same three marked historical reports.
