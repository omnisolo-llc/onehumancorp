...
| Memory worker boundaries | Added injectable, deadline-bound summarization; explicit system authority for cross-tenant acquisition/filesystem ingestion; and organization-scoped failure/final mutations | 5 Cargo worker tests and the Bazel workers target passed; real Postgres/RLS assertions remained skipped because `OMNISOLO_DATABASE_URL` was unset |
...
**Status (2026-07-13): Removed from the active application and deployment graph.** The repository owner confirmed in this thread on 2026-07-13 that there was no real-customer or production Chatwoot deployment or data. That owner statement was not independently verified against an external production system. On that owner-confirmed basis, no Chatwoot data migration was performed. The native OmniSolo omnichannel inbox remains in place; expanding its channel and inbox capabilities belongs to later native-chat projects rather than this removal.
...
- `cargo check -p omnisolo --locked` exited 0. Cargo emitted four existing unused-variable warnings for `db` in `src/server/api/staff_mesh.rs`; no warning was suppressed for this work.
...
(
  ...
  cp -a deploy/helm/omnisolo/. "$evidence_dir/chart/ohc/"
...
- `cargo check -p omnisolo --locked` exited 0 after 28.38 seconds. It emitted the same four unused-`db` warnings in `src/server/api/staff_mesh.rs`; no warning was suppressed.
...
| Built-in agent gRPC (`src/agents/builtin/service.rs`) | Token auth or a non-serializable trusted in-process extension | No request tenant; successful memory writes use process-global `OMNISOLO_ORGANIZATION_ID`, defaulting to `system` | Provider clients and each run attempt have deadlines; output channel is bounded, but producer ignores receiver closure | Instrumentation skips request values | Authentication improved; tenant attribution and cancellation fail requirements |
...
**Status (2026-07-12): Remediated.** Commits `7ad1f0fc4` and `aaf947781` require a non-system `OMNISOLO_ORGANIZATION_ID` for cloud/cluster agent startup, capture it as an immutable process capability, and use it for both semantic search and completion records. Standalone mode may explicitly use the local `system` tenant. The focused memory regression, all 517 built-in-agent tests, and `//src/agents/builtin:omnisolo_builtin_agent_lib_unit_test` pass.

After a successful task, the built-in service assigns memory to `OMNISOLO_ORGANIZATION_ID`, defaulting to `system`, rather than an authenticated request tenant (`src/agents/builtin/service.rs:1095`). The public task request carries no enforced tenant identity. A shared agent process can therefore misattribute or combine memories across callers.
...
|---|---|---|
| `OMNISOLO_REQUIRE_POSTGRES_TESTS=1 cargo test -p server_auth multitenancy_isolation:: -- --nocapture` with disposable pgvector PostgreSQL and admin/application-role URLs | 6 passed, 0 failed in 5.13s | **Verified:** all six bodies began as `ohc_security_test` with `rolsuper=false`, `rolinherit=false`, `rolbypassrls=false`, explicit bypass-role membership, and `row_security=on`; system context explicitly switched roles |
| `cargo test -p omnisolo --lib agent_memory_pipeline -- --nocapture` | 3 passed | SQLite and timeout behavior covered; real Postgres isolation remains environment-dependent |
| `cargo test -p omnisolo --lib services::agent -- --nocapture` | 7 passed | Includes the cross-organization agent/resource/mutation isolation regression |
| `cargo test -p omnisolo --lib orchestration::queue -- --nocapture` | 17 passed | SQLite behavior covered; Postgres/RLS claims require a configured database to be considered verified |
| `cargo test -p omnisolo_builtin_agent service -- --nocapture` | 9 passed | Current filtered service suite includes verified-mTLS identity, captured-tenant memory, and receiver-drop cancellation regressions |
| `cargo test -p omnisolo_builtin_agent_tools --lib` | 157 passed | Tenant-aware tool schemas no longer expose tenant selection; tools regressions remain green |
| `cargo test -p omnisolo_builtin_agent --lib` | 520 passed | Process tenant, captured-memory, cancellation, and emitted-log redaction regressions pass with the full agent suite |
| `bazel test //src/agents/builtin:omnisolo_builtin_agent_lib_unit_test` | 1 target passed | Bazel build/test graph includes and validates the tenant-capability changes |
| `cargo test -p omnisolo --lib agent_memory_pipeline` | 5 passed | Deterministic summary deadline and scoped SQL shape pass; the Postgres-named test skipped its database body because `OMNISOLO_DATABASE_URL` was unset |
...
```text
CARGO_TARGET_DIR=/tmp/agent-benchmark-e6fa-target cargo build --release -p omnisolo_builtin_agent --example agent_path_baseline
taskset -c 0 /tmp/agent-benchmark-e6fa-target/release/examples/agent_path_baseline
CARGO_TARGET_DIR=/tmp/agent-benchmark-56766-target cargo build --release -p omnisolo_builtin_agent --example agent_path_baseline
...
- Exact removed application/deployment surfaces: `src/server/integrations/chatwoot/`, `deploy/helm/omnisolo/templates/chatwoot.yaml`, `deploy/helm/omnisolo/templates/chatwoot-service.yaml`, docker-compose services, and prometheus configurations.
- Exact commands, exit results, and test counts from Steps 1–2:
  ...
  - `helm lint deploy/helm/omnisolo` passed
  - `helm template omnisolo deploy/helm/omnisolo` passed
  - `bazel query //...` passed
