...
- `pnpm exec vitest run`: 223 files and 891 tests passed, zero failures. Voice regressions cover normal and unmount cleanup, late and overlapping media acquisition, exact-once track release, callback/send suppression, pointer and Enter/Space hold behavior, repeat suppression, assistive synthetic-click toggling, dynamic pressed/label state, and polite/assertive live announcements.
