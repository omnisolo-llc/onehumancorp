...
**Design Doc**: A "Text Alerts" settings panel where owners can toggle automatic SMS reminders for appointments or order shipments. OHC formats short, concise messages and dispatches them via the SMS API (e.g., Twilio). For campaigns, a broadcast tool similar to the email feature, but restricted by character limits and strictly enforcing opt-out tracking.
