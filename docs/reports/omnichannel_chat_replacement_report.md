...
issue_description: |
  ...
  **Findings & Competitive Analysis:**
  - **Source Code Audit**: A clone and audit of legacy open-source omnichannel platforms reveals a robust architecture that we must adapt to Rust. Key components to replicate:
    - Multi-tenant data models: `Account` (Tenant), `Inbox`, `Channel`, `Conversation`, `Message`, `Contact`.
    - Real-time WebSocket infrastructure for live updates (we need a native Rust solution like `tokio` + `axum-ws` or `tungstenite`).
  ...
  **CUJ & Acceptance Criteria**:
  ...
  7. **Crucial**: The entire backend implementation must be in Rust, replacing the conceptual need for any external messaging provider.
