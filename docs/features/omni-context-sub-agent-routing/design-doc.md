...
The feature hooks into the `DelegateMission` function of the orchestration layer (`srcs/server/orchestration/sip.go`).
...
*   **SIPDB Modification:** The `SIPDB` struct in `srcs/server/orchestration/sip.go` requires a `ContextRoot` string field to identify where to search for grounding files.
