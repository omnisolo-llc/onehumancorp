...
## 1. Testing Strategy
Validate the end-to-end functionality, security boundaries, and performance constraints of the Just In Time Tool Synthesis feature using hermetic, table-driven tests. Ensure we use the Database Seeder pattern to establish deterministic starting states.
...
- **Action:** Prompt an agent to calculate a complex cryptographic hash. Verify it writes a custom script, registers it, executes it successfully, and the temporary tool is purged after the session.
