...
- **Architecture:**
  ...
  - A distributed State Machine (PENDING -> IN_PROGRESS -> COMPLETED/FAILED/BLOCKED) leveraging `SELECT ... FOR UPDATE` (or SQLite equivalent) to safely handle concurrent task claiming.
...
- **Architecture:**
  ...
  - Mocked runtime-memory logging under `${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/memory/mesh_mock.log` for Standalone Mode.
