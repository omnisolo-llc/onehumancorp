...
# Research Report
- **Tool:** Twilio API for SMS.
- **Target Persona:** Fatima (Food Cart Operator)
- **Advantages:** Invisible to the user. They just toggle "Send SMS reminders" in their settings. High open rate and immediate notification.
...
# Design Doc
- **Integration Trigger:** User goes to Settings and toggles "Send me SMS for new orders".
- **User Flow:** User enters their verified mobile number.
- **Action Flow:** When an order is paid, OHC dispatches async jobs to send SMS messages via Twilio API. The Operations Agent decides the optimal time to send the reminder.
