...
1. Create a `PermissionManager` in `srcs/server/auth/permissions.go`.
2. Define `PermissionRule` struct: `ToolName`, `Pattern`, `Action` (Allow, Deny, Ask).
3. Update `srcs/server/orchestration/statemachine/machine.go` to handle the `PENDING_APPROVAL` state.
