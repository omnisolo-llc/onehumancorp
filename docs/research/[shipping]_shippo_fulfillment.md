...
- **Advantages:** Very high value once configured natively. User just clicks 'Buy Label & Print' without leaving OHC.
...
# Design Doc
- **Integration Trigger:** When an order is placed, OHC sends the dimensions/weight to Shippo to get live rates natively during checkout.
- **User Flow:** The user clicks a native 'Fulfill Order' button.
- **Action Flow:** The Operations agent shows the cheapest shipping option. OHC purchases the label via Shippo and saves the tracking number. OHC automatically emails the customer the tracking number.
