...
- Fits OmniSolo’s "The Promoter" agent to automate posts and "The Ambassador" to draft replies.
- Non-technical users benefit by never leaving the OmniSolo interface.
...
**Implementation Prompt**: Implement an integration where users can link Instagram and Facebook, allowing OmniSolo AI agents to read incoming messages and draft replies in the unified inbox, and schedule out outbound picture posts.
**Priority**: P1
...
- It is lightweight (Go + PostgreSQL), aligning perfectly with the OmniSolo backend stack.
- Zero extra SaaS costs for OmniSolo Standalone users; minimal scaling costs for Cloud.
...
**Implementation Prompt**: Integrate Listmonk as the underlying email engine to allow users to trigger marketing emails to specific customer segments directly from the OmniSolo dashboard.
**Priority**: P2
**Estimated Scope**: Medium
...
- Works for both Cloud (via OmniSolo platform account) and Standalone (user supplies API keys).
**Design Doc**:
...
**Implementation Prompt**: Add Mercado Pago as a payment provider alternative to Stripe, allowing users in supported LATAM countries to accept local payment methods via the OmniSolo checkout flow.
**Priority**: P1
**Estimated Scope**: Large
