...
| F06: unusable connection flow | Encrypted, tenant/provider-bound vault; supported OpenAI API/Stripe connection controls and read-only verification, revocation, recheck and stale-verification states. | Verified. Connection missing credentials or invalid IDs fail closed. Unsupported subscription modes rejected. Strict 501 bounds for unsupported integrations confirmed. No live credentials were tested. |
...
| F14: economics/owner outcomes | Workload usage records and build/resource timing available; research keeps costs, owner correction time and actual outcome evidence separate. | Verified blocked / no-work outcome due to missing prerequisites and owner economic/metric data. |
...
| F04 | Model paths disagree on usage; proposal adapter returns default usage; proxy forwards streams | Verified provider-native capture preserves known counts and unknown states. Verified proxy-stream accumulates usage correctly and proposal-adapter respects the contract. No invented free usage. | Closed |
