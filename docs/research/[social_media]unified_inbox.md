...
*   **Ease of Use**: Once connected, the user never has to leave OmniSolo. The initial setup requires logging into Meta.
...
## Design Doc
*   **Integration Trigger**: User connects their Meta/WhatsApp accounts via a "Connect Socials" button in OmniSolo Settings.
*   **Action**: Webhooks receive incoming messages and route them to a unified "Inbox" view in the OmniSolo app. Replies sent from OmniSolo are pushed back to the respective platform.
