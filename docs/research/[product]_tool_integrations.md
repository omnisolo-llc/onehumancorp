...
## Problem Statement
Business owners need to integrate external tools for scheduling, communications, payments, and fulfillment directly into their OHC dashboard to centralize operations without leaving the platform.

## Research Report
We evaluated various providers for 7 core categories:
1. **Social Media**: Meta Graph API for unified WhatsApp, Instagram, and Facebook inbox.
2. **Scheduling**: Cal.com for embedding timezone-aware bookings.
3. **Email Marketing**: Resend for developer-friendly transactional and bulk emails.
4. **Payments (LATAM)**: Mercado Pago for local payment methods like PIX.
5. **Shipping**: EasyPost for aggregating 100+ carriers into a single rating/label API.
6. **SMS Alerts**: Twilio for global reach.
7. **Video Calls**: Google Meet for ubiquitous free-tier virtual appointments.

## Design Doc
### Architecture
Each integration will use the `AgentRunConfig` to securely inject API tokens. Background workers will periodically pull from webhook tables to orchestrate AI drafts (e.g., Customer Success agent drafting an IG reply).

### UX Flow
1. User navigates to Settings -> Integrations.
2. Clicks "Connect" on the desired tool.
3. Completes OAuth or provides an API key.
4. Platform provisions a webhook and begins syncing data to the tenant's dedicated partition.

## Implementation Prompt
1. Extend `src/agents/builtin/memory_store.rs` or create `integrations.rs` with empty function stubs for each of the 7 tools to prepare for Phase 2 implementation.
2. Update the Unified Inbox UI mockups.

## Priority
P0 - P2 depending on the tool.
