...
1.  **File Path:** `srcs/server/agents/harness/bwrap_sandbox.go`
...
5.  **Testing:** Create `srcs/server/agents/harness/bwrap_sandbox_test.go` with 100% coverage, ensuring successful execution and proper isolation (e.g., verifying network access is blocked).
