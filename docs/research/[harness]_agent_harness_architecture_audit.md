...
1.  **File Path:** `srcs/server/agent/harness/bwrap_sandbox.go`
...
5.  **Testing:** Create `srcs/server/agent/harness/bwrap_sandbox_test.go` with 100% coverage, ensuring successful execution and proper isolation (e.g., verifying network access is blocked).
