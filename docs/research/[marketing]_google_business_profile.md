...
- **Integration Potential**: OmniSolo can integrate with the GBP APIs to allow users to authenticate and authorize OmniSolo to manage their profile. The "Marketing & Advertising" agent can then automatically update hours, add photos, generate and schedule posts, and reply to reviews (via the "Customer Success" agent).
- **Usability for Non-Technical Users**: The user simply clicks "Connect Google Business Profile" and authorizes OmniSolo. All subsequent management happens within the OmniSolo UI or invisibly via AI agents.
...
- **Authentication**: OmniSolo integrates Google OAuth 2.0 to request the `https://www.googleapis.com/auth/business.manage` scope.
- **Location Management**: Fetch the user's accounts and locations to link the GBP location to the OmniSolo tenant.
- **Data Sync**: Automatically sync core business data (name, address, phone, hours, special hours, attributes, categories) between OmniSolo and GBP using the Business Information API.
...
3. **Location Selection**: User selects which GBP location corresponds to their OmniSolo business.
