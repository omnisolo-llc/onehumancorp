...
The server now uses `sqlx::migrate!("./src/server/migrations")` to embed the exact SQL and checksums into the executable. `src/server/build.rs`, explicitly declared in the root Cargo package, tracks migration-directory changes so newly added migrations invalidate cached binaries on stable Rust. The first follow-up (`18ff5d311`, CI run 35490999358) exposed that the repository's tracked-source ownership guard rejects root-level Rust files, even when Git tracks them. The build script was therefore moved into the owned server source tree rather than weakening that guard or broadening root-file exclusions. That failed run's skipped jobs are not passes.
