# [AI_Agents] Auto-Reply DM Agent

## Problem Statement
Maya (baker, 28) currently sells via Instagram DMs. She misses leads because she is busy baking.
...
## Design Doc
- **Entity Types:** `AIAgentsEntity`, `Task`, `AIAgentPersona`.
...
- **UI Flow:**
  ...
  2. User taps "Auto-Reply DM Agent".
  3. User toggles "Enable AI Automation".
  4. System confirms activation via a subtle Glassmorphism toast notification.
- **Integration Points:** Meta Graph API, NLP Router, OHC Core Service, OHC AI Orchestrator.

## Implementation Prompt
Implement the Auto-Reply DM Agent feature. The agent must have real-time access to the user's data.
...
**Acceptance Criteria:**
- Agent successfully executes the core task (Agent parses DM, queries catalog, sends checkout link.).
...
| Platform | Approach to Auto-Reply DM Agent | Drawbacks |
| :--- | :--- | :--- |
