### Title
Architectural Mapping of the End-to-End Business Journey for OHC Personas (Detailed)
...
| Persona | Key Pain Points | Mitigation Strategy |
| :--- | :--- | :--- |
| **Maya** | Missing IG DMs while sleeping. | 1-tap AI responses queueing in dashboard. |
| **Carlos** | Complex quoting and back-and-forth scheduling. | AI-drafted quotes for review; unified inbox. |
| **Priya** | Managing inventory across physical and online stores. | Native POS integration and omnichannel sync. |
| **Leo** | Generating Zoom links and managing recurring payments. | Automated service scheduling with native Zoom generation. |
| **Fatima** | Too many buttons, complex English terms, slow loading. | Arabic UI support, audio notifications, offline drafting. |
...
-   **Visual Excellence**: All journey flows must adhere to the OHC Premium design tokens, including Glassmorphism and Outfit/Inter typography.
...
sequenceDiagram
    ...
    Maya->>OHC: Downloads App & Opens (Acquisition)
    OHC->>AI_Mark: Trigger Onboarding Wizard
    AI_Mark->>Maya: Asks "What do you sell?"
    Maya->>AI_Mark: "Custom vegan cakes" (Onboarding)
...
sequenceDiagram
    ...
    Carlos->>AI_Mark: "Plumbing, Painting" (Onboarding)
...
sequenceDiagram
    ...
    Priya->>Search: Searches "Easy online store for boutique" (Acquisition)
    Priya->>OHC: Signs up
    OHC->>AI_Mark: Trigger Onboarding
    AI_Mark->>Priya: Syncs initial inventory (Onboarding)
...
sequenceDiagram
    ...
    OHC->>AI_Mark: Generates Profile & Subscriptions (Onboarding)
...
sequenceDiagram
    ...
    OHC->>AI_Mark: Fast menu creation (Photos + Prices) (Onboarding)
...
### Mobile UX Flow
1. **Acquisition & Landing:** Clean landing page highlighting simple sign-up (<3 seconds load time on 4G).
2. **Onboarding Wizard:** 1 question per screen. Minimal text. Native keyboards for inputs.
3. **AI Generation (Loading):** While generating, display a premium Glassmorphism shimmer effect to indicate progress.
4. **Dashboard View:**
   - **Top:** Daily Analytics Summary (Orders, Revenue).
   - **Middle:** AI Agent "Action Feed" requiring 1-Tap approvals.
   - **Bottom Nav:** Primary tabs (Home, Messages, Inventory/Bookings, Settings).
5. **Offline Support:** Ability to draft messages and view local task queues offline, syncing via the Teammate Mesh upon reconnection.

## Implementation Prompt
**To Implementer Agent:**
Implement the foundational user onboarding flow and dashboard state management that supports the progression from Acquisition to Activation for the 5 key personas. The system should define the required data models to capture the user's business type and minimal initial configuration. Build the mobile-first (375px) UI wizard that guides a user through the initial setup, ensuring that advanced configurations are deferred via Progressive Disclosure. The final step of the wizard should instantly generate a functional "Storefront/Booking Page" view, satisfying the 'Activation' milestone. Ensure that interactions feel premium (Glassmorphism, correct typography, subtle motion) and are resilient to network issues (optimistic updates). Do not prescribe the specific database schema or backend routing; focus on the unified API contract and the user journey transitions. Include full E2E test coverage verifying a successful run-through from login to the generated storefront.
