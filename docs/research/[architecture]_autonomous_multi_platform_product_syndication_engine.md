...
- **OmniSolo Target**: Zero-config syndication. The AI Marketing Agent handles taxonomy mapping invisibly.
...
4. **Unified Order Ingestion**: Orders originating from TikTok or Instagram are ingested, normalized into the OmniSolo `ORDER` format, and pushed to the Operations Agent, decrementing the global `INVENTORY_LEDGER`.
...
3. **Invisible Magic**: She is returned to the OmniSolo app. A shimmering translucent glass card says: "Agent is optimizing and syncing your catalog... 45/120 products synced."
