...
*   **Pricing**: Pay-as-you-go model ($0.05 per label) or free for USPS only. They also provide deep discounted carrier rates (up to 90% off USPS) which directly benefits our small business users, adding instant value to the OmniSolo platform.
*   **Ease of Use for Non-Technical Users**: The user simply clicks "Fulfill Order" in OmniSolo, sees the cheapest shipping option automatically selected by AI, and clicks "Print Label". The label is generated as a PDF or image, ready to print on a standard printer or thermal label printer. The tracking number is automatically emailed to the customer by the Operations agent. No carrier accounts needed for the user (OmniSolo acts as the master account or uses Shippo's default carrier accounts).
...
*   **Competitor Comparison**: Shopify has built-in Shopify Shipping (powered by Shippo/similar). Wix uses third-party apps like ShipStation or Shippo. OmniSolo can differentiate by making it completely invisible and AI-driven (e.g., "The Manager" agent automatically buys the cheapest label when an item is packed).
...
*   **Acceptance Criteria 1**: A non-technical user can generate a shipping label for an order directly from the OmniSolo mobile or web app with a single click.
