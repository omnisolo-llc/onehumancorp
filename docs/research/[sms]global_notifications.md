...
*   **Modes**: Cloud (requires OmniSolo to manage billing/credits). Standalone (user inputs their own Twilio credentials).

## Design Doc
*   **Integration Trigger**: An appointment is approaching (24h before) or an order ships.
*   **Action**: OmniSolo triggers an SMS payload to the provider API.
