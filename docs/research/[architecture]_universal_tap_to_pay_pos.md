...
2.  **Mobile POS:** Priya opens the OmniSolo app on her phone, selects the dress from her synced inventory, and taps "Charge."
3.  **Tap to Pay:** The OmniSolo app activates the phone's NFC reader. The prompt "Hold card near reader" appears.
...
sequenceDiagram
    ...
    participant OmniSoloBackend as OmniSolo Backend
    participant PaymentAPI as Payment Provider API
    participant Ledger as Unified Ledger

    App->>OmniSoloBackend: Request Connection Token
    OmniSoloBackend->>PaymentAPI: Generate Terminal Connection Token
    PaymentAPI-->>OmniSoloBackend: Token
    OmniSoloBackend-->>App: Token
    App->>TerminalSDK: Initialize & Connect Reader (Tap to Pay)
    App->>OmniSoloBackend: Create PaymentIntent
    OmniSoloBackend->>PaymentAPI: Create PaymentIntent
    PaymentAPI-->>OmniSoloBackend: Client Secret
    OmniSoloBackend-->>App: Client Secret
    ...
    App->>OmniSoloBackend: Capture/Verify Payment
    OmniSoloBackend->>Ledger: Record Transaction & Update Inventory
    OmniSoloBackend-->>App: Sale Complete
```
