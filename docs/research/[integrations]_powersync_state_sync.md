...
2. Integrate the PowerSync client SDK into the Go standalone daemon (`srcs/server/orchestration/hybrid_sync/hybrid_sync.go` and `srcs/server/auth/powersync.go`), ensuring it syncs the SQLite database locally to the cloud.
...
**File Paths:** `srcs/server/orchestration/hybrid_sync/hybrid_sync.go`, `srcs/server/auth/powersync.go`
