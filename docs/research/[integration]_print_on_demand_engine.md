...
- **Competitor Landscape**: Shopify relies heavily on Printful/Printify apps. Squarespace has limited native integrations. By building this deeply into OmniSolo, we remove the "app installation" friction entirely. The user just sees a "Merch" tab.
- **Evaluation of Tool (Printful API)**:
  ...
  - Supports both Cloud (multi-tenant) and can be accessed via OmniSolo's backend.
  - Generous developer ecosystem and reliable SLA.

## Design Doc
- **Integration with OmniSolo**:
    - **Catalog Sync**: OmniSolo backend pulls the Printful product catalog (t-shirts, hoodies, mugs) and base costs.
    - **Design & Mockup Flow**:
      - The user accesses the "Merchandising" tab in the OmniSolo app.
      - The user uploads a logo/design or asks the "Marketing & Advertising" AI Agent to generate a design.
      - OmniSolo calls Printful's Mockup Generator API to instantly create realistic product photos.
      - The item is immediately listed on the user's OmniSolo storefront.
    - **Order Fulfillment Loop**:
      - A customer buys the t-shirt on the OmniSolo storefront.
      - OmniSolo processes the payment (Stripe).
      - The OmniSolo "Operations" Agent automatically pushes the order details (shipping address, design file, product ID) to the Printful API.
      - Printful prints and ships the item. Webhooks update the order status in OmniSolo, triggering the "Customer Success" Agent to send the customer a tracking link.
...
- Handle Printful webhooks to update shipping and tracking status in the OmniSolo unified order manager.
