...
2. Define the gRPC/REST API endpoints for the Tauri mobile app to request these connection tokens securely, ensuring strict multi-tenant isolation (`tenant_id` validation).
...
Do not implement the Tauri UI or Stripe SDK integration yet—focus on the backend infrastructure, database schema changes, and API contracts required to support Terminal operations.
