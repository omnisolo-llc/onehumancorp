...
```mermaid
graph TD
    A[Customer Messaging via IG/FB/WhatsApp] --> B(Meta Webhook Event)
    B --> C[OHC Inbox Service]
    C --> D[Store Message in DB]
    C --> E[Trigger AI Ambassador Agent]
    E --> F[Generate Reply Draft]
    F --> G[Present Draft to Maya in UI]
```
...
```mermaid
graph TD
    A[Public Booking Link] --> B(Customer Selects Time)
    B --> C[OHC Booking Service]
    C --> D[Cal.com Platform API]
    D --> E[Sync with Owner's Google/Outlook]
    C --> F[Trigger Operations Agent]
    F --> G[Send Booking Confirmation]
```
...
```mermaid
graph TD
    A[User Drafts Message & Adds Photo] --> B(Trigger AI Promoter)
    B --> C[Generate HTML Email Template]
    C --> D[Select Target Customers]
    D --> E[OHC Marketing Service]
    E --> F[Resend API]
    F --> G[Deliver Emails to Customers]
```

### Implementation Prompt
Build a simple email campaign tool that integrates with the existing customer directory. The user should be able to draft a plain-text/image message, which the AI automatically transforms into a styled HTML email using OHC's design system. Integrate with Resend API to handle the actual integration. Provide basic analytics (open rates, click rates) in plain language.
...
```mermaid
graph TD
    A[Customer Initiates Checkout] --> B(OHC Payment Gateway Router)
    B --> C{Determine Region/Preference}
    C -->|LATAM| D[Mercado Pago Adapter]
    C -->|US/Global| E[Stripe Adapter]
    D --> F[Process Local Payment e.g., PIX]
    E --> G[Process Credit Card/Apple Pay]
    F --> H[Order Fulfillment Flow]
    G --> H
```
...
```mermaid
graph TD
    A[User Clicks Buy Shipping Label] --> B(OHC Shipping Module)
    B --> C[Shippo API - Fetch Rates]
    C --> D[Select Cheapest Carrier]
    D --> E[Charge User & Generate Label]
    E --> F[Download PDF Label]
    E --> G[Update Tracking & Notify Ambassador]
```
...
```mermaid
graph TD
    A[Order Status Updated to Ready] --> B(Trigger Notification System)
    B --> C[Check Notification Preferences]
    C --> D{SMS Enabled?}
    D -->|Yes| E[Send to Twilio API]
    E --> F[Deliver SMS to Customer/Owner]
    D -->|No| G[Fallback to Email/Push]
```
...
```mermaid
graph TD
    A[Student Books Online Service] --> B(OHC Booking Service)
    B --> C[Daily.co API - Create Room]
    C --> D[Generate Unique Video Link]
    D --> E[Save Link to Booking Record]
    E --> F[Include in Confirmation Email & Calendar Invite]
```
