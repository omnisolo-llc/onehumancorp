...
## Problem Statement
Email open rates are low. Many critical business communications (e.g., appointment reminders, emergency cancellations, delivery updates) are missed if sent only via email. Owners need a reliable way to reach customers instantly.
...
## Research Report
Twilio is the industry standard for programmatic SMS. While it requires a bit more setup (purchasing a number, handling A2P 10DLC registration in the US), it offers unparalleled reliability, global reach, and extensive API documentation.
...
For users with lower English proficiency or digital literacy, SMS is often the primary mode of communication. Implementing this feature is essential for inclusivity and ensuring OHC works effectively across diverse customer bases.

Use Case Focus: A local mechanic shop uses OHC to manage repair jobs. An automated SMS via Twilio alerting a customer that their car is ready results in same-day pickup, improving cash flow.
...
## Design Doc
OHC will allow users to connect their Twilio credentials. The system will define key trigger events (e.g., 'Appointment Confirmed', 'Order Shipped'). When an event occurs, OHC will construct the SMS payload and dispatch it via the Twilio API.
...
Create a Twilio configuration page. Implement the message sending logic. Crucially, build an interface that allows owners to toggle which automated SMS notifications are active. Ensure the system gracefully handles invalid phone numbers and logs delivery failures.
...
### Onboarding UX Flow
1. This is a complex onboarding. OHC will provide a 'Setup Wizard' for Twilio.
2. Step 1: Enter Account SID and Auth Token.
3. Step 2: OHC pulls available numbers; user selects one to use as their primary sender.
4. Step 3: A simple checklist guides the user through any required regulatory registration (like A2P 10DLC).
...
### Testing Strategy
1. Write tests ensuring invalid phone numbers (e.g., missing country codes) are gracefully caught and formatted before calling Twilio.
2. Mock the Twilio message dispatch API and assert that 'Sent' and 'Failed' statuses are correctly logged in OHC.
3. Test the 'Opt-out' webhook logic to ensure 'STOP' messages correctly flag a contact as unsubscribed in OHC.
