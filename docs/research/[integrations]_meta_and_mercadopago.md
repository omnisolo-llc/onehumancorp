...
**Problem Statement:** Business owners like Maya (The Home Baker) rely heavily on Instagram DMs and WhatsApp for customer acquisition and order placement. Currently, OmniSolo lacks a unified inbox connection to these platforms, forcing owners to switch contexts and losing AI capabilities.
**Research Report:** Meta provides Graph API for Instagram Direct Messages and WhatsApp Business Platform API. Integrating these into the OmniSolo Unified Inbox will allow the "Customer Success" and "Operations" AI agents to interact directly with customers on their preferred platforms.
**Design Doc:**
- Connect Instagram/Facebook/WhatsApp Business accounts via Meta OAuth.
- Implement Webhooks to receive incoming messages into the OmniSolo unified inbox.
- OmniSolo Backend uses Graph API/WhatsApp API to send replies.
...
**Problem Statement:** To effectively serve the LATAM market, OmniSolo needs to support local payment methods like PIX, OXXO, and local installments, which Stripe does not fully cover in all regions.
