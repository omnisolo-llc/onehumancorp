...
## Problem Statement
Currently, OmniSolo (OmniSolo) handles online payments efficiently via Stripe Checkout and custom payment routing. However, our real-world personas, like Priya (the boutique owner) and Carlos (the handyman), frequently transact in person. Priya needs to seamlessly ring up customers in her store using her phone (Tap to Pay) or a dedicated card reader, while Carlos needs to accept final payments on-site after a repair. The lack of a native in-person Point of Sale (POS) solution forces users to rely on disjointed third-party systems, breaking the "all-in-one" OmniSolo promise and muddying financial reporting.

## Research Report
- **OmniSolo Gaps:** A review of `src/server/integrations/stripe/` reveals implementations for Checkout, Subscription, and Routing (Card vs. ACH, Razorpay, MercadoPago), but zero support for Stripe Terminal.
...
- **Opportunity:** Integrating Stripe Terminal will enable OmniSolo users to accept in-person payments directly within the OmniSolo app, utilizing Tap to Pay on compatible devices or connecting to Stripe readers (e.g., BBPOS WisePad 3, Stripe Reader S700). This provides a unified ledger and centralized inventory management.

## Design Doc
### Architecture Diagram
```mermaid
graph TD
  Client[OmniSolo Mobile App - POS View] -->|Request Connection Token| API[OmniSolo Backend API]
...
- Glassmorphism design elements (translucent overlays, clean typography) adhering to OmniSolo's visual standards.
...
As an implementer, your task is to integrate Stripe Terminal into OmniSolo's payment architecture.
1. Add a new `terminal.rs` module under `src/server/integrations/stripe/` to handle `ConnectionToken` generation.
2. Create secure API endpoints for the mobile client to request these tokens.
3. Ensure the `PaymentIntent` creation flow supports in-person capture methods.
4. Integrate the transaction flow into the unified ledger, ensuring offline and online sales are consolidated.
5. The end result should allow a user (like Priya) to open the OmniSolo app, select an item, and successfully prompt a "Tap to Pay" or card reader interaction, resulting in a recorded, unified transaction in her dashboard.
