...
*   **Discovery:** OmniSolo requires a Universal Staff Mesh deeply integrated into the Core Ledger and POS. AI Operations and Finance Departments will co-manage this: Operations handles shift scheduling (optimizing based on forecasted demand and staff availability), while Finance automatically calculates wages, handles tip splitting (integrated with offline POS), and queues payroll drafts. The system must operate offline-first (e.g., clocking in from a basement worksite).
