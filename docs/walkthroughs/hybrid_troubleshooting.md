...
graph TD
    ...
    B -- Yes (SQLite) --> C[Check Local ${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/status]
...
- **Database Lock Contention (SQLite)**: Ensure multiple background processes are not writing to `${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime` directories simultaneously without respecting the file lock protocol.
