...
## Problem Statement
Currently, OHC relies on the external chat service as an external third-party service for omnichannel customer support and chat functionality. However, based on the engineering standards, the external chat service must be retired and replaced with a native Rust implementation to ensure performance, multi-tenant isolation, and alignment with OHC's architectural vision.

## Research Findings
After auditing the the external chat service source code (`https://github.com/chatwoot/chatwoot`), the following core data models and relationships were identified as critical for a minimum viable native Rust chat system:
...
Implement the foundational data model and API for the native Rust omnichannel chat system to replace the external chat service.
...
The user-facing outcome is that OHC can natively manage chat data without relying on external the external chat service services, laying the groundwork for the unified owner inbox UI.
