    ...
    steps:
      ...
      - name: Patch Bazel Cache
        run: |
          ...
          for f in ~/.cache/bazel/*/*/external/rules_android++android_sdk_repository_extension+androidsdk/*.bzl; do
            if [ -f "$f" ]; then
          ...
          for f in ~/.cache/bazel/*/*/external/rules_flutter+/flutter/private/package_generation.bzl; do
            if [ -f "$f" ]; then
