...
for f in files:
    ...
    if 'status: PENDING' in content or 'status: OPEN' in content:
        print(f"Blocking {f}")
        content = content.replace('status: PENDING', 'status: BLOCKED\nblockers: Domain mismatch. I am a Flutter agent (Palette) and this mission requires a Go Backend Implementer.')
        content = content.replace('status: OPEN', 'status: BLOCKED\nblockers: Domain mismatch. I am a Flutter agent (Palette) and this mission requires a Go Backend Implementer.')
