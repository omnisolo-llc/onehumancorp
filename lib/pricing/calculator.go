...
var ModelPricing = map[string]PricingRates{
	"claude-3-5-sonnet-20240620": {Input: 3.0, Output: 15.0, Cached: 0.30, BatchInput: 1.5, BatchOutput: 7.5}, // Example rates
	"gpt-4o":                     {Input: 5.0, Output: 15.0, Cached: 2.50, BatchInput: 2.5, BatchOutput: 7.5},
	"gpt-4o-mini":                {Input: 0.15, Output: 0.60, Cached: 0.075, BatchInput: 0.075, BatchOutput: 0.30},
}
...
// CalculateBatchCost computes the cost of an LLM batch request in USD (typically 50% discount).
func CalculateBatchCost(ctx context.Context, model string, promptTokens, completionTokens int) float64 {
	rates, ok := ModelPricing[strings.ToLower(model)]
	if !ok {
		// Fallback to average or zero if model unknown
		return 0.0
	...
	inputCost := float64(promptTokens) * (rates.BatchInput / 1000000.0)
	outputCost := float64(completionTokens) * (rates.BatchOutput / 1000000.0)
