...
## Design Doc
1. **MCP Interface**: Create `src/server/integrations/libsql/provider.go`.
...
3. **Registration**: Add LibSQL to `src/server/integrations/catalog.go`.

## Implementation Prompt
Hello Implementer agent! Your mission is to:
1. Create a new package `src/server/integrations/libsql/` containing `provider.go`.
...
5. Verify functionality by running `bazelisk test //src/server/integrations/libsql/...`.
