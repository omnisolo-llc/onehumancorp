...
## Overview
OmniSolo (OmniSolo) platform relies on a high-performance serving layer that delivers instant page loads and strong SEO metrics, leveraging edge caching architectures such as Cloudflare or Varnish. This document outlines the CDN integration pattern and how cache invalidations are automatically triggered when tenants update their products, storefronts, or settings.

## Edge Caching Mechanism
The OmniSolo backend is designed to interact natively with edge CDNs. The dynamic storefront endpoints (e.g. `/api/v1/storefront/{tenant_id}/{product_id}` or the builder routes) pre-render HTML and SEO metadata server-side and instruct the edge CDN to cache the result.
...
## Cache Invalidation
When inventory updates, products are modified, or tenant settings change, the central control plane (the `omnisolo-core` app servers) triggers a targeted cache purge.
...
2. **Pub/Sub Notification:** Immediately after the transaction, the API publishes a JSON payload to the Redis Pub/Sub topic `cache_invalidation_events`. The payload looks like this:
   ...
   ```
3. **Invalidator Service:** The internal `Cache Invalidator Service` (in `omnisolo-core`) listens to the `cache_invalidation_events` topic. It purges the tags from the internal `CDN_CACHE` and/or dispatches HTTP `PURGE` requests to the external edge CDN (like Cloudflare or Varnish) to instantly invalidate the corresponding `Surrogate-Key`.
...
By adopting this edge-caching model, OmniSolo storefronts enjoy sub-100ms TTFB globally and ensure search engines index the complete, fully rendered SEO content.
