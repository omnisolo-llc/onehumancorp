#!/bin/bash
# OmniSolo Premium Setup Verification
...
echo -e "${BOLD}${CYAN}   🔍 OmniSolo Environment Audit                    ${RESET}"
echo -e "${BOLD}${BLUE}===============================================${RESET}"
...
# Check OMNISOLO_SOURCE_MODE
if [ -z "$OMNISOLO_SOURCE_MODE" ]; then
    echo -e "${RED}✗ OMNISOLO_SOURCE_MODE is not set.${RESET}"
    ISSUES=$((ISSUES+1))
else
    echo -e "${GREEN}✓ OMNISOLO_SOURCE_MODE is set to ${OMNISOLO_SOURCE_MODE}.${RESET}"
fi
...
- **OMNISOLO_SOURCE_MODE:** ${OMNISOLO_SOURCE_MODE:-Unset}
MDEOF
