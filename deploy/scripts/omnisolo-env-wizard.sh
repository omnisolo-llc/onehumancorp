#!/bin/bash
# OmniSolo Hybrid Agentic OS - Day One Interactive Environment Wizard
...
echo -e "${BOLD}${CYAN}     OmniSolo: Interactive Environment Wizard (.env)       ${RESET}"
echo -e "${BOLD}${BLUE}======================================================${RESET}"
echo ""
...
update_env "OMNISOLO_MULTITENANT" "$multitenant"

if [ "$multitenant" = "false" ]; then
    update_env "OMNISOLO_SOURCE_MODE" "standalone"
else
    update_env "OMNISOLO_SOURCE_MODE" "cloud"
fi
...
echo -e "${DIM}Note: You can run this wizard again anytime via deploy/scripts/omnisolo-env-wizard.sh.${RESET}"
echo ""
