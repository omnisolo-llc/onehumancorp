...
echo -e "${BOLD}${CYAN}   🚀 OmniSolo Quick Start (Day One Onboarding)    ${RESET}"
echo -e "${BOLD}${BLUE}======================================================${RESET}"
echo ""
...
if [ ! -f .env ]; then
  ...
  echo "PORT=8080" >> .env
  echo "OMNISOLO_MULTITENANT=false" >> .env
  echo "OMNISOLO_HEADLESS=false" >> .env
  echo "OMNISOLO_SOURCE_MODE=standalone" >> .env
  chmod 0600 .env
fi
...
yes | bash deploy/scripts/omnisolo-diagnostics.sh || true

echo -e "${BOLD}Next steps:${RESET}"
echo -e "  Use ${CYAN}./deploy/scripts/omnisolo_hybrid_cli.sh${RESET} to manage the OS, switch to Cloud Mode, or seed mock data."
