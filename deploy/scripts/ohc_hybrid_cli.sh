...
while true; do
    ...
    echo -e "  7) Swarm Status Viewer"
    echo -e "  8) Seed Database with Mock Data"
    echo -e "  9) Launch Cloud Native Quick Start"
    echo -e "  0) Exit"
    ...
    case $choice in
        ...
        6)
            ...
            ;;
        7) (set -e; bash "$SCRIPT_DIR/ohc-swarm-status.sh") || echo -e "${PURPLE}Swarm Status Viewer returned non-zero exit status ($?).${RESET}" ;;
        8) (set -e; bash "$SCRIPT_DIR/ohc-seed-data.sh") || echo -e "${PURPLE}Data Seeder returned non-zero exit status ($?).${RESET}" ;;
        9) (set -e; bash "$SCRIPT_DIR/ohc-cloud-start.sh") || echo -e "${PURPLE}Cloud Native Quick Start returned non-zero exit status ($?).${RESET}" ;;
