...
echo -e "${BOLD}${CYAN}      OmniSolo Hybrid Agentic OS - Master CLI              ${RESET}"
echo -e "${BOLD}${BLUE}======================================================${RESET}"
...
while true; do
    ...
    case $choice in
        1) (set -e; bash "$SCRIPT_DIR/omnisolo-setup.sh") || echo -e "${PURPLE}Developer Setup returned non-zero exit status ($?).${RESET}" ;;
        2) (set -e; bash "$SCRIPT_DIR/omnisolo-env-wizard.sh") || echo -e "${PURPLE}Environment Wizard returned non-zero exit status ($?).${RESET}" ;;
        3) (set -e; bash "$SCRIPT_DIR/omnisolo-diagnostics.sh") || echo -e "${PURPLE}Diagnostics returned non-zero exit status ($?).${RESET}" ;;
        4) (set -e; bash "$SCRIPT_DIR/omnisolo-standalone.sh") || echo -e "${PURPLE}Quick Start returned non-zero exit status ($?).${RESET}" ;;
        5) (bash "$SCRIPT_DIR/omnisolo-agent-wizard.sh") || echo -e "${PURPLE}Agent Provisioning returned non-zero exit status ($?).${RESET}" ;;
        6)
            if ! command -v sqlite3 &> /dev/null; then
                echo -e "${PURPLE}✗ sqlite3 is not installed. Please install it to perform the DB Health Check.${RESET}"
            elif [ -f "$HOME/.omnisolo-local-data/standalone.db" ]; then
                echo -e "${GREEN}✓ Standalone DB found. Checking tables...${RESET}"
                sqlite3 "$HOME/.omnisolo-local-data/standalone.db" ".tables" || echo -e "${PURPLE}DB Check failed with exit status $?.${RESET}"
            else
                echo -e "${PURPLE}✗ standalone.db not found in $HOME/.omnisolo-local-data/.${RESET}"
            fi
            ;;
