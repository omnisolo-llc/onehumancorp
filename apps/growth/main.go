...
func NewGrowthMux(tracker *analytics.Tracker, rdb *redis.Client) *http.ServeMux {
	...
	mux.HandleFunc("/api/v1/growth/referrals/bulk_invite", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(req.InviteeEmails) > 100 {
			http.Error(w, "too many emails in bulk invite, maximum is 100", http.StatusBadRequest)
			return
		}
		...
		for _, email := range req.InviteeEmails {
			...
			if err != nil {
				// Log but continue
				log.Printf("Failed to save referral for email %s: %v", email, err)
				continue
			}
			links = append(links, fmt.Sprintf("ohc://join?ref=%s", referral.ID))
		}

		err := referralService.ProcessBulkInvites(r.Context(), spiffeID, req.InviteeEmails)
		if err != nil {
			http.Error(w, "Failed to process bulk invites", http.StatusInternalServerError)
			return
		}
