...
func NewGrowthMux(tracker *analytics.Tracker, rdb *redis.Client) *http.ServeMux {
	...
	teamInviteRepo := growth.NewTeamInviteRepository(rdb)
	teamInviteService := growth.NewTeamInviteService(tracker, teamInviteRepo)

	mux.HandleFunc("/api/v1/growth/team/invite", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.TenantID == "" || req.InviteeEmail == "" {
			http.Error(w, "missing tenant_id or invitee_email", http.StatusBadRequest)
			return
		}

		if req.TenantID != spiffeID { // Mock auth check
			http.Error(w, "unauthorized to invite to this tenant", http.StatusForbidden)
			return
		}

		invite := &growth.TeamInvite{
			ID:           fmt.Sprintf("team-inv-%d", time.Now().UnixNano()),
		...
		err := teamInviteRepo.SaveInvite(r.Context(), invite)
		if err != nil {
			http.Error(w, "Failed to save invite", http.StatusInternalServerError)
			return
		}

		_ = teamInviteService.ProcessTeamInvite(r.Context(), req.TenantID, spiffeID, req.InviteeEmail)
	...
	mux.HandleFunc("/api/v1/growth/team/accept", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
		}
		...
		inv, err := teamInviteRepo.GetInviteByID(r.Context(), req.InviteID)
		if err != nil {
			http.Error(w, "Invite not found", http.StatusNotFound)
			return
		}

		if inv.InviteeEmail != req.UserEmail {
			http.Error(w, "unauthorized to accept this invite", http.StatusForbidden)
			return
		}
		...
		inv.Status = "ACCEPTED"
		err = teamInviteRepo.SaveInvite(r.Context(), inv)
		if err != nil {
			http.Error(w, "Failed to accept invite", http.StatusInternalServerError)
			return
		}

		_ = teamInviteService.AcceptTeamInvite(r.Context(), req.InviteID)
	...
	mux.HandleFunc("/api/v1/growth/team/invites", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		...
		if tenantID != spiffeID { // Mock auth check
			http.Error(w, "unauthorized to view these invites", http.StatusForbidden)
			return
		}

		invites, err := teamInviteRepo.GetInvitesByTenant(r.Context(), tenantID)
		if err != nil {
	...
	mux.HandleFunc("/api/v1/growth/referrals/bulk_invite", authMiddleware(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
		}
		...
		if len(req.InviteeEmails) == 0 {
			http.Error(w, "missing invitee_emails", http.StatusBadRequest)
			return
		}
		...
		_ = referralService.ProcessBulkInvites(r.Context(), spiffeID, req.InviteeEmails)
