...
3. **Create Go Interface and Concrete Implementation:**
   ...
   - Include `RAGSyncRecord`, `RAGSyncService` interfaces, `SyncStatus` constants, and initialize OpenTelemetry counters `rag_records_synced_total` and `rag_sync_errors_total` globally via `otel.Meter("github.com/onehumancorp/mono/srcs/server/hub")`.
