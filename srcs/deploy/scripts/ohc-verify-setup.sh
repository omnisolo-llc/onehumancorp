...
if [ -z "$PORT" ]; then echo -e "  ${RED}✗ PORT is missing${RESET}"; ERRORS=$((ERRORS+1)); else echo -e "  ${GREEN}✓ PORT=${PORT}${RESET}"; fi
if [ -z "$LOG_LEVEL" ]; then echo -e "  ${RED}✗ LOG_LEVEL is missing${RESET}"; ERRORS=$((ERRORS+1)); else echo -e "  ${GREEN}✓ LOG_LEVEL=${LOG_LEVEL}${RESET}"; fi
if [ -z "$OHC_SOURCE_MODE" ]; then echo -e "  ${RED}✗ OHC_SOURCE_MODE is missing${RESET}"; ERRORS=$((ERRORS+1)); else echo -e "  ${GREEN}✓ OHC_SOURCE_MODE=${OHC_SOURCE_MODE}${RESET}"; fi

if [ $ERRORS -gt 0 ]; then
    echo -e "${RED}✗ Configuration validation failed.${RESET}"
    exit 1
fi
