...
func TestCujAuditDashboardLoadMetrics(t *testing.T) {
	...
	if !hasDashboard {
		t.Log("Dashboard page heading not found")
	}
	if !hasOverview {
...
func TestCujAuditHireAgentWizardHasRequiredSteps(t *testing.T) {
	...
	if count == 0 {
		t.Log("Hire Agent wizard missing 'Role' step")
	}
	...
	if count == 0 {
		t.Log("Hire Agent wizard missing 'Deploy Agent' button")
	}
...
func TestCujAuditChatScreenHasMessageInput(t *testing.T) {
	...
	if count == 0 {
		t.Log("Chat screen missing text input field")
	}
...
func TestCujAuditHandoffsScreenShowsPendingItems(t *testing.T) {
	...
	if count == 0 {
		t.Log("Handoffs screen missing title")
	}
...
func TestCujAuditAllCoreRoutesAreReachable(t *testing.T) {
	...
	for _, route := range routes {
		...
		if err != nil {
			t.Logf("Route %s (%s) failed: %v", route.name, route.path, err)
		} else {
...
func TestCujAuditNavigationSidebarHasRequiredLinks(t *testing.T) {
	...
	for _, item := range navItems {
		...
		count, _ := elem.Count()
		if count == 0 {
			t.Logf("Navigation missing '%s' link", item)
		}
	}
...
func TestCujAuditAgentsScreenHasHireButton(t *testing.T) {
	...
	if count == 0 {
		t.Log("Agents screen missing 'Hire Agent' button")
	}
...
func TestCujAuditSettingsScreenHasSecuritySection(t *testing.T) {
	...
	if count == 0 {
		t.Log("Settings screen missing title")
	}
