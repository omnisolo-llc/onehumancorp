...
ResourceExhaustion
// CorruptAgentLock simulates agent lock file corruption.
CorruptAgentLock
)
...
mu       sync.Mutex
rand     *rand.Rand
// LockDir is the directory used for simulated agent-lock corruption.
// It defaults to a directory inside the OS temp folder so that chaos
// tests never write inside the source tree.
LockDir string
}
...
LockDir: filepath.Join(os.TempDir(), "ohc-chaos", "agent-lock"),
}
}
