...
import (
	...
	"github.com/onehumancorp/mono/srcs/interop/autogen"
	"github.com/onehumancorp/mono/srcs/interop/crewai"
	"github.com/onehumancorp/mono/srcs/interop/openclaw"
	"github.com/onehumancorp/mono/srcs/interop/semantickernel"
...
func TestSwarmInteropStateSync(t *testing.T) {
	...
	openClaw, err := openclaw.NewOpenClawAdapter("spiffe://ohc.os/agent/openclaw-01")
	if err != nil {
		t.Fatalf("Failed to create OpenClaw adapter: %v", err)
	}
	autoGen, err := autogen.NewAutoGenAdapter("spiffe://ohc.os/agent/autogen-01")
	if err != nil {
		t.Fatalf("Failed to create AutoGen adapter: %v", err)
	}
	crewAI, err := crewai.NewCrewAIAdapter("spiffe://ohc.os/agent/crewai-01")
	if err != nil {
		t.Fatalf("Failed to create CrewAI adapter: %v", err)
	}
	semanticKernel, err := semantickernel.NewSemanticKernelAdapter("spiffe://ohc.os/agent/semantickernel-01")
	if err != nil {
	...
	for _, id := range invalidIDs {
		err := interop.ValidateSPIFFEID(id)
		if err == nil {
			t.Errorf("Expected error for invalid SPIFFE ID: %s", id)
		}
	}

	// Simulate shared K8s / LangGraph interop.State
	sharedState := &interop.State{
...
func TestExecuteCommand(t *testing.T) {
	...
	tests := []struct {
		name     string
		adapter  interop.UniversalAdapter
		cmd      string
		expected string
		wantErr  bool
	}{
		{
			...
			adapter:  func() interop.UniversalAdapter { a, _ := openclaw.NewOpenClawAdapter("spiffe://ohc.os/agent/openclaw-01"); return a }(),
			cmd:      "scan-metrics",
		...
		},
		{
			...
			adapter:  func() interop.UniversalAdapter { a, _ := autogen.NewAutoGenAdapter("spiffe://ohc.os/agent/autogen-01"); return a }(),
			cmd:      "analyze-data",
		...
		},
		{
			...
			adapter:  func() interop.UniversalAdapter { a, _ := crewai.NewCrewAIAdapter("spiffe://ohc.os/agent/crewai-01"); return a }(),
			cmd:      "research-topic",
		...
		},
		{
			...
			adapter: func() interop.UniversalAdapter {
				a, _ := semantickernel.NewSemanticKernelAdapter("spiffe://ohc.os/agent/semantickernel-01")
				return a
			}(),
			cmd:      "solve-problem",
...
func TestSyncStateNil(t *testing.T) {
	ctx := context.Background()
	openClaw, _ := openclaw.NewOpenClawAdapter("spiffe://ohc.os/agent/openclaw-01")
	err := openClaw.SyncState(ctx, nil)
	if err == nil {
		t.Errorf("Expected error when syncing nil state")
	}

	autoGen, _ := autogen.NewAutoGenAdapter("spiffe://ohc.os/agent/autogen-01")
	err = autoGen.SyncState(ctx, nil)
	if err == nil {
		t.Errorf("Expected error when syncing nil state")
	}

	crewAI, _ := crewai.NewCrewAIAdapter("spiffe://ohc.os/agent/crewai-01")
	err = crewAI.SyncState(ctx, nil)
	if err == nil {
		t.Errorf("Expected error when syncing nil state")
	}

	semanticKernel, _ := semantickernel.NewSemanticKernelAdapter("spiffe://ohc.os/agent/semantickernel-01")
...
func TestAdapters_InvalidIdentity(t *testing.T) {
	...
	tests := []struct {
		name    string
		factory func(id string) (interop.UniversalAdapter, error)
	}{
		{"autogen.NewAutoGenAdapter", func(id string) (interop.UniversalAdapter, error) { return autogen.NewAutoGenAdapter(id) }},
		{"crewai.NewCrewAIAdapter", func(id string) (interop.UniversalAdapter, error) { return crewai.NewCrewAIAdapter(id) }},
		{"openclaw.NewOpenClawAdapter", func(id string) (interop.UniversalAdapter, error) { return openclaw.NewOpenClawAdapter(id) }},
		{"semantickernel.NewSemanticKernelAdapter", func(id string) (interop.UniversalAdapter, error) { return semantickernel.NewSemanticKernelAdapter(id) }},
	}
...
func TestValidateSPIFFEID_EdgeCases(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            err := interop.ValidateSPIFFEID(tt.id)
            if (err != nil) != tt.wantErr {
                t.Errorf("interop.ValidateSPIFFEID() error = %v, wantErr %v", err, tt.wantErr)
            }
        })
    }
}
