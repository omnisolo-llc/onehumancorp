...
func BenchmarkStreamLatency(b *testing.B) {
	...
	hub.RegisterAgent(handoff.Agent{ID: "agent1", Status: handoff.StatusIdle})
	hub.RegisterAgent(handoff.Agent{ID: "agent2", Status: handoff.StatusIdle})
...
func BenchmarkPublish_Concurrent(b *testing.B) {
	...
	for i := 0; i < numAgents; i++ {
		hub.RegisterAgent(handoff.Agent{ID: fmt.Sprintf("agent%d", i), Status: handoff.StatusIdle})
	}
