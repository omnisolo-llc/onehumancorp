...
func NewSIPDB(dbPath string) (*SIPDB, error) {
	// For high-concurrency modernc.org/sqlite:
	// set WAL mode and busy_timeout to prevent SQLITE_BUSY
	dsn := dbPath
	if dbPath != ":memory:" {
...
func (s *SIPDB) GetPendingMissions(ctx context.Context, role string) ([]Message, error) {
	...
	err := withRetry(ctx, func() error {
		...
		for rows.Next() {
			...
			// We attempt to unmarshal into the wrapper first to cleanly filter by role.
			if err := json.Unmarshal([]byte(payloadStr), &wrapper); err == nil && wrapper.Role != "" {
				...
				// Legacy or fallback parsing
				if err := json.Unmarshal([]byte(payloadStr), &msg); err != nil {
					// String fallback
					if role != "" && payloadStr != role {
						continue
					}
