...
func TestPublish_ContextSummarization_Success(t *testing.T) {
	...
	hub.RegisterAgent(core.Agent{ID: "pm-1", Name: "PM", Role: "PRODUCT_MANAGER", OrganizationID: "org-1"})
	hub.RegisterAgent(core.Agent{ID: "swe-1", Name: "SWE", Role: "SOFTWARE_ENGINEER", OrganizationID: "org-1"})

	meeting := hub.OpenMeeting("kickoff", []string{"pm-1", "swe-1"})

	for i := 0; i < 16; i++ {
		err := hub.Publish(core.Message{
...
func TestStreamMessages_SendErrorOnInitialSend(t *testing.T) {
	hub := NewHub()
	hub.RegisterAgent(core.Agent{ID: "sender", Name: "Sender", Role: "R1", OrganizationID: "O1"})
	hub.RegisterAgent(core.Agent{ID: "receiver", Name: "Receiver", Role: "R2", OrganizationID: "O1"})
	server := NewHubServiceServer(hub)

	_ = hub.Publish(core.Message{
		ID:         "msg-1",
		FromAgent:  "sender",
		ToAgent:    "receiver",
		Type:       core.EventTask,
...
func TestService_CoverageExt(t *testing.T) {
	...
	hub.RegisterAgent(core.Agent{ID: "del-sender", Role: "ROUTER"})
	hub.RegisterAgent(core.Agent{ID: "del-receiver", Role: "WORKER"})

	time.Sleep(10 * time.Millisecond)

	err = hub.DelegateTask("del-sender", "del-receiver", core.Message{ID: "msg-del"})
	if err != nil {
	...
	largeSlice := make([]core.Message, 0, 2048)
	...
	err = hub.Publish(core.Message{FromAgent: "del-sender", ToAgent: "del-receiver"})
	if err != nil {
