...
func TestEventLogWorker_CoverageGaps(t *testing.T) {
	...
	for i := 0; i < cap(hub.eventLogChan); i++ {
		hub.eventLogChan <- handoff.Message{ID: "fill"}
	}
	...
	hub.LogEvent(handoff.Message{ID: "dropped"})
	...
	hub2 := &Hub{
		agents:        make(map[string]handoff.Agent),
		inbox:         make(map[string][]handoff.Message),
		meetings:      make(map[string]handoff.MeetingRoom),
	...
	// Send an event
	hub2.LogEvent(handoff.Message{ID: "m1", Content: "test content"})
