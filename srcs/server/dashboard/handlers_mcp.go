...
import (
	...
	"github.com/onehumancorp/mono/srcs/server/tools/fsmcp"
...
func (s *Server) invokeMCPTool(ctx context.Context, req mcpInvokeRequest) (map[string]any, error) {
	...
	case "fs-mcp":
		...
		fsProvider, err := fsmcp.NewProviderFactory("./")
		if err != nil {
			return nil, fmt.Errorf("failed to create fsmcp provider: %w", err)
		}

		inspector := fsmcp.NewFSMCP(fsProvider)
		var params map[string]interface{}
		if err := json.Unmarshal(req.Params, &params); err != nil {
...
func (s *Server) handleMcpRagSync(w http.ResponseWriter, r *http.Request) {
	ctx, span := otel.Tracer("github.com/onehumancorp/mono/srcs/server/dashboard").Start(r.Context(), "handleMcpRagSync")
	defer span.End()
