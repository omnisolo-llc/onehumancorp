...
func (s *Server) handleSecretsSyncUp(w http.ResponseWriter, r *http.Request) {
	...
	if s.hub != nil && s.hub.SIPDB() != nil {
		...
		if pool != nil {
			for _, secret := range payload.Secrets {
				...
				if key != "" && value != "" {
					_, err := pool.Exec(ctx, "INSERT INTO cloud_secrets (org_id, key, value) VALUES ($1, $2, $3) ON CONFLICT(org_id, key) DO UPDATE SET value = excluded.value", claims.OrganizationID, key, value)
					if err != nil {
						// Log the error but continue syncing other secrets
					}
				}
			}
		}
	...
	writeJSON(w, map[string]string{"status": "success", "message": "secrets synced up successfully"})
}

// handleSecretsSyncDown handles providing cloud secrets to standalone daemon.
func (s *Server) handleSecretsSyncDown(w http.ResponseWriter, r *http.Request) {
	ctx, span := otel.Tracer("github.com/onehumancorp/mono/srcs/server/dashboard").Start(r.Context(), "handleSecretsSyncDown")
	defer span.End()
	...
	writeJSON(w, map[string]interface{}{
		"status":  "success",
		"message": "secrets synced down successfully",
		"secrets": secrets,
	})
}
