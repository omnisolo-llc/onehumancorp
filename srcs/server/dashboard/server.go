...
func NewServer(org domain.Organization, hub *orchestration.Hub, tracker *billing.Tracker, authStore ...*auth.Store) http.Handler {
	...
	psAuth := auth.NewPowerSyncAuthHandlers(server.authHandlers.Store())
	mux.HandleFunc("/api/auth/powersync/token", psAuth.HandleToken)
	mux.HandleFunc("/api/auth/powersync/jwks", psAuth.HandleJWKS)
