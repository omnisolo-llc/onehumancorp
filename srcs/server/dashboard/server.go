...
func NewServer(org domain.Organization, hub *orchestration.Hub, tracker *billing.Tracker, authStore ...*auth.Store) http.Handler {
	...
	mux.HandleFunc("/api/auth/powersync/token", auth.RequireRole("viewer", auth.HandlePowerSyncToken()))
	mux.HandleFunc("/api/auth/powersync/jwks", auth.HandlePowerSyncJWKS())
