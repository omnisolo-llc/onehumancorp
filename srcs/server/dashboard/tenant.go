...
func (r *TenantRegistry) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if req.URL.Path == "/api/auth/login" || req.URL.Path == "/api/auth/register" || req.URL.Path == "/api/health/hybrid" || req.URL.Path == "/healthz" || req.URL.Path == "/readyz" || req.URL.Path == "/metrics" || req.URL.Path == "/login" || req.URL.Path == "/favicon.ico" || req.URL.Path == "/" || strings.HasPrefix(req.URL.Path, "/assets/") {
