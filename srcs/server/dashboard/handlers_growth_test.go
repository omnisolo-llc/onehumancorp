...
func TestHandleTeamInvites(t *testing.T) {
	...
	// Test POST
	payload := `{"inviterId": "user-123", "inviteeEmail": "peer@example.com"}`
	...
	if created.InviteeEmail != "peer@example.com" {
		t.Errorf("expected inviteeEmail 'peer@example.com', got '%s'", created.InviteeEmail)
	}
