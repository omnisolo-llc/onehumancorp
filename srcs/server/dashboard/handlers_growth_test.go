...
func TestHandleTeamInvites(t *testing.T) {
	...
	// Test POST
	payload := `{"inviterId": "user-1", "inviteeEmail": "colleague@example.com"}`
	...
	if created.InviterID != "user-1" {
		t.Errorf("expected inviterId 'user-1', got '%s'", created.InviterID)
	}
	if created.InviteeEmail != "colleague@example.com" {
		t.Errorf("expected inviteeEmail 'colleague@example.com', got '%s'", created.InviteeEmail)
	}
