...
func TestHandleSecretsSyncUp(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/secrets/sync/up", nil)
...
func TestHandleSecretsSyncDown(t *testing.T) {
	org := domain.NewSoftwareCompany("test-org", "Test Org", "CEO", time.Now())
	hub := orchestration.NewHub()
	defer hub.Close()
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/secrets/sync/down", nil)
	rr := httptest.NewRecorder()
