	...
	if sipdbErr == nil {
		...
		if os.Getenv("OHC_STANDALONE") == "true" {
			...
			if cloudEndpoint != "" && envBoolDefault("OHC_TELEMETRY_ENABLED", false) {
				telemetryEndpoint := strings.TrimRight(cloudEndpoint, "/") + "/api/telemetry/sync"
				telemetry.StartSyncDaemon(ctx, sipdb.SyncBufferedMetrics, telemetryEndpoint, 5*time.Minute)
			}
