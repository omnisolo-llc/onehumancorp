...
func (w *McpSyncWorker) syncOnce(ctx context.Context) {
	...
	rows, err := w.provider.DB().QueryContext(ctx, "SELECT id, metric_name, value FROM local_telemetry_buffer WHERE sync_status = 'pending' LIMIT 100")
	if err != nil {
		log.Printf("McpSyncWorker failed to query local_telemetry_buffer: %v", err)
		return
	}
	defer rows.Close()
	...
	for _, id := range pendingIDs {
		_, err := w.provider.DB().ExecContext(ctx, "UPDATE local_telemetry_buffer SET sync_status = 'synced' WHERE id = ?", id)
		if err != nil {
