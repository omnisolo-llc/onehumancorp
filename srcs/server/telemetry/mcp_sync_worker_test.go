...
func TestMcpSyncWorker_SyncOnce(t *testing.T) {
	...
	id1 := uuid.New().String()
	_, err = provider.DB().Exec(`
		INSERT INTO local_telemetry_buffer (id, metric_name, value, labels_json, timestamp, sync_status)
		VALUES (?, ?, ?, ?, ?, 'pending')
	...
	// Verify sync status
	var status string
	err = provider.DB().QueryRow("SELECT sync_status FROM local_telemetry_buffer WHERE id = ?", id1).Scan(&status)
	if err != nil {
