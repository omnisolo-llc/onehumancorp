...
func TestStandaloneTelemetryPIIDoesNotMutateOriginal(t *testing.T) {
	...
	_, err = db.Exec("CREATE TABLE local_telemetry_buffer (id TEXT PRIMARY KEY, metric_name TEXT, value REAL, labels_json TEXT, timestamp DATETIME, sync_status TEXT)")
	if err != nil {
	...
	// Read from db
	var payload string
	err = db.QueryRow("SELECT labels_json FROM local_telemetry_buffer LIMIT 1").Scan(&payload)
	if err != nil {
