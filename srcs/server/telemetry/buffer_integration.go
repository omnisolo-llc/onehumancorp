...
func bufferMetricHelper(ctx context.Context, name string, value float64, attrs map[string]interface{}) {
	if globalSyncEngine != nil && isTelemetryEnabled() {
		...
		if isStandalone {
			redactedAttrs, ok := redactInterfacePII(attrs).(map[string]interface{})
			if !ok {
				// fail closed on redaction error to prevent leakage
				_ = globalSyncEngine.BufferMetric(ctx, name, value, nil)
				return
			}
