...
// CRDTPush pushes local unsynced CRDT deltas to the cloud.
func (p *DBStateSyncProvider) CRDTPush(ctx context.Context, claims *auth.Claims) (map[string]interface{}, error) {
	if p.dbWrapper == nil || !p.dbWrapper.IsSQLite() {
		return nil, fmt.Errorf("local database not configured or not running in standalone mode")
	}

	rows, err := p.dbWrapper.Query(ctx, "SELECT id, entity_id, data, updated_at FROM crdt_deltas WHERE synced_to_cloud = false LIMIT 50")
	if err != nil {
		return nil, fmt.Errorf("query unsynced crdt deltas: %w", err)
	}
	defer rows.Close()
	...
	if len(deltas) == 0 {
		return map[string]interface{}{
			"status": "success",
			"synced_count": 0,
			"message": "No pending CRDT deltas to push.",
		}, nil
	}

	_, err = p.sendToCloud(ctx, "/api/v1/sync/mcp-deltas", http.MethodPost, map[string]interface{}{
		"deltas": deltas,
	}, claims)

	if err != nil {
		return nil, fmt.Errorf("crdt push to cloud failed: %w", err)
	}

	for _, id := range ids {
		_, _ = p.dbWrapper.Exec(ctx, "UPDATE crdt_deltas SET synced_to_cloud = true WHERE id = $1", id)
	}
...
// CRDTPull pulls CRDT deltas from the cloud.
func (p *DBStateSyncProvider) CRDTPull(ctx context.Context, claims *auth.Claims) (map[string]interface{}, error) {
	if p.dbWrapper == nil || !p.dbWrapper.IsSQLite() {
		return nil, fmt.Errorf("local database not configured or not running in standalone mode")
	}

	body, err := p.sendToCloud(ctx, "/api/v1/sync/mcp-deltas", http.MethodGet, nil, claims)
	if err != nil {
		return nil, fmt.Errorf("crdt pull from cloud failed: %w", err)
	}

	var response map[string]interface{}
	if err := json.Unmarshal(body, &response); err != nil {
		return nil, fmt.Errorf("failed to parse crdt pull payload: %w", err)
	}

	if deltas, ok := response["deltas"].([]interface{}); ok {
		for _, d := range deltas {
			if deltaMap, ok := d.(map[string]interface{}); ok {
				id, _ := deltaMap["id"].(string)
				entityID, _ := deltaMap["entity_id"].(string)
				data, _ := deltaMap["data"].(string)
				updatedAtStr, _ := deltaMap["updated_at"].(string)

				if id != "" && entityID != "" && updatedAtStr != "" {
					updatedAt, err := time.Parse(time.RFC3339, updatedAtStr)
					if err == nil {
						_, _ = p.dbWrapper.Exec(ctx, "INSERT INTO crdt_deltas (id, entity_id, data, updated_at, synced_to_cloud) VALUES ($1, $2, $3, $4, true) ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at, synced_to_cloud = true", id, entityID, data, updatedAt)
					}
				}
			}
		}
	...
	return map[string]interface{}{
		"status": "success",
		"message": "CRDT Pull completed successfully.",
		"data": response,
	}, nil
}
