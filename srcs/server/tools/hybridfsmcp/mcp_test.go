...
func (m *MockProvider) SearchFiles(ctx context.Context, path, pattern string) ([]string, error) {
    return []string{"mock.txt"}, nil
}
...
func TestHybridFSMCP(t *testing.T) {
    ...
    // Search
    res, err = mcp.CallTool(ctx, "search_files", map[string]interface{}{"path": ".", "pattern": "mock"})
    if err != nil {
        t.Fatalf("unexpected error: %v", err)
    }
    resMap = res.(map[string]interface{})
