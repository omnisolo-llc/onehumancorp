...
func (m *mockDBProviderTools) Query(ctx context.Context, sql string, optionsAndArgs ...any) (db.Rows, error) {
	...
	m.queryCalls++
	return &mockRowToolss{count: 1}, nil
}
...
type mockRowToolss struct { count int }
func (m *mockRowToolss) Close() {}
func (m *mockRowToolss) Err() error { return nil }
func (m *mockRowToolss) Next() bool {
	if m.count > 0 {
		m.count--
		return true
	}
...
func (m *mockRowToolss) Scan(dest ...any) error {
	if len(dest) == 3 {
...
func (m *mockRowToolss) Columns() ([]string, error) { return nil, nil }
...
// The uncovered lines are in filepath.WalkDir's walkFn:
// 39: relPath error -> error from filepath.Rel
// 44: info error -> error from d.Info()
...
func TestWorkspaceSyncTool_WalkFn_Coverage(t *testing.T) {
	...
	// The prompt says: "Write 100% coverage unit tests in srcs/server/interop/mcp/tools_test.go mocking the proxy database."
