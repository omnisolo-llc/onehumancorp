...
func TestProvider_ClaimTask_Postgres(t *testing.T) {
	...
	err = provider.ClaimTask(context.Background(), "tenant-1", "task-1")
	assert.NoError(t, err)
	assert.NoError(t, mock.ExpectationsWereMet())

	// Test lock already acquired by another redis client
	mr.Set("task_lock:task-2", "locked")
	err = provider.ClaimTask(context.Background(), "tenant-1", "task-2")
	assert.Error(t, err)
...
func TestProvider_ClaimTask_SQLite(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT status FROM tasks WHERE id = \? AND tenant_id = \?`).
		WithArgs("task-1", "tenant-1").
		WillReturnRows(sqlmock.NewRows([]string{"status"}).AddRow("PENDING"))
	mock.ExpectExec(`UPDATE tasks SET status = 'IN_PROGRESS', updated_at = CURRENT_TIMESTAMP WHERE id = \? AND tenant_id = \? AND status = 'PENDING'`).
		WithArgs("task-1", "tenant-1").
		WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestProvider_ClaimTask_Postgres_Errors(t *testing.T) {
	...
	err = provider.ClaimTask(context.Background(), "tenant-1", "task-tx")
	assert.Error(t, err)
	...
	err = provider2.ClaimTask(context.Background(), "tenant-1", "task-q")
	assert.Error(t, err)
	...
	err = provider3.ClaimTask(context.Background(), "tenant-1", "task-miss")
	assert.Error(t, err)
	...
	err = provider4.ClaimTask(context.Background(), "tenant-1", "task-claimed")
	assert.Error(t, err)
...
func TestProvider_ClaimTask_SQLite_Errors(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE tasks SET status = 'IN_PROGRESS', updated_at = CURRENT_TIMESTAMP WHERE id = \? AND tenant_id = \? AND status = 'PENDING'`).WillReturnError(errors.New("exec error"))
	err = provider.ClaimTask(context.Background(), "tenant-1", "task-1")
	assert.Error(t, err)
}
...
func TestProvider_CreateTask_Postgres_Errors(t *testing.T) {
	...
	task := &Task{ID: "task-err", TenantID: "tenant-err", Status: "PENDING"}

	mock.ExpectQuery(`INSERT INTO tasks \(id, tenant_id, status, created_at, updated_at\)`).
		WithArgs("task-err", "tenant-err", "PENDING").
...
func TestProvider_ClaimTask_TenantIsolation_Exploit(t *testing.T) {
	os.Setenv("OHC_STANDALONE", "false")
	defer os.Unsetenv("OHC_STANDALONE")
	...
	// Try to claim task-1 with the WRONG tenant-id (tenant-2 instead of tenant-1)
	mock.ExpectBegin()
	mock.ExpectQuery(`SELECT status FROM tasks WHERE id = \$1 AND tenant_id = \$2 FOR UPDATE SKIP LOCKED`).
		WithArgs("task-1", "tenant-2").
		WillReturnError(sql.ErrNoRows) // Emulate isolation filtering out the row

	err = provider.ClaimTask(context.Background(), "tenant-2", "task-1")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "task not found")
}
