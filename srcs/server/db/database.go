...
func (p *DB) RunMigrations(ctx context.Context) error {
	...
	for _, f := range files {
		...
		if p.Provider.IsSQLite() {
			...
			sqlStr = strings.ReplaceAll(sqlStr, "vector(1536)", "TEXT")
			sqlStr = strings.ReplaceAll(sqlStr, "UUID REFERENCES", "TEXT REFERENCES")
			...
			sqlStr = strings.ReplaceAll(sqlStr, "ohc_memory.autodream_vectors", "ohc_memory_autodream_vectors")
			sqlStr = strings.ReplaceAll(sqlStr, "ohc_tasks.mission_queue", "ohc_tasks_mission_queue")
			sqlStr = strings.ReplaceAll(sqlStr, "ohc_tasks.sub_agent_queue", "ohc_tasks_sub_agent_queue")
