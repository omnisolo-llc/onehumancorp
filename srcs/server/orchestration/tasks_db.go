...
type TaskStoreV4 interface {
	ClaimTaskV4(ctx context.Context, organizationID string, agentID string) (*SharedTaskV4, error)
}

type PostgresTaskStoreV4 struct {
	db *sql.DB
}

func NewPostgresTaskStoreV4(db *sql.DB) *PostgresTaskStoreV4 {
	return &PostgresTaskStoreV4{db: db}
}

func (s *PostgresTaskStoreV4) ClaimTaskV4(ctx context.Context, organizationID string, agentID string) (*SharedTaskV4, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
	...
	query := `SELECT id, organization_id, title, status, dependencies FROM shared_tasks_v4 WHERE status = 'PENDING' AND organization_id = $1 FOR UPDATE SKIP LOCKED LIMIT 1`
	row := tx.QueryRowContext(ctx, query, organizationID)

	task := &SharedTaskV4{}
	err = row.Scan(&task.ID, &task.OrganizationID, &task.Title, &task.Status, &task.Dependencies)
	if err != nil {
		return nil, err
	}

	updateQuery := `UPDATE shared_tasks_v4 SET status = 'IN_PROGRESS' WHERE id = $1`
...
type SqliteTaskStoreV4 struct {
	db *sql.DB
	mu sync.Mutex
}

func NewSqliteTaskStoreV4(db *sql.DB) *SqliteTaskStoreV4 {
	return &SqliteTaskStoreV4{db: db}
}

func (s *SqliteTaskStoreV4) ClaimTaskV4(ctx context.Context, organizationID string, agentID string) (*SharedTaskV4, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	query := `SELECT id, organization_id, title, status, dependencies FROM shared_tasks_v4 WHERE status = 'PENDING' AND organization_id = ? LIMIT 1`
	row := tx.QueryRowContext(ctx, query, organizationID)
	...
	updateQuery := `UPDATE shared_tasks_v4 SET status = 'IN_PROGRESS' WHERE id = ?`
	_, err = tx.ExecContext(ctx, updateQuery, task.ID)
	if err != nil {
		return nil, err
	}
