...
func (to *SharedTaskOrchestrator) ClaimDecompositionTask(ctx context.Context, agentID string) (*SharedTaskDecomposition, error) {
    ...
    if to.dbProvider.IsSQLite() {
        return to.claimDecompositionTaskSQLite(ctx, claims.OrganizationID, agentID)
    }
    return to.claimDecompositionTaskPostgres(ctx, claims.OrganizationID, agentID)
}

func (to *SharedTaskOrchestrator) claimDecompositionTaskSQLite(ctx context.Context, orgID, agentID string) (*SharedTaskDecomposition, error) {
    to.mu.Lock()
    defer to.mu.Unlock()
    ...
    query := `
        ...
        AND NOT EXISTS (
            ...
            WHERE d.status != 'COMPLETED' AND d.status != 'DONE'
        )
        LIMIT 1
    `
    row := tx.QueryRow(ctx, query, orgID)
...
func (to *SharedTaskOrchestrator) claimDecompositionTaskPostgres(ctx context.Context, orgID, agentID string) (*SharedTaskDecomposition, error) {
    tx, err := to.dbProvider.Begin(ctx)
    if err != nil {
        return nil, fmt.Errorf("failed to begin transaction: %w", err)
    }
