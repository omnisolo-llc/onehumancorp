...
// WithAutoDream adds autodream memory consolidator
func (to *SharedTaskOrchestrator) WithAutoDream(ad autodream.MemoryConsolidator) *SharedTaskOrchestrator {
	to.autodream = ad
	return to
}

// WithMesh adds mesh transport
func (to *SharedTaskOrchestrator) WithMesh(mesh MeshTransport) *SharedTaskOrchestrator {
	to.mesh = mesh
	return to
}
...
func (to *SharedTaskOrchestrator) TransitionTask(ctx context.Context, taskID, agentID, fromState, toState, reason string) error {
	...
	if toState == "COMPLETED" {
		// Fetch payload and deliberation_log
		var payloadText string
		var deliberationLog string
		_ = to.dbProvider.QueryRow(context.Background(), "SELECT COALESCE(payload, '{}'), COALESCE(deliberation_log, '{}') FROM shared_tasks_v4 WHERE id = $1", taskID).Scan(&payloadText, &deliberationLog)

		if to.autodream != nil {
			go func() {
				logs := []string{"Task " + taskID + " completed successfully."}
				if payloadText != "" && payloadText != "{}" {
					logs = append(logs, "Payload: "+payloadText)
				}
				if deliberationLog != "" {
