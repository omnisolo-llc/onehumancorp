...
func (to *SharedTaskOrchestrator) claimTaskSQLite(ctx context.Context, orgID, agentID string) (*SharedTaskDB, error) {
    ...
    query := `
        ...
        AND NOT EXISTS (
            ...
            JOIN shared_tasks_v4 d ON d.id = td.depends_on_task_id
    ...
    if _, err = tx.Exec(ctx, "UPDATE shared_tasks_v4 SET status = 'IN_PROGRESS', assigned_agent_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2", agentID, task.ID); err != nil {
