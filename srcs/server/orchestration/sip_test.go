...
func TestSIPDB_PruneStaleMissions_RequeueStuck(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "test.db")
	db, err := NewSIPDB(dbPath)
	if err != nil {
		t.Fatalf("Failed to create test DB: %v", err)
	...
	_, err = db.db.Exec(ctx, "INSERT INTO agent_missions (id, status, payload, organization_id) VALUES ('m-stuck', 'STUCK', '{}', 'system')")
	if err != nil {
		t.Fatalf("Failed to insert: %v", err)
	}
	...
	err = db.db.QueryRow(ctx, "SELECT status, payload FROM agent_missions WHERE id = 'm-stuck'").Scan(&status, &payload)
	if err != nil {
	...
	// It should have retry_count: 1
	var payloadMap map[string]interface{}
	if err := json.Unmarshal([]byte(payload), &payloadMap); err != nil {
		t.Fatalf("Failed to unmarshal payload: %v", err)
	}

	retryCount := int(payloadMap["retry_count"].(float64))
	if retryCount != 1 {
