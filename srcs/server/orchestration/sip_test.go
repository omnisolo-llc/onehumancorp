...
func TestSIPDB_NewSIPDB_Fail(t *testing.T) {
	...
	_, err := NewSIPDBFromPath("/root/illegal/path/db.sqlite")
	if err == nil {
...
func TestSIPDB_PollMissions_ScanError(t *testing.T) {
	...
	ctx := context.Background()
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('123', 'PENDING', '{\"role\":\"SOFTWARE_ENGINEER\",\"task\":\"invalid-json\"}')")
	if err != nil {
...
func TestSIPDB_CompleteMission_ExecError(t *testing.T) {
	...
	_, err = db.provider.Exec(context.Background(), "DROP TABLE agent_missions")
	if err != nil {
...
func TestSIPDB_PruneStaleMissions(t *testing.T) {
	...
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload, created_at) VALUES ('1', 'PENDING', '{\"role\":\"ROLE\",\"task\":\"task\"}', datetime('now'))")
	if err != nil {
		t.Fatal(err)
	}

	// 2. Completed (should be deleted regardless of age)
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload, created_at) VALUES ('2', 'COMPLETED', '{\"role\":\"ROLE\",\"task\":\"task\"}', datetime('now'))")
	if err != nil {
		t.Fatal(err)
	}

	// 3. Pending but old (should be deleted)
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload, created_at) VALUES ('3', 'PENDING', '{\"role\":\"ROLE\",\"task\":\"task\"}', datetime('now', '-2 days'))")
	if err != nil {
		t.Fatal(err)
	}
	...
	var count int
	err = db.provider.QueryRow(ctx, "SELECT COUNT(*) FROM agent_missions").Scan(&count)
	if err != nil {
		t.Fatal(err)
	}
	...
	var id string
	err = db.provider.QueryRow(ctx, "SELECT id FROM agent_missions").Scan(&id)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSIPDB_GetPendingMissions_ScanError2(t *testing.T) {
	...
	_, err = db.provider.Exec(ctx, "DROP TABLE agent_missions")
	_, err = db.provider.Exec(ctx, "CREATE TABLE agent_missions (id TEXT PRIMARY KEY)")
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('123', 'PENDING', '{\"role\":\"SOFTWARE_ENGINEER\"}')")
...
func TestSIPDB_ScanErrors(t *testing.T) {
	db, err := NewSIPDBFromPath(filepath.Join(t.TempDir(), "test_scan.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	_, _ = db.provider.Exec(ctx, "INSERT INTO capability_plugins (plugin_id, name, version, manifest_url, status, registered_at) VALUES (NULL, 'name', '1', 'url', 'stat', 'time')")
	_, _ = db.GetCapabilityPlugins(ctx, "")
	_, _ = db.provider.Exec(ctx, "DELETE FROM capability_plugins")

	_, _ = db.provider.Exec(ctx, "INSERT INTO swarm_memory_embeddings (memory_id, context, vector_embedding, source_plugin, created_at) VALUES (NULL, 'ctx', 'vec', 'plg', 'time')")
	_, _ = db.GetEpisodicMemoriesByPlugin(ctx, "")
	_, _ = db.provider.Exec(ctx, "DELETE FROM swarm_memory_embeddings")

	_, _ = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES (NULL, 'PENDING', '{\"role\":\"ROLE\",\"task\":\"task\"}')")
	_, _ = db.GetPendingMissions(ctx, "ROLE")
	_, _ = db.provider.Exec(ctx, "DELETE FROM agent_missions")
...
func TestSIPDB_GetPendingMissions_Fallbacks(t *testing.T) {
	...
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('m-1', 'PENDING', '{\"role\":\"TESTER1\",\"Content\":\"Direct Content\"}')")
	if err != nil {
	...
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('m-3', 'PENDING', '{\"role\":\"TESTER2\",\"task\":\"some string\"}')")
	if err != nil {
		t.Fatalf("Failed to insert: %v", err)
	}
