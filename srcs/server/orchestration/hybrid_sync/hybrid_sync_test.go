...
func TestMissionSynchronizer_SyncLocalToCloud(t *testing.T) {
	// Setup SQLite in-memory db
	sqlDB, err := sql.Open("sqlite", ":memory:")
	if err != nil {
		t.Fatalf("failed to open sqlite db: %v", err)
	...
	// Mock cloud API
	var receivedMission AgentMission
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/sync/mission" && r.Method == http.MethodPost {
	...
	sync := NewDefaultMissionSynchronizer(dbWrapper, srv.URL)

	mission := &AgentMission{
		ID:             "mission_1",
		OrganizationID: "org_1",
		Status:         "PENDING",
		Payload:        `{"rag_context":"secret email is test@example.com"}`,
		CreatedAt:      time.Now(),
		UpdatedAt:      time.Now(),
	}

	err = sync.SyncLocalToCloud(context.Background(), mission)
	if err != nil {
		t.Fatalf("failed to sync local to cloud: %v", err)
	}

	expectedPayload := `{"rag_context":"secret email is [REDACTED_EMAIL]"}`
	if receivedMission.Payload != expectedPayload {
		t.Errorf("expected payload %q, got %q", expectedPayload, receivedMission.Payload)
	}
}
