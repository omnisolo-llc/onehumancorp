...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		// SQLite doesn't support UPDATE RETURNING with LIMIT, so we use SELECT then UPDATE
		selectQuery := `
			SELECT st.id
			FROM shared_tasks st
		...
		errQuery = tx.QueryRow(ctx, selectQuery, taskID, claims.OrganizationID).Scan(&selectedID)
		...
		if errQuery == nil {
			...
			errQuery = tx.QueryRow(ctx, updateQuery, agentID, selectedID).Scan(
		...
		// PostgreSQL with UPDATE ... RETURNING ... SKIP LOCKED to prevent TOCTOU
		query := `
			UPDATE shared_tasks
			SET status = 'IN_PROGRESS', agent_id = $1, updated_at = CURRENT_TIMESTAMP
			WHERE id = (
...
func (tm *TaskManager) CompleteTask(ctx context.Context, taskID, agentID string) error {
	...
	`
	var updatedID string
	err = tx.QueryRow(ctx, query, taskID, agentID, claims.OrganizationID).Scan(&updatedID) // Use = here because err was declared above
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	// Find dependent tasks that might now be unblocked (their dependencies are met)
	// For shared_tasks, status remains PENDING but they are no longer blocked by this task
	depQuery := `
		SELECT t.id
		FROM shared_tasks t
	...
	for _, pid := range pendingTasks {
		...
		if err == nil && pendingCount == 0 {
			// All dependencies completed. It's still 'PENDING' but now it's unblocked.
			// It will be picked up by the next PollTasks / ClaimTask.
	...
	if tm.hub != nil {
		go func() {
			...
			// Optionally broadcast unblocked tasks
