...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		...
		// We found a task to claim, so let's transition it using the state machine
		err = tm.stateMachine.TransitionWithTx(ctx, tx, fetchedTaskID, "SHARED_TASK", statemachine.StateInProgress, agentID, "Task claimed")
		if err != nil {
			return nil, fmt.Errorf("failed to transition task to IN_PROGRESS: %w", err)
		}
		...
		`
		errQuery = tx.QueryRow(ctx, selectQuery, fetchedTaskID).Scan(
...
func (tm *TaskManager) PollTasks(ctx context.Context, agentID string, limit int) ([]*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		...
		err = tm.stateMachine.BulkTransitionWithTx(ctx, tx, taskIDs, "SHARED_TASK", statemachine.StateInProgress, agentID, "Task polled and claimed")
		if err != nil {
			return nil, fmt.Errorf("failed to bulk transition tasks to IN_PROGRESS: %w", err)
		}

		// build IN clause for selection
		var placeholders2 []string
		var args2 []interface{}
		for i, id := range taskIDs {
			placeholders2 = append(placeholders2, fmt.Sprintf("$%d", i+1))
		...
		taskRows, errQuery := tx.Query(ctx, selectQuery, args2...)
		if errQuery != nil {
			return nil, fmt.Errorf("failed to query claimed tasks: %w", errQuery)
		}
