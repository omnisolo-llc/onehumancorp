...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		// SQLite doesn't support FOR UPDATE. To prevent TOCTOU, we use a single UPDATE ... RETURNING statement
		// leveraging a subquery to find the ID.
		query := `
			UPDATE shared_tasks
	...
	if errQuery != nil {
		if errors.Is(errQuery, sql.ErrNoRows) || errQuery.Error() == "no rows in result set" {
