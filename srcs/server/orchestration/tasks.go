...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	// State machine transition for the task
	_, errMachine := tm.stateMachine.TransitionWithTx(ctx, tx, fetchedTaskID, "SHARED_TASK", statemachine.StateInProgress, agentID, "Task claimed by agent")
	if errMachine != nil {
		return nil, fmt.Errorf("failed to transition state: %w", errMachine)
	}

	// Fetch updated_at since the state machine updated it
	err = tx.QueryRow(ctx, `SELECT updated_at FROM shared_tasks WHERE id = $1`, fetchedTaskID).Scan(&task.UpdatedAt)
	if err != nil {
		return nil, fmt.Errorf("failed to read updated_at after transition: %w", err)
	}
	task.Status = statemachine.StateInProgress
