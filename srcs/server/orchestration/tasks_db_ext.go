...
type SharedTaskDBManager struct {
	dbProvider db.Provider
	mu         sync.Mutex // For SQLite concurrent assignment locking
}

func NewSharedTaskDBManager(dbProvider db.Provider) *SharedTaskDBManager {
	return &SharedTaskDBManager{
		dbProvider: dbProvider,
	}
}

// ClaimTask attempts to claim a task. Returns the task ID and an error if one occurred.
func (to *SharedTaskDBManager) ClaimTask(ctx context.Context, agentID string) (*SharedTaskDB, error) {
...
func (to *SharedTaskDBManager) claimTaskSQLite(ctx context.Context, orgID, agentID string) (*SharedTaskDB, error) {
	to.mu.Lock()
	defer to.mu.Unlock()
...
func (to *SharedTaskDBManager) claimTaskPostgres(ctx context.Context, orgID, agentID string) (*SharedTaskDB, error) {
	tx, err := to.dbProvider.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to begin transaction: %w", err)
	}
