...
func (m *UltraPlanManager) UpdatePlanStatus(ctx context.Context, planID string, newStatus string, stateMachine map[string]interface{}) error {
	...
	err = tx.QueryRow(ctx, query, planID).Scan(&currentStatus, &currentStateMB, &updatedAt)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	currentStateMB, err = decompressStateMachine(currentStateMB)
	if err == nil {
		var oldStateMachine map[string]interface{}
		if err := json.Unmarshal(currentStateMB, &oldStateMachine); err == nil {
