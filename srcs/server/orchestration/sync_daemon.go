...
func (d *HybridMCPRAGDaemon) SyncPendingMissions(ctx context.Context) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.id, &m.status, &m.payload); err != nil {
			log.Printf("sync_daemon: failed to scan mission row: %v", err)
			continue
		}
	...
	for _, m := range missions {
		...
		if err != nil {
			...
			if dbErr != nil {
				log.Printf("sync_daemon: failed to update sync_error for mission %s: %v", m.id, dbErr)
			}
			continue
		}
		...
		<-throttleSemaphore
		if err != nil {
			log.Printf("sync_daemon: failed to mark mission %s as synced: %v", m.id, err)
			continue
		}
	...
	if syncedCount > 0 {
		log.Printf("sync_daemon: successfully synced agent_missions: count=%d", syncedCount)
	}
