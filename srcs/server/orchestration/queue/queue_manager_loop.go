...
func (q *QueueManager) MarkCompleted(ctx context.Context, jobID string) error {
	if q.sm != nil {
		return q.sm.Transition(ctx, jobID, "SUB_AGENT_JOB", statemachine.StateCompleted, "", "SUBAGENT_COMPLETED")
	}
...
func (q *QueueManager) MarkFailed(ctx context.Context, jobID string, reason string) error {
	if q.sm != nil {
		return q.sm.Transition(ctx, jobID, "SUB_AGENT_JOB", statemachine.StateFailed, "", reason)
	}
