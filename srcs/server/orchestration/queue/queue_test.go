...
func TestQueueManager(t *testing.T) {
	...
	if err := qm.Enqueue(ctx, job); err != nil {
		t.Fatalf("Enqueue failed: %v", err)
	}

	dequeued, err := qm.Poll(ctx, []string{"worker"})
	if err != nil {
	...
	if dequeued.ID != "manager-job-1" {
		t.Fatalf("Expected job ID manager-job-1, got %s", dequeued.ID)
	}

	if err := qm.Complete(ctx, "manager-job-1"); err != nil {
		t.Fatalf("Complete failed: %v", err)
	}
}
