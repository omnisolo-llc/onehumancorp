...
// TaskQueueService manages pushing, claiming (with locking), and completing tasks
type TaskQueueService struct {
	db db.Provider
}

// NewTaskQueueService creates a new TaskQueueService
func NewTaskQueueService(db db.Provider) *TaskQueueService {
	return &TaskQueueService{db: db}
}

// PushTask pushes a new task to the queue
func (s *TaskQueueService) PushTask(ctx context.Context, title string, payload json.RawMessage, epicID string) (string, error) {
	id := generateID()
	...
	query := `
		INSERT INTO shared_tasks (id, title, status, payload, epic_id, created_at, updated_at)
		VALUES ($1, $2, 'PENDING', $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
	`
	_, err := s.db.Exec(ctx, query, id, title, payload, epic)
	if err != nil {
...
func (s *TaskQueueService) ClaimTask(ctx context.Context, agentID string) (*SharedTask, error) {
	...
	var query string
	if s.db.IsSQLite() {
		// SQLite fallback implementation for FOR UPDATE SKIP LOCKED
		query = `
			UPDATE shared_tasks
	...
	err := s.db.QueryRow(ctx, query, agentID).Scan(
		&task.ID, &task.ParentID, &task.EpicID, &task.Title,
		&task.Status, &task.AssignedAgent, &payloadStr,
...
func (s *TaskQueueService) CompleteTask(ctx context.Context, taskID string) error {
	query := `
		...
		WHERE id = $1
	`
	res, err := s.db.Exec(ctx, query, taskID)
	if err != nil {
