...
func TestPostgresTaskStore_ReportMissionHandover_Success(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	...
	mock.ExpectExec("UPDATE agent_missions.*SET status = 'blocked'").
		WithArgs("my blockers", "mission-1").
		WillReturnResult(sqlmock.NewResult(1, 1))

	err = store.ReportMissionHandover(context.Background(), "mission-1", "my blockers")
	assert.NoError(t, err)
}

func TestPostgresTaskStore_ReportMissionHandover_Error(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
...
func TestSqliteTaskStore_ReportMissionHandover_Success(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
...
func TestSqliteTaskStore_ReportMissionHandover_Error(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
