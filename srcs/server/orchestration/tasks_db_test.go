...
func TestSharedTaskOrchestrator_ClaimTask_WithDependencies(t *testing.T) {
	dbProvider, err := db.NewSqliteProvider("file::memory:?cache=shared")
	if err != nil {
		t.Fatalf("failed to create sqlite provider: %v", err)
	}
	...
	// Claiming should only return 'task-parent' since 'task-child' depends on it
	task1, err := to.ClaimTask(ctxWithClaims, "agent-1")
	if err != nil {
		t.Fatalf("ClaimTask failed: %v", err)
	}
	if task1 == nil {
	...
	if task1.ID != "task-parent" {
		t.Errorf("expected task ID 'task-parent', got '%s'", task1.ID)
	}

	// Claiming again should return nil since 'task-child' is blocked
	task2, err := to.ClaimTask(ctxWithClaims, "agent-2")
	if err != nil {
		t.Fatalf("ClaimTask failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("failed to update parent task status: %v", err)
	}

	// Claiming should now return 'task-child'
