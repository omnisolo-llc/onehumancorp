...
func (s *SIPDB) SyncMissions(ctx context.Context, remoteEndpoint string) (int, error) {
	...
	err := withSipRetry(ctx, func() error {
		...
		tasks := make([]func() error, len(missions))
		for i, m := range missions {
			m := m
			tasks[i] = func() error {
				var rawData interface{}
				if err := json.Unmarshal([]byte(m.payload), &rawData); err != nil {
					slog.Warn("Failed to unmarshal mission payload for sanitization", "mission_id", m.id)
					if syncMissionsErr != nil { syncMissionsErr.Add(ctx, 1) }
					return nil
				}
				...
				if err != nil {
					slog.Warn("Failed to marshal sanitized payload", "mission_id", m.id)
					if syncMissionsErr != nil { syncMissionsErr.Add(ctx, 1) }
					return nil
				}
				...
				if err == nil {
					if (resp.StatusCode >= 200 && resp.StatusCode < 300) || resp.StatusCode == http.StatusConflict {
						if _, updateErr := tx.Exec(ctx, "UPDATE agent_missions SET status = \047SYNCED\047 WHERE id = $1", m.id); updateErr == nil {
							syncedCount++
							if syncMissionsOk != nil { syncMissionsOk.Add(ctx, 1) }
						} else if syncMissionsErr != nil { syncMissionsErr.Add(ctx, 1) }
					} else if syncMissionsErr != nil { syncMissionsErr.Add(ctx, 1) }
					resp.Body.Close()
				} else {
					if syncMissionsErr != nil { syncMissionsErr.Add(ctx, 1) }
