...
func (s *SIPDB) CompleteMission(ctx context.Context, missionID string) error {
	...
	err := withSipRetry(ctx, func() error {
		...
		errStatus := s.db.QueryRow(ctx, "SELECT status, updated_at FROM agent_missions WHERE id = $1 AND organization_id = $2", missionID, s.orgID).Scan(&oldStatus, &updatedAt)
		if errStatus == nil && oldStatus != "COMPLETED" {
			shouldRecord = true
			transitionDuration = time.Since(updatedAt)
		}
	...
	if err == nil && shouldRecord {
		transitionType := strings.ToLower(oldStatus) + "_to_completed"
		telemetry.RecordAgentTransitionLatency(ctx, transitionType, transitionDuration)
	}
	return err
}
...
func (s *SIPDB) BurstMission(ctx context.Context, missionID string, remoteEndpoint string) error {
	...
	err := withSipRetry(ctx, func() error {
		...
		var updatedAt time.Time
		errStatus := s.db.QueryRow(ctx, "SELECT status, updated_at FROM agent_missions WHERE id = $1 AND organization_id = $2", missionID, s.orgID).Scan(&oldStatus, &updatedAt)
		if errStatus == nil && oldStatus != "BURSTING" {
			shouldRecord = true
			transitionDuration = time.Since(updatedAt)
		}
	...
	if shouldRecord {
		transitionType := strings.ToLower(oldStatus) + "_to_bursting"
		telemetry.RecordAgentTransitionLatency(ctx, transitionType, transitionDuration)
	}
...
func (s *SIPDB) UpsertMission(ctx context.Context, missionID, status, payload string, forceLocal bool) error {
	...
	err := withSipRetry(ctx, func() error {
		...
		var updatedAt time.Time
		errStatus := tx.QueryRow(ctx, "SELECT status, updated_at FROM agent_missions WHERE id = $1 AND organization_id = $2", missionID, s.orgID).Scan(&oldStatus, &updatedAt)
		if errStatus == nil && oldStatus != status {
			shouldRecord = true
			transitionDuration = time.Since(updatedAt)
		}
	...
	if err == nil && shouldRecord {
		transitionType := strings.ToLower(oldStatus) + "_to_" + strings.ToLower(status)
		telemetry.RecordAgentTransitionLatency(ctx, transitionType, transitionDuration)
	}
...
func (s *SIPDB) PruneStaleMissions(ctx context.Context, ageThreshold time.Duration) error {
	return withSipRetry(ctx, func() error {
		...
		_, err := s.db.Exec(ctx, "UPDATE agent_missions SET status = 'FAILED', updated_at = CURRENT_TIMESTAMP WHERE status = 'PENDING' AND created_at < $1 AND organization_id = $2", thresholdTime, s.orgID)
		if err != nil {
			return err
		}
