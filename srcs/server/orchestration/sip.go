...
func (s *SIPDB) CompleteMission(ctx context.Context, missionID string) error {
	var id string
	var oldTimestampStr sql.NullString
	err := withSipRetry(ctx, func() error {
		// Fetch old timestamp before updating
		_ = s.db.QueryRow(ctx, "SELECT COALESCE(updated_at, created_at) FROM agent_missions WHERE id = $1 AND organization_id = $2", missionID, s.orgID).Scan(&oldTimestampStr)
	...
	if err == nil && oldTimestampStr.Valid {
		if t, parseErr := time.Parse(time.RFC3339Nano, oldTimestampStr.String); parseErr == nil {
			telemetry.RecordAgentTransitionLatency(ctx, "running_to_completed", time.Since(t))
		} else if t, parseErr := time.Parse("2006-01-02 15:04:05", oldTimestampStr.String); parseErr == nil {
			telemetry.RecordAgentTransitionLatency(ctx, "running_to_completed", time.Since(t))
		}
	}
	return err
...
func (s *SIPDB) BurstMission(ctx context.Context, missionID string, remoteEndpoint string) error {
	...
	if err == nil && oldTimestampStr.Valid {
		if t, parseErr := time.Parse(time.RFC3339Nano, oldTimestampStr.String); parseErr == nil {
			telemetry.RecordAgentTransitionLatency(ctx, "pending_to_running", time.Since(t))
		} else if t, parseErr := time.Parse("2006-01-02 15:04:05", oldTimestampStr.String); parseErr == nil {
			telemetry.RecordAgentTransitionLatency(ctx, "pending_to_running", time.Since(t))
		}
	}
