...
// this replaces the former filesystem-based ${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/status/<taskID>.yml.
...
func (s *DefaultSubAgentSpawner) executeTask(task *SharedTask) error {
	...
	// Emit heartbeat to database (replaces former ${XDG_STATE_HOME:-$HOME/.local/state}/ohc/runtime/status/<taskID>.yml).
