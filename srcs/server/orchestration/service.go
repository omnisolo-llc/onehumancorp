...
func (s *HubServiceServer) AdvertiseCapabilities(ctx context.Context, caps *pb.AgentCapabilities) (*pb.PublishMessageResponse, error) {
	if s.mesh != nil {
		err := s.mesh.AdvertiseCapabilities(ctx, *caps)
		if err != nil {
			return nil, err
		}
		return pb.PublishMessageResponse_builder{Success: proto.Bool(true)}.Build(), nil
...
func (s *HubServiceServer) DiscoverAgents(req *pb.Query, stream pb.HubService_DiscoverAgentsServer) error {
	if s.mesh != nil {
		ctx := stream.Context()
		capsChan, err := s.mesh.SubscribeCapabilities(ctx)
		if err != nil {
			return err
		}
...
func (s *HubServiceServer) StreamMeshEvents(req *pb.EventStreamRequest, stream pb.HubService_StreamMeshEventsServer) error {
	...
	topic := req.GetTopic()
	ctx := stream.Context()
	eventsChan, err := s.mesh.SubscribeMeshEvents(ctx, topic)
	if err != nil {
		return err
	}

	for range eventsChan {
		// Need to fix struct literal based on proto fields if any or just return basic event
		event := &pb.MeshEvent{}
		// We don't have exact field names here, assuming we just send empty event as hack
