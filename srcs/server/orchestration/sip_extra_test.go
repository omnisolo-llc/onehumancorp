...
func TestSIPDB_NewSIPDB_InvalidPath(t *testing.T) {
	...
	db, err := NewSIPDBFromPath("/root/locked.db")
	if err == nil {
		db.Close()
...
func TestSIPDB_GetPendingMissions_Fallback(t *testing.T) {
	...
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('m2', 'PENDING', '{\"role\":\"ROLE\",\"task\":\"invalid_json\"}')")
	if err != nil {
...
func TestSIPDB_GetPendingMissions_MissingID(t *testing.T) {
	...
	_, err = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES ('m3', 'PENDING', '{\"role\":\"ROLE\",\"task\":{\"type\":\"TASK\"}}')")
	if err != nil {
		t.Fatalf("failed to insert: %v", err)
	}
...
func TestSIPDB_GetCapabilityPlugins_ScanError(t *testing.T) {
	...
	ctx := context.Background()
	_, err = db.provider.Exec(ctx, "DROP TABLE capability_plugins")
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.provider.Exec(ctx, "CREATE TABLE capability_plugins (plugin_id TEXT, name TEXT, version TEXT, manifest_url TEXT, status TEXT, registered_at TEXT)")
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.provider.Exec(ctx, "INSERT INTO capability_plugins (plugin_id, name, version, manifest_url, status, registered_at) VALUES (NULL, 'name', 'version', 'url', 'status', '2023-01-01 00:00:00')")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSIPDB_GetEpisodicMemoriesByPlugin_ScanError(t *testing.T) {
	...
	ctx := context.Background()
	_, err = db.provider.Exec(ctx, "DROP TABLE swarm_memory_embeddings")
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.provider.Exec(ctx, "CREATE TABLE swarm_memory_embeddings (memory_id TEXT, context TEXT, vector_embedding BLOB, source_plugin TEXT, created_at TEXT)")
	if err != nil {
		t.Fatal(err)
	}
	_, err = db.provider.Exec(ctx, "INSERT INTO swarm_memory_embeddings (memory_id, context, vector_embedding, source_plugin, created_at) VALUES (NULL, 'ctx', NULL, 'plugin', '2023-01-01 00:00:00')")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSIPDB_GetPendingMissions_ScanError_Coverage(t *testing.T) {
	...
	defer db.Close()
	ctx := context.Background()
	_, _ = db.provider.Exec(ctx, "DROP TABLE agent_missions")
	_, _ = db.provider.Exec(ctx, "CREATE TABLE agent_missions (id TEXT, status TEXT, payload TEXT, created_at DATETIME)")
	_, _ = db.provider.Exec(ctx, "INSERT INTO agent_missions (id, status, payload) VALUES (NULL, 'PENDING', '{\"role\":\"ROLE\"}')")
	_, err = db.GetPendingMissions(ctx, "ROLE")
	if err == nil {
