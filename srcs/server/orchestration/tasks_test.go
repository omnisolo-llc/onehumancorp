...
func TestTaskManager_ClaimTask(t *testing.T) {
	...
	// Claim when empty
	task, err := tm.ClaimTask(ctx, "non-existent-task-id", "agent-1", "mission-1")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if task != nil {
	...
	// Claim task
	claimedTask, err := tm.ClaimTask(ctx, createdTask.ID, "agent-1", "mission-1")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if claimedTask == nil {
	...
	task3, err := tm.ClaimTask(ctx, "another-non-existent-id", "agent-2", "mission-1")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if task3 != nil {
...
func TestTaskManager_PollTasks(t *testing.T) {
	...
	// Poll when empty
	tasks, err := tm.PollTasks(ctx, "agent-1", "mission-1", 5)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if len(tasks) != 0 {
	...
	tasks, err = tm.PollTasks(ctx, "agent-1", "mission-1", 2)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if len(tasks) != 2 {
	...
	tasks2, err := tm.PollTasks(ctx, "agent-2", "mission-1", 5)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if len(tasks2) != 1 {
...
func TestTaskManager_CompleteTask(t *testing.T) {
	...
	claimedTask, _ := tm.ClaimTask(ctx, task.ID, "agent-1", "mission-1")
	...
	// Complete task
	err := tm.CompleteTask(ctx, claimedTask.ID, "agent-1", "mission-1")
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	err = tm.CompleteTask(ctx, "non-existent", "agent-1", "mission-1")
	if err == nil {
