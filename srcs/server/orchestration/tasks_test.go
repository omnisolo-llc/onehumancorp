...
func TestTaskManager_CreateTask(t *testing.T) {
	...
	task, err := tm.CreateTask(ctx, "mission-1", "Test Task", `{"desc": "Payload"}`)
	if err != nil {
...
func TestTaskManager_ClaimTask(t *testing.T) {
	...
	// Create task
	createdTask, err := tm.CreateTask(ctx, "mission-1", "Test Task", `{"desc": "Payload"}`)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	if claimedTask.AssignedAgentID.String != "agent-1" {
		t.Errorf("expected AssignedAgentID 'agent-1', got %s", claimedTask.AssignedAgentID.String)
	}
...
func TestTaskManager_CompleteTask(t *testing.T) {
	...
	ctx := context.Background()
	task, _ := tm.CreateTask(ctx, "mission-1", "Test Task", `{"desc": "Payload"}`)
