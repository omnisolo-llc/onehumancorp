...
// CloudProfile returns a pre-configured EnvConfig for cloud mode.
func CloudProfile() *EnvConfig {
	return &EnvConfig{
		Mode:             "cloud",
		MultiTenant:      true,
		Headless:         false,
		...
		ApiEndpoint:      "",
		DatabaseURL:      "postgresql://user:pass@localhost:5432/ohc", // Dummy placeholder for UI
	}
}

// StandaloneProfile returns a pre-configured EnvConfig for standalone mode.
func StandaloneProfile() *EnvConfig {
	return &EnvConfig{
		Mode:             "standalone",
		MultiTenant:      false,
		Headless:         false,
...
// GetWizardProfileHandler returns the preset profile based on the requested mode.
func GetWizardProfileHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if mode == "cloud" {
		...
		http.Error(w, "Invalid mode requested", http.StatusBadRequest)
		return
	}
