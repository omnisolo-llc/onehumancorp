...
func TestAPIEndToEndFlow(t *testing.T) {
	...
	assert.NoError(t, err)
	req1.Header.Set("Authorization", "Bearer jwt-token-"+startRes.TenantID)
	...
	req2.Header.Set("Authorization", "Bearer jwt-token-"+startRes.TenantID)
...
func TestAPIStateFlow(t *testing.T) {
	...
	assert.NoError(t, err)
	req1.Header.Set("Authorization", "Bearer jwt-token-"+tenant.ID)
	...
	assert.NoError(t, err)
	req2.Header.Set("Authorization", "Bearer jwt-token-"+tenant.ID)
