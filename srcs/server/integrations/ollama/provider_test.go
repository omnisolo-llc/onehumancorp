...
func TestOllamaIntegration_Metadata(t *testing.T) {
	integration := &OllamaIntegration{}
	metadata := integration.Metadata()

	if metadata.GetId() != "ollama" {
		t.Errorf("Expected ID to be 'ollama', got '%s'", metadata.GetId())
	}
	if metadata.GetName() != "Ollama" {
		t.Errorf("Expected Name to be 'Ollama', got '%s'", metadata.GetName())
	}
}

func TestOllamaIntegration_WizardSteps(t *testing.T) {
	integration := &OllamaIntegration{}
