...
cleanup_tmp_files() {
  if [[ "${OHC_STANDALONE:-true}" == "true" ]]; then
    ...
    if [ -d "${STATE_DIR}" ]; then
      ...
      # Exclude internal Linear orchestrator state to prevent disrupting local-to-cloud handoffs
      find "${STATE_DIR}" -name "*.tmp" ! -name "*Linear*" -type f -mmin +60 -delete 2>/dev/null || true
    fi
  fi
}
