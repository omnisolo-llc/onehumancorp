...
script_path="${RUNFILES_DIR}/_main/srcs/server/standalone_ohc.sh"
if [[ ! -f "${script_path}" ]]; then
    script_path="${RUNFILES_DIR}/${TEST_WORKSPACE:-mono}/srcs/server/standalone_ohc.sh"
    if [[ ! -f "${script_path}" ]]; then
...
touch "${STATE_DIR}/linear_task.tmp"
...
touch -t 200001010000 "${STATE_DIR}/linear_task.tmp"
