...
func TestRAGSyncWorker_Coverage(t *testing.T) {
	...
	worker := NewRAGSyncWorker(service, database)

	// Call syncToCloud directly to test logic
	worker.syncToCloud(ctx)
	...
	if len(pending) != 0 {
		t.Errorf("expected 0 pending records after syncToCloud, got %d", len(pending))
	}

	// Test the worker start method to ensure no panic
	ctxCancel, cancel := context.WithCancel(ctx)
	worker.Start(ctxCancel, 10*time.Millisecond)
