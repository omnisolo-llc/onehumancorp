...
// HandleTelemetrySync handles receiving synced local telemetry metrics from Standalone mode.
func HandleTelemetrySync(w http.ResponseWriter, r *http.Request) {
	ctx, span := otel.Tracer("github.com/onehumancorp/mono/srcs/server/api").Start(r.Context(), "HandleTelemetrySync")
	defer span.End()
