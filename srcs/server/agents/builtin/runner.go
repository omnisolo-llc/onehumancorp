// Package builtin provides the default builtin agent runner.
//
// The Runner registers itself in the orchestration Hub, watches for incoming
// TaskAssignment messages, and executes each task using the full builtin agent
...
// Runner listens on a registered Hub agent and dispatches builtin agent tasks.
// One Runner manages a single registered agent identity; multiple Runner
// instances can be created to provide a pool of builtin agents.
type Runner struct {
	agentID string
	hub     Hub
	cfg     AgentConfig
...
func (r *Runner) Start(ctx context.Context) {
	...
	slog.Info("builtin agent runner: registered", "agent_id", r.agentID)
...
func (r *Runner) drainInbox(ctx context.Context) {
	...
	for _, msg := range messages {
		...
		case "TaskDelegation":
			// TaskDelegation messages arrive from orchestration.DelegateSubTask.
			// Treat the content as a free-form prompt for the agent.
			r.handleTaskDelegation(ctx, msg)
		case "Kill":
			var payload struct {
		...
		default:
			slog.Debug("builtin agent runner: unhandled message type", "type", msg.Type, "from", msg.FromAgent)
		}
	}
}

// handleTaskAssignment decodes and spawns a builtin agent for a task assignment.
func (r *Runner) handleTaskAssignment(ctx context.Context, msg HubMessage) {
	var payload TaskAssignmentPayload
	if err := json.Unmarshal([]byte(msg.Content), &payload); err != nil {
		slog.Error("builtin agent runner: failed to parse TaskAssignment", "err", err)
		return
	}
	...
	if err != nil {
		slog.Error("builtin agent runner: failed to spawn task", "err", err)
		r.hub.RegisterAgent(HubAgent{
			ID:           r.agentID,
			Name:         "builtin-agent",
			Role:         "SOFTWARE_ENGINEER",
	...
	slog.Info("builtin agent runner: task started", "task_id", state.ID, "description", description)

	// Watch the task completion in a separate goroutine so Start doesn't block.
	go r.watchTask(ctx, state, msg)
}

// handleTaskDelegation handles a TaskDelegation message from DelegateSubTask.
// The content is a free-form instruction string (not a JSON payload).
func (r *Runner) handleTaskDelegation(ctx context.Context, msg HubMessage) {
	prompt := msg.Content
	if prompt == "" {
		slog.Warn("builtin agent runner: empty TaskDelegation content, ignoring", "from", msg.FromAgent)
		return
	}
	...
	state, err := SpawnTask(ctx, description, prompt, workDir, r.cfg)
	if err != nil {
		slog.Error("builtin agent runner: failed to spawn delegated task", "err", err)
		r.hub.RegisterAgent(HubAgent{
			ID:           r.agentID,
			Name:         "builtin-agent",
			Role:         "SOFTWARE_ENGINEER",
	...
	slog.Info("builtin agent runner: delegated task started", "task_id", state.ID, "from", msg.FromAgent)
...
func (r *Runner) publishCompletion(state *TaskState, originMsg HubMessage) {
	...
	if replyTo == "" || replyTo == "SYSTEM" {
		slog.Info("builtin agent runner: task complete", "task_id", state.ID, "status", status, "summary", summaryText)
		return
	}
	...
	if err := r.hub.Publish(reply); err != nil {
		slog.Error("builtin agent runner: failed to publish task result", "err", err, "task_id", state.ID)
	}
}
