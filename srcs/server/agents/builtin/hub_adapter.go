...
// OrchestrationHubAdapter wraps orchestration.Hub to satisfy the builtin.Hub interface.
// It is compiled only when the orchestration package is available (Bazel build).
type OrchestrationHubAdapter struct {
	hub *orchestration.Hub
}

// NewOrchestrationHubAdapter wraps an orchestration.Hub for use with a builtin Runner.
