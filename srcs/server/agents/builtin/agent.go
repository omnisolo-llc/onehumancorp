...
const (
	...
	// systemPrompt is the default system prompt injected into every builtin agent.
	// It mirrors the CC-Source GENERAL_PURPOSE_AGENT prompt with additional guidance.
...
- Performing multi-step research and engineering tasks
...
- Prefer targeted edits (file_edit) over full rewrites (file_write) whenever possible.
...
- Run bash commands to explore the codebase, run tests, and validate changes.
- When you are done, provide a concise summary of what was done and any key findings.
  The caller will relay this to the user, so it only needs the essentials.
- If you encounter errors, understand them and fix them before reporting completion.
- Do not ask clarifying questions; make reasonable assumptions and proceed.
- Use the todo_write tool to track multi-step tasks and keep yourself on track.`
)
...
func NewAgent(state *TaskState, cfg AgentConfig) *Agent {
	...
	if cfg.MaxTurns <= 0 {
		...
		// OHC_AGENT_MAX_TURNS is the canonical name; OHC_LOCAL_AGENT_MAX_TURNS is
		// kept for backward compatibility.
		for _, envKey := range []string{"OHC_AGENT_MAX_TURNS", "OHC_LOCAL_AGENT_MAX_TURNS"} {
	...
	// Wire tool_search with the full tool list so it can enumerate them.
	for _, t := range cfg.Tools {
		if ts, ok := t.(*toolSearchTool); ok {
			ts.tools = cfg.Tools
		}
	}
