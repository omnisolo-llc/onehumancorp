...
void main() {
  ...
  group('SettingsScreen – button clicks', () {
    testWidgets('Sign Out list tile is rendered and tappable', (tester) async {
      final fakeManager = _FakeLocalManagerService();
      await tester.pumpWidget(
        _wrap(
          const SettingsScreen(),
          overrides: [
            authStateProvider.overrideWith(() => _FakeAuthNotifier(_fakeUser)),
            localManagerServiceProvider.overrideWithValue(fakeManager),
            clientSettingsProvider.overrideWith((ref) {
              final notifier = ClientSettingsNotifier(ref);
              // mock a resolved initial state to prevent timeout
    ...
    testWidgets('shows user name and email when logged in', (tester) async {
      ...
      expect(find.text('Dev User'), findsWidgets);
