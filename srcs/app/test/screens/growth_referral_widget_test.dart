...
void main() {
  testWidgets('GrowthReferralWidget renders correctly and responds to hover', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: const Scaffold(body: Center(child: GrowthReferralWidget())),
      ),
    );
    ...
    await gesture.addPointer(
      location: tester.getCenter(find.byType(GrowthReferralWidget)),
    );
    await tester.pumpAndSettle();

    // Check if hovered state is applied
    final AnimatedScale hoverScaleWidget = tester.widget<AnimatedScale>(
      find.byType(AnimatedScale),
    );
    ...
    final AnimatedScale exitScaleWidget = tester.widget<AnimatedScale>(
      find.byType(AnimatedScale),
    );
