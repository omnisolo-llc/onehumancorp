...
class _AnimatedMessageItemState extends State<_AnimatedMessageItem> with SingleTickerProviderStateMixin {
  ...
  IconData _getIconForAction(String action) {
    ...
    if (lower.contains('fail') || lower.contains('error')) return Icons.error_outline;
    if (lower.contains('start') || lower.contains('init')) return Icons.play_circle_outline;
    if (lower.contains('done') || lower.contains('finish') || lower.contains('success')) return Icons.check_circle_outline;
    if (lower.contains('message') || lower.contains('send')) return Icons.message_outlined;
    return Icons.sync_alt;
  }

  Color _getColorForAction(String action) {
    final lower = action.toLowerCase();
    if (lower.contains('fail') || lower.contains('error')) return Colors.redAccent;
    if (lower.contains('start') || lower.contains('init')) return Colors.blueAccent;
    if (lower.contains('done') || lower.contains('finish') || lower.contains('success')) return Colors.greenAccent;
    if (lower.contains('message') || lower.contains('send')) return Colors.purpleAccent;
  ...
  Widget build(BuildContext context) {
    ...
    final iconData = _getIconForAction(widget.message.action);
    final iconColor = _getColorForAction(widget.message.action);
