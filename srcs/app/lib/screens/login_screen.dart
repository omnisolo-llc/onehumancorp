...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _oauthLogin(String provider) async {
    ...
    try {
      await ref.read(authStateProvider.notifier).oauthLogin(provider);
      if (mounted) {
      ...
      if (mounted) {
        setState(() {
          _error = 'OAuth provider unreachable.';
          _loading = false;
        });
      }
    }
  ...
  Future<void> _showSettings(BuildContext context) async {
    ...
    if (result != null && result.isNotEmpty) {
      await ref.read(clientSettingsProvider.notifier).updateBackendUrl(result);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Backend URL updated to $result',
              style: const TextStyle(fontFamily: 'Inter'),
            ),
