use crate::queue::JobQueue;
    ...
    #[async_trait]
    impl crate::queue::JobPayloadHandler for MockHandler {
    ...
    async fn test_in_mem_job_queue_worker_pool() {
        let queue = Arc::new(crate::queue::InMemJobQueue::new());
        let handler = Arc::new(MockHandler);
        let pool = crate::queue::WorkerPool::new(queue.clone(), "test_topic".to_string(), 3, handler);
    ...
    async fn test_task_queue_service_push_claim() {
        ...
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            let service = crate::queue::TaskQueueService::new(pool.clone());
            ...
            let task = crate::queue::SharedTaskModel {
                id: task_id.clone(),
    ...
    async fn test_queue_manager_tenant_isolation() {
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            let job2 = crate::queue::SubAgentJob {
                id: job_id2.clone(),
                tenant_id: org_id.clone(),
                parent_task_id: "task-1".to_string(),
    ...
    async fn test_task_queue_service_with_dependencies() {
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            let parent_task = crate::queue::SharedTaskModel {
            ...
            let child_task = crate::queue::SharedTaskModel {
