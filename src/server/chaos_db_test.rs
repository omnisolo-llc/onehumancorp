...
mod chaos_db_tests {
    ...
    async fn test_chaos_parity_audit_sqlite_postgres_identical_queries() {
        let pg_pool_opt = if let Ok(database_url) = std::env::var("OHC_DATABASE_URL") {
        ...
        assert_eq!(sqlite_count, 1, "SQLite insert should succeed");
    ...
    async fn test_chaos_parity_audit_comprehensive() {
        ...
        let mut pg_db_opt = None;
        if let Some(pg_pool) = &pg_pool_opt {
            pg_db_opt = Some(Arc::new(DB {
                pool: pg_pool.clone(),
        ...
        assert_eq!(sqlite_sorted.len(), 2, "Task count SQLite failed");

        if let Some(pg_db) = &pg_db_opt {
