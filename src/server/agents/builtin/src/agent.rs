...
impl Agent {
        ...
        F: FnMut(AgentEvent) + Send,
    {
        ...
        // Hook 1: Input Guardrail
        if let Some(guardrails) = &cfg.guardrails {
            if let Err(err) = crate::guardrails::check_input(initial_message, guardrails) {
        ...
        for iteration in 0..max_iterations {
            ...
            if tool_calls.is_empty() {
                ...
                // Hook 3: Output Guardrail
                if let Some(guardrails) = &cfg.guardrails {
                    if let Err(err) = crate::guardrails::check_output(&last_assistant_content, guardrails) {
                        on_event(AgentEvent::TaskError { error: err.clone() });
                        return Err(err.into());
                    }
                }
            ...
            for tc in &tool_calls {
                // Hook 2: Tool Guardrail
                if let Some(guardrails) = &cfg.guardrails {
                    if let Err(err) = crate::guardrails::check_tool(&tc.name, &tc.arguments.to_string(), guardrails) {
                        on_event(AgentEvent::TaskError { error: err.clone() });
                        return Err(err.into());
                    }
                }
        ...
        // Hook 3: Output Guardrail (fallback if we hit max iterations)
        if let Some(guardrails) = &cfg.guardrails {
            if let Err(err) = crate::guardrails::check_output(&last_assistant_content, guardrails) {
                on_event(AgentEvent::TaskError { error: err.clone() });
                return Err(err.into());
...
mod tests {
    ...
    #[tokio::test]
    async fn test_guardrails_input() {
        let client = Arc::new(MockLlmClient {
            responses: tokio::sync::Mutex::new(vec![]),
        });
        ...
        let mut cfg = AgentRunConfig::default();
        cfg.guardrails = Some(GuardrailConfig {
            blocked_keywords: vec!["DANGER".to_string()],
        });

        let mut events = vec![];
        let mut on_event = |e| { events.push(e); };

        let result = agent.run(&cfg, "This is a DANGER input", &mut on_event).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Input contains blocked keyword: DANGER"));
    }

    #[tokio::test]
    async fn test_guardrails_tool() {
        let client = Arc::new(MockLlmClient {
            responses: tokio::sync::Mutex::new(vec![
                ChatResponse {
                    message: Message {
                        ...
                        tool_calls: vec![ToolCall {
                            id: "call_1".to_string(),
                            name: "test_tool".to_string(),
                            arguments: serde_json::json!({ "arg": "DANGER" }),
                        }],
                        tool_results: vec![],
                    },
                    usage: Usage::default(),
        ...
        let result = agent.run(&cfg, "Hello", &mut on_event).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Tool test_tool contains blocked keyword: DANGER"));
    }

    #[tokio::test]
    async fn test_guardrails_output() {
        let client = Arc::new(MockLlmClient {
            responses: tokio::sync::Mutex::new(vec![
                ChatResponse {
                    message: Message::assistant("This is a DANGER output"),
                    usage: Usage::default(),
                    stop_reason: "stop".to_string(),
                },
            ]),
        ...
        let result = agent.run(&cfg, "Hello", &mut on_event).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Output contains blocked keyword: DANGER"));
    }
}
