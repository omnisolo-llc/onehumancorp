...
pub struct DistributedLock {
    inner: crate::mesh::interop::HybridLock,
}

impl DistributedLock {
    pub fn new(client: redis::Client, key: &str) -> Self {
        let is_standalone = std::env::var("DATABASE_URL").unwrap_or_default().starts_with("sqlite") || std::env::var("DATABASE_URL").is_err();
        let inner = if is_standalone {
            crate::mesh::interop::HybridLock::new_standalone(key)
        } else {
            crate::mesh::interop::HybridLock::new_cloud(client, key)
        };
        DistributedLock { inner }
    }

    pub async fn acquire(&self, timeout: Duration, expiration: Duration) -> Result<(), String> {
        self.inner.acquire(timeout, expiration).await
    }
