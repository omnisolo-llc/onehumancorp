use ohc_builtin_agent::sandbox::session::ShellSession;
use sqlx::SqlitePool;

pub struct LocalExecutionRouter {
    db: SqlitePool,
    session: ShellSession,
}

impl LocalExecutionRouter {
    ...
    pub async fn route_execution(&self, command: &str) -> Result<String, String> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_route_execution_success() {
        let db_id = format!("db_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
        ...
        let router = LocalExecutionRouter::new(pool.clone(), session).await.unwrap();
        let res = router.route_execution("echo 'proxytest'").await.unwrap();
    ...
    #[tokio::test]
    async fn test_route_execution_error() {
        let db_id = format!("db_{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
        let uri = format!("sqlite:file:{}?mode=memory&cache=shared", db_id);
        let pool = SqlitePoolOptions::new().connect(&uri).await.unwrap();
        ...
        let router = LocalExecutionRouter::new(pool.clone(), session).await.unwrap();
        let res = router.route_execution("nonexistent_command_12345").await;
        assert!(res.is_err());
