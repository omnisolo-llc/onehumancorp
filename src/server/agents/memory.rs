...
impl VectorRepository {
    ...
    pub async fn upsert(&self, record: &EmbeddingRecord) -> Result<(), String> {
        ...
        match &self.store {
            VectorMemoryStore::Postgres(pool) => {
                sqlx::query(
                    "INSERT INTO consolidated_memory (id, organization_id, agent_id, content, embedding, source_type, created_at)                      VALUES ($1, $2, $3, $4, $5::vector, $6, $7)                      ON CONFLICT(id) DO UPDATE SET                          content=excluded.content,                          embedding=excluded.embedding,                          created_at=excluded.created_at"
                )
                .bind(&record.id)
            ...
            VectorMemoryStore::Sqlite(pool) => {
                sqlx::query(
                    "INSERT INTO consolidated_memory (id, organization_id, agent_id, content, embedding, source_type, created_at)                      VALUES (?, ?, ?, ?, ?, ?, ?)                      ON CONFLICT(id) DO UPDATE SET                          content=excluded.content,                          embedding=excluded.embedding,                          created_at=excluded.created_at"
                )
                .bind(&record.id)
                .bind(&record.organization_id)
                .bind(&record.agent_id)
    ...
    pub async fn semantic_search(&self, organization_id: &str, query_embedding: &[f32], limit: i64) -> Result<Vec<EmbeddingRecord>, String> {
        ...
        match &self.store {
            VectorMemoryStore::Postgres(pool) => {
                let rows = sqlx::query(
                    "SELECT id, organization_id, COALESCE(agent_id, '') as agent_id, content, embedding::text, source_type, created_at                      FROM consolidated_memory                      WHERE organization_id = $1                      ORDER BY embedding <-> $2::vector                      LIMIT $3"
                )
            ...
            VectorMemoryStore::Sqlite(pool) => {
                let rows = sqlx::query(
                    "SELECT id, organization_id, COALESCE(agent_id, '') as agent_id, content, embedding, source_type, created_at                      FROM consolidated_memory                      WHERE organization_id = ?                      ORDER BY vec_distance_cosine(embedding, ?)                      LIMIT ?"
                )
                .bind(organization_id)
                .bind(emb_str)
                .bind(limit)
