...
type mockPGProviderStore struct {
	db.Provider
	execArgs  []interface{}
	queryArgs []interface{}
	execErr   error
...
func (m *mockPGProviderStore) IsSQLite() bool { return false }

func (m *mockPGProviderStore) Exec(ctx context.Context, sql string, arguments ...any) (int64, error) {
	m.execArgs = arguments
	return 0, m.execErr
}

func (m *mockPGProviderStore) Query(ctx context.Context, sql string, optionsAndArgs ...any) (db.Rows, error) {
...
func TestPGVectorStore_Store_InvalidMetadata(t *testing.T) {
	store := NewPGVectorStore(&mockPGProviderStore{})
...
func TestSQLiteVectorStore_Store_InvalidMetadata(t *testing.T) {
	store := NewSQLiteVectorStore(&mockPGProviderStore{})
	err := store.Store(context.Background(), "id", []float32{1.0}, map[string]any{"invalid": make(chan int)}, "content")
	if err == nil {
		t.Fatal("expected error, got nil")
	}
