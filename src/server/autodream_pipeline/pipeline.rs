        ...
        tokio::spawn(async move {
            loop {
                ...
                if let Err(e) = pipeline.process_closed_tasks().await {
                    ::server_telemetry::record_error_signal("[bug] AutoDreamPipeline worker error");
...
mod tests {
    ...
    async fn test_process_closed_tasks_with_cache() {
        let database_url = std::env::var("OMNISOLO_DATABASE_URL").unwrap_or_else(|_| "dummy".to_string());
