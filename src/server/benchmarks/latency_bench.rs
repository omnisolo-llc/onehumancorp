...
pub async fn bench_db_query_time() {
    ...
    if _database_url.starts_with("postgres") {
        let pg_pool = sqlx::postgres::PgPoolOptions::new().connect(&_database_url).await.unwrap_or_else(|e| panic!("Failed to connect to DB at {}: {}", _database_url, e));
...
pub async fn bench_dashboard_snapshot() {
    ...
    let db = if _database_url.starts_with("sqlite") {
        let pool = sqlx::sqlite::SqlitePoolOptions::new()
            ...
            .connect(&_database_url).await.unwrap_or_else(|e| panic!("Failed to connect to DB at {}: {}", _database_url, e));
...
mod tests {
    ...
    async fn test_stress_verification_cloud_standalone() {
        ...
        let _database_url = std::env::var("OHC_DATABASE_URL").unwrap_or_else(|_| "sqlite::memory:".to_string());
        if _database_url.starts_with("postgres") {
            if let Ok(pg_pool) = sqlx::postgres::PgPoolOptions::new().after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) }).connect(&_database_url).await {
                let pg_queue = Arc::new(PostgresTaskQueue::new(pg_pool));
...
pub async fn bench_advisory_insights_latency() {
    ...
    if _database_url != "sqlite::memory:" && _database_url.starts_with("postgres") {
        let pg_pool = sqlx::postgres::PgPoolOptions::new().connect(&_database_url).await.unwrap_or_else(|e| panic!("Failed to connect to DB at {}: {}", _database_url, e));
