...
pub async fn bench_dashboard_snapshot() {
    ...
    let db = if database_url.starts_with("sqlite") {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new().after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) }).connect(&database_url).await.unwrap();
