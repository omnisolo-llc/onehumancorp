...
pub async fn bench_api_response_time() {
    ...
    let fallback_pg = sqlx::PgPool::connect_lazy("sqlite::memory:").unwrap_or_else(|_| panic!("Failed to connect to fallback memory pool"));
...
pub async fn bench_dashboard_snapshot() {
    ...
    let db = if database_url.starts_with("sqlite") {
        ...
        let pg_pool = sqlx::PgPool::connect_lazy("sqlite::memory:").unwrap_or_else(|_| panic!("Failed to connect to fallback memory pool"));
