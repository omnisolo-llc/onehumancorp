...
pub async fn bench_api_response_time() {
    ...
    let fallback_pg = sqlx::PgPool::connect_lazy("postgres://invalid-dummy-url-for-typing-only").unwrap();
...
pub async fn bench_dashboard_snapshot() {
    ...
    let db = if database_url.starts_with("sqlite") {
        ...
        let pg_pool = sqlx::PgPool::connect_lazy("postgres://invalid-dummy-url-for-typing-only").unwrap();
