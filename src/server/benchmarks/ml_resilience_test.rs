...
mod ml_resilience_tests {
    ...
    async fn test_token_budget_server_side() {
        ...
        let decision = omnisolo_builtin_agent::budget::check_token_budget(&mut tracker, budget, global_turn_tokens);
        // It should continue since we haven't reached 1000 or diminishing returns
        assert_eq!(decision.action, omnisolo_builtin_agent::budget::BudgetAction::Continue, "Token budget must enforce limits server-side");

        let global_turn_tokens_exceeded = 950; // > 90% (threshold is 0.9)
        let decision_stop = omnisolo_builtin_agent::budget::check_token_budget(&mut tracker, budget, global_turn_tokens_exceeded);
        assert_eq!(decision_stop.action, omnisolo_builtin_agent::budget::BudgetAction::Stop, "Token budget must stop server-side execution if exceeded");
    }
}
