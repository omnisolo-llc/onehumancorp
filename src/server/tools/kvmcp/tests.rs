...
async fn test_kv_get_set_list_delete_standalone() {
    with_var("OHC_STANDALONE_MODE", Some("true"), || async {
        ...
        let spiffe_id = "spiffe://ohc.os/org/test_org/agent/test_agent".to_string();
...
async fn test_tenant_id_parsing() {
    ...
    let spiffe_id = "spiffe://ohc.os/org/test_org/agent/test_agent";
    assert_eq!(server.get_tenant_id(spiffe_id).unwrap(), "test_org");

    let bad_spiffe = "spiffe://ohc.os/something/else";
    assert!(server.get_tenant_id(bad_spiffe).is_err());

    let empty_spiffe = "spiffe://ohc.os/org//agent/test_agent";
