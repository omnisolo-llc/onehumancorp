...
async fn test_kv_get_set_list_delete_standalone() {
    with_var("OHC_STANDALONE_MODE", Some("true"), || async {
        ...
        let spiffe_id = "spiffe://trust.domain/workload/ohc/tenant/test_org/agent/test_agent".to_string();
...
async fn test_tenant_id_parsing() {
    ...
    let spiffe_id = "spiffe://trust.domain/workload/ohc/tenant/test_org/agent/test_agent";
