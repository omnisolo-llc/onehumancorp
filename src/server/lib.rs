...
async fn load_ui_agent_approvals_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, department, description, status, action_risk, NULL as payload FROM agent_approvals WHERE tenant_id = $1 AND status IN ('DRAFT', 'PAUSED') ORDER BY id ASC LIMIT $2")
            } else {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, department, description, status, action_risk, NULL as payload FROM agent_approvals WHERE tenant_id = ? AND status IN ('DRAFT', 'PAUSED') ORDER BY id ASC LIMIT ?")
            } else {
...
async fn load_ui_ledger_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, event_type, department, '{}'::jsonb as payload, created_at FROM ohc_universal_ledger WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2")
            } else {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, event_type, department, '{}' as payload, created_at FROM ohc_universal_ledger WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?")
            } else {
...
async fn load_ui_triage_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            if mobile_optimized {
                sqlx::query(
                    "SELECT t.id, '' as tenant_id, '' as customer_id, t.source, t.priority, '' as context, t.status, t.created_at, a.action_type, '' AS action_payload FROM triage_items t LEFT JOIN triage_proposed_actions a ON t.id = a.triage_item_id WHERE t.tenant_id = $1 AND t.status != 'resolved' AND t.status != 'dismissed' ORDER BY t.created_at DESC LIMIT 50"
                )
            } else {
                sqlx::query(
                    "SELECT t.id, t.tenant_id, t.customer_id, t.source, t.priority, t.context, t.status, t.created_at, a.action_type, a.payload AS action_payload FROM triage_items t LEFT JOIN triage_proposed_actions a ON t.id = a.triage_item_id WHERE t.tenant_id = $1 AND t.status != 'resolved' AND t.status != 'dismissed' ORDER BY t.created_at DESC LIMIT 50"
    ...
    match &db.store {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            let rows_res = if mobile_optimized {
                sqlx::query("SELECT id, organization_id, tenant_id, title, '' as description, status, created_at, updated_at FROM shared_tasks WHERE status IN ('PENDING', 'IN_PROGRESS') ORDER BY created_at DESC LIMIT ?")
            } else {
...
async fn load_ui_agent_feed_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, event_source, NULL as context_payload, NULL as proposed_action, lifecycle_state, created_at, updated_at FROM agent_feed_items WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2")
            } else {
                sqlx::query("SELECT * FROM agent_feed_items WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT $2")
            }
            .bind(tenant_id)
            .bind(limit)
            .fetch_all(&db.pool)
        ...
        crate::db::DbStore::Sqlite(pool) => {
            if mobile_optimized {
                sqlx::query("SELECT id, '' as tenant_id, event_source, NULL as context_payload, NULL as proposed_action, lifecycle_state, created_at, updated_at FROM agent_feed_items WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?")
            } else {
                sqlx::query("SELECT * FROM agent_feed_items WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?")
            }
            .bind(tenant_id)
            .bind(limit)
            .fetch_all(pool)
