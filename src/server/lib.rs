...
async fn create_ui_bom_item_handler(
    ...
    let app = axum::Router::new()
        ...
        .route("/api/v1/sync/offline", axum::routing::post({ let db = db.clone(); let mesh = mesh_transport.clone(); move |headers: axum::http::HeaderMap, payload: axum::Json<api::offline_sync::OfflineSyncRequest>| async move { api::offline_sync::offline_sync_handler(axum::extract::State((db.pool.clone(), mesh.clone())), headers, payload).await } })).route("/api/v1/sync/mutations", axum::routing::post({ let db = db.clone(); move |headers: axum::http::HeaderMap, payload: axum::Json<api::sync_mutations_handler::GenericOfflineSyncRequest>| async move { api::sync_mutations_handler::sync_mutations_handler(axum::extract::State(db.pool.clone()), headers, payload).await } }))
