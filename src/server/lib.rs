...
async fn load_ui_ledger_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            if mobile_optimized { sqlx::query("SELECT id, event_type, department, CAST(created_at AS TEXT) AS created_at FROM ohc_universal_ledger WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?") } else { sqlx::query("SELECT id, tenant_id, event_type, department, payload, CAST(created_at AS TEXT) AS created_at FROM ohc_universal_ledger WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?") }
                .bind(tenant_id)
                .bind(limit_ledger)
                .fetch_all(pool)
                .await.map(|rows| rows.into_iter().map(|row| {
                        ...
                        for row in rows {
                            ...
                            let item = if mobile_optimized {
                                serde_json::json!({
                                    "id": row.get::<String, _>("id"),
                                    "status": row.try_get::<String, _>("status").unwrap_or_default(),
                                    "created_at": match row.try_get::<String, _>("created_at") { Ok(dt) => dt, Err(_) => "".to_string() },
                                    "action_type": row.try_get::<String, _>("action_type").unwrap_or_default(),
                                })
                            } else {
                                serde_json::json!({
