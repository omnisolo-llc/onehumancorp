...
impl HubService for MyHubService {
    ...
    async fn get_my_plan(
        ...
        let tenant_id_string = request.metadata().get("x-spiffe-id").and_then(|v| v.to_str().ok()).and_then(|v| ::server_auth::parse_spiffe_id(v).ok()).map(|(id, _)| id).unwrap_or_else(|| "default".to_string());
        let tenant_id = tenant_id_string.as_str();
    ...
    async fn select_plan(
        ...
        let tenant_id = request.metadata().get("x-spiffe-id").and_then(|v| v.to_str().ok()).and_then(|v| ::server_auth::parse_spiffe_id(v).ok()).map(|(id, _)| id).unwrap_or_else(|| "default".to_string());
