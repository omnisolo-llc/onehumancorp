...
impl HubService for MyHubService {
    ...
    async fn get_my_plan(
        ...
        let tenant_id_str = request.metadata().get("x-spiffe-id").and_then(|v| v.to_str().ok()).unwrap_or("");
        let (tenant_id_string, _) = ::server_auth::parse_spiffe_id(tenant_id_str).unwrap_or(("default".to_string(), "".to_string()));
    ...
    async fn select_plan(
        ...
        let tenant_id_str = request.metadata().get("x-spiffe-id").and_then(|v| v.to_str().ok()).unwrap_or("");
        let (tenant_id, _) = ::server_auth::parse_spiffe_id(tenant_id_str).unwrap_or(("default".to_string(), "".to_string()));
