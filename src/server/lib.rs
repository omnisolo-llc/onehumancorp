...
async fn load_ui_priority_tasks_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            if mobile_optimized {
                sqlx::query(
                    "SELECT id, title, status, created_at, updated_at FROM shared_tasks WHERE tenant_id = $1 AND status IN ('PENDING', 'IN_PROGRESS') ORDER BY created_at DESC LIMIT $2"
                )
                .bind(tenant_id)
                .bind(limit)
                .fetch_all(&db.pool)
                ...
                sqlx::query(
                    "SELECT id, title, description, status, created_at, updated_at FROM shared_tasks WHERE tenant_id = $1 AND status IN ('PENDING', 'IN_PROGRESS') ORDER BY created_at DESC LIMIT $2"
                )
                .bind(tenant_id)
                .bind(limit)
                .fetch_all(&db.pool)
...
async fn create_ui_bom_item_handler(
    ...
    let mesh_service = crate::services::mesh::mesh_service::MyMeshService::new(hub.redis_client.clone());
    ...
    Server::builder()
        ...
        .add_service(::server_ohc::inventory::inventory_sync_service_server::InventorySyncServiceServer::with_interceptor(inventory_sync_service, spiffe_interceptor.clone()))
        .add_service(::server_ohc::mesh::unified_mesh_service_server::UnifiedMeshServiceServer::with_interceptor(mesh_service, spiffe_interceptor))
