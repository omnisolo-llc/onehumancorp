...
pub async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let webhook_router = axum::Router::new()
        .route("/api/v1/billing/pricing", axum::routing::get(pricing_plans_handler))
...
async fn pricing_plans_handler(axum::extract::State(webhook_state): axum::extract::State<crate::api::billing_webhook::WebhookState>) -> impl axum::response::IntoResponse {
    match crate::billing::get_pricing_plans(&webhook_state.db).await {
        Ok(plans) => axum::response::Json(serde_json::json!({ "success": true, "plans": plans })).into_response(),
        Err(e) => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, axum::response::Json(serde_json::json!({ "success": false, "error": e }))).into_response()
    }
}
