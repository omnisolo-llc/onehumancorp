...
pub async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    ops_worker.start();
    let yield_worker = crate::workers::department_workers::FinanceYieldWorker::new(db.clone());
    yield_worker.start();
...
async fn create_ui_bom_item_handler(
    ...
    let app = axum::Router::new()
        ...
        .route("/api/ui/yield-opportunities", axum::routing::get(server_api::yield_opportunities::list_ui_yield_opportunities_handler).with_state(db.clone()))
        .route("/api/ui/yield-opportunities/:id/approve", axum::routing::post(server_api::yield_opportunities::approve_yield_opportunity_handler).with_state(db.clone()))
