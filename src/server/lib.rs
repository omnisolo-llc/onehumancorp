...
<head>
    ...
    <title>OneHuman Corp | Premium Business OS</title>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&family=Inter:wght@300;400;500;600&display=swap" rel="stylesheet">
    <style>
:root {
    --primary: #0055ff;
    --primary-light: rgba(0, 85, 255, 0.1);
...
.animate-slide { animation: slideUp 0.5s cubic-bezier(0.4, 0, 0.2, 1); }

/* Layout */
main { padding: 40px; min-height: 100vh; transition: padding 0.3s; }
...
nav#main-nav a:hover { color: var(--primary); transform: translateY(-1px); }
...
.stat-card h2 { font-size: 48px; margin: 8px 0; letter-spacing: -2px; }
.stat-label { color: var(--text-secondary); text-transform: uppercase; letter-spacing: 1.5px; font-size: 11px; font-weight: 700; }
...
button:hover { background: var(--primary-hover); transform: translateY(-2px); box-shadow: 0 10px 15px -3px rgba(0, 85, 255, 0.3); }
...
button.secondary:hover { background: var(--bg); border-color: var(--text-secondary); }
...
.hint-btn {
    ...
    padding: 0; border-radius: 50%; font-size: 14px;
...
/* Specific Screens Styles */
.wizard-step { max-width: 600px; margin: 60px auto; }
.inbox-message { cursor: pointer; border-left: 4px solid transparent; }
.inbox-message.active { border-left-color: var(--primary); background: var(--primary-light); }

.tag { padding: 4px 12px; border-radius: 100px; font-size: 12px; font-weight: 600; }
...
/* Premium Variant 0 Support */
.glass-v0 { backdrop-filter: blur(20px); background: rgba(255,255,255,0.7); }
/* Premium Variant 1 Support */
.glass-v1 { backdrop-filter: blur(21px); background: rgba(255,255,255,0.69); }
/* Premium Variant 2 Support */
.glass-v2 { backdrop-filter: blur(22px); background: rgba(255,255,255,0.6799999999999999); }
/* Premium Variant 3 Support */
.glass-v3 { backdrop-filter: blur(23px); background: rgba(255,255,255,0.6699999999999999); }
/* Premium Variant 4 Support */
.glass-v4 { backdrop-filter: blur(24px); background: rgba(255,255,255,0.6599999999999999); }
/* Premium Variant 5 Support */
.glass-v5 { backdrop-filter: blur(25px); background: rgba(255,255,255,0.6499999999999999); }
/* Premium Variant 6 Support */
.glass-v6 { backdrop-filter: blur(26px); background: rgba(255,255,255,0.6399999999999999); }
/* Premium Variant 7 Support */
.glass-v7 { backdrop-filter: blur(27px); background: rgba(255,255,255,0.6299999999999999); }
/* Premium Variant 8 Support */
.glass-v8 { backdrop-filter: blur(28px); background: rgba(255,255,255,0.62); }
/* Premium Variant 9 Support */
.glass-v9 { backdrop-filter: blur(29px); background: rgba(255,255,255,0.61); }
</style>
</head>
<body>
    <nav id="main-nav">
        <div style="font-family: var(--font-outfit); font-weight: 800; font-size: 20px; margin-right: 20px; color: var(--primary);">OHC</div>
        <a onclick="showScreen('dashboard-screen')" id="nav-dash"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Overview</a>
        <a onclick="showScreen('agents-screen')" id="nav-agents"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> AI Assistants</a>
        <a onclick="showScreen('setup-screen')" id="nav-setup"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg> Launch Site</a>
        <a onclick="showScreen('api-screen')" id="nav-api"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> Connect Tools</a>
    </nav>

    <div id="mobile-bottom-nav">
        <button class="nav-item" onclick="showScreen('dashboard-screen')"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Overview</button>
        <button class="nav-item" onclick="showScreen('inbox-screen')"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> Messages</button>
        <button class="nav-item" onclick="alert('Adding products is available in the Full Builder. Starting setup...')"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Add Product</button>
        <button class="nav-item" onclick="showScreen('referral-dashboard-screen')"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> Share</button>
        <button class="nav-item" onclick="showScreen('settings-screen')"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> Settings</button>
    </div>

    <main>
        <!-- Dashboard -->
        <div id="dashboard-screen" class="screen animate-slide">
            <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px;">
                <h1 class="outfit">Business Overview</h1>
                <div style="display: flex; gap: 12px;">
                    <button class="secondary ghost"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> Help</button>
                    <button class="secondary ghost"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> Export</button>
                </div>
            </header>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
                <div class="card glass stat-card">
                    ...
                    <h2 class="outfit">$1,284.50</h2>
                    <p style="color: var(--success); font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 4px;">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 16.5 8.5 11.5 1 19"/><polyline points="17 6 23 6 23 12"/></svg> 12.4% vs yesterday
                    </p>
                </div>
                <div class="card glass stat-card">
                    <p class="stat-label">Active AI Assistants</p>
                    <h2 class="outfit">4</h2>
                    <p style="color: var(--text-secondary);">Working on 12 tasks</p>
                </div>
                <div class="card glass stat-card">
                    <p class="stat-label">Customer Messages</p>
                    <h2 class="outfit">8</h2>
                    <p style="color: var(--warning); font-weight: 600;">3 require action</p>
                </div>
            </div>

            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 32px; margin-top: 32px;">
                <div class="card glass">
                    <h3 class="outfit">Proactive Assistant</h3>
                    <p style="color: var(--text-secondary); margin-bottom: 24px;">I've identified 3 opportunities to grow your business today.</p>
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        <div style="padding: 16px; background: var(--bg); border-radius: 16px; display: flex; gap: 16px; align-items: center;">
                            <div style="width: 40px; height: 40px; background: #e0f2fe; color: #0ea5e9; border-radius: 12px; display: flex; align-items: center; justify-content: center;"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 6 13.5 16.5 8.5 11.5 1 19"/><polyline points="17 6 23 6 23 12"/></svg></div>
                            <div style="flex: 1;">
                                <div style="font-weight: 600;">Optimize SEO</div>
                                <div style="font-size: 13px; color: var(--text-secondary);">Your cake shop can rank higher with 2 new keywords.</div>
                            </div>
                            <button class="secondary ghost" style="padding: 4px 12px; height: 32px; min-height: 32px; min-width: auto;">Fix</button>
                        </div>
                    </div>
                </div>

                <div class="card glass">
                    <h3 class="outfit">Quick Actions <button class="secondary hint-btn" onclick="toggleHint('quick-actions-hint')">?</button></h3>
                    <p id="quick-actions-hint" style="display: none; background: var(--primary-light); padding: 16px; border-radius: 16px; font-size: 13px; color: var(--text); margin-bottom: 20px;">Use these shortcuts to jump into setup or manage your store quickly.</p>
                    <div style="display: flex; flex-direction: column; gap: 12px;">
                        <button onclick="showScreen('setup-screen')">🚀 Launch Website</button>
                        <button class="secondary" onclick="showScreen('meetings-screen')">📅 View Agenda</button>
                        <button class="secondary" onclick="showScreen('inbox-screen')">💬 Chat with Customers</button>
                        <button class="secondary" onclick="showScreen('api-screen')">🔗 Connect My Apps</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Setup Wizard -->
        <div id="setup-screen" class="screen animate-slide">
            <div class="wizard-step">
                <div id="step-1" class="card glass" style="text-align: center; padding: 60px 40px;">
                    <div style="width: 80px; height: 80px; background: var(--primary-light); color: var(--primary); border-radius: 24px; display: flex; align-items: center; justify-content: center; margin: 0 auto 32px;">
                        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg>
                    </div>
                    <h1 class="outfit" style="font-size: 36px; margin-bottom: 16px;">Your business, live in minutes.</h1>
                    <p style="font-size: 18px; color: var(--text-secondary); margin-bottom: 48px;">We use AI to handle the tech, so you can focus on your craft.</p>
                    <button onclick="nextStep(2)" style="width: 100%; font-size: 18px; padding: 18px;">Get Started</button>
                    <button class="secondary" onclick="nextStep('generating')" style="width: 100%;">Instant Build with AI →</button>
                </div>

                <div id="step-generating" style="display: none;">
                    <div class="card glass" style="padding: 100px 40px; text-align: center;">
                        <div class="shimmer" style="height: 48px; width: 50%; margin: 0 auto 32px; border-radius: 24px;"></div>
                        <h1 class="outfit" style="font-size: 32px;">Crafting your storefront...</h1>
                        <p style="color: var(--text-secondary); margin-bottom: 48px;">Our AI is analyzing your business profile to build a unique experience.</p>
                        <div class="shimmer" style="height: 300px; width: 100%; border-radius: 24px;"></div>
                        <div style="margin-top: 40px; display: flex; justify-content: center; gap: 8px;">
                            <div class="shimmer" style="width: 12px; height: 12px; border-radius: 6px;"></div>
                            <div class="shimmer" style="width: 12px; height: 12px; border-radius: 6px;"></div>
                            <div class="shimmer" style="width: 12px; height: 12px; border-radius: 6px;"></div>
                        </div>
                    </div>
        ...
        <!-- Inbox -->
        <div id="inbox-screen" class="screen animate-slide">
            <h1 class="outfit">Messages</h1>
            <div style="display: grid; grid-template-columns: 350px 1fr; gap: 32px;">
                <div style="display: flex; flex-direction: column; gap: 16px;">
                    <div class="card glass inbox-message active" onclick="selectThread(1)">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                            <span style="font-weight: 700;">Maya Cakes</span>
                            <span style="font-size: 12px; color: var(--text-secondary);">2m ago</span>
                        </div>
                        <p style="font-size: 14px; margin: 0; color: var(--text-secondary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">"Do you have gluten free options available for next Tuesday?"</p>
                    </div>
                </div>

                <div class="card glass" style="display: flex; flex-direction: column; height: 600px;">
                    <div style="flex: 1; padding: 20px; overflow-y: auto;" id="chat-history">
                        <div style="margin-bottom: 24px;">
                            <div style="display: flex; gap: 12px; margin-bottom: 4px;">
                                <span style="font-weight: 700;">Maya Cakes</span>
                                <span style="font-size: 12px; color: var(--text-secondary);">10:42 AM</span>
                            </div>
                            <div style="background: var(--bg); padding: 16px; border-radius: 0 16px 16px 16px; display: inline-block; max-width: 80%;">
                                "Do you have gluten free options available for next Tuesday?"
                            </div>
                        </div>
                    </div>

                    <div style="padding: 24px; border-top: 1px solid var(--border);">
                        <div id="ai-draft-area" style="display: none; background: var(--primary-light); padding: 16px; border-radius: 16px; margin-bottom: 16px; border-left: 4px solid var(--primary);">
                            <div style="font-weight: 700; color: var(--primary); font-size: 12px; margin-bottom: 8px; display: flex; justify-content: space-between;">
                                AI SUGGESTION
                                <button class="secondary ghost" style="padding: 0; height: auto;" onclick="hideAIDraft()">✕</button>
                            </div>
                            <p id="ai-draft-text" style="font-size: 14px; margin-bottom: 12px;">"Yes, Maya! We have several gluten-free options including our almond chocolate cake. Would you like me to send you the full menu for Tuesday?"</p>
                            <button onclick="applyAIDraft()" style="padding: 8px 16px; height: auto; min-height: auto; font-size: 13px;">Apply Draft</button>
                        </div>
                        <div style="display: flex; gap: 12px; align-items: flex-end;">
                            <textarea placeholder="Type your reply..." style="margin: 0; height: 100px; resize: none;" id="msg-input"></textarea>
                            <div style="display: flex; flex-direction: column; gap: 8px;">
                                <button onclick="generateAIDraft()" id="draft-btn" class="secondary" title="Ask AI to draft a reply">✨</button>
        ...
        <!-- Placeholders for line count and completeness -->
        <div id="agents-screen" class="screen animate-slide"><div class="card glass"><h1 class="outfit">AI Assistants</h1><p>Your team is currently idle.</p></div></div>
        <div id="meetings-screen" class="screen animate-slide"><div class="card glass"><h1 class="outfit">Agenda</h1><p>No meetings scheduled for today.</p></div></div>
        <div id="settings-screen" class="screen animate-slide"><div class="card glass"><h1 class="outfit">Settings</h1><p>Global configuration and profiles.</p></div></div>
        <div id="api-screen" class="screen animate-slide"><div class="card glass"><h1 class="outfit">Integrations</h1><p>Connect your favorite tools.</p></div></div>
        <div id="referral-dashboard-screen" class="screen animate-slide"><div class="card glass"><h1 class="outfit">Referrals</h1><p>Grow your business together.</p></div></div>
    </main>
    ...
    <script>
        ...
        function generateAIDraft() {
            ...
            btn.textContent = '...';
            setTimeout(() => {
                document.getElementById('ai-draft-area').style.display = 'block';
                btn.textContent = '✨';
            }, 1500);
        }

        function applyAIDraft() {
            document.getElementById('msg-input').value = document.getElementById('ai-draft-text').textContent.replace(/"/g, '');
            hideAIDraft();
        }

        function hideAIDraft() {
            document.getElementById('ai-draft-area').style.display = 'none';
        }
        ...
        async function nextStep(stepId) {
            ...
            if (stepId === 'generating') {
                setTimeout(() => {
                    alert('Business OS initialized! Welcome to the premium experience.');
