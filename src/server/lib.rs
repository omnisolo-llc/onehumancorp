            ...
            if event.action == "ProductCreated" || event.action == "ProductUpdated" || event.action == "tenant.product.created" || event.action == "tenant.product.updated" {
                if let Ok(payload_str) = String::from_utf8(event.payload.clone()) {
                    if let Ok(payload_json) = serde_json::from_str::<serde_json::Value>(&payload_str) {
                        ...
                        let event_type = if event.action == "ProductCreated" || event.action == "tenant.product.created" {
...
async fn create_ui_bom_item_handler(
    ...
    let app = axum::Router::new()
        ...
        .route("/api/v1/settings/integrations/whatsapp_cloud_api/templates", axum::routing::get(api::integrations_settings::get_whatsapp_templates).with_state(std::sync::Arc::new(crate::integrations::registry::IntegrationsRegistry::new())))
        .route("/api/v1/settings/integrations/whatsapp_cloud_api/health", axum::routing::get(api::integrations_settings::get_whatsapp_health).with_state(std::sync::Arc::new(crate::integrations::registry::IntegrationsRegistry::new())))
        .route("/api/v1/settings/integrations/whatsapp_cloud_api/send_template", axum::routing::post(api::integrations_settings::send_whatsapp_template).with_state(std::sync::Arc::new(crate::integrations::registry::IntegrationsRegistry::new())))
