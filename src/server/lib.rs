...
async fn create_ui_bom_item_handler(
    ...
    let app = axum::Router::new()
        ...
        .route("/api/v1/sync/mesh", axum::routing::post({ let db = db.clone(); let mesh = mesh_transport.clone(); move |headers: axum::http::HeaderMap, payload: axum::Json<api::sync_mesh::MeshSyncRequest>| async move { api::sync_mesh::sync_mesh_handler(axum::extract::State((db.pool.clone(), mesh.clone())), headers, payload).await } }))
