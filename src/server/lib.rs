    ...
    match &db.store {
        crate::db::DbStore::Postgres => {
            let query_str = if mobile_optimized {
                "SELECT o.id, CAST(COALESCE(o.total_amount, 0.0) AS DOUBLE PRECISION) AS total_amount, COALESCE(o.status, '') AS status, COALESCE(o.created_at::text, '') AS created_at FROM orders o WHERE o.tenant_id = $1 ORDER BY o.created_at DESC LIMIT 50"
            } else {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            let query_str = if mobile_optimized {
                "SELECT o.id, CAST(COALESCE(o.total_amount, 0.0) AS REAL) AS total_amount, COALESCE(o.status, '') AS status, COALESCE(CAST(o.created_at AS TEXT), '') AS created_at FROM orders o WHERE o.tenant_id = ? ORDER BY o.created_at DESC LIMIT 50"
            } else {
...
async fn load_ui_priority_tasks_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            // Note: SQLite fallback currently fetches * and filters by tenant in memory.
...
async fn load_ui_agent_feed_from_db(db: &crate::db::DB, tenant_id: &str, mobile_optimized: bool) -> Result<Vec<serde_json::Value>, sqlx::Error> {
    ...
    match &db.store {
        ...
        crate::db::DbStore::Sqlite(pool) => {
            let query_str = if mobile_optimized {
                "SELECT id, tenant_id, event_source, proposed_action, lifecycle_state, CAST(created_at AS TEXT) as created_at, CAST(updated_at AS TEXT) as updated_at FROM agent_feed_items WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?"
            } else {
...
async fn ui_dashboard_unified_feed_handler(
    ...
    // DB queries return optimized payloads for mobile.
