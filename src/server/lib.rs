...
impl HubService for MyHubService {
    async fn get_my_plan(
        ...
        let ai_used = self
            .hub
            .tracker()
            .get_tenant_actions_used(tenant_id)
            .await
            .unwrap_or(0);
        let storage_used_bytes = self
            .hub
            .tracker()
    ...
    async fn poll_tasks(
        ...
        let mapped_tasks: Vec<Result<SharedTask, Status>> = tasks
            .into_iter()
            .map(|task| {
                Ok(SharedTask {
                    id: task.id,
    ...
    async fn get_pending_approvals(
        ...
        let mapped_tasks: Vec<SharedTask> = tasks
            .into_iter()
            .map(|task| SharedTask {
                id: task.id,
                organization_id: task.organization_id,
...
pub async fn run_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vector_repo = std::sync::Arc::new(match &db.store {
        crate::db::DbStore::Postgres => {
            ohc_builtin_agent::memory_store::ConsolidatedMemoryRepository::new(db.pool.clone())
        }
        crate::db::DbStore::Sqlite(sqlite_pool) => {
            ohc_builtin_agent::memory_store::ConsolidatedMemoryRepository::new_sqlite(
                sqlite_pool.clone(),
            )
        }
    });
    ...
    tokio::spawn(async move {
        ...
        let cfg = ohc_builtin_agent::service::AgentConfig {
            ...
            max_tokens: std::env::var("OHC_MAX_TOKENS")
                .ok()
    ...
    let app = axum::Router::new()
        ...
        .route(
            "/api/v1/mesh/connect",
            axum::routing::get(api::mesh_handler::mesh_ws_handler),
        )
        .route(
            "/api/mesh/v2/broadcast",
            axum::routing::post(api::mesh_handler::broadcast_handler),
        )
        .nest(
            "/api/v1/autodream",
            api::autodream::router(autodream_worker.clone()),
        )
        .nest(
            "/api/v1/builder",
            crate::builder::api::router(db.pool.clone()),
        )
        .nest("/api/agents", api::agents::hire::router(hub.clone()))
        .nest(
            "/api/onboarding",
            api::onboarding::router(std::sync::Arc::new(
                crate::services::onboarding::onboarding_agent::OnboardingAgent::new(
                    db.clone(),
                    hub.clone(),
                ),
            ))
        ...
        .nest(
            "/api/v1/growth",
            api::growth::router(db.pool.clone(), hub.clone()),
        )
        .nest(
            "/api/agents/approvals",
            api::agents::approvals::router(dept_orchestrator.clone()),
        )
    ...
    if ::server_config::get().telemetry_enabled {
        ...
        let telemetry_daemon = crate::services::sync::telemetry_sync::TelemetrySyncDaemon::new(
            db.pool.clone(),
            cloud_url.clone(),
        );
