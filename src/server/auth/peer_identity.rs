...
pub fn authenticate_spiffe_request<T>(
    ...
    let claimed_identity = request
        ...
        .transpose()?;
    let peer_certificates = request.extensions().get::<tonic::transport::server::TlsConnectInfo<tonic::transport::server::TcpConnectInfo>>().and_then(|info| info.peer_certs().map(|c| c.to_vec()));
