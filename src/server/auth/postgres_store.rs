...
impl UserRepository for PgUserRepository {
    ...
    async fn get_by_id(&self, id: &str, org_id: &str) -> Result<User, String> {
        let query = if !crate::config::get().multitenant && (org_id == "system" || org_id.is_empty()) {
        ...
        let row = if !crate::config::get().multitenant && (org_id == "system" || org_id.is_empty()) {
    ...
    async fn list_users(&self, org_id: &str) -> Result<Vec<User>, String> {
        ...
        let rows = if !crate::config::get().multitenant && (org_id == "system" || org_id.is_empty()) {
    ...
    async fn update_user(&self, user: User, org_id: &str) -> Result<(), String> {
        ...
        let res = if !crate::config::get().multitenant && (org_id == "system" || org_id.is_empty()) {
            sqlx::query(query)
                .bind(&user.id)
                .bind(&user.username)
                .bind(&user.email)
...
mod security_tests {
    ...
    async fn test_multitenant_idor_system_bypass_prevention() {
        ...
        let org_id = "system";
        let should_bypass = !is_multitenant && (org_id == "system" || org_id.is_empty());
...
mod tests {
    ...
    // Add additional tests to complete the suite if needed
}
