...
#[cfg(test)]
mod sentinel_security_regression_tests {
    use super::*;
    use std::time::Duration;
    use sqlx::postgres::PgPoolOptions;

    #[tokio::test]
    async fn test_tenant_isolation_regression() {
        let database_url = match std::env::var("DATABASE_URL") {
            Ok(url) => url,
            Err(_) => return,
        };
        ...
        ::server_common::auth_utils::set_org_context(&mut *tx, tenant_a).await.unwrap();

        let current_tenant: String = sqlx::query_scalar("SELECT current_setting('app.current_tenant', true)")
            .fetch_one(&mut *tx)
            .await
            .unwrap();

        assert_eq!(current_tenant, tenant_a);
