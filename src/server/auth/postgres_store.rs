...
mod security_tests {
    ...
    async fn test_multitenant_idor_system_bypass_prevention() {
        ...
        let pool = PgPoolOptions::new().after_release(|conn: &mut sqlx::PgConnection, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) })
    ...
    async fn test_revoke_token_uses_transaction_and_tenant_context() {
        ...
        let res: Result<(), String> = repo.revoke_token("test-token-jti".to_string(), exp, "test-tenant").await;
