...
impl UserRepository for PgUserRepository {
    ...
    async fn get_by_id(&self, id: &str, org_id: &str) -> Result<User, String> {
        let query = if !crate::config::get().multitenant && (org_id.is_empty() || org_id == "system") {
        ...
        let row = if !crate::config::get().multitenant && (org_id.is_empty() || org_id == "system") {
    ...
    async fn list_users(&self, org_id: &str) -> Result<Vec<User>, String> {
        ...
        let rows = if !crate::config::get().multitenant && (org_id.is_empty() || org_id == "system") {
    ...
    async fn update_user(&self, user: User, org_id: &str) -> Result<(), String> {
        ...
        let res = if !crate::config::get().multitenant && (org_id.is_empty() || org_id == "system") {
            sqlx::query(query)
                .bind(&user.id)
                .bind(&user.username)
                .bind(&user.email)
