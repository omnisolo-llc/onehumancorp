...
impl UserRepository for PgUserRepository {
    ...
    async fn get_by_id(&self, id: &str, org_id: &str) -> Result<User, String> {
        ...
        let row_opt = sqlx::query(query).bind(id).bind(org_id).fetch_optional(&mut *tx).await.map_err(|e| e.to_string())?;
    ...
    async fn get_by_username(&self, username: &str, org_id: &str) -> Result<User, String> {
        ...
        let row_opt = sqlx::query(query).bind(username).bind(org_id).fetch_optional(&mut *tx).await.map_err(|e| e.to_string())?;
    ...
    async fn get_by_email(&self, email: &str, org_id: &str) -> Result<User, String> {
        ...
        let row_opt = sqlx::query(query).bind(email).bind(org_id).fetch_optional(&mut *tx).await.map_err(|e| e.to_string())?;
    ...
    async fn get_by_oidc_subject(&self, sub: &str, org_id: &str) -> Result<User, String> {
        ...
        let row_opt = sqlx::query(query).bind(sub).bind(org_id).fetch_optional(&mut *tx).await.map_err(|e| e.to_string())?;
