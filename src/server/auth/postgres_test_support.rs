...
async fn initialize_postgres(admin_url: &str) -> Result<(), String> {
    ...
    match MIGRATOR.run(&admin_pool).await { Ok(_) => {}, Err(e) => { if !e.to_string().contains("duplicate key value violates unique constraint") { return Err(format!("run src/server/migrations: {e}")); } } };
