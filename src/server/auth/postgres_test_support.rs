...
async fn initialize_postgres(admin_url: &str) -> Result<(), String> {
    ...
    // Acquire database-level session advisory lock to prevent concurrent migration conflicts across processes
    sqlx::query("SELECT pg_advisory_lock(721377771)")
        .execute(&admin_pool)
        .await
        .map_err(|error| format!("acquire session advisory lock: {error}"))?;

    let migration_result = MIGRATOR.run(&admin_pool).await;

    // Always release the advisory lock
    let _ = sqlx::query("SELECT pg_advisory_unlock(721377771)")
        .execute(&admin_pool)
        .await;
