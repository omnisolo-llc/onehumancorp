...
async fn initialize_postgres(admin_url: &str) -> Result<(), String> {
    ...
    let mut conn = admin_pool.acquire().await.map_err(|e| format!("acquire conn for lock: {e}"))?;
    sqlx::query("SELECT pg_advisory_lock(742385923)").execute(&mut *conn).await.map_err(|e| format!("acquire advisory lock: {e}"))?;
    let res = MIGRATOR.run(&admin_pool).await;
    sqlx::query("SELECT pg_advisory_unlock(742385923)").execute(&mut *conn).await.map_err(|e| format!("release advisory lock: {e}"))?;
