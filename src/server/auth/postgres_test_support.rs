...
async fn initialize_postgres(admin_url: &str) -> Result<(), String> {
    ...
    // We increase max_connections to 2 so that we can hold an advisory lock on one connection
    // and let MIGRATOR run on another.

    let mut conn = admin_pool
        .acquire()
        .await
        .map_err(|error| format!("acquire connection: {error}"))?;

    sqlx::query("SELECT pg_advisory_lock(743289573)")
        .execute(&mut *conn)
        .await
    ...
    sqlx::query("SELECT pg_advisory_unlock(743289573)")
        .execute(&mut *conn)
        .await
