...
"#;
    ...
    pub async fn cleanup_stagnant_missions(
        ...
        let affected = match &self.store {
            DbStore::Sqlite(sqlite_pool) => {
                sqlx::query("DELETE FROM agent_missions WHERE status = 'STUCK' OR ((status = 'PENDING' OR status = 'RUNNING') AND updated_at < ?)")
            ...
            DbStore::Postgres => {
                let mut tx = self.pool.begin().await?;
                ::server_common::auth_utils::set_system_context(&mut *tx).await?;
                let affected = sqlx::query("DELETE FROM agent_missions WHERE status = 'STUCK' OR ((status = 'PENDING' OR status = 'RUNNING') AND updated_at < $1)")
                    .bind(threshold)
                    .execute(&mut *tx)
                    .await?.rows_affected();
                tx.commit().await?;
