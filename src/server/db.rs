...
mod autodream_db_tests {
    ...
    async fn test_local_sqlite_encryption_hardening_mock() {
        ...
        let pool_result = sqlx::sqlite::SqlitePoolOptions::new().after_connect(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("PRAGMA secure_delete = ON").await?; Ok(()) }) })
            .connect_with(opts)
            .await;
