...
pub async fn insert_autodream_memory(
    ...
    pub async fn cleanup_stagnant_missions(&self, timeout_secs: i64) -> Result<u64, Box<dyn std::error::Error>> {
        ...
        let affected = match &self.store {
            DbStore::Sqlite(sqlite_pool) => {
                sqlx::query("UPDATE agent_missions SET status = 'FAILED' WHERE status = 'STUCK' OR ((status = 'PENDING' OR status = 'RUNNING' OR status = 'BURSTING') AND updated_at < ?)")
            ...
            DbStore::Postgres => {
                let mut tx = self.pool.begin().await?;
                ::server_common::auth_utils::set_system_context(&mut *tx).await?;
                let affected = sqlx::query("UPDATE agent_missions SET status = 'FAILED' WHERE status = 'STUCK' OR ((status = 'PENDING' OR status = 'RUNNING' OR status = 'BURSTING') AND updated_at < $1)")
                    .bind(threshold)
                    .execute(&mut *tx)
                    .await?.rows_affected();
                tx.commit().await?;
