...
pub async fn insert_autodream_memory(
    ...
    pub async fn cleanup_stagnant_missions(&self, timeout_secs: i64) -> Result<u64, Box<dyn std::error::Error>> {
        ...
        let affected = match &self.store {
            DbStore::Sqlite(sqlite_pool) => {
                sqlx::query("DELETE FROM agent_missions WHERE (status = 'PENDING' OR status = 'RUNNING' OR status = 'STUCK') AND updated_at < ?")
            ...
            DbStore::Postgres => {
                let mut tx = self.pool.begin().await?;
                set_org_context(&mut *tx, "system").await?;
                let affected = sqlx::query("DELETE FROM agent_missions WHERE (status = 'PENDING' OR status = 'RUNNING' OR status = 'STUCK') AND updated_at < $1")
                    .bind(threshold)
                    .execute(&mut *tx)
                    .await?.rows_affected();
                tx.commit().await?;
