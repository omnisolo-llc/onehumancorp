...
pub fn get_pool() -> PgPool {
    GLOBAL_POOL.get().cloned().unwrap_or_else(|| {
        ...
        sqlx::postgres::PgPoolOptions::new()
            .before_acquire(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("SET app.current_tenant = ''").await?; Ok(true) }) }).after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) })
...
mod e2e_tenant_isolation_tests {
    ...
    async fn test_before_acquire_resets_tenant() {
        ...
        // We can't trivially introspect the options object cleanly to confirm there is a before_acquire hook,
        // but we verify that the pool options can be built successfully and inherently injects a tenant reset.
        let _pool = pool_opts.connect_lazy(database_url).unwrap();

        // If the pool initialized with the `before_acquire` hook, this is a success.
