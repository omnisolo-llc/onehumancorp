...
mod tests {
    ...
    async fn test_fork_agent_state_match() {
        ...
        // Wait for the tokio spawn to run
        for _ in 0..50 {
            {
                let inbox = hub.inbox.read().unwrap();
                if let Some(child_inbox) = inbox.get(&child_id) {
                    if child_inbox.len() == 2 {
                        break;
                    }
                }
            }
