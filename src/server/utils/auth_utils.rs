    ...
    E: Executor<'a, Database = Postgres>,
{
    if org_id == "system" || (!crate::config::get().multitenant && org_id.is_empty()) {
        ...
        // Another option: "SELECT set_config('app.current_tenant', 'system', true)" is all we need! We don't strictly *need* to set current_tenant to empty.
        query("SELECT set_config('app.current_tenant', 'system', true)")
            .execute(executor)
            .await?;
    } else {
