    ...
    E: Executor<'a, Database = Postgres>,
{
    if org_id.is_empty() {
        ...
        // Wait, we can use `query` instead of `executor.execute`, because `query` takes `executor` which we can borrow if we used `&executor`, but wait, we had errors with `&executor` too because E doesn't implement `Executor` for `&mut E`.
