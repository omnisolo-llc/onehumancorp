    ...
    E: Executor<'a, Database = Postgres>,
{
    if org_id.is_empty() {
        ...
        // SET ROLE is used to ensure role is elevated for the session if not in explicit transaction block.
        // The global connection pool hook `after_release` executes `RESET ROLE` to prevent leaks.
