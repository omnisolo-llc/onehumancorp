    ...
    E: Executor<'a, Database = Postgres>,
{
    if crate::config::get().multitenant && org_id.is_empty() {
        return Err(sqlx::Error::Configuration("CRITICAL SECURITY ERROR: organization_id is missing in multi-tenant mode. Access denied to prevent accidental global leak.".into()));
    }
