...
pub async fn tenant_middleware(req: Request, next: Next) -> Response {
    ...
    let is_auth_bypass = path.starts_with("/api/v1/auth") || path.starts_with("/api/agents/webhook") || path.starts_with("/api/v1/webhook") || path.starts_with("/health") || path.starts_with("/api/v1/storefront") || path.starts_with("/metrics");
    if is_auth_bypass {
         return next.run(req).await;
    }
