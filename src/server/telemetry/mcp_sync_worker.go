...
func (w *McpSyncWorker) syncOnce(ctx context.Context) {
	...
	if err != nil {
		slog.Info("McpSyncWorker failed to query telemetry_buffer", "error", err)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &metricName, &value); err != nil {
			slog.Info("McpSyncWorker failed to scan row", "error", err)
			continue
		}
	...
	if err := rows.Err(); err != nil {
		slog.Info("McpSyncWorker row iteration error", "error", err)
		return
	}
	...
	for _, id := range pendingIDs {
		...
		if err != nil {
			slog.Info("McpSyncWorker failed to update status", "id", id, "error", err)
		}
	}
