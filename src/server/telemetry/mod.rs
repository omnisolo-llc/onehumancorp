...
pub fn get_sub_agent_lock_contention_total() -> &'static Counter<u64> {
    SUB_AGENT_LOCK_CONTENTION_TOTAL.get_or_init(|| {
        ...
        meter
            ...
            .with_description("Tracks the number of database lock delays within the QueueManager polling")
            .build()
    })
}
...
pub fn record_sub_agent_queue_delay(delay: f64, deployment_mode: &str) {
    ...
    histogram.record(delay, &[opentelemetry::KeyValue::new("mode", deployment_mode.to_string())]);
}

pub fn record_sub_agent_spawn_error(deployment_mode: &str) {
    let counter = get_sub_agent_spawn_errors_total();
    counter.add(1, &[opentelemetry::KeyValue::new("mode", deployment_mode.to_string())]);
}

pub fn record_sub_agent_lock_contention(deployment_mode: &str, store: &str) {
    let counter = get_sub_agent_lock_contention_total();
    counter.add(1, &[
        opentelemetry::KeyValue::new("mode", deployment_mode.to_string()),
...
pub async fn record_storage_rw_cost(
    ...
    buffer_metric(
        pool,
        "ohc_storage_rw_cost_cents",
        "counter",
        ((size_bytes as f64 / (1024.0 * 1024.0 * 1024.0)) * 0.10 * 100.0) as f32,
        serde_json::json!({
            "organization_id": organization_id,
            "operation": operation,
        }),
...
pub async fn record_email_send_cost(
    ...
    buffer_metric(
        pool,
        "ohc_email_send_cost_cents",
        "counter",
        ((count as f64) * 0.001 * 100.0) as f32,
        serde_json::json!({
            "organization_id": organization_id,
        }),
    )
