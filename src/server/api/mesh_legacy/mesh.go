...
func (m *TeammateMesh) HandleSubscribe(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Info("Failed to upgrade to websocket", "error", err)
		return
	}
	defer conn.Close()
	...
	m.Subscribe(ctx, topic, func(msg MeshMessage) {
		if err := conn.WriteJSON(msg); err != nil {
			slog.Info("Failed to write to websocket", "error", err)
			cancel()
		}
	})
			...
			for {
				...
				case redisMsg := <-ch:
					...
					if err := json.Unmarshal([]byte(redisMsg.Payload), &msg); err != nil {
						slog.Info("Failed to unmarshal mesh message", "error", err)
						continue
					}
					handler(msg)
				}
