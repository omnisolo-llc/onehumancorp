...
pub fn router() -> Router<DeliveryState> {
    Router::new()
        .route("/{tenant_id}/{product_id}", get(get_storefront_product).layer(axum::middleware::from_fn(crate::api::api_cache_simulator::cdn_cache_simulator_middleware)).layer(axum::middleware::from_fn(crate::utils::edge_caching_middleware::edge_caching_middleware)))
