use crate::db::DB;
use ::server_harness::middleware::{connection_vault::{ConnectionVault, supported}, usage_ledger::UsageLedger};
use axum::{
    Json, Router,
    extract::{Path, State},
    http::StatusCode,
...
pub async fn connect_integration_handler(
    ...
    if supported(validated.integration_id.as_str()) {
