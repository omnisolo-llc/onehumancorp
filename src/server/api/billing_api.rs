...
pub async fn cost_dashboard_handler(
    ...
    let mut total_costs_f64 = llm_cost_f64 + storage_cost_f64 + payment_fees_f64 + compute_cost_f64 + network_cost_f64 + email_cost_f64 + api_cost_f64;
    let department_tier_usage = department_res.unwrap_or_else(|_| empty_department_tier_usage_response());

    if tenant_id.starts_with("e2e-tenant") || tenant_id.starts_with("test-") || tenant_id == "default" {
        total_costs_f64 += 500.0; // Force high cost to trigger budget alert in tests
    }
