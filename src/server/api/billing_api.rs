...
pub async fn cost_dashboard_handler(
    ...
    let tenant_id = match request.extensions().get::<::server_auth::orchestration::AuthInfo>() {
        ...
        None => return Json(CostDashboardResponse { total_revenue: 0, total_costs: 0, llm_cost: 0, storage_cost: 0, payment_fees: 0, network_cost: 0, bandwidth_savings: 0, prompt_cache_savings: 0, cache_hit_rate: 0.0, cost_per_1k_tokens: 0.0, period_start: "2024-05-01".to_string(), period_end: "2024-05-31".to_string(), trend: vec![] })
    };
    ...
    // Estimate savings using the calculated average cost per token
    let prompt_cache_savings_f64 = (cached_tokens as f64 / 1000.0) * cost_per_1k_tokens;
    ...
    let resp = CostDashboardResponse {
        ...
        prompt_cache_savings: (prompt_cache_savings_f64 * 100.0).round() as i64,
