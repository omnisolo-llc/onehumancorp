...
pub async fn my_plan_handler(
    ...
    let tier_task = tokio::spawn({
        let tracker = tracker.clone();
        let tid = tenant_id.to_string();
        async move { tracker.get_tenant_tier(&tid).await }
    });
    let ai_used_task = tokio::spawn({
        let tracker = tracker.clone();
        let tid = tenant_id.to_string();
        async move { tracker.get_tenant_actions_used(&tid).await }
    });
    let storage_used_bytes_task = tokio::spawn({
        let tracker = tracker.clone();
        let tid = tenant_id.to_string();
        async move { tracker.get_tenant_storage_used(&tid).await }
    });

    let (tier_res, ai_used_res, storage_used_bytes_res) = tokio::join!(tier_task, ai_used_task, storage_used_bytes_task);
