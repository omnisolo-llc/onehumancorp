            ...
            if let Some(tenant_id) = tenant_id_opt {
                ...
                let advances = sqlx::query!(
                    "SELECT a.id, a.total_owed, a.total_repaid, o.repayment_percentage
                     ...
                     WHERE a.tenant_id = $1 AND a.status = 'active'", tenant_id
                )
                .fetch_all(pool)
                .await
                .unwrap_or_default();
                ...
                for adv in advances {
                    // Safely extract the repayment percentage from the macro result
                    // We fall back to 0.1 (10%) to handle trait bounds for the generic decimal type
                    let repayment_amount = (amount as f64) * 0.1;
                    if repayment_amount > 0.0 {
                        ...
                        .bind(uuid::Uuid::new_v4().to_string()).bind(tenant_id).bind(&adv.id).bind(&payload.id).bind(repayment_amount as f64)
                        .execute(pool)
                        .await;
                    }
                }
