    ...
    S: Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route("/referrals/extend-trial", post(handle_extend_trial))
...
async fn handle_extend_trial(
    Extension(_state): Extension<GrowthState>,
    Json(_req): Json<TrialExtensionRequest>,
) -> Result<Json<TrialExtensionResponse>, StatusCode> {
    // Simulated trial extension logic
    let end_date = chrono::Utc::now() + chrono::Duration::days(30);
    Ok(Json(TrialExtensionResponse {
        success: true,
        new_trial_end: end_date.to_rfc3339(),
    }))
}
