    ...
    S: Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route("/referrals/social-share", post(handle_social_share_reward))
...
async fn handle_social_share_reward(
    Extension(state): Extension<GrowthState>,
    axum::extract::Extension(auth_info): axum::extract::Extension<::server_auth::orchestration::AuthInfo>,
    Json(_req): Json<ReferralIdRequest>,
) -> Result<Json<()>, StatusCode> {
    match sqlx::query("UPDATE tenant_usage_limits SET remaining_ai_actions = remaining_ai_actions + 100 WHERE tenant_id = $1")
        .bind(&auth_info.org_id)
        .execute(&state.pool)
        .await
    {
