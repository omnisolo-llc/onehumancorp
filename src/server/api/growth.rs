...
pub async fn handle_zero_click_generate(
    ...
    for task_title in tasks_to_insert {
        ...
        let org_id_clone = _start_res.organization_id.clone();
        let query = sqlx::query("INSERT INTO shared_tasks (id, tenant_id, title, description, status) VALUES ($1, $2, $3, $4, 'PENDING')")
            .bind(task_id)
            .bind(org_id_clone)
            .bind(task_title)
