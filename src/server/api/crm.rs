...
pub async fn list_opportunities_handler(
    ...
    let tenant_id = match claims.organization_id {
        Some(ref org) if !org.is_empty() => org.clone(),
        _ => query.tenant_id.unwrap_or_else(|| "default".to_string()),
    };
    let mobile_optimized = query.mobile_optimized.unwrap_or(false);

    let cache_key = format!(
        "crm_opportunities:{}:mobile:{}",
    ...
    if let Some((cached, is_stale)) = cache.get_with_swr(&cache_key).await {
        if !is_stale {
            ...
            headers.insert(
                "x-total-count",
                axum::http::HeaderValue::from_str(&cached.total_count.to_string())
        ...
        tokio::spawn(async move {
            ...
            let (list_res, count_res) = tokio::join!(
                ...
                tokio::spawn(async move {
                    ...
                    let count: (i64,) =
                        sqlx::query_as("SELECT count(*) FROM opportunities WHERE tenant_id = $1")
                            .bind(&t_id2)
                            .fetch_one(&mut *tx)
                            .await?;
                    tx.commit().await?;
            ...
            if let (Ok(Ok(rows)), Ok(Ok(total_count)), Some(c)) =
                (list_res, count_res, CRM_OPPORTUNITIES_CACHE.get())
            {
                let _ = c
                    .set(
                        &cache_key_bg,
                        CachedOpportunities { rows, total_count },
    ...
    headers.insert(
        "x-total-count",
        axum::http::HeaderValue::from_str(&total_count.to_string())
            .unwrap_or(axum::http::HeaderValue::from_static("0")),
    );
    let _ = cache
        .set(
...
mod tests {
    ...
    fn test_opportunity_mobile_optimization() {
        ...
        assert_eq!(
            ...
            "lead_id must be preserved for navigation"
        );
    }
}
