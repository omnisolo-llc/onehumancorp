...
impl ResearcherLlmClient for AdapterLlm {
    async fn chat(
        ...
        let (response_text, usage) = if let Some(response) = forced_response {
            ...
            (r#"[{"description": "AI Labor", "unit_price_cents": 15000, "quantity": 1, "is_optional": false, "service_item_id": null}]"#.to_string(), Usage { input_tokens: 10, output_tokens: 20, cache_read_input_tokens: 0, cache_creation_input_tokens: 0 })
        } else {
