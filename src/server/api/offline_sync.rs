...
pub async fn sync_mesh_handler(
    ...
    let batch_id = payload.batch_id.unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
    ...
    for mutation in &payload.mutations {
        ...
        let sync_event_id = uuid::Uuid::new_v4().to_string();
        ...
        if mutation.action_type == "JobCompleted" {
            ...
            let _ = sqlx::query(
                "INSERT INTO department_tasks (id, tenant_id, department, event_type, payload, status)
                 VALUES ($1, $2, 'operations', 'SyncEvent:JobCompleted', $3, 'PENDING')"
            )
            .bind(&ai_task_id)
            .bind(&tenant_id)
            .bind(&ai_payload)
