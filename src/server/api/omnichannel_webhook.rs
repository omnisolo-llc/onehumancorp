...
mod tests {
    ...
    async fn test_handle_omnichannel_webhook() {
        ...
        let res = handle_omnichannel_webhook(State(app_state), axum::Extension(::server_common::Claims { sub: "test".to_string(), organization_id: Some("test".to_string()), iat: 0, exp: 0, username: "".to_string(), email: "".to_string(), roles: vec![], jti: "".to_string(), session_id: None }), Json(payload)).await.into_response();
