...
pub async fn reserve_inventory_handler(
    ...
    let tenant_id = match auth_info {
        ...
        None => {
            ...
            if let Some(tenant_override) = _headers.get("x-tenant-id").and_then(|v| v.to_str().ok()).filter(|_| spiffe_id_str.starts_with("spiffe://ohc.os/org/") && spiffe_id_str.contains("/agent/")) {
