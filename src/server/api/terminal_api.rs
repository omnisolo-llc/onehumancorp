...
pub async fn sync_offline_transactions_handler(
    ...
    let _ = sqlx::query(
        "INSERT INTO pos_terminal_sessions (id, tenant_id, device_id, status, started_at, last_synced_at, offline_changes_count, sync_status)
         VALUES ($1, $2, $3, 'ACTIVE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $4, 'SYNCING')
         ON CONFLICT (tenant_id, device_id) DO UPDATE SET last_synced_at = CURRENT_TIMESTAMP, offline_changes_count = pos_terminal_sessions.offline_changes_count + $4, sync_status = 'SYNCING'"
    )
    .bind(&session_id)
    .bind(&tenant_id)
    .bind(&client_id)
