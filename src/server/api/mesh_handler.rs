        ...
        while let Some(msg) = rx.recv().await {
            ...
            if msg.encode(&mut buf).is_ok() {
                ...
                ::server_telemetry::record_error_signal(
                    "[bug] Failed to encode mesh message to protobuf",
                );
...
mod tests {
    ...
    async fn test_mesh_direct_handler() {
        ...
        let app = Router::new()
            .route(
                "/api/v1/mesh/v2/direct",
                axum::routing::post(direct_handler),
            )
            .with_state(transport);
        ...
        let res = client
            ...
            .header(
                "x-spiffe-id",
                "spiffe://ohc.local/org/example.org/agent/agent-1",
            )
            .json(&req_body)
            .send()
            .await
    ...
    async fn test_mesh_mailbox_handler() {
        ...
        let app = Router::new()
            .route(
                "/api/v1/mesh/v2/mailbox",
                axum::routing::post(mailbox_handler),
            )
            .with_state(transport);
