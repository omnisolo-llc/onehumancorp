...
pub async fn health_handler(
    ...
    // ADDED: Additional health metrics for job queues to improve observability
    let pending_jobs: i64 = sqlx::query_scalar("SELECT count(*) FROM ohc_job_queue WHERE status = 'PENDING'")
        .fetch_one(&hub.pool)
        .await
        .unwrap_or(0);

    let failed_jobs: i64 = sqlx::query_scalar("SELECT count(*) FROM ohc_job_queue WHERE status = 'FAILED'")
        .fetch_one(&hub.pool)
        .await
        .unwrap_or(0);
