    ...
    S: Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route("/orders_and_inventory", get(get_orders_and_inventory_handler))
...
async fn get_orders_and_inventory_handler(
    State(_hub): State<Arc<Hub>>,
    Query(query): Query<PosQuery>,
) -> Json<Value> {
    let tenant_id = query.tenant_id.unwrap_or_else(|| "default".to_string());
    ...
    let (orders_rows, inventory_rows) = tokio::join!(
        sqlx::query("SELECT id, total_amount, status, created_at FROM orders WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 20").bind(&tenant_id).fetch_all(&pool),
        sqlx::query("SELECT id, title, description, price_cents, currency, inventory_count FROM products WHERE tenant_id = $1").bind(&tenant_id).fetch_all(&pool)
    );

    let orders: Vec<Value> = orders_rows.unwrap_or_default().into_iter().map(|row| {
        json!({
            "id": row.get::<String, _>("id"),
            "total_amount": row.get::<f64, _>("total_amount"),
            "status": row.get::<String, _>("status"),
    ...
    let inventory: Vec<Value> = inventory_rows.unwrap_or_default().into_iter().map(|row| {
        json!({
            "id": row.get::<String, _>("id"),
            "name": row.get::<String, _>("title"),
            "description": row.get::<Option<String>, _>("description"),
    ...
    Json(json!({ "orders": orders, "inventory": inventory }))
}
