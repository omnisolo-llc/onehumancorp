    ...
    match tracker.get_task(payload.task_id).await {
        Ok(Some(task)) => {
            if let Err(e) = tracker.update_task_status(payload.task_id, &payload.status, payload.result).await {
                tracing::error!(task = %payload.task_id, "Failed to update MCP task: {}", e);
                return (
            ...
            tracing::info!(agent = %task.agent_id, tenant = %task.tenant_id, task = %task.id, "KAIROS Hook: Reactivating agent");
        ...
        Err(e) => {
            tracing::error!(task = %payload.task_id, "Database error fetching MCP task: {}", e);
            (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(McpWebhookResponse {
                    success: false,
