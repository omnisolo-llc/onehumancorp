...
pub async fn handle_voice_command(
    ...
    if audio_data.is_empty() {
        return (
            ...
            Json(serde_json::json!({"error": "audio data is required"})),
        )
            .into_response();
    }
    ...
    let preferred_language = target_languages
        .first()
        .cloned()
    ...
    let (dept, description, mut action_payload, final_transcription) = if api_key.is_empty()
        || api_key == "fake-key"
    {
        ...
        if transcription.contains("quote") {
            ...
            (
                ...
                format!(
                    "Drafted Quote for Voice Request: ${:.2} total, ${:.2} deposit.",
                    total as f64 / 100.0,
            ...
            (
                ...
                "Voice initiated: 3x Chicken Tacos".to_string(),
                serde_json::json!({
            ...
            Ok(raw_json) => {
                ...
                if let Ok(val) = serde_json::from_str::<serde_json::Value>(clean_res) {
                    ...
                    let desc = val
                        .get("description")
                        .and_then(|v| v.as_str())
                        .unwrap_or(&format!("Voice initiated: {}", translated_text))
                        .to_string();
                    ...
                    (
                        ...
                        serde_json::json!({ "raw_transcription": transcription, "items": ["Chicken Tacos"], "quantities": [3], "special_requests": ["none"] }),
                        transcription.clone(),
                    )
                }
            }
    ...
    if action_payload.get("feature_type").and_then(|v| v.as_str()) == Some("quote_draft") {
        let materials_cents = action_payload
            .get("materials_cents")
            .and_then(|v| v.as_i64())
            .unwrap_or(0);
        let labor_cents = action_payload
            .get("labor_cents")
            .and_then(|v| v.as_i64())
            .unwrap_or(0);
        let total_amount_cents = action_payload
            .get("total_amount_cents")
            .and_then(|v| v.as_i64())
            .unwrap_or(materials_cents + labor_cents);
        let required_deposit_cents = action_payload
            .get("required_deposit_cents")
            .and_then(|v| v.as_i64())
            .unwrap_or(total_amount_cents / 2);
        ...
        if required_deposit_cents > 0 {
            ...
            match stripe_client
                .create_payment_link("Service Deposit", required_deposit_cents)
                .await
            {
                Ok(url) => stripe_payment_link = Some(url),
                Err(e) => tracing::error!(
                    "Failed to generate Stripe payment link for voice quote: {}",
                    e
                ), // pii-safe
            }
        }
        ...
        let customer_id = action_payload
            .get("customer_id")
            .and_then(|v| v.as_str())
            .unwrap_or("00000000-0000-0000-0000-000000000000"); // default dummy or extract actual
