...
{
    Router::new()
        .route("/{department}", post(update_setting).get(get_setting))
...
async fn get_setting(
    ...
    let tenant_id = match claims.organization_id.as_deref() {
        ...
        None => return (StatusCode::UNAUTHORIZED, Json(None::<DepartmentConfig>)).into_response(),
    };

    match orchestrator.get_department_config(&tenant_id, &department).await {
        Ok(config) => (StatusCode::OK, Json(config)).into_response(),
        Err(_) => (StatusCode::INTERNAL_SERVER_ERROR, Json(None::<DepartmentConfig>)).into_response(),
    }
}
