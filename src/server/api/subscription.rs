...
async fn get_subscription_overview(
    ...
    match fetch_subscription_overview(&state.pool, &tenant_id).await {
...
async fn get_plans(
    ...
    match fetch_subscription_plans(&state.pool, &tenant_id).await {
...
async fn get_subscribers(
    ...
    match fetch_subscribers(&state.pool, &tenant_id).await {
...
async fn get_fulfillment_batches(
    ...
    match fetch_fulfillment_batches(&state.pool, &tenant_id).await {
...
async fn create_fulfillment_batch(
    ...
    let service = SubscriptionService::new(Arc::new(state.pool.clone()));
...
async fn get_subscription_by_id(
    ...
    let mut transaction = match begin_subscription_tenant_transaction(&state.pool, &tenant_id).await {
        Ok(transaction) => transaction,
        Err(_) => return (StatusCode::INTERNAL_SERVER_ERROR, "DB Error").into_response(),
    };
