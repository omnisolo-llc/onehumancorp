...
mod tests {
    ...
    async fn test_autodream_sync_endpoint() {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .before_acquire(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("SELECT set_config('app.current_tenant', 'system', false)").await?; Ok(true) }) }).connect_lazy(database_url)
            .unwrap();
