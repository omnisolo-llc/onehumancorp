...
async fn update_feed_item_state(
    ...
    match repo.update_state(&tenant_id, &id, &payload.state).await {
        Ok(updated_item) => {
            ...
            // Execute dynamic action handler based on action intent
            if payload.state == "APPROVED" {
                if let Ok(Some(item)) = repo.get(&tenant_id, &id).await {
                    let mut feature_type = item.event_source.as_str();
                    let payload = item.proposed_action.clone().or(item.context_payload.clone()).unwrap_or(sqlx::types::Json(serde_json::json!({})));

                    if let Some(ft) = payload.get("feature_type").and_then(|v| v.as_str()) {
                        feature_type = ft;
                    }

                    let _ = crate::domain::agent_feed::action_router::ActionRouter::execute(
                        &pool,
                        &tenant_id,
                        feature_type,
                        &payload,
