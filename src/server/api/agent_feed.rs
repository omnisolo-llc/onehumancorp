...
use crate::domain::action_router::get_action_router;
                ...
                if let Ok(Some(item)) = repo.get(&tenant_id, &id).await {
                    ...
                    if item.event_source == "incident_resolution" {
                        if let Some(ref payload) = item.context_payload {
                            if let serde_json::Value::Object(map) = &payload.0 {
                                let val = serde_json::Value::Object(map.clone());
                                let _ = get_action_router().dispatch("incident_resolution", tenant_id.clone(), val, pool.clone()).await;
                            }
                        }
                    }
                    ...
                    if let Some(payload) = item.proposed_action.clone().or(item.context_payload.clone()) {
                        if let serde_json::Value::Object(map) = &payload.0 {
                            let val = serde_json::Value::Object(map.clone());
                            let feature_type = payload.0.get("feature_type").and_then(|v| v.as_str()).unwrap_or("");
                            if !feature_type.is_empty() {
                                let _ = get_action_router().dispatch(feature_type, tenant_id.clone(), val, pool.clone()).await;
                            }
                        }
                    }
                }
