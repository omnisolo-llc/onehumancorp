...
func (d *HybridMCPRAGDaemon) ProcessSync(ctx context.Context) bool {
	...
	if err != nil {
		telemetry.RecordSyncDaemonError(ctx, "Standalone", "transaction_begin_failed")
	...
	if err != nil {
		telemetry.RecordSyncDaemonError(ctx, "Standalone", "query_failed")
	...
	if err := d.sendToCloud(ctx, payloads); err != nil {
		telemetry.RecordSyncDaemonError(ctx, "Standalone", "cloud_send_failed")
	...
	if len(ids) > 0 {
		...
		if err != nil {
			telemetry.RecordSyncDaemonError(ctx, "Standalone", "update_failed")
	...
	if err := tx.Commit(ctx); err != nil {
		telemetry.RecordSyncDaemonError(ctx, "Standalone", "commit_failed")
