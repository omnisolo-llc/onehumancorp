...
func (d *HybridMCPRAGDaemon) SyncPendingMissions(ctx context.Context) error {
	rows, err := d.db.QueryContext(ctx, "SELECT id, status, payload, mission_log FROM agent_missions WHERE synced_to_cloud = false AND (status = 'CLOUD_ESCALATION' OR status = 'BURSTING') LIMIT 100")
	if err != nil {
