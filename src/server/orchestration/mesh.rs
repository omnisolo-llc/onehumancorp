...
impl TeammateMesh for CentrifugeNode {
    ...
    async fn ping(&self) -> Result<(), String> {
        self.publish_with_ack("mesh:health:ping", b"ping".to_vec())
            .await
    }
...
pub async fn get_mesh_transport(
    ...
    match db_store {
        crate::db::DbStore::Postgres => {
            ...
            let transport = ohc_builtin_agent::mesh::transport::RedisTransport::new(&redis_url)
                .await
        ...
        crate::db::DbStore::Sqlite(pool) => {
            ...
            match ohc_builtin_agent::mesh::transport::SqliteTransport::new(pool.clone()).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "Failed to initialize SqliteTransport: {}. Falling back to MemoryTransport.",
                        e
                    );
