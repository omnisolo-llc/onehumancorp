...
impl DepartmentOrchestrator {
                        ...
                        .unwrap_or(false)
                    {
                        ...
                        if !success {
                            ::server_telemetry::record_error_signal(
                                "[cleanup] Dead-letter logging for event  after 3 failed retries. Error",
                            );
                            tracing::error!(
                                "Dead-letter logging for event {} after 3 failed retries. Error: {}",
                                event.id,
                                last_err
                            );
                            let dl_id = Uuid::new_v4().to_string();
                            let redacted_payload =
                                ::server_telemetry::redact_interface_pii(event.payload.clone());
                            let dl_payload =
                            ...
                            match &self.db.store {
                                DbStore::Postgres => {
                                    ...
                                    if let Err(err) = res {
                                        ::server_telemetry::record_error_signal(
                                            "[bug] Failed to insert dead letter into DB",
                                        );
                                        tracing::error!(
                                            "Failed to insert dead letter into DB: {}",
                                            err
                                        );
                                    }
                                    ...
                                    .bind(&event.tenant_id)
                                    .bind(format!("AI Agent Paused: {}", dep_type))
    ...
    pub async fn soft_lock_booking_slot(
        &self,
        tenant_id: &str,
        service_id: &str,
        _suggested_time_str: &str,
        ttl: std::time::Duration,
    ) -> Result<Option<String>, String> {
        // Attempt to parse time or default to next day if natural language (simplified for now)
        let start_time = if let Ok(parsed) =
            chrono::DateTime::parse_from_rfc3339(_suggested_time_str)
        {
        ...
        let lock_acquired = self
            .mesh
            .acquire_lock(&time_id, "sales_agent", ttl.as_secs())
            .await
            .unwrap_or(false);
    ...
    pub async fn confirm_booking_slot(&self, tenant_id: &str, slot_id: &str) -> Result<(), String> {
        match &self.db.store {
            ...
            crate::db::DbStore::Sqlite(pool) => {
                sqlx::query(
                    "UPDATE booking_slots SET status = 'booked' WHERE id = ? AND tenant_id = ?",
                )
                .bind(slot_id)
                .bind(tenant_id)
                .execute(pool)
    ...
    pub async fn execute_action(
        ...
        let within_budget = self
            ...
            .await
            .map_err(|_| "Unable to verify AI action budget".to_string())?;
        if !within_budget {
            tracing::info!(
                "Tenant {} action budget exhausted; action was not admitted.",
                tenant_id
            ); // pii-safe
            return Err(
                "Token budget exhausted. Please upgrade your plan to continue using AI actions."
                    .to_string(),
            );
        }
        ...
        match risk {
            ...
            ActionRisk::DraftForReview => {
                ...
                self.add_approval_request(req.clone()).await?;
                // The persisted in-app feed is the default notification. Paid
                // external notifications require a separate authorized policy.
    ...
    pub async fn add_approval_request(&self, req: ApprovalRequest) -> Result<(), String> {
        if req.tenant_id.trim().is_empty() || req.id.trim().is_empty() {
            return Err("An approval requires tenant and request identities".into());
        }
        let now = Utc::now();
        ...
        if req.action_risk == ActionRisk::DraftForReview
            ...
            && (payload.get("feature_type").and_then(|v| v.as_str()) == Some("invoice_followup")
                || payload.get("feature_type").and_then(|v| v.as_str()) == Some("quote_draft"))
        {
        ...
        match &self.db.store {
            DbStore::Postgres => {
                let mut tx = self
                    ...
                    .begin()
                    .await
                    .map_err(|_| "Approval storage unavailable")?;
                ::server_common::auth_utils::set_org_context(&mut *tx, &req.tenant_id)
                    .await
                    .map_err(|_| "Approval tenant context unavailable")?;
                {
            ...
            DbStore::Sqlite(pool) => {
                let context_payload_str =
                    serde_json::json!({"description": req.description}).to_string();
                ...
                let fetch_res = if let Ok(mut tx) = self.db.pool.begin().await {
                        ...
                        .is_ok()
                    {
                        ...
                        Err(sqlx::Error::Configuration(
                            "failed to set tenant context".into(),
                        ))
                    }
                } else {
                    Err(sqlx::Error::Configuration(
                        "failed to begin tenant query".into(),
                    ))
                };
                if let Ok(rows) = fetch_res {
                    ...
                    for row in rows {
                        ...
                        let payload_opt: Option<serde_json::Value> =
                            match row.try_get::<String, _>("payload") {
    ...
    pub async fn decide_approval(
        ...
        let lock_key = format!("ohc:lock:agent_approval:{}:{}", tenant_id, request_id);
        ...
        let opt_dept_payload = match &self.db.store {
            DbStore::Postgres => {
                let row = if let Ok(mut tx) = self.db.pool.begin().await {
                    ...
                    Err(sqlx::Error::Configuration(
                        "failed to begin tenant update".into(),
                    ))
                };
                match row {
                    Ok(Some(r)) => {
                        ...
                        let payload_val: Option<serde_json::Value> =
                            match r.try_get::<String, _>("payload") {
                                Ok(p) => serde_json::from_str(&p).unwrap_or(None),
                                Err(_) => r.try_get::<serde_json::Value, _>("payload").ok(),
                            };
            ...
            if approved {
                ...
                if let Some(payload) = payload_to_use {
                    if payload.get("feature_type").and_then(|v| v.as_str())
                        == Some("field_service_quote")
                        || payload.get("feature_type").and_then(|v| v.as_str())
                            == Some("autonomous_quote")
                    {
                        let price = payload
                            .get("suggested_price")
                            .and_then(|v| v.as_f64())
                        ...
                        let deposit_amount = payload
                            ...
                            .and_then(|v| v.as_i64())
                            .unwrap_or((price * 0.20 * 100.0) as i64);
                        ...
                        let start_time = payload
                            .get("start_time")
                            .and_then(|v| v.as_str())
                            .unwrap_or(start_time_arr);
                        let end_time = payload
                            .get("end_time")
                            .and_then(|v| v.as_str())
                            .unwrap_or(end_time_arr);
                        ...
                        let stripe =
                            crate::integrations::stripe::client::StripeClient::new(api_key);
                        let stripe_link = stripe
                            .create_checkout_session(
                                &quote_id,
                                &customer_id_to_use,
                                (deposit_amount as f64) / 100.0,
                                None,
                                None,
                                None,
                            )
                        ...
                        let inbox_message_id = payload
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("");
                        let mut generated_reply = payload
                        ...
                        if !inbox_message_id.is_empty()
                            && let Err(e) = sqlx::query("UPDATE inbox_messages SET draft_reply = $1, status = 'auto_replied' WHERE id = $2 AND tenant_id = $3")
                            ...
                            .await
                        {
                            Ok(draft_invoice) => {
                                ...
                                match stripe.finalize_and_send_invoice(&draft_invoice.id).await {
                                    Ok(sent_invoice) => {
                                        tracing::info!(
                                            "Finalized and sent invoice via Stripe: {}",
                                            sent_invoice.id
                                        ); // pii-safe
                                        ...
                                        tracing::info!(
                                            "Omnichannel Dispatcher sent invoice email to {}",
                                            customer_id_to_use
                                        ); // pii-safe
                                    }
                                    Err(e) => {
                    ...
                    if payload.get("feature_type").and_then(|v| v.as_str()) == Some("quote_draft") {
                        ...
                        if let DbStore::Postgres = &self.db.store {
                            ...
                            if !inbox_message_id.is_empty()
                                && let Err(e) = sqlx::query("UPDATE inbox_messages SET draft_reply = ?, status = 'auto_replied' WHERE id = ? AND tenant_id = ?")
                                    .bind(&generated_reply)
                                    .bind(inbox_message_id)
                                    .bind(tenant_id)
                                    .execute(pool)
                ...
                if let Some(payload) = payload_to_use
                    && payload.get("feature_type").and_then(|v| v.as_str())
                        == Some("stockout_restock_and_price")
                    && let Some(product_id) = payload.get("product_id").and_then(|v| v.as_str())
                {
                    ...
                    if let DbStore::Postgres = &self.db.store {
                        ...
                        let reorder_quantity = payload
                            .get("suggested_reorder_quantity")
                            .and_then(|v| v.as_i64())
                            .unwrap_or(50);
                ...
                if let Some(payload) = payload_to_use
                    && payload
                        .get("context")
                        .and_then(|c| c.get("smart_pricing"))
                        .and_then(|v| v.as_bool())
                        .unwrap_or(false)
                    && let Some(product_id) = payload
                        .get("context")
                        .and_then(|c| c.get("product_id"))
                        .and_then(|v| v.as_str())
                {
                    let _discount_amount = payload
                        .get("context")
                        .and_then(|c| c.get("discount_amount"))
                        .and_then(|v| v.as_f64())
                        .unwrap_or(0.0);
                    let now = Utc::now();
                    let _expires_at = now + chrono::Duration::days(2);
                    ...
                    if let DbStore::Postgres = &self.db.store {
                                ...
                                && let Err(e) = sqlx::query("UPDATE products SET price = ?, price_cents = ? WHERE id = ? AND tenant_id = ?")
                                    .bind(new_price)
                                    .bind(new_price_cents)
                                    .bind(product_id)
                                    .bind(tenant_id)
                ...
                if let Some(payload) = payload_to_use {
                            ...
                            == Some("quote_draft")
                    {
                        ...
                        let customer_id = payload
                            ...
                            .and_then(|v| v.as_str())
                            .unwrap_or("");
                        let _generated_reply = payload
                            .get("generated_response")
                            .and_then(|v| v.as_str())
                            .unwrap_or("");
                        tracing::info!(
                            "Simulated sending of personalized invoice reminder for {} to customer {}",
                            invoice_id,
                            customer_id
                        ); // pii-safe
                        ...
                        == Some("dispute_resolution")
                    {
                        ...
                        let stripe =
                            crate::integrations::stripe::client::StripeClient::new(api_key);
                        if let Err(e) = stripe
                            .submit_dispute_evidence(dispute_id, payload.clone())
                            .await
                        {
    ...
    pub async fn update_inbox_message_status(
        &self,
        message_id: &str,
        tenant_id: &str,
        new_status: &str,
        ...
        match &self.db.store {
            DbStore::Postgres => {
                sqlx::query(
                    "UPDATE inbox_messages SET status = $1 WHERE id = $2 AND tenant_id = $3",
                )
                .bind(new_status)
                .bind(message_id)
                .bind(tenant_id)
    ...
    pub async fn update_inbox_message_draft(
        &self,
        message_id: &str,
        tenant_id: &str,
        draft_reply: &str,
        ...
        match &self.db.store {
            DbStore::Postgres => {
                sqlx::query(
                    "UPDATE inbox_messages SET draft_reply = $1 WHERE id = $2 AND tenant_id = $3",
                )
                .bind(draft_reply)
                .bind(message_id)
                .bind(tenant_id)
            ...
            DbStore::Sqlite(pool) => {
                ...
                .map_err(|e| e.to_string())?;
                sqlx::query(
                    "UPDATE omni_inbox_messages SET draft_reply = ? WHERE id = ? AND tenant_id = ?",
                )
                .bind(draft_reply)
                .bind(message_id)
                .bind(tenant_id)
    ...
    pub async fn predict_replenishment(
        &self,
        tenant_id: &str,
        customer_id: &str,
    ) -> Result<Option<String>, String> {
        ...
        for i in 0..(orders.len() - 1) {
            total_duration += orders[i].0 - orders[i + 1].0;
        }
    ...
    pub async fn query_long_term_memory(
        ...
        let records = self
            .memory_repo
            .cross_department_search(tenant_id, query_embedding, limit)
            .await?;
        Ok(records.into_iter().map(|r| r.content).collect())
    }

    pub async fn append_to_timeline(
        &self,
        event: crate::orchestration::departments::types::TimelineEvent,
    ) -> Result<(), String> {
        let meta_str = event
            .metadata
            .map(|v| v.to_string())
    ...
    pub async fn get_customer_timeline(
        ...
        customer_id: &str,
        limit: i64,
    ) -> Result<Vec<crate::orchestration::departments::types::TimelineEvent>, String> {
        let mut results = Vec::new();
        match &self.db.store {
            crate::db::DbStore::Postgres => {
    ...
    pub async fn update_department_config(
        ...
        tenant_id: &str,
        department: &str,
        config: crate::orchestration::departments::types::DepartmentConfig,
    ) -> Result<(), String> {
        let deps = self.departments.read().await;
        let dep_type =
            crate::orchestration::departments::types::DepartmentType::from_str(department)?;
    ...
    pub async fn get_customer360(
        ...
        tenant_id: &str,
        customer_id: &str,
    ) -> Result<Option<crate::orchestration::departments::types::Customer360>, String> {
        match &self.db.store {
            crate::db::DbStore::Postgres => {
                let row = sqlx::query(
                    "SELECT * FROM customer360 WHERE tenant_id = $1 AND customer_id = $2",
                )
                .bind(tenant_id)
                .bind(customer_id)
            ...
            crate::db::DbStore::Sqlite(pool) => {
                let row = sqlx::query(
                    "SELECT * FROM customer360 WHERE tenant_id = ? AND customer_id = ?",
                )
                .bind(tenant_id)
                .bind(customer_id)
                .fetch_optional(pool)
    ...
    pub async fn upsert_customer360(
        &self,
        c: &crate::orchestration::departments::types::Customer360,
    ) -> Result<(), String> {
        let prefs_str = c
            .preferences
            .as_ref()
        ...
        match &self.db.store {
            ...
            crate::db::DbStore::Sqlite(pool) => {
                let exists = sqlx::query(
                    "SELECT 1 FROM customer360 WHERE tenant_id = ? AND customer_id = ?",
                )
                .bind(&c.tenant_id)
                .bind(&c.customer_id)
                .fetch_optional(pool)
    ...
    pub async fn update_customer_mood(
        &self,
        tenant_id: &str,
        customer_id: &str,
        mood: &str,
    ...
    pub async fn get_loyalty_ledger(
        &self,
        tenant_id: &str,
        customer_id: &str,
    ) -> Result<Option<crate::orchestration::departments::types::LoyaltyLedger>, String> {
        match &self.db.store {
            crate::db::DbStore::Postgres => {
                let row = sqlx::query(
                ...
                if let Some(r) = row {
                    use sqlx::Row;
                    Ok(Some(
                        crate::orchestration::departments::types::LoyaltyLedger {
                            id: r.get("id"),
                            tenant_id: r.get("tenant_id"),
                            customer_id: r.get("customer_id"),
                ...
                if let Some(r) = row {
                    ...
                    Ok(Some(
                        crate::orchestration::departments::types::LoyaltyLedger {
                            ...
                            last_updated: Some(
                                r.try_get::<chrono::DateTime<chrono::Utc>, _>("last_updated")
                                    .unwrap_or_else(|_| chrono::Utc::now()),
                            ),
                        },
                    ))
        ...
        match &self.db.store {
            crate::db::DbStore::Postgres => {
                ...
                if let Some(r) = row {
                    ...
                    Ok(Some((
                        r.get("customer_id"),
                        r.get::<f64, _>("total_amount_cents"),
                    )))
                } else {
                    Ok(None)
                }
    ...
    pub async fn get_service_by_name_like(
        &self,
        tenant_id: &str,
        name: &str,
    ...
    pub async fn get_booking(
        ...
        match &self.db.store {
            crate::db::DbStore::Postgres => {
                let row = sqlx::query(
                    "SELECT customer_id FROM bookings WHERE tenant_id = $1 AND id = $2",
                )
                .bind(tenant_id)
                .bind(booking_id)
                .fetch_optional(&self.db.pool)
            ...
            crate::db::DbStore::Sqlite(pool) => {
                let row =
                    sqlx::query("SELECT customer_id FROM bookings WHERE tenant_id = ? AND id = ?")
                        .bind(tenant_id)
                        .bind(booking_id)
                        .fetch_optional(pool)
                        .await
...
mod tests {
    ...
    async fn test_orchestrator_initialization() {
        ...
        assert!(orchestrator.departments.read().await.is_empty());
        assert!(orchestrator.event_subscriptions.read().await.is_empty());
        ...
        orchestrator.register_department(dummy.clone()).await;
        assert_eq!(orchestrator.departments.read().await.len(), 1);
        assert!(
            orchestrator
                .departments
                .read()
                .await
                .contains_key(&DepartmentType::Operations)
        );
        assert_eq!(
            orchestrator
                .event_subscriptions
                ...
                .await
                .get("test_event"),
            Some(&vec![DepartmentType::Operations])
        );
        assert_eq!(
            dummy.read().await.department_type(),
            DepartmentType::Operations
        );
        assert!(
            dummy
