            ...
            if approved {
                ...
                if let Some(payload) = payload_to_use {
                    ...
                    if payload.get("feature_type").and_then(|v| v.as_str()) == Some("quote_draft") {
                        ...
                        if let DbStore::Postgres = &self.db.store {
                            if let Err(e) = sqlx::query("INSERT INTO quotes (id, tenant_id, customer_id, status, total_amount_cents, required_deposit_cents, stripe_payment_link, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())")
                                .bind(&quote_id)
                                .bind(tenant_id)
                                .bind(&customer_id_to_use)
                                .bind("SENT")
                            ...
                            if let Err(e) = sqlx::query("INSERT INTO quotes (id, tenant_id, customer_id, status, total_amount_cents, required_deposit_cents, stripe_payment_link, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)")
                                .bind(&quote_id)
                                .bind(tenant_id)
                                .bind(&customer_id_to_use)
                                .bind("SENT")
