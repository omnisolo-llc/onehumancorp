    ...
    pub async fn decide_approval(&self, request_id: &str, tenant_id: &str, approved: bool) -> Result<(), String> {
        ...
        if let Some((dep, original_payload)) = opt_dept_payload {
            ...
            if approved {
                ...
                // Promoter Agent: Social Promotion Approval
                if let Some(p) = &original_payload {
                    if p.get("feature_type").and_then(|v| v.as_str()) == Some("social_promotion") {
                        let social_payload = serde_json::json!({
                            "tenant_id": tenant_id,
                            "product_name": p.get("product_name"),
                            "captions": p.get("captions"),
                            "approved_at": Utc::now().to_rfc3339()
                        });
                        let _ = self.mesh.publish(
                            "agent:marketing:social_scheduled",
                            serde_json::to_vec(&social_payload).unwrap_or_default()
                        ).await;
                    }
                }
