...
impl DepartmentOrchestrator {
    ...
    pub async fn add_approval_request(&self, req: ApprovalRequest) {
        ...
        match &self.db.store {
            DbStore::Postgres => {
                let _ = sqlx::query(
                    "INSERT INTO agent_approvals (id, tenant_id, department, description, status, action_risk,
                            payload: None, payload, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)"
                )
                .bind(&req.id)
                .bind(&req.tenant_id)
            ...
            DbStore::Sqlite(pool) => {
                let _ = sqlx::query(
                    "INSERT INTO agent_approvals (id, tenant_id, department, description, status, action_risk,
                            payload: None, payload, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                )
                .bind(&req.id)
                .bind(&req.tenant_id)
                .bind(req.department.to_string())
        ...
        match &self.db.store {
            DbStore::Postgres => {
                let fetch_res = if let Some(ref cur) = cursor {
                    sqlx::query("SELECT id, tenant_id, department, description, status, action_risk,
                            payload: None, payload FROM agent_approvals WHERE tenant_id = $1 AND status = 'PENDING' AND id > $2 ORDER BY id ASC LIMIT $3")
                        .bind(tenant_id)
                        .bind(cur)
                        .bind(limit)
                    ...
                    sqlx::query("SELECT id, tenant_id, department, description, status, action_risk,
                            payload: None, payload FROM agent_approvals WHERE tenant_id = $1 AND status = 'PENDING' ORDER BY id ASC LIMIT $2")
                        .bind(tenant_id)
                        .bind(limit)
                        .fetch_all(&self.db.pool)
                        .await
            ...
            DbStore::Sqlite(pool) => {
                let fetch_res = if let Some(ref cur) = cursor {
                    sqlx::query("SELECT id, tenant_id, department, description, status, action_risk,
                            payload: None, payload FROM agent_approvals WHERE tenant_id = ? AND status = 'PENDING' AND id > ? ORDER BY id ASC LIMIT ?")
                        .bind(tenant_id)
                        .bind(cur)
                        .bind(limit)
                        .fetch_all(pool)
                    ...
                    sqlx::query("SELECT id, tenant_id, department, description, status, action_risk,
                            payload: None, payload FROM agent_approvals WHERE tenant_id = ? AND status = 'PENDING' ORDER BY id ASC LIMIT ?")
                        .bind(tenant_id)
                        .bind(limit)
                        .fetch_all(pool)
                        .await
