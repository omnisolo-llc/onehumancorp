...
impl Department for OperationsAgent {
    ...
    async fn handle_event(&self, event: &DepartmentEvent) -> Result<(), String> {
        ...
        if event.event_type == "tenant.inventory.updated" || event.event_type == "tenant.pricing.updated" {
            ...
            let invalidation_event = serde_json::json!({ "event": event.event_type, "tags": tags });
            if let Some(redis_client) = crate::get_redis_client() {
                if let Ok(mut conn) = redis_client.get_multiplexed_async_connection().await {
                    let _: redis::RedisResult<()> = redis::cmd("PUBLISH").arg("cache_invalidation_events").arg(invalidation_event.to_string()).query_async(&mut conn).await;
                }
            }
            ...
            if let Ok(tenant_uuid) = uuid::Uuid::parse_str(&event.tenant_id) {
                ...
                let cache_clone = crate::builder::edge::get_edge_cache();
