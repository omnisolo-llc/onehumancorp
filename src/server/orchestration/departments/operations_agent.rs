...
impl Department for OperationsAgent {
    ...
    async fn handle_event(&self, event: &DepartmentEvent) -> Result<(), String> {
        ...
        if event.event_type == "voice.intent.synced" {
            ...
            self.orchestrator.write_long_term_memory(ohc_builtin_agent::memory_store::EmbeddingRecord {
                id: uuid::Uuid::new_v4().to_string(),
                tenant_id: event.tenant_id.clone(),
                agent_id: "operations_agent".to_string(),
                source_type: "Department".to_string(),
                content: format!("Parsed offline voice intent: {}", transcription),
                embedding: vec![], // Generate later or async
                metadata: Some(serde_json::json!({"department": "Operations", "type": "offline_intent"}).to_string()),
