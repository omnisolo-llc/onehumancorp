...
mod tests {
    ...
    async fn test_marketing_product_created_social_post() {
        ...
        let tenant_id = "test-tenant-marketing-product".to_string();
        let product_id = "test-product-123".to_string();
        ...
        match &db.store {
            DbStore::Postgres => {
                ...
                let _ = sqlx::query("INSERT INTO products (id, tenant_id, title, description, type, inventory_count) VALUES ($1, $2, $3, $4, $5, 10) ON CONFLICT DO NOTHING")
                    .bind(&product_id)
                    .bind(&tenant_id)
                    .bind("Vegan Chocolate Cake")
                    .bind("Delicious dairy-free chocolate cake.")
                    .bind("physical")
                    .execute(&db.pool)
                    .await;
            }
        ...
        let event = DepartmentEvent {
            ...
            payload: serde_json::json!({
                ...
                "media_url": "https://example.com/cake.jpg"
            }),
        };
        ...
        for _ in 0..10 {
            tokio::time::sleep(std::time::Duration::from_millis(50)).await;
            let pending = orchestrator.get_pending_approvals(&tenant_id, None, 100).await;
            if pending.iter().any(|req| req.description.contains("Draft social media post for new product: Vegan Chocolate Cake")) {
                has_social_post_draft = true;
                break;
            }
        }

        assert!(has_social_post_draft, "Marketing Agent should draft a social post when a new product is created.");
    }
}
