...
impl Department for SalesAgent {
    ...
    async fn handle_event(&self, event: &DepartmentEvent) -> Result<(), String> {
        if event.event_type == "agent:sales:approved" {
            // Processing is now handled directly in orchestrator.rs to ensure database tasks and invoices
            // are properly emitted inside the same transaction block/context as the approval.
            return Ok(());
        }
