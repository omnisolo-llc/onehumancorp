...
impl Department for FinanceAgent {
    ...
    fn subscribed_events(&self) -> Vec<String> {
        vec!["payment.received".to_string(), "invoice.overdue".to_string()]
    }

    async fn handle_event(&self, event: &DepartmentEvent) -> Result<(), String> {
        let risk = ActionRisk::DraftForReview;

        let _ = self.request_approval(format!("Handled finance event: {}", event.event_type), event.tenant_id.clone(), risk).await;
        Ok(())
    }

    async fn query_memory(&self, query: &str) -> Result<Vec<String>, String> {
        Ok(vec![format!("Memory result for finance: {}", query)])
    }
