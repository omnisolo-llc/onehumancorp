...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_claim")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
...
func (tm *TaskManager) ReviewTask(ctx context.Context, taskID, agentID string) error {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_review")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
...
func (tm *TaskManager) CompleteTaskWithResult(ctx context.Context, taskID, agentID, result string) error {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_complete")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
...
func (tm *TaskManager) PollTasks(ctx context.Context, agentID string, limit int) ([]*SharedTask, error) {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_poll")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
...
func (tm *TaskManager) UpdateTask(ctx context.Context, task *SharedTask) error {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_update")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
...
func (tm *TaskManager) DeleteTask(ctx context.Context, taskID string) error {
	...
	if tm.db.IsSQLite() {
		if !tm.mu.TryLock() {
			telemetry.RecordPostgresLockContention(ctx, "task_manager_delete")
			tm.mu.Lock()
		}
		defer tm.mu.Unlock()
	}
