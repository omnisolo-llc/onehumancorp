	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &status); err == nil {
			if tm.mesh != nil {
				...
				_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", "", "READY", payloadBytes)
			} else if tm.hub != nil {
...
func (tm *TaskManager) CreateTaskWithPlan(ctx context.Context, organizationID string, missionID string, parentPlanID string, dependencies []string, title, description, priority string) (*SharedTask, error) {
	...
	if tm.mesh != nil {
		...
		_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", task.AssignedAgentID, "CREATE", payloadBytes)
	} else if tm.hub != nil {
		go func() {
...
func (tm *TaskManager) ClaimTask(ctx context.Context, taskID, agentID string) (*SharedTask, error) {
	...
	if tm.mesh != nil {
		...
		_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", agentID, "CLAIM", payloadBytes)
	} else if tm.hub != nil {
		go func() {
			payload := map[string]interface{}{
				"task_id":  task.ID,
...
func (tm *TaskManager) ReviewTask(ctx context.Context, taskID, agentID string) error {
	...
	if tm.mesh != nil {
		...
		_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", agentID, "REVIEW", payloadBytes)
	} else if tm.hub != nil {
		go func() {
			payload := map[string]interface{}{
				"task_id":  taskID,
...
func (tm *TaskManager) CompleteTaskWithResult(ctx context.Context, taskID, agentID, result string) error {
	...
	if tm.mesh != nil {
		...
		_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", agentID, "COMPLETE", payloadBytes)
	} else if tm.hub != nil {
		go func() {
			payload := map[string]interface{}{
				"task_id":  taskID,
...
func (tm *TaskManager) UpdateTask(ctx context.Context, task *SharedTask) error {
	...
	if tm.mesh != nil {
		...
		_ = tm.mesh.PublishTeammateMeshEvent(ctx, "teammate_mesh", task.AssignedAgentID, "UPDATE", payloadBytes)
	} else if tm.hub != nil {
		go func() {
			payload := map[string]interface{}{
				"task_id":  task.ID,
