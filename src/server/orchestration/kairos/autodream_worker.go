...
func NewAutoDreamConsolidator(dbProvider db.Provider, redisClient *redis.Client, llmClient EmbeddingClient) *AutoDreamConsolidator {
...
func (c *AutoDreamConsolidator) ProcessBacklog(ctx context.Context) error {
	...
	for _, mem := range memories {
		...
		if c.redisClient != nil {
			res := c.redisClient.SetNX(ctx, lockKey, "locked", 2*time.Minute)
		...
		func() {
			defer func() {
				if c.redisClient != nil {
					c.redisClient.Del(context.Background(), lockKey)
				}
			}()
