...
func (d *HybridSyncDaemon) ProcessCRDTSync(ctx context.Context) {
	...
	jsonData, err := json.Marshal(redactedPayload)
	if err != nil {
...
func (d *HybridSyncDaemon) SyncLocalToCloud(ctx context.Context, mission *AgentMission) error {
	...
	redactedPayloadToSync := telemetry.RedactInterfacePII(payloadToSync)
	jsonData, err := json.Marshal(redactedPayloadToSync)
	if err != nil {
