...
impl HybridSyncDaemon {
    ...
    pub async fn sync_step(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for row in rows {
            ...
            let res = sqlx::query("INSERT INTO sub_agent_jobs (id, tenant_id, parent_task_id, payload, status, scheduled_at, created_at, updated_at) VALUES ($1, 'system', NULL, $2, 'QUEUED', $3, $3, $3)")
                .bind(&queue_id)
                .bind(payload.to_string())
                .bind(now)
                .execute(&mut *tx)
