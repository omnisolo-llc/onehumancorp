...
impl HybridSyncDaemon {
    ...
    pub async fn prune_stuck_agent_missions(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let sqlite_insert = format!("INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message) SELECT id, tenant_id, 'mission_stuck', 'agent_missions', COALESCE(CAST(payload AS TEXT), '{{}}'), '[cleanup] Mission became stuck' FROM agent_missions WHERE {}", SQLITE_WHERE_CLAUSE);
    ...
    pub async fn prune_stuck_ohc_job_queue(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let sqlite_running_insert = format!("INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message) SELECT id, tenant_id, 'job_stuck', 'ohc_job_queue', COALESCE(CAST(payload AS TEXT), '{{}}'), '[cleanup] Stagnant backlog item stuck in RUNNING for > 1 hour' FROM ohc_job_queue WHERE {}", SQLITE_RUNNING_WHERE);
        ...
        let sqlite_queued_insert = format!("INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message) SELECT id, tenant_id, 'job_failed', 'ohc_job_queue', COALESCE(CAST(payload AS TEXT), '{{}}'), '[cleanup] Stagnant backlog item stuck in QUEUED for > 24 hours' FROM ohc_job_queue WHERE {}", SQLITE_QUEUED_WHERE);
