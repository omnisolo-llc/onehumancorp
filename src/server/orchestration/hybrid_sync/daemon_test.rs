    ...
    temp_env::with_vars(
        ...
        || {
            ...
            tokio::runtime::Builder::new_current_thread()
                ...
                .block_on(async {
                    let daemon =
                        super::daemon::HybridSyncDaemon::new(sqlite_pool.clone(), pg_pool.clone());
...
async fn test_hybrid_sync_pos_offline_transactions() {
    ...
    let synced_row =
        sqlx::query("SELECT status FROM pos_offline_transactions WHERE id = 'pos-tx-1'")
            .fetch_one(&sqlite_pool)
            .await
            .unwrap();
...
async fn test_hybrid_sync_pos_offline_transactions_chaos_degradation() {
    ...
    let _ = sqlx::query("DROP TABLE IF EXISTS pos_offline_transactions CASCADE")
        .execute(&pg_pool)
        .await;
    let daemon = super::daemon::HybridSyncDaemon::new(sqlite_pool.clone(), pg_pool.clone());
    ...
    let synced_row =
        sqlx::query("SELECT status FROM pos_offline_transactions WHERE id = 'pos-tx-chaos-1'")
            .fetch_one(&sqlite_pool)
            .await
            .unwrap();
    let status: String = sqlx::Row::get(&synced_row, "status");
...
async fn test_prune_stuck_missions_and_queue() {
    ...
    // Insert stuck tasks
    sqlx::query("INSERT INTO agent_missions (id, status, last_synced_at, updated_at, tenant_id, payload) VALUES ('stuck_mission_sqlite', 'IN_PROGRESS', datetime('now', '-2 hour'), datetime('now', '-2 hour'), 'tenant1', '{}')")
            .execute(&sqlite_pool).await.unwrap();

    sqlx::query("INSERT INTO agent_missions (id, status, last_synced_at, updated_at, tenant_id, payload) VALUES ('stuck_mission_pg', 'RUNNING', NOW() - INTERVAL '2 hours', NOW() - INTERVAL '2 hours', 'tenant1', '{}')")
            .execute(&pg_pool).await.unwrap();
    ...
    let row_sqlite =
        sqlx::query("SELECT status FROM agent_missions WHERE id = 'stuck_mission_sqlite'")
    ...
    let row_queue_sqlite =
        sqlx::query("SELECT status FROM ohc_job_queue WHERE id = 'stuck_queue_sqlite'")
            .fetch_one(&sqlite_pool)
            .await
            .unwrap();
    ...
    let dl_sqlite_mission: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE event_type = 'mission_stuck'",
    )
    .fetch_one(&sqlite_pool)
    .await
    .unwrap();
    ...
    let dl_pg_mission: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE event_type = 'mission_stuck'",
    )
    .fetch_one(&pg_pool)
    .await
    ...
    let dl_sqlite_job: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE event_type = 'job_stuck'",
    )
    .fetch_one(&sqlite_pool)
    .await
    .unwrap();
    ...
    let dl_pg_job: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE event_type = 'job_stuck'",
    )
    .fetch_one(&pg_pool)
    .await
...
async fn test_prune_stuck_queued_items() {
    ...
    use sqlx::Row;
    let row_queue_sqlite =
        sqlx::query("SELECT status FROM ohc_job_queue WHERE id = \'stuck_queued_sqlite\'")
            .fetch_one(&sqlite_pool)
            .await
            .unwrap();
    assert_eq!(row_queue_sqlite.get::<String, _>("status"), "FAILED");
    ...
    // Verify PG queue is failed
    let row_queue = sqlx::query("SELECT status FROM ohc_job_queue WHERE id = \'stuck_queued_pg\'")
        .fetch_one(&pg_pool)
        .await
        .unwrap();
    assert_eq!(row_queue.get::<String, _>("status"), "FAILED");
    ...
    let row_running_sqlite =
        sqlx::query("SELECT status FROM ohc_job_queue WHERE id = \'stuck_running_sqlite\'")
            .fetch_one(&sqlite_pool)
            .await
            .unwrap();
    ...
    let row_running =
        sqlx::query("SELECT status FROM ohc_job_queue WHERE id = \'stuck_running_pg\'")
            .fetch_one(&pg_pool)
            .await
            .unwrap();
    ...
    // Verify dead letters were created for running jobs
    let dl_sqlite: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE id = 'stuck_running_sqlite'",
    )
    .fetch_one(&sqlite_pool)
    .await
    .unwrap();
    ...
    let dl_pg: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE id = 'stuck_running_pg'",
    )
    .fetch_one(&pg_pool)
    .await
    .unwrap();
...
async fn test_agent_mission_failure_categorization() {
    ...
    sqlx::query("INSERT INTO agent_missions (id, status, last_synced_at, updated_at, tenant_id, payload) VALUES ('stuck_mission_sqlite_cat', 'IN_PROGRESS', datetime('now', '-2 hour'), datetime('now', '-2 hour'), 'tenant1', '{}')")
            .execute(&sqlite_pool).await.unwrap();

    sqlx::query("INSERT INTO agent_missions (id, status, last_synced_at, updated_at, tenant_id, payload) VALUES ('stuck_mission_pg_cat', 'RUNNING', NOW() - INTERVAL '2 hours', NOW() - INTERVAL '2 hours', 'tenant1', '{}')")
            .execute(&pg_pool).await.unwrap();
    ...
    let dl_sqlite: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE id = 'stuck_mission_sqlite_cat'",
    )
    .fetch_one(&sqlite_pool)
    .await
    .unwrap();
    ...
    let dl_pg: (i64,) = sqlx::query_as(
        "SELECT count(*) FROM department_dead_letters WHERE id = 'stuck_mission_pg_cat'",
    )
    .fetch_one(&pg_pool)
    .await
    .unwrap();
