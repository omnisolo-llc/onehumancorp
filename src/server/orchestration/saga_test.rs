...
async fn test_saga_orchestration() {
    ...
    let saga_id = orchestrator
        .start_saga(tenant_id, "ProjectIntake", context)
        .await
        .unwrap();

    // Add steps
    let step1_id = orchestrator
        .add_step(tenant_id, saga_id, "Draft Proposal", "SalesAgent")
        .await
        .unwrap();
    let step2_id = orchestrator
        .add_step(tenant_id, saga_id, "Create Tasks", "OperationsAgent")
        .await
        .unwrap();

    // Complete step 1
    orchestrator
        .complete_step(tenant_id, step1_id)
        .await
        .unwrap();
