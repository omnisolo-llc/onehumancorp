...
func (w *KairosAutoDreamWorker) processCompletedTasks(ctx context.Context) error {
	...
	query := "SELECT id, tenant_id, title, payload FROM shared_tasks WHERE status = 'COMPLETED' LIMIT 50"
	...
	for rows.Next() {
		var t taskData
		if err := rows.Scan(&t.id, &t.tenantID, &t.title, &t.payload); err != nil {
	...
	for _, t := range tasks {
		...
		if err := w.storeEmbedding(ctx, tx, t.tenantID, "autodream", "task_memory", content, embedding); err != nil {
