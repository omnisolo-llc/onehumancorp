...
func (c *CentrifugeNode) Broadcast(ctx context.Context, topic string, event *MeshEvent) error {
...
func (s *HubServiceServerImpl) StreamMeshEvents(req *EventStreamRequest, stream HubService_StreamMeshEventsServer) error {
...
func (s *HubServiceServerImpl) PublishMeshEvent(ctx context.Context, req *PublishMeshEventRequest) (*PublishMessageResponse, error) {
	if req == nil || req.Event == nil {
		return &PublishMessageResponse{Success: false}, fmt.Errorf("invalid request")
	}
	err := s.transport.Publish(ctx, req.Event.Topic, req.Event.Payload)
	return &PublishMessageResponse{Success: err == nil}, err
}
...
type PublishMeshEventRequest struct {
	Event *MeshEvent
}
...
func (UnimplementedHubServiceServer) PublishMeshEvent(context.Context, *PublishMeshEventRequest) (*PublishMessageResponse, error) {
	return nil, nil
}
