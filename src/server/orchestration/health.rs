...
pub async fn run_health_monitor(
    ...
    let sync_pool = monitor_hub.pool.clone();
    ...
    loop {
        tokio::select! {
            _ = sync_interval.tick() => {
                if let Ok(count) = sqlx::query_scalar::<_, i64>("SELECT count(*) FROM agent_missions WHERE _sync_status = 'pending'")
                    .fetch_one(&sync_pool)
                    .await
                {
                    if count > 0 {
                        tracing::warn!("HEALTH MONITOR: {} agent_missions pending sync", count);
                    }
                }
            }
            _ = interval.tick() => {
...
mod tests {
    ...
    async fn test_health_monitor_sync_probe() {
        ...
        // Sleep to let the probe execute and exit without crashing
