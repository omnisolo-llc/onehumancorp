...
impl HandoffManager {
    ...
    pub async fn start_listener(&self) -> Result<Box<dyn Fn() + Send + Sync>, String> {
        ...
        let handler = Box::new(move |msg: MeshMessage| {
            if let Ok(handoff) = SyncStateHandoff::decode(&msg.payload[..]) {
                ...
                tokio::spawn(async move {
                    ...
                    if let Ok(true) = transport
                        .acquire_lock(&lock_key, "handoff_manager", 60)
                        .await
                    {
        ...
        self.transport
            .subscribe("mesh:coordination:handoff", handler)
            .await
    }
    ...
    pub async fn initiate_handoff(
        ...
        self.transport
            .publish("mesh:coordination:handoff", msg)
            .await
    }
}
...
mod tests {
    ...
    async fn test_handoff_manager() {
        ...
        let res = manager
            .initiate_handoff("tenant1", "state1", b"some_state".to_vec())
            .await;
        assert!(res.is_ok());
    ...
    async fn test_handoff_listener() {
        ...
        transport
            .publish("mesh:coordination:handoff", msg2)
            .await
            .unwrap();
