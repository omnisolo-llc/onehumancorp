        ...
        match &self.db.store {
            ...
            DbStore::Sqlite(sqlite_pool) => {
                ...
                let _lock = match lock_result {
                    Ok(guard) => guard,
                    Err(_) => {
                        let _ = ::server_lib::telemetry::record_sqlite_lock_contention(&self.db.pool, "ClaimTask").await;
    ...
    pub async fn update_status(&self, id: &str, new_status: &str, agent_id: &str) -> Result<(), String> {
        ...
        match &self.db.store {
            DbStore::Postgres => {
                ...
                if new_status == "COMPLETED" {
                    let autodream = ::server_lib::autodream::AutoDreamWorker::new(self.db.clone());
