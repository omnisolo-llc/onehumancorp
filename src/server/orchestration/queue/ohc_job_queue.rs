...
impl OHCJobQueue {
    ...
    pub async fn dequeue(&self, job_types: Vec<&str>) -> Result<Option<OHCJob>, String> {
        ...
        let type_placeholders = job_types
            .iter()
            .enumerate()
            .map(|(i, _)| format!("${}", i + 1))
            .collect::<Vec<_>>()
        ...
        let job_opt = tokio::time::timeout(
            std::time::Duration::from_secs(60),
            query.fetch_optional(&mut *tx),
        )
        .await
    ...
    pub async fn cleanup_stale_jobs(&self) -> Result<u64, String> {
        ...
        let query_str = if is_standalone {
            "INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message)
             SELECT lower(hex(randomblob(16))), tenant_id, 'job_failed', 'job_queue', json_insert(COALESCE(CAST(payload AS TEXT), '{}'), '$.original_id', id), '[cleanup] Stagnant backlog item stuck in PENDING for > 24 hours'
             FROM ohc_job_queue
             WHERE status = 'PENDING' AND created_at < datetime('now', '-24 hours')"
        } else {
            "INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message)
             SELECT gen_random_uuid()::text, tenant_id, 'job_failed', 'job_queue', jsonb_set(COALESCE(payload, '{}'::jsonb), '{original_id}', to_jsonb(id::text))::text, '[cleanup] Stagnant backlog item stuck in PENDING for > 24 hours'
             FROM ohc_job_queue
        ...
        if let Err(e) = sqlx::query(query_str).execute(&mut *tx).await {
            ::server_telemetry::record_error_signal(
                "[bug] Failed to insert dead letter for stagnant backlog items",
            );
        ...
        if stagnant_result.rows_affected() > 0 {
            ::server_telemetry::record_error_signal(
                "[cleanup] Stagnant backlog item stuck in PENDING for > 24 hours",
            );
        }
    ...
    pub async fn fail(&self, job_id: &str, max_retries: i32, reason: &str) -> Result<(), String> {
        ...
        let row = sqlx::query(
            "SELECT retry_count, tenant_id, payload FROM ohc_job_queue WHERE id = $1 FOR UPDATE",
        )
        .bind(job_id)
        ...
        if let Some(r) = row {
            ...
            if next_retry >= max_retries {
                ...
                let new_run_after = (chrono::Utc::now()
                    + chrono::Duration::seconds(backoff_seconds as i64))
                .to_rfc3339();
