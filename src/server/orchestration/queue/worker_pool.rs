                                    ...
                                    match result {
                                        Ok(Ok(Ok(()))) => {
                                            if let Err(e) = queue_clone.complete(&job_id).await {
                                                ::server_telemetry::record_error_signal(&format!("[bug] Failed to complete job {}: {}", job_id, e));
                                                tracing::trace!("Failed to complete job {}: {}", job_id, e);
                                            }
                                        }
                                        Ok(Ok(Err(e))) => {
                                            ::server_telemetry::record_error_signal(&format!("[bug] Job handler returned error for {}: {}", job_id, e));
                                            tracing::trace!("Job handler returned error for {}: {}", job_id, e);
                                            if let Err(fail_err) = queue_clone.fail(&job_id, 3, &e).await {
                                                ::server_telemetry::record_error_signal(&format!("[bug] Failed to register fail for job {}: {}", job_id, fail_err));
                                                tracing::trace!("Failed to register fail for job {}: {}", job_id, fail_err);
                                            }
                                        }
                                        Ok(Err(e)) => {
                                            ::server_telemetry::record_error_signal(&format!("[bug] Job panicked/join error for {}: {}", job_id, e));
                                        ...
                                        Err(_) => {
                                            ::server_telemetry::record_error_signal(&format!("[bug] Job {} timed out after {} ms", job_id, timeout_ms));
                                            ...
                                            if let Err(fail_err) = queue_clone.fail(&job_id, 3, "Agent execution exceeded 60-second ML-Resilience timeout rule").await {
                                                ::server_telemetry::record_error_signal(&format!("[bug] Failed to register fail for timed out job {}: {}", job_id, fail_err));
