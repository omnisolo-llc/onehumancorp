...
async fn test_sqlite_task_queue() {
    ...
    sqlx::query("UPDATE ohc_job_queue SET retry_count = 2 WHERE id = 'job-fail-sqlite-dead'")
        .execute(&pool)
        .await
        .unwrap();

    let dequeued_opt = queue
        .dequeue(vec!["test-role".to_string()], 100, 100)
        .await
        .unwrap();
    if dequeued_opt.is_none() {
        return;
    }
...
async fn test_sqlite_fail_backoff() {
    ...
    queue.enqueue(job).await.unwrap();
    // For SQLite enqueue doesn't preserve retry_count (defaults to 0), so we must update it
    sqlx::query("UPDATE ohc_job_queue SET retry_count = 0 WHERE id = 'job-fail-1'")
        .execute(&pool)
        .await
        .unwrap();
    ...
    use sqlx::Row;
    let row = sqlx::query(
        "SELECT retry_count, status, next_retry_at FROM ohc_job_queue WHERE id = 'job-fail-1'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    ...
    let next_retry_at: chrono::DateTime<chrono::Utc> = next_retry_at_str
        .parse()
        .expect("next_retry_at must be a valid ISO 8601 string");
...
async fn test_sqlite_fail_max_retries_dead_letter() {
    ...
    queue
        .fail("job-fail-sqlite-dead", "test reason")
        .await
        .unwrap();

    // Verify job is marked as FAILED
    use sqlx::Row;
    let row = sqlx::query("SELECT status FROM ohc_job_queue WHERE id = 'job-fail-sqlite-dead'")
        .fetch_one(&pool)
        .await
        .unwrap();
    let status: String = row.get("status");
