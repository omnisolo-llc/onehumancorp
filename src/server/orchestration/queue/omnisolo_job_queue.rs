...
pub struct OmniSoloJobQueue {
    pool: Arc<PgPool>,
}

impl OmniSoloJobQueue {
    ...
    pub async fn dequeue(&self, job_types: Vec<&str>) -> Result<Option<OmniSoloJob>, String> {
    ...
    pub async fn cleanup_stale_jobs(&self) -> Result<u64, String> {
        ...
        // This queue owns a PostgreSQL pool, so its maintenance statements must
        // always use PostgreSQL date expressions even when the process is
        // configured for standalone mode elsewhere.
        let query_str = "UPDATE ohc_job_queue
        ...
        let query_str = "INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message)
             SELECT id, tenant_id, 'job_failed', 'job_queue', jsonb_set(COALESCE(payload, '{}'::jsonb), '{original_id}', to_jsonb(id::text))::text, '[cleanup] Stagnant backlog item stuck in PENDING for > 24 hours'
             FROM ohc_job_queue
        ...
        let query_str = "UPDATE ohc_job_queue SET status = 'FAILED', updated_at = CURRENT_TIMESTAMP
             WHERE status = 'PENDING' AND created_at < CURRENT_TIMESTAMP - INTERVAL '24 hours'";
    ...
    pub async fn fail(&self, job_id: &str, max_retries: i32, reason: &str) -> Result<(), String> {
        ...
        if let Some(r) = row {
            ...
            if next_retry >= max_retries {
                ...
                let _ = sqlx::query(
                    r#"
                    INSERT INTO shared_tasks (id, organization_id, title, description, status, action_risk, approval_status, proposed_content)
                    VALUES ($1, $2, 'AI Agent Paused: System Queue', 'A background agent job failed permanently and is paused.', 'PENDING', 'LOW', 'PAUSED', 'System is paused. Please manually check business performance or wait for the system to recover.')
                    "#
                )
