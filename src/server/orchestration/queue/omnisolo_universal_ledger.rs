...
pub struct OmniSoloUniversalLedger {
    pool: Arc<PgPool>,
}

impl OmniSoloUniversalLedger {
    ...
    pub async fn get_entries(&self, tenant_id: &str, limit: i64) -> Result<Vec<OmniSoloLedgerEntry>, String> {
        let mut tx = self.pool.begin().await.map_err(|e| e.to_string())?;
        ::server_common::auth_utils::set_org_context(&mut *tx, tenant_id).await.map_err(|e| e.to_string())?;
