...
// mockPGProviderTaskDB wraps a db.Provider to simulate Postgres
type mockPGProviderTaskDB struct {
	db.Provider
}

func (m *mockPGProviderTaskDB) IsSQLite() bool {
	return false
}

// Intercept queries with FOR UPDATE SKIP LOCKED
func (m *mockPGProviderTaskDB) Begin(ctx context.Context) (db.Tx, error) {
...
func TestTasksDBClaimTask_Postgres(t *testing.T) {
	...
	pgProvider := &mockPGProviderTaskDB{Provider: provider}
