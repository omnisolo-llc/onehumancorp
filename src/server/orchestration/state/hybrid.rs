...
use super::{MeshLockGuard, StateManager};
...
#[async_trait]
impl StateManager for HybridStateManager {
    async fn transition_state(
        ...
        let owner = match &self.db.store {
            DbStore::Postgres => "cloud_state_manager".to_string(),
            DbStore::Sqlite(_) => "standalone_state_manager".to_string(),
        };

        let acquire_future = MeshLockGuard::acquire(self.mesh.clone(), lock_key.clone(), owner, 30);
        ...
        match &self.db.store {
            DbStore::Postgres => {
                ...
                let _lock_guard = match tokio::time::timeout(std::time::Duration::from_secs(2), acquire_future).await {
                    ...
                    Ok(Err(e)) => return Err(e),
                    Err(_) => {
                        tracing::warn!("Lock timeout in HybridStateManager::pull_available_tasks (Postgres), fail-safing to empty list.");
                        return Ok(vec![]);
                    }
                };
                ...
                let rows = match tokio::time::timeout(std::time::Duration::from_secs(2), rows_future).await {
                    ...
                    Err(_) => {
                        tracing::warn!("Database timeout in HybridStateManager::pull_available_tasks (Postgres), fail-safing to empty list.");
                        return Ok(vec![]);
                    }
                };
            ...
            DbStore::Sqlite(pool) => {
                let acquire_and_fetch = async {
                    let lock_guard = MeshLockGuard::acquire(self.mesh.clone(), lock_key.clone(), owner, 30).await?;
                ...
                let (_lock_guard, mut tx, rows) = match tokio::time::timeout(std::time::Duration::from_secs(2), acquire_and_fetch).await {
                    ...
                    Ok(Err(e)) => {
                        if e.contains("Timeout acquiring lock") {
                            tracing::warn!("Lock timeout in HybridStateManager::pull_available_tasks (Sqlite), fail-safing to empty list.");
                            return Ok(vec![]);
                        }
                        return Err(e);
                    },
                    Err(_) => {
                        tracing::warn!("Database/Lock timeout in HybridStateManager::pull_available_tasks (Sqlite), fail-safing to empty list.");
                        return Ok(vec![]);
                    }
                };
