...
impl DbStateManager {
    ...
    async fn transition_state_inner(
        ...
        // Depending on Postgres vs SQLite for system tasks
        match &self.db.store {
            DbStore::Postgres => {
                let mut tx = self.db.pool.begin().await.map_err(|e| e.to_string())?;
                ::server_common::auth_utils::set_org_context(&mut *tx, "system").await.map_err(|e| e.to_string())?;

                // 1. Verify current state. Use specific locking for Postgres.
                let row = sqlx::query("SELECT status, dependencies, payload, tenant_id FROM swarm_tasks WHERE id = $1::uuid FOR UPDATE")
                    .bind(task_id)
                ...
                if to_state == "EXECUTING" {
                    ...
                    let deps_str = deps_val.to_string();
            ...
            DbStore::Sqlite(sqlite_pool) => {
                ...
                // SQLite handles concurrency via its connection pool and lock guard, no row-level lock needed here.
                let row = sqlx::query("SELECT status, dependencies, tenant_id FROM swarm_tasks WHERE id = $1")
                    .bind(task_id)
                    .fetch_optional(&mut *tx)
                    .await.map_err(|e| e.to_string())?;
                ...
                let now = Utc::now();
                sqlx::query(
                    "UPDATE swarm_tasks SET status = $1, assigned_agent_id = $2, updated_at = $3 WHERE id = $4"
                )
                .bind(to_state)
                .bind(agent_id)
                .bind(now.to_rfc3339())
...
#[async_trait]
impl StateManager for DbStateManager {
    async fn transition_state(
        ...
        let transition_future = async {
            let lock_guard = MeshLockGuard::acquire(self.mesh.clone(), lock_key.clone(), "db_state_manager".to_string(), 30).await?;
    ...
    async fn pull_available_tasks(&self, limit: i64) -> Result<Vec<SharedTask>, String> {
        ...
        let acquire_future = MeshLockGuard::acquire(self.mesh.clone(), lock_key.clone(), "db_state_manager".to_string(), 30);
        let _lock_guard = match tokio::time::timeout(std::time::Duration::from_secs(2), acquire_future).await {
            Ok(Ok(guard)) => guard,
            Ok(Err(e)) => return Err(e),
            Err(_) => {
                tracing::warn!("Lock timeout in DbStateManager::pull_available_tasks, fail-safing to empty list.");
                return Ok(vec![]);
            }
        };
        ...
        match &self.db.store {
            DbStore::Postgres => {
                ...
                let rows = match tokio::time::timeout(std::time::Duration::from_secs(2), rows_future).await {
                    ...
                    Err(_) => {
                        tracing::warn!("Database timeout in DbStateManager::pull_available_tasks, fail-safing to empty list.");
                        return Ok(vec![]);
                    }
                };
