use super::{StateManager, MeshLockGuard};
    ...
    async fn transition_state_inner(
        ...
        if is_sqlite {
            let sqlite_pool = match &self.db.store {
                ...
                _ => return Err("UniversalStateManager internal error: Expected DbStore::Sqlite".to_string()),
            };
...
#[async_trait]
impl StateManager for UniversalStateManager {
    async fn transition_state(
        ...
        let acquire_future = MeshLockGuard::acquire(self.mesh.clone(), lock_key.clone(), "universal_state_manager".to_string(), 30);
    ...
    async fn pull_available_tasks(&self, limit: i64) -> Result<Vec<SharedTask>, String> {
        ...
        let _lock_guard = match tokio::time::timeout(std::time::Duration::from_secs(2), acquire_future).await {
            ...
            Ok(Err(e)) => return Err(e),
            Err(_) => {
                tracing::warn!("Lock timeout in UniversalStateManager::pull_available_tasks, fail-safing to empty list.");
                return Ok(vec![]);
            }
        };
        ...
        if is_sqlite {
            let sqlite_pool = match &self.db.store {
                DbStore::Sqlite(pool) => pool,
                _ => return Err("UniversalStateManager internal error".to_string()),
            };
            let mut tx = sqlite_pool.begin().await.map_err(|e| e.to_string())?;
            ...
            let rows = match tokio::time::timeout(std::time::Duration::from_secs(2), rows_future).await {
                ...
                Err(_) => {
                    tracing::warn!("Database timeout in UniversalStateManager::pull_available_tasks, fail-safing to empty list.");
                    return Ok(vec![]);
                }
            };
