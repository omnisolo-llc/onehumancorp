...
impl TransportLockGuard {
    async fn acquire(transport: Arc<dyn MeshTransport>, key: String) -> Result<Self, String> {
        let acquired = transport.acquire_lock(&key, "cloud_state_manager", 30).await?;
        if acquired {
...
impl StateManager for CloudStateManager {
    async fn transition_state(
        ...
        let lock_guard = TransportLockGuard::acquire(self.transport.clone(), lock_key).await?;
