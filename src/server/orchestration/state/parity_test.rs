...
mod parity_tests {
    ...
    async fn setup_sqlite_db() -> Arc<DB> {
        ...
        let db = DB {
            pool: PgPoolOptions::new().after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) }).connect_lazy("postgres://localhost/dummy?statement_cache_capacity=0").unwrap(),
