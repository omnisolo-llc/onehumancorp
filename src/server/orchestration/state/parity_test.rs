...
mod parity_tests {
    ...
    async fn test_parity_null_handling() {
        ...
        // SQLite
        if let DbStore::Sqlite(pool) = &sqlite_db.store {
            sqlite_db.execute_with_retry("insert_task", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES (?, 'tenant_1', ?, NULL, ?)")
                .bind(&task_id)
                .bind(mission_id)
                .bind(title)
                .execute(pool)
        ...
        if let Some(ref db) = pg_db {
            ...
            db.execute_with_retry("insert_task", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES ($1, 'tenant_1', $2, NULL, $3)")
                .bind(parsed_id)
                .bind(mission_id)
                .bind(title)
                .execute(&db.pool)
    ...
    async fn test_parity_null_equivalence() {
        ...
        // SQLite
        if let DbStore::Sqlite(pool) = &sqlite_db.store {
            sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES (?, 'tenant_1', ?, NULL, ?)")
                .bind(&task_id1)
                .bind(mission_id)
                .bind(title)
                .execute(pool)
            ...
            sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES (?, 'tenant_1', ?, 'plan_1', ?)")
                .bind(&task_id2)
                .bind(mission_id)
                .bind(title)
                .execute(pool)
        ...
        if let Some(ref db) = pg_db {
            ...
            sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES ($1, 'tenant_1', $2, NULL, $3)")
                .bind(parsed_id1)
                .bind(mission_id)
                .bind(title)
                .execute(&db.pool)
            ...
            sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, parent_plan_id, title) VALUES ($1, 'tenant_1', $2, 'plan_1', $3)")
                .bind(parsed_id2)
                .bind(mission_id)
                .bind(title)
                .execute(&db.pool)
    ...
    async fn test_parity_transaction_isolation() {
        ...
        if let DbStore::Sqlite(pool) = &sqlite_db.store {
            sqlite_db.execute_with_retry("insert_task_tx", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, title, status) VALUES (?, 'tenant_1', ?, ?, \'PENDING\')")
                .bind(&task_id)
                .bind(mission_id)
                .bind(title)
                .execute(pool)
        ...
        if let Some(ref db) = pg_db {
            let parsed_id = uuid::Uuid::parse_str(&task_id).unwrap();
            db.execute_with_retry("insert_task_tx", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, title, status) VALUES ($1, 'tenant_1', $2, $3, \'PENDING\')")
                .bind(parsed_id)
                .bind(mission_id)
                .bind(title)
                .execute(&db.pool)
    ...
    async fn test_parity_timezones() {
        ...
        if let DbStore::Sqlite(pool) = &sqlite_db.store {
            sqlite_db.execute_with_retry("insert_task_tz", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, title, status, created_at) VALUES (?, 'tenant_1', ?, ?, \'PENDING\', ?)")
                .bind(&task_id)
                .bind(mission_id)
                .bind(title)
                .bind(dt)
        ...
        if let Some(ref db) = pg_db {
            let parsed_id = uuid::Uuid::parse_str(&task_id).unwrap();
            db.execute_with_retry("insert_task_tz", || async { sqlx::query("INSERT INTO swarm_tasks (id, tenant_id, mission_id, title, status, created_at) VALUES ($1, 'tenant_1', $2, $3, \'PENDING\', $4)")
                .bind(parsed_id)
                .bind(mission_id)
                .bind(title)
                .bind(dt)
