...
mod parity_tests {
    ...
    #[tokio::test]
    async fn test_parity_transition_state() {
        let sqlite_db = setup_sqlite_db().await;
        let pg_db = setup_postgres_db().await;

        let task_id = uuid::Uuid::new_v4().to_string();
        let mission_id = "mission_123";
        let title = "Test Transition State Title";

        // We insert a task in PENDING state
        let dt = chrono::Utc::now();
        ...
        if let DbStore::Sqlite(pool) = &sqlite_db.store {
            ...
            let row = sqlx::query("SELECT status, assigned_agent_id FROM swarm_tasks WHERE id = ?")
                .bind(&task_id)
                .fetch_one(pool)
                .await
                .unwrap();
            ...
            let assigned_agent_id: String = row.try_get("assigned_agent_id").unwrap_or_default();

            assert_eq!(status, "IN_PROGRESS");
            assert_eq!(assigned_agent_id, "agent_1");
        }
        ...
        if let Some(ref db) = pg_db {
            ...
            let row = sqlx::query("SELECT status, assigned_agent_id FROM swarm_tasks WHERE id = $1")
                .bind(parsed_id)
                .fetch_one(&db.pool)
                .await
                .unwrap();
