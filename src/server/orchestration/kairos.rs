...
impl KairosOrchestrator {
    ...
    pub async fn submit_for_approval(&self, task_id: &str, tenant_id: &str, proposed_content: &str, risk: &str) -> Result<(), KairosError> {
        let now = Utc::now();
        match &self.db.store {
            DbStore::Postgres => {
                ...
                sqlx::query(
                    "UPDATE shared_tasks SET approval_status = 'PENDING', proposed_content = $1, action_risk = $2, updated_at = $3 WHERE id = $4 AND (organization_id = $5 OR tenant_id = $5)"
                )
            ...
            DbStore::Sqlite(sqlite_pool) => {
                ...
                sqlx::query(
                    "UPDATE shared_tasks SET approval_status = 'PENDING', proposed_content = ?, action_risk = ?, updated_at = ? WHERE id = ? AND (organization_id = ? OR tenant_id = ?)"
                )
                .bind(proposed_content)
                .bind(risk)
                .bind(now.to_rfc3339())
    ...
    pub async fn approve_task(&self, task_id: &str, tenant_id: &str, approved: bool) -> Result<(), KairosError> {
        ...
        match &self.db.store {
            DbStore::Postgres => {
                ...
                sqlx::query(
                    "UPDATE shared_tasks SET approval_status = $1, status = $2, updated_at = $3 WHERE id = $4 AND (organization_id = $5 OR tenant_id = $5)"
                )
            ...
            DbStore::Sqlite(sqlite_pool) => {
                ...
                sqlx::query(
                    "UPDATE shared_tasks SET approval_status = ?, status = ?, updated_at = ? WHERE id = ? AND (organization_id = ? OR tenant_id = ?)"
                )
                .bind(new_approval_status)
                .bind(new_status)
                .bind(now.to_rfc3339())
