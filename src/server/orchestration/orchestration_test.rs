...
async fn test_task_decomposition_service() {
    ...
    let db = DB { pool: _dummy_pool.clone(), store: DbStore::Sqlite(sqlite_pool) };
...
async fn test_task_decomposition_dag_blocked() {
    let sqlite_pool = sqlx::sqlite::SqlitePoolOptions::new()
        ...
        .await
        .expect("Failed to initialize database");
    let _dummy_pool = sqlx::postgres::PgPoolOptions::new().after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("DISCARD ALL").await?; Ok(true) }) })
