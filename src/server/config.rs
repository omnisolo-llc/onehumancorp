...
mod tests {
    ...
    fn test_load_defaults() {
        ...
        temp_env::with_vars(vec![("OHC_LISTEN_ADDR", None::<&str>), ("DATABASE_URL", None::<&str>)], || {
            let cfg = load().unwrap();
    ...
    fn test_load_env_vars() {
        let _lock = ENV_MUTEX.lock().unwrap();
        temp_env::with_vars(vec![("OHC_LISTEN_ADDR", Some(":9999")), ("DATABASE_URL", Some("postgres://localhost/testdb"))], || {
            let cfg = load().unwrap();
    ...
    fn test_telemetry_enabled_override() {
        let _lock = ENV_MUTEX.lock().unwrap();
        temp_env::with_vars(vec![("OHC_TELEMETRY_ENABLED", Some("true"))], || {
            let cfg = load().unwrap();
