        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for tenant_id in tenants {
                    ...
                    // Collect context from the canonical booking/order tables. The historical
                    // calendar_events/pos_orders names were never in the active PostgreSQL schema.
                    let mut unconfirmed_bookings = 0;
                    match &db.store {
                        crate::db::DbStore::Postgres => {
                            if let Ok(count) = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM bookings WHERE tenant_id = $1 AND status IN ('pending', 'pending_payment') AND created_at > CURRENT_TIMESTAMP - INTERVAL '7 days'")
                                .bind(&tenant_id)
                        ...
                        crate::db::DbStore::Sqlite(_) => {
                            if let Ok(count) = sqlx::query_scalar::<_, i32>("SELECT COUNT(*) FROM bookings WHERE tenant_id = $1 AND status IN ('pending', 'pending_payment') AND created_at > datetime('now', '-7 days')")
                                .bind(&tenant_id)
                                .fetch_one(&db.pool).await {
