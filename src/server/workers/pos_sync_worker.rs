    ...
    pub async fn handle(&self, job: crate::queue::Job) -> Result<Result<(), String>, String> {
        ...
        if let Some(mutation) = payload.get("mutation") {
            ...
            if let Ok(Some(row)) = current_stock_res {
                ...
                let _ = sqlx::query("UPDATE products SET inventory_count = GREATEST(0, inventory_count - $1), available_quantity = GREATEST(0, available_quantity - $1), pn_counter_n = pn_counter_n + $1 WHERE id = $2 AND tenant_id = $3")
