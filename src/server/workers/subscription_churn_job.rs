                ...
                match &db.store {
                    crate::db::DbStore::Postgres => {
                        let rows = sqlx::query(
                            ...
                            FROM subscribers
                            WHERE status = 'ACTIVE'
                            AND (last_health_check_at IS NULL OR last_health_check_at <= NOW() - INTERVAL '24 hours')
                            "#
                        )
                        .fetch_all(&db.pool)
                        .await
                    ...
                    crate::db::DbStore::Sqlite(sqlite_pool) => {
                         let rows = sqlx::query(
                            ...
                            FROM subscribers
                            WHERE status = 'ACTIVE'
                            AND (last_health_check_at IS NULL OR datetime(last_health_check_at) <= datetime('now', '-24 hours'))
                            "#
                        )
