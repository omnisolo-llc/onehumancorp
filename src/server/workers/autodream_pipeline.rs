...
mod tests {
    ...
    async fn test_autodream_pipeline_sqlite() {
        ...
        let pg_pool = sqlx::postgres::PgPoolOptions::new().max_connections(1).acquire_timeout(std::time::Duration::from_millis(10)).connect_lazy("postgres://dummy").unwrap();
        ...
        let pg_pool = sqlx::postgres::PgPoolOptions::new().max_connections(1).acquire_timeout(std::time::Duration::from_millis(10)).connect_lazy("postgres://postgres:postgres@localhost:5432/test").unwrap_or_else(|_| panic!("Failed lazy"));
