            ...
            loop {
                ...
                if !has_recent_booking {
                    ...
                    let customer_name = match &db.store {
                        ...
                        .await
                        .unwrap_or(None)
                        .unwrap_or("Valued Customer".to_string()),
                    ...
                    match &db.store {
                        crate::db::DbStore::Postgres => {
                            let service =
                                crate::services::agent_feed::service::AgentFeedService::new(
                                    pool.clone(),
                                );
                            let _ = service
                                .process_event(&tenant_id, "sales", &context_payload)
                                .await;
                        }
                        crate::db::DbStore::Sqlite(_) => {
                            // AgentFeedService only supports Postgres pool currently, so we fallback for sqlite manually
                            let ai_prompt = format!(
                                "Generate a friendly, context-aware re-engagement SMS message for a customer named {} who hasn't booked in a while. Suggest they book a new slot.",
                                customer_name
                            );
