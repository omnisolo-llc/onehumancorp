            ...
            loop {
                ...
                if let Some((job_id, tenant_id, payload)) = job {
                    if let (Some(subscriber_id), Some(customer_id)) = (
                        ...
                        let (status, last_engagement_at) = match &db.store {
                                crate::db::DbStore::Postgres => {
                                    sqlx::query("SELECT status, last_engagement_at FROM subscribers WHERE id = $1 AND tenant_id = $2")
                                        .bind(subscriber_id).bind(&tenant_id).fetch_optional(&mut **postgres_transaction.as_mut().expect("postgres transaction")).await.unwrap_or(None).map(|r| {
                        ...
                        match &db.store {
                            crate::db::DbStore::Postgres => {
                                let _ = sqlx::query("UPDATE subscribers SET health_score = $1 WHERE id = $2 AND tenant_id = $3").bind(health_score).bind(subscriber_id).bind(&tenant_id).execute(&mut **postgres_transaction.as_mut().expect("postgres transaction")).await;
                            }
                            crate::db::DbStore::Sqlite(sqlite_pool) => {
                            ...
                            let already_drafted = match &db.store {
                                    crate::db::DbStore::Postgres => {
                                        sqlx::query("SELECT id FROM agent_action_requests WHERE tenant_id = $1 AND payload->>'subscriber_id' = $2 AND created_at > NOW() - INTERVAL '30 days'")
                                            .bind(&tenant_id).bind(subscriber_id).fetch_optional(&mut **postgres_transaction.as_mut().expect("postgres transaction")).await.unwrap_or(None).is_some()
                                    },
                                    crate::db::DbStore::Sqlite(sqlite_pool) => {
                    ...
                    match &db.store {
                        crate::db::DbStore::Postgres => {
                            let _ = sqlx::query("UPDATE ohc_job_queue SET status = 'COMPLETED', updated_at = NOW() WHERE id = $1").bind(job_id).execute(&mut **postgres_transaction.as_mut().expect("postgres transaction")).await;
                        }
                        crate::db::DbStore::Sqlite(sqlite_pool) => {
                            let _ = sqlx::query("UPDATE ohc_job_queue SET status = 'COMPLETED', updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(job_id).execute(sqlite_pool).await;
                        }
                    }
                }
                ...
                if let Some(transaction) = postgres_transaction {
                    if let Err(error) = transaction.commit().await {
                        tracing::warn!("subscription_health worker failed to commit: {}", error);
                    }
                }
            }
        });
