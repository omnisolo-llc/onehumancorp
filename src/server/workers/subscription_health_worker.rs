        ...
        tokio::spawn(async move {
            loop {
                match self.poll().await {
                    ...
                    Err(e) => {
                        tracing::error!("SubscriptionHealthWorker error: {}", e);
                        tokio::time::sleep(Duration::from_millis(5000)).await;
                    }
                }
            }
    ...
    pub async fn poll(&self) -> Result<bool, String> {
        let job = match &self.db.store {
            crate::db::DbStore::Postgres => {
                ...
                let row = sqlx::query(
                    ...
                    FROM ohc_job_queue
                    WHERE status = 'PENDING' AND next_retry_at <= NOW() AND job_type = 'subscription_health_check'
            ...
            crate::db::DbStore::Sqlite(sqlite_pool) => {
                ...
                let row = sqlx::query(
                    ...
                    SELECT id, tenant_id, payload
                    FROM ohc_job_queue
                    WHERE status = 'PENDING' AND next_retry_at <= CURRENT_TIMESTAMP AND job_type = 'subscription_health_check'
                    ORDER BY next_retry_at ASC, created_at ASC
                    LIMIT 1
                    "#
                )
        ...
        if let Some((job_id, tenant_id, payload)) = job {
            if let Some(subscription_id) = payload.get("subscription_id").and_then(|v| v.as_str()) {
                ...
                let reason = payload.get("reason").and_then(|v| v.as_str()).unwrap_or("no activity in 30 days");

                let proposed_action = serde_json::json!({
                    "action_type": "subscription_churn_risk",
                    "draft_reply": format!("Hi {}, we noticed you haven't booked a lesson with us in a while. Is everything okay? We'd love to offer you a free 15-minute consultation to get back on track!", customer_name),
                    "subscription_id": subscription_id,
                    "customer_id": customer_id
                });

                let context_payload = serde_json::json!({
                    "description": format!("Health Score dropped due to {}", reason),
