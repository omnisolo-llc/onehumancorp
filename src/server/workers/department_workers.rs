                        ...
                        if let Ok(payload_json) = serde_json::from_str::<serde_json::Value>(&payload_str) {
                            ...
                            if !product_id.is_empty() && !product_name.is_empty() {
                                        ...
                                        _ => {
                                            ...
                                            if attempts == MAX_RETRIES {
                                                let _ = sqlx::query(
                                                    ...
                                                    VALUES ($1, $2, 'marketing', 'The AI agent responsible for drafting social posts is paused because the AI service is unavailable.', 'DRAFT', 'LOW', '{"feature_type": "promoter_draft", "title": "AI Agent Paused: Marketing"}'::jsonb, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                                                    "#
                                                )
                                                .bind(Uuid::new_v4().to_string())
                                                .bind(&org_id)
                                ...
                                let payload = serde_json::json!({
                                    "feature_type": "promoter_draft",
                                    "title": title,
                                    "captions": parsed
                                });
                                let redacted = ::server_telemetry::redact_interface_pii(payload);
                                let proposed_content = serde_json::to_string(&redacted).unwrap_or_else(|_| "{}".to_string());
                                ...
                                match &db_for_products.store {
                                    crate::db::DbStore::Postgres => {
                                        let _ = sqlx::query(
                                            r#"
                                            INSERT INTO agent_approvals (id, tenant_id, department, description, status, action_risk, payload, created_at, updated_at)
                                            VALUES ($1, $2, 'marketing', $3, 'DRAFT', 'LOW', $4::jsonb, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                                            "#
                                        )
                                        .bind(&task_id)
                                        .bind(&org_id)
                                    ...
                                    crate::db::DbStore::Sqlite(pool) => {
                                        let _ = sqlx::query(
                                            r#"
                                            INSERT INTO agent_approvals (id, tenant_id, department, description, status, action_risk, payload, created_at, updated_at)
                                            VALUES (?, ?, 'marketing', ?, 'DRAFT', 'LOW', ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                                            "#
                                        )
                                        .bind(&task_id)
                                        .bind(&org_id)
