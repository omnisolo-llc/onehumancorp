        ...
        if let Some((id, tenant_id, payload, event_type)) = task {
            ...
            if event_type == "InventoryConflictEvent" {
                ...
                let description = format!(
                    "Inventory discrepancy detected. We oversold the item {} by {}. Should I cancel the online order or draft a rush supply order for transaction {}?",
                    product_id, deficit, transaction_id
                );

                let mut drafted_msg = format!(
                    "We oversold the item {} by {}. Please advise if we should cancel order {} or draft a restock.",
                    product_id, deficit, transaction_id
                );

                let prompt = format!(
                    "Draft a concise message to our customer apologizing that their order {} for product {} is delayed because we oversold it by {} units due to an inventory sync issue. Offer them a refund or a delayed shipment.",
                    transaction_id, product_id, deficit
                );
...
impl PromoterWorker {
    ...
    pub fn start(&self) {
        ...
        tokio::spawn(async move {
            let mut product_rx = hub_for_products
        ...
        tokio::spawn(async move {
            let mut promoter_rx = hub_for_promoter
