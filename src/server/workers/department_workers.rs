...
async fn connect_to_hub() -> Result<
    ::server_ohc::orchestration::hub_service_client::HubServiceClient<tonic::transport::Channel>,
    String,
> {
    let url = std::env::var("OHC_HUB_URL").unwrap_or_else(|_| "http://127.0.0.1:8081".to_string());
    let endpoint = match tonic::transport::Endpoint::from_shared(url) {
        Ok(e) => e.connect_timeout(std::time::Duration::from_secs(2)),
        Err(e) => return Err(format!("Invalid URL: {}", e)),
    };
    ::server_ohc::orchestration::hub_service_client::HubServiceClient::connect(endpoint)
        .await
        ...
        if let Some((id, tenant_id, payload, event_type)) = task {
            ...
            if event_type == "InventoryConflictEvent" {
                ...
                while attempts < MAX_RETRIES {
                    let ai_op = async {
                        if let Ok(mut client) = connect_to_hub().await {
                                        ...
                                        let ai_op = async {
                                            if let Ok(mut client) = connect_to_hub().await {
                                                ...
                                                if let Ok(res) = client
                                                    .reason(tonic::Request::new(reason_req))
                                                    .await
                                                {
                                                    return Ok(res.into_inner().content);
                                                }
                    ...
                    match Self::poll(&db).await {
                        ...
                        Err(e) => {
                            ::server_telemetry::record_error_signal(
                                "[bug] CustomerSuccessWorker error",
                            );
            ...
            if let Ok(api_key) = std::env::var("MINIMAX_API_KEY") {
                if !api_key.is_empty() {
                    ...
                    while attempts < MAX_RETRIES {
                        match timeout(
                            AI_AGENT_TIMEOUT,
                            minimax.reason(&::server_pricing::compression::reduce_tokens(&prompt)),
                        )
                        .await
                        {
                            Ok(Ok(res)) => {
                ...
                if event.action == "ProductCreated" || event.action == "ProductUpdated" {
                    if let Ok(payload_str) = String::from_utf8(event.payload.clone()) {
                            ...
                            serde_json::from_str::<serde_json::Value>(&payload_str)
                        {
                            ...
                            if let Some(pid) =
                                payload_json.get("product_id").and_then(|p| p.as_str())
                            {
                            ...
                            if !product_id.is_empty() && !product_name.is_empty() {
                                ...
                                while attempts < MAX_RETRIES {
                                    let ai_op = async {
                                        if let Ok(mut client) = connect_to_hub().await {
                                            let reason_req =
                                                ::server_ohc::orchestration::ReasonRequest {
                                                    prompt:
                                                        ::server_pricing::compression::reduce_tokens(
                                                            &prompt,
                                                        ),
                                                    from_agent_id: "The Promoter".into(),
                                                };
                                            if let Ok(res) =
                                                client.reason(tonic::Request::new(reason_req)).await
                                            {
                                                return Ok(res.into_inner().content);
                                            }
                                        }
...
impl AdvisorWorker {
    ...
    pub async fn periodic_check(db: Arc<DB>) {
        ...
        for tenant_id in tenants {
            ...
            let mut product_counts: std::collections::HashMap<String, i32> =
            ...
            let thresholds = vec![
                (
                    ...
                    "revenue_100k",
                    "🌟 Six-Figure Club",
                    "Your business has surpassed $100,000 in total revenue!",
                ),
                (
                    10000.0,
                    "revenue_10k",
                    "💎 Five-Figure Club",
                    "Your business has surpassed $10,000 in total revenue!",
                ),
                (
                    1000.0,
                    "revenue_1k",
                    "💰 Four-Figure Club",
                    "Your business has surpassed $1,000 in total revenue!",
                ),
            ];
            ...
            for (threshold, milestone_type, milestone_title, milestone_desc) in thresholds {
                if gross_sales >= threshold {
                    let milestone_msg =
                        format!("{} (Current: ${:.2})", milestone_desc, gross_sales);
                    let milestone_id = Uuid::new_v4().to_string();
...
mod tests {
    ...
    async fn test_operations_worker_inventory_check() {
        ...
        if let DbStore::Sqlite(pool) = &db.store {
            ...
            let row = sqlx::query(
                "SELECT title, approval_status FROM shared_tasks WHERE tenant_id = 'tenant1'",
            )
            .fetch_optional(pool)
            .await
            .unwrap();
            ...
            let status: String =
                sqlx::query_scalar("SELECT status FROM department_tasks WHERE id = 'task1'")
                    .fetch_one(pool)
                    .await
                    .unwrap();
    ...
    async fn test_operations_worker_predictive_inventory_check() {
        ...
        if let DbStore::Sqlite(pool) = &db.store {
            ...
            // Verify task was marked COMPLETED
            let status: String =
                sqlx::query_scalar("SELECT status FROM department_tasks WHERE id = 'task2'")
                    .fetch_one(pool)
                    .await
                    .unwrap();
            assert_eq!(status, "COMPLETED");
    ...
    async fn test_operations_worker_inventory_conflict() {
        ...
        if let DbStore::Sqlite(pool) = &db.store {
            ...
            let status: String =
                sqlx::query_scalar("SELECT status FROM department_tasks WHERE id = 'task3'")
                    .fetch_one(pool)
                    .await
                    .unwrap();
            assert_eq!(status, "COMPLETED");
    ...
    async fn test_customer_success_worker_draft_reply() {
        ...
        if let DbStore::Sqlite(pool) = &db.store {
            ...
            assert!(
                title == "Draft Reply"
                    || title == "AI Agent Paused: Customer Success"
    ...
    async fn test_advisor_worker_revenue_milestone() {
        ...
        if let DbStore::Sqlite(pool) = &db.store {
            ...
            let ledger_entries: Vec<(String, serde_json::Value)> = sqlx::query_as(
                "SELECT action_type, state_change FROM ohc_universal_ledger WHERE tenant_id = ?",
            )
            .bind(tenant_id)
            .fetch_all(pool)
            .await
    ...
    async fn test_operations_worker_100_orders_milestone() {
        ...
        let _ =
            sqlx::query("CREATE TABLE IF NOT EXISTS orders (id TEXT PRIMARY KEY, tenant_id TEXT);")
                .execute(&pool)
                .await;
        let _ = sqlx::query("CREATE TABLE IF NOT EXISTS business_milestones (id TEXT PRIMARY KEY, tenant_id TEXT, milestone_type TEXT, reached_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(tenant_id, milestone_type));").execute(&pool).await;
        let _ = sqlx::query("CREATE TABLE IF NOT EXISTS shared_tasks (id TEXT PRIMARY KEY, tenant_id TEXT, title TEXT, description TEXT, status TEXT, priority TEXT, action_risk TEXT, approval_status TEXT, proposed_content TEXT);").execute(&pool).await;
        ...
        for i in 0..99 {
            let _ =
                sqlx::query("INSERT INTO orders (id, tenant_id) VALUES ($1, 'tenant-100-orders')")
                    .bind(format!("order-{}", i))
                    .execute(&pool)
                    .await;
        }
        ...
        let _ = sqlx::query(
            "INSERT INTO orders (id, tenant_id) VALUES ('order-100', 'tenant-100-orders')",
        )
        .execute(&pool)
        .await;
