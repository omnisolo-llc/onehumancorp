-- 057_harden_remaining_rls_policies.sql
-- Drop vulnerable policies with empty string checks and recreate securely
...
CREATE POLICY tenant_isolation_swarm_memory ON swarm_memory USING (organization_id = current_setting('app.current_tenant', true));
...
CREATE POLICY tenant_isolation_agent_status ON agent_status USING (organization_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS tenant_isolation_capability_plugins ON capability_plugins;
CREATE POLICY tenant_isolation_capability_plugins ON capability_plugins USING (organization_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS tenant_isolation_swarm_memory_embeddings ON swarm_memory_embeddings;
CREATE POLICY tenant_isolation_swarm_memory_embeddings ON swarm_memory_embeddings USING (organization_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS tenant_isolation_telemetry_buffer ON telemetry_buffer;
CREATE POLICY tenant_isolation_telemetry_buffer ON telemetry_buffer USING (organization_id = current_setting('app.current_tenant', true));
...
CREATE POLICY tenant_isolation_meeting_rooms ON meeting_rooms USING (organization_id = current_setting('app.current_tenant', true));

DROP POLICY IF EXISTS tenant_isolation_agent_inbox ON agent_inbox;
CREATE POLICY tenant_isolation_agent_inbox ON agent_inbox USING (organization_id = current_setting('app.current_tenant', true));
...
CREATE POLICY tenant_isolation_products ON products USING (organization_id = current_setting('app.current_tenant', true));
...
CREATE POLICY tenant_isolation_tool_integrations ON tool_integrations USING (tenant_id = current_setting('app.current_tenant', true));
