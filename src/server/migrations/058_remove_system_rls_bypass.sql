-- 058_remove_system_rls_bypass.sql
-- Drop vulnerable policies with system bypass and recreate securely
...
CREATE POLICY tenant_isolation_task_dependencies ON task_dependencies USING (
    task_id IN (SELECT id FROM shared_tasks WHERE organization_id = current_setting('app.current_tenant', true))
);
