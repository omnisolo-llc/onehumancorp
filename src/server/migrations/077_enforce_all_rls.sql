-- +goose Up
-- Migration 077: Systematically enforce Row Level Security on all tenant-specific tables
...
-- Fix Missing RLS Policies on remaining tables

-- Add tenant_id if not exists (most probably do, but we ensure it for safety or rely on dynamic RLS script)
DO $$
DECLARE
    t_name text;
    pol_name text;
