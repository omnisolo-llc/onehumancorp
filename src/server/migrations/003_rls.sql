...
-- Enable Row Level Security (RLS) for all tables introduced in 002_missing_tables.sql

-- Note: agent_session_data does not have a tenant_id or organization_id column
...
-- Note: swarm_tasks does not have a tenant_id or organization_id column
...
CREATE POLICY tenant_isolation_hybrid_fs_sync_queue ON hybrid_fs_sync_queue USING (organization_id::text = current_setting('app.current_tenant', true));
