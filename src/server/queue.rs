...
impl TaskQueue for PostgresTaskQueue {
    ...
    async fn dequeue(&self, roles: Vec<String>) -> Result<Option<Job>, String> {
        ...
        sqlx::query("SELECT set_config('app.current_tenant', 'system', true)").execute(&mut *tx).await.map_err(|e| e.to_string())?;
...
impl QueueManager {
    ...
    pub async fn poll(&self, worker_id: &str) -> Result<Option<SubAgentJob>, sqlx::Error> {
        ...
        sqlx::query("SELECT set_config('app.current_tenant', 'system', true)").execute(&mut *tx).await?;
