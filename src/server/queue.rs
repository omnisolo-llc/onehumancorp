...
impl TaskQueue for PostgresTaskQueue {
    ...
    async fn dequeue(&self, roles: Vec<String>) -> Result<Option<Job>, String> {
        ...
        let row = sqlx::query("UPDATE sub_agent_queue SET status = 'RUNNING' WHERE id = (SELECT id FROM sub_agent_queue WHERE status = 'PENDING' AND scheduled_at <= CURRENT_TIMESTAMP AND payload::json->>'agent_role' = ANY($1) ORDER BY scheduled_at ASC LIMIT 25 FOR UPDATE SKIP LOCKED) RETURNING id, organization_id, parent_task_id, payload, status, scheduled_at")
            .bind(&roles)
...
impl QueueManager {
    ...
    pub async fn poll(&self, worker_id: &str) -> Result<Option<SubAgentJob>, sqlx::Error> {
        ...
        let row = sqlx::query("UPDATE sub_agent_queue SET status = 'RUNNING', worker_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = (SELECT id FROM sub_agent_queue WHERE status = 'QUEUED' ORDER BY created_at ASC LIMIT 25 FOR UPDATE SKIP LOCKED) RETURNING id, organization_id, parent_task_id, payload, status, worker_id, created_at, updated_at")
            .bind(worker_id)
            .fetch_optional(&mut *tx)
            .await?;
        tx.commit().await?;
