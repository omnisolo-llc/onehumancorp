...
pub struct Job {
    pub id: String,
    pub tenant_id: tenant_id: String,
...
pub trait TaskQueue: Send + Sync {
    ...
    async fn dequeue(&self, roles: Vec<String>) -> Result<Option<Job>, String>;
        async fn complete(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), String>;
    async fn fail(&self, job_id: &str, tenant_id: tenant_id: &str, reason: &str) -> Result<(), String>;
...
impl TaskQueue for MemoryTaskQueue {
    ...
    async fn complete(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), String> {
        if let Some(mut job) = self.jobs.get_mut(job_id) {
            if job.tenant_id: tenant_id != tenant_id: tenant_id {
...
impl TaskQueue for PostgresTaskQueue {
    async fn requeue(&self, job: Job) -> Result<(), String> {
        ...
        set_org_context(&mut *tx, &job.tenant_id: tenant_id).await.map_err(|e| e.to_string())?;
        sqlx::query("UPDATE sub_agent_queue SET status = 'QUEUED', payload = $3, scheduled_at = $4 WHERE id = $1 AND tenant_id: tenant_id = $2")
            .bind(&job.id)
            .bind(&job.tenant_id: tenant_id)
            .bind(new_payload)
            .bind(job.run_after)
            .execute(&mut *tx)
            .await
    ...
    async fn enqueue_batch(&self, jobs: Vec<Job>) -> Result<(), String> {
        ...
        for job in &jobs {
            let org_id = if job.tenant_id: tenant_id.is_empty() {
                let payload_map: serde_json::Value = serde_json::from_str(&job.payload).unwrap_or_else(|_| serde_json::json!({}));
                payload_map["tenant_id: tenant_id"].as_str().unwrap_or("").to_string()
            } else {
        ...
        let counts: Vec<(String, i64)> = sqlx::query_as("SELECT tenant_id: tenant_id, COUNT(*) FROM sub_agent_queue WHERE tenant_id: tenant_id = ANY($1) AND status = 'PENDING' GROUP BY tenant_id: tenant_id")
            .bind(&org_ids_vec)
            .fetch_all(&mut *tx)
            .await
            .unwrap_or_default();

        for (tenant_id: tenant_id, count) in counts {
            current_depths.insert(tenant_id: tenant_id, count);
        }

        let mut builder = sqlx::QueryBuilder::new("INSERT INTO sub_agent_queue (id, tenant_id: tenant_id, parent_task_id, payload, status, scheduled_at) ");
    ...
    async fn enqueue(&self, job: Job) -> Result<(), String> {
        ...
        let count_row: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM sub_agent_queue WHERE tenant_id: tenant_id = $1 AND status = 'PENDING'")
            .bind(&org_id)
            .fetch_one(&mut *tx)
            .await
            .unwrap_or((0,));
        ...
        sqlx::query("INSERT INTO sub_agent_queue (id, tenant_id: tenant_id, parent_task_id, payload, status, scheduled_at) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(job.id)
            .bind(org_id)
    ...
    async fn dequeue(&self, roles: Vec<String>) -> Result<Option<Job>, String> {
        ...
        let row = sqlx::query("UPDATE sub_agent_queue SET status = 'RUNNING' WHERE id = (SELECT id FROM sub_agent_queue WHERE status = 'PENDING' AND scheduled_at <= CURRENT_TIMESTAMP AND payload::json->>'agent_role' = ANY($1) ORDER BY scheduled_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING id, tenant_id: tenant_id, parent_task_id, payload, status, scheduled_at")
            .bind(&roles)
        ...
        if let Some(row) = row {
            ...
            let tenant_id: tenant_id: String = row.get("tenant_id: tenant_id");
    ...
    async fn complete(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), String> {
        sqlx::query("UPDATE sub_agent_queue SET status = 'COMPLETED', completed_at = CURRENT_TIMESTAMP WHERE id = $1 AND tenant_id: tenant_id = $2")
            .bind(job_id)
    ...
    async fn fail(&self, job_id: &str, tenant_id: tenant_id: &str, reason: &str) -> Result<(), String> {
        ...
        sqlx::query("UPDATE sub_agent_queue SET status = 'FAILED', payload = COALESCE(payload::jsonb, '{}'::jsonb) || $2::jsonb WHERE id = $1 AND tenant_id: tenant_id = $3")
            .bind(job_id)
            .bind(error_payload)
            .bind(tenant_id: tenant_id)
            .execute(&self.pool)
                        ...
                        Ok(Some(job)) => {
                            ...
                            match handler_res {
                                Ok(_) => {
                                    ...
                                    let _ = self.queue.complete(&job.id, &job.tenant_id: tenant_id).await;
                                }
                                Err(e) => {
                                    ...
                                    if job.attempts < job.max_attempts {
                                        ...
                                        let _ = self.queue.fail(&job.id, &job.tenant_id: tenant_id, &e).await;
                                    }
                                }
                            }
                        }
...
impl QueueManager {
    ...
    pub async fn enqueue(&self, job: SubAgentJob) -> Result<(), sqlx::Error> {
        ...
        set_org_context(&mut *tx, &job.tenant_id: tenant_id).await?;
        sqlx::query("INSERT INTO sub_agent_queue (id, tenant_id: tenant_id, parent_task_id, payload, status, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)")
            .bind(job.id)
            .bind(job.tenant_id: tenant_id)
            .bind(job.parent_task_id)
            .bind(payload_str)
    ...
    pub async fn poll(&self, worker_id: &str) -> Result<Option<SubAgentJob>, sqlx::Error> {
        ...
        let row = loop {
            match sqlx::query("UPDATE sub_agent_queue SET status = 'RUNNING', worker_id = $1, updated_at = CURRENT_TIMESTAMP WHERE id = (SELECT id FROM sub_agent_queue WHERE status = 'QUEUED' AND (scheduled_at IS NULL OR scheduled_at <= CURRENT_TIMESTAMP) ORDER BY created_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING id, tenant_id: tenant_id, parent_task_id, payload, status, worker_id, created_at, updated_at")
                .bind(worker_id)
                .fetch_optional(&mut *tx)
                .await {
                Ok(r) => break r,
        ...
        if let Some(row) = row {
            ...
            Ok(Some(SubAgentJob {
                id: row.get("id"),
                tenant_id: tenant_id: row.get("tenant_id: tenant_id"),
    ...
    pub async fn mark_completed(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), sqlx::Error> {
        let mut tx = self.pool.begin().await?;
        set_org_context(&mut *tx, tenant_id: tenant_id).await?;
        let row = sqlx::query("UPDATE sub_agent_queue SET status = 'COMPLETED', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND tenant_id: tenant_id = $2 RETURNING updated_at, created_at")
            .bind(job_id)
            .bind(tenant_id: tenant_id)
            .fetch_optional(&mut *tx)
            .await?;
    ...
    pub async fn requeue(&self, job_id: &str, tenant_id: tenant_id: &str, payload: serde_json::Value) -> Result<(), sqlx::Error> {
        ...
        let mut tx = self.pool.begin().await?;
        set_org_context(&mut *tx, tenant_id: tenant_id).await?;
        sqlx::query("UPDATE sub_agent_queue SET status = 'QUEUED', payload = $3, updated_at = CURRENT_TIMESTAMP, scheduled_at = CURRENT_TIMESTAMP + INTERVAL '5 seconds' WHERE id = $1 AND tenant_id: tenant_id = $2")
            .bind(job_id)
            .bind(tenant_id: tenant_id)
            .bind(payload_str)
            .execute(&mut *tx)
    ...
    pub async fn mark_failed(&self, job_id: &str, _reason: &str, tenant_id: tenant_id: &str) -> Result<(), sqlx::Error> {
        let mut tx = self.pool.begin().await?;
        set_org_context(&mut *tx, tenant_id: tenant_id).await?;
        sqlx::query("UPDATE sub_agent_queue SET status = 'FAILED', updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND tenant_id: tenant_id = $2")
            .bind(job_id)
            .bind(tenant_id: tenant_id)
            .execute(&mut *tx)
            .await?;
                            ...
                            Ok(Some(mut job)) => {
                                ...
                                match handler_res {
                                    Ok(_) => {
                                        ...
                                        let _ = self.mark_completed(&job.id, &job.tenant_id: tenant_id).await;
                                    }
                                    Err(e) => {
                                        ...
                                        if attempts < max_attempts {
                                            ...
                                            let _ = self.requeue(&job.id, &job.tenant_id: tenant_id, retry_job.payload).await;
                                        } else {
                                            let _ = self.mark_failed(&job.id, &e, &job.tenant_id: tenant_id).await;
                                        }
                                    }
                                }
                            }
...
impl TaskQueueService {
    ...
    pub async fn push_task(&self, task: SharedTaskModel) -> Result<(), sqlx::Error> {
        ...
        let mut tx = self.pool.begin().await?;
        set_org_context(&mut *tx, &task.tenant_id: tenant_id).await?;
        sqlx::query("INSERT INTO shared_tasks (id, parent_id, epic_id, title, status, assigned_agent, payload, tenant_id: tenant_id, dependencies, action_risk, approval_status, proposed_content) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10, $11, $12)")
            .bind(task.id)
            .bind(task.parent_id)
            .bind(task.epic_id)
            .bind(task.title)
    ...
    pub async fn get_completed_tasks(&self, limit: i64) -> Result<Vec<SharedTaskModel>, sqlx::Error> {
        let rows = sqlx::query("SELECT id, tenant_id: tenant_id, parent_id, epic_id, title, status, assigned_agent, payload, dependencies::text AS dependencies, created_at, updated_at, action_risk, approval_status, proposed_content FROM shared_tasks WHERE status = 'COMPLETED' LIMIT $1")
            .bind(limit)
            .fetch_all(&self.pool)
            .await?;
...
impl TaskQueue for SqliteTaskQueue {
    async fn enqueue_batch(&self, jobs: Vec<Job>) -> Result<(), String> {
        if jobs.is_empty() { return Ok(()); }
        let mut builder = sqlx::QueryBuilder::new("INSERT INTO local_queue_jobs (id, tenant_id: tenant_id, task_id, role, payload) ");
        builder.push_values(jobs.into_iter(), |mut b, job| {
            b.push_bind(job.id)
             .push_bind(job.tenant_id: tenant_id)
    ...
    async fn enqueue(&self, job: Job) -> Result<(), String> {
        ...
        sqlx::query("INSERT INTO local_queue_jobs (id, tenant_id: tenant_id, task_id, role, payload) VALUES (?, ?, ?, ?, ?)")
            .bind(job.id)
            .bind(job.tenant_id: tenant_id)
            .bind(job.parent_task_id)
            .bind(job.agent_role)
    ...
    async fn dequeue(&self, roles: Vec<String>) -> Result<Option<Job>, String> {
        ...
        let query_str = format!(
            "UPDATE local_queue_jobs SET status = 'RUNNING' WHERE id = (SELECT id FROM local_queue_jobs WHERE status = 'PENDING' AND role IN ({}) LIMIT 1) RETURNING id, tenant_id: tenant_id, task_id, role, payload, status",
            role_placeholders
        );
    ...
    async fn complete(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), String> {
        sqlx::query("UPDATE local_queue_jobs SET status = 'COMPLETED' WHERE id = ? AND tenant_id: tenant_id = ?")
            .bind(job_id)
            .bind(tenant_id: tenant_id)
            .execute(&self.pool)
            .await
    ...
    async fn fail(&self, job_id: &str, tenant_id: tenant_id: &str, _reason: &str) -> Result<(), String> {
        sqlx::query("UPDATE local_queue_jobs SET status = 'FAILED' WHERE id = ? AND tenant_id: tenant_id = ?")
            .bind(job_id)
            .bind(tenant_id: tenant_id)
            .execute(&self.pool)
            .await
    ...
    async fn requeue(&self, job: Job) -> Result<(), String> {
        ...
        sqlx::query("UPDATE local_queue_jobs SET status = 'PENDING', payload = ? WHERE id = ? AND tenant_id: tenant_id = ?")
            .bind(new_payload)
            .bind(&job.id)
            .bind(&job.tenant_id: tenant_id)
            .execute(&self.pool)
...
impl TaskQueue for RedisTaskQueue {
    async fn enqueue_batch(&self, jobs: Vec<Job>) -> Result<(), String> {
        ...
        for job in jobs {
            let queue_job = ::server_ohc::interop::QueueJob {
                id: job.id,
                tenant_id: tenant_id: job.tenant_id: tenant_id,
        ...
        if let Some((_, payload_bytes)) = result {
            if let Ok(queue_job) = <::server_ohc::interop::QueueJob as prost::Message>::decode(&payload_bytes[..]) {
                let job = Job {
                    ...
                    tenant_id: tenant_id: queue_job.tenant_id: tenant_id,
    ...
    async fn complete(&self, job_id: &str, tenant_id: tenant_id: &str) -> Result<(), String> {
        ...
        if let Some(payload_bytes) = result {
            if let Ok(queue_job) = <::server_ohc::interop::QueueJob as prost::Message>::decode(&payload_bytes[..]) {
                if queue_job.tenant_id: tenant_id != tenant_id: tenant_id {
                    return Err("tenant mismatch".to_string());
                }
...
mod tests {
    ...
    async fn test_task_queue_service_push_claim() {
        ...
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            sqlx::query("CREATE TABLE IF NOT EXISTS shared_tasks (id VARCHAR PRIMARY KEY, parent_id VARCHAR, epic_id VARCHAR, title VARCHAR NOT NULL, status VARCHAR NOT NULL, assigned_agent VARCHAR, payload JSONB, tenant_id: tenant_id VARCHAR, dependencies JSONB DEFAULT '[]', created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP)")
                .execute(&pool)
                .await
                .unwrap();
            ...
            let task = SharedTaskModel {
                id: task_id.clone(),
                tenant_id: tenant_id: "org1".to_string(),
                parent_id: None,
                epic_id: None,
    ...
    async fn test_queue_manager_tenant_isolation() {
        if let Ok(db_url) = std::env::var("DATABASE_URL") {
            ...
            let _ = sqlx::query("CREATE TABLE IF NOT EXISTS sub_agent_queue (id VARCHAR PRIMARY KEY, tenant_id: tenant_id VARCHAR NOT NULL, parent_task_id VARCHAR, payload TEXT, status VARCHAR, worker_id VARCHAR, scheduled_at TIMESTAMP, completed_at TIMESTAMP, created_at TIMESTAMP, updated_at TIMESTAMP)")
                .execute(&pool)
                .await;

            let job = SubAgentJob {
                id: job_id.clone(),
                tenant_id: tenant_id: org_id.clone(),
