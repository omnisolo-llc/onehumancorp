    ...
    async fn cleanup_stale_jobs(&self) -> Result<u64, String> {
        ...
        sqlx::query(
            "INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message)
             SELECT id, tenant_id, 'job_failed', 'sub_agent_queue', COALESCE(payload::text, '{}'), '[cleanup] stagnant backlog item in QUEUED for > 24 hours'
...
impl TaskQueue for SqliteTaskQueue {
    ...
    async fn cleanup_stale_jobs(&self) -> Result<u64, String> {
        ...
        let _ = sqlx::query(
            "INSERT INTO department_dead_letters (id, tenant_id, event_type, department, payload, error_message)
             SELECT id, tenant_id, 'job_failed', 'sub_agent_queue', COALESCE(payload, '{}'), '[cleanup] stagnant backlog item in QUEUED for > 24 hours'
             FROM sub_agent_queue
