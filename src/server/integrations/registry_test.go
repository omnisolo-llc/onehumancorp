...
func TestNewRegistryAllDisconnected(t *testing.T) {
	...
	for _, i := range r.Instances() {
		if i.Status != StatusDisconnected {
			t.Errorf("integration %q should start disconnected, got %q", i.IntegrationID, i.Status)
		}
	}
}
...
func TestIntegrationsByCategory(t *testing.T) {
	...
	chat := r.InstancesByCategory(CategoryChat)
	for _, i := range chat {
		if i.Category != CategoryChat {
	...
	git := r.InstancesByCategory(CategoryGit)
	if len(git) < 2 {
		t.Errorf("expected at least 2 git integrations, got %d", len(git))
	}

	issues := r.InstancesByCategory(CategoryIssues)
	if len(issues) < 2 {
...
func TestIntegrationsByCategoryUnknown(t *testing.T) {
	r := NewRegistry()
	result := r.InstancesByCategory(Category("unknown"))
	if result != nil {
...
func TestConnectUpdatesStatus(t *testing.T) {
	...
	LookupIPFunc = func(host string) ([]net.IP, error) { return []net.IP{net.ParseIP("8.8.8.8")}, nil }
...
func TestDefaultIntegrationsHaveRequiredFields(t *testing.T) {
	for _, i := range GetCatalog() {
		meta := i.Metadata()
		if meta.GetId() == "" {
			t.Errorf("integration missing ID: %+v", meta)
		}
		if meta.GetName() == "" {
			t.Errorf("integration %q missing Name", meta.GetId())
		}
		if meta.GetType() == "" {
			t.Errorf("integration %q missing Type", meta.GetId())
		}
		if meta.GetCategory() == "" {
			t.Errorf("integration %q missing Category", meta.GetId())
		}
		if meta.GetDescription() == "" {
			t.Errorf("integration %q missing Description", meta.GetId())
		}
	}
}
