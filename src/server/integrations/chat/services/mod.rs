...
use crate::domain::{ChatInbox, ChatChannel, ChatContact, ChatConversation, ChatMessage};
...
impl ChatService {
    ...
    pub async fn create_contact(
        ...
        let contact = sqlx::query_as::<_, ChatContact>(
            r#"
            INSERT INTO chat_contacts (id, tenant_id, name, email, phone, identifier)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id, tenant_id, name, email, phone, identifier, created_at, updated_at
            "#
        )
        .bind(Uuid::new_v4())
        .bind(tenant_id)
    ...
    pub async fn send_message(
        ...
        let msg = sqlx::query_as::<_, ChatMessage>(
            ...
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id, tenant_id, conversation_id, sender_type, sender_id, content, metadata, created_at, updated_at
            "#
        )
        .bind(Uuid::new_v4())
        .bind(tenant_id)
        ...
        let _ = self.enqueue_ai_triage_job(&msg).await;

        Ok(msg)
    }

    async fn enqueue_ai_triage_job(&self, _msg: &ChatMessage) -> Result<(), sqlx::Error> {
        Ok(())
    }
}
