...
pub struct IntegrationsRegistry {
    messages: RwLock<std::collections::HashMap<String, Vec<::server_omnisolo::orchestration::ChatMessage>>>,
    instances: RwLock<std::collections::HashMap<String, ::server_omnisolo::orchestration::IntegrationInstance>>,
    pull_requests: RwLock<std::collections::HashMap<String, Vec<::server_omnisolo::orchestration::PullRequest>>>,
...
impl IntegrationsRegistry {
    pub fn new() -> Self {
        ...
        for provider in crate::integrations::catalog::get_catalog() {
            ...
            instances.insert(id.clone(), ::server_omnisolo::orchestration::IntegrationInstance {
                id: id.clone(),
    ...
    // Chat methods
    pub fn test_connection(&self, integration_id: &str, _creds: ::server_omnisolo::orchestration::ChatTestRequest) -> Result<(), String> {
    ...
    pub fn chat_messages(&self, integration_id: &str) -> Vec<::server_omnisolo::orchestration::ChatMessage> {
        let msgs = self.messages.read().unwrap();
        msgs.get(integration_id).cloned().unwrap_or_default()
    }

    pub fn send_chat_message(&self, integration_id: &str, channel: &str, from_agent: &str, content: &str, thread_id: &str) -> Result<::server_omnisolo::orchestration::ChatMessage, String> {
                     ...
                     if !creds.from_phone.is_empty() {
                         ...
                         if let Some(client) = clients.get(integration_id) {
                             ...
                             tokio::spawn(async move {
                                ...
                                if let Err(e) = result {
                                     ::server_telemetry::record_error_signal("[bug] Failed to send Twilio SMS");
                         ...
                         if let Some(client) = client {
                             ...
                             tokio::spawn(async move {
                                 ...
                                 if let Err(e) = client.send_message(platform, &to, &text).await {
                                     ::server_telemetry::record_error_signal("[bug] Failed to send Meta message");
    ...
    pub fn instances_by_category(&self, category: &str) -> Vec<::server_omnisolo::orchestration::IntegrationInstance> {
        let insts = self.instances.read().unwrap();
        insts.values().filter(|i| i.category == category).cloned().collect()
    }

    pub fn connect(&self, integration_id: &str, base_url: &str, creds: ::server_omnisolo::orchestration::ConnectIntegrationRequest) -> Result<::server_omnisolo::orchestration::IntegrationInstance, String> {
    ...
    pub fn disconnect(&self, integration_id: &str) -> Result<::server_omnisolo::orchestration::IntegrationInstance, String> {
        let mut insts = self.instances.write().unwrap();
    ...
    pub fn pull_requests(&self, integration_id: &str) -> Vec<::server_omnisolo::orchestration::PullRequest> {
        let prs = self.pull_requests.read().unwrap();
        prs.get(integration_id).cloned().unwrap_or_default()
    }

    pub fn create_pull_request(&self, integration_id: &str, _repository: &str, title: &str, body: &str, source_branch: &str, target_branch: &str, created_by: &str) -> Result<::server_omnisolo::orchestration::PullRequest, String> {
    ...
    pub fn merge_pull_request(&self, pr_id: &str) -> Result<::server_omnisolo::orchestration::PullRequest, String> {
        let mut prs = self.pull_requests.write().unwrap();
    ...
    pub fn close_pull_request(&self, pr_id: &str) -> Result<::server_omnisolo::orchestration::PullRequest, String> {
        let mut prs = self.pull_requests.write().unwrap();
        for v in prs.values_mut() {
            if let Some(pr) = v.iter_mut().find(|p| p.id == pr_id) {
    ...
    pub fn create_issue(&self, integration_id: &str, _project: &str, title: &str, description: &str, created_by: &str, priority: &str, labels: Vec<String>) -> Result<::server_omnisolo::orchestration::Issue, String> {
    ...
    pub fn update_issue_status(&self, issue_id: &str, status: &str) -> Result<::server_omnisolo::orchestration::Issue, String> {
        let mut issues = self.issues.write().unwrap();
    ...
    pub fn assign_issue(&self, issue_id: &str, assignee: &str) -> Result<::server_omnisolo::orchestration::Issue, String> {
        let mut issues = self.issues.write().unwrap();
        for v in issues.values_mut() {
            if let Some(issue) = v.iter_mut().find(|i| i.id == issue_id) {
