...
mod tests {
    ...
    #[tokio::test]
    async fn delete_event_sends_delete_request_and_handles_success() {
        let (base_url, request_rx) = start_google_calendar_server("").await;
        let client =
            RealGoogleCalendarClient::with_base_url_for_test("valid-token".to_string(), base_url);

        let result = client.delete_event("test-event-id-123").await;

        assert!(result.is_ok());

        let request = request_rx.await.unwrap();
        assert!(request.starts_with(
            "DELETE /calendar/v3/calendars/primary/events/test-event-id-123 HTTP/1.1"
        ));
        assert!(
            request.contains("authorization: Bearer valid-token")
                || request.contains("Authorization: Bearer valid-token")
