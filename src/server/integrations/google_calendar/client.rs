...
pub trait GoogleCalendarClientWrapper: Send + Sync {
    ...
    async fn generate_meet_link(&self, summary: &str, start_time: &str, end_time: &str) -> Result<String, String>;
}
            ...
            Ok(resp) => {
                if resp.status().is_success() {
                    let _ = ::server_telemetry::record_api_call_cost(
                        &crate::db::get_pool(),
                        "unknown",
                        "google_calendar_generate_meet_link",
                        0.01
                    ).await;
                    Ok("https://meet.google.com/mock-link".to_string()) // Returning mock meet link
                } else {
                    Err(format!("Google Calendar API error: {}", resp.status()))
                }
            }
