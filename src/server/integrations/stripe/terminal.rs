...
impl StripeClient {
    ...
    pub async fn capture_terminal_payment_intent(&self, payment_intent_id: &str) -> Result<String, String> {
        let api_key = self.require_api_key()?;
        if payment_intent_id.trim().is_empty() {
            return Err("payment_intent_id is required".to_string());
        }

        let res = reqwest::Client::new().post(format!("{}/v1/payment_intents/{}/capture", Self::api_base(), payment_intent_id))
            .basic_auth(api_key, Some(""))
            .send()
            .await
            .map_err(|e| format!("Stripe API request failed: {}", e))?;
        ...
        let json: serde_json::Value = res.json().await.map_err(|e| format!("Failed to parse response: {}", e))?;
        let status = json["status"].as_str().ok_or_else(|| "Missing status in response".to_string())?;
...
mod tests {
    ...
    async fn test_capture_terminal_payment_intent_requires_configured_key() {
        ...
        let err = result.expect_err("Terminal capture must not be mocked when Stripe credentials are missing");
        assert!(err.contains("Stripe API key"));
    }
}
