...
impl StripeClient {
    pub async fn create_terminal_payment_intent(&self, tenant_id: &str, amount_cents: i64, currency: &str, product_id: Option<&str>, quantity: Option<i32>, order_id: Option<&str>) -> Result<String, String> {
        let api_key = self.require_api_key()?;
        if amount_cents <= 0 {
