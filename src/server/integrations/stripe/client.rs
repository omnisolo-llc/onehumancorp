...
impl StripeClient {
    ...
    pub async fn create_checkout_session(&self, price_id_or_name: &str, customer_id: &str, amount_usd: f64, is_subscription: bool, tenant_id: Option<&str>, inventory_lock_id: Option<&str>) -> Result<String, String> {
        ...
        if let Some(tid) = tenant_id {
            form.insert("metadata[tenant_id]".to_string(), tid.to_string());
        }
        if let Some(lock) = inventory_lock_id {
            form.insert("metadata[inventory_lock_id]".to_string(), lock.to_string());
        }
