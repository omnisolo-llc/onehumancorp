...
func TestWeaviateIntegration_Metadata(t *testing.T) {
	integration := &WeaviateIntegration{}
	metadata := integration.Metadata()

	if metadata.GetId() != "weaviate" {
		t.Errorf("Expected ID to be 'weaviate', got '%s'", metadata.GetId())
	}
	if metadata.GetName() != "Weaviate" {
		t.Errorf("Expected Name to be 'Weaviate', got '%s'", metadata.GetName())
	}
}

func TestWeaviateIntegration_WizardSteps(t *testing.T) {
	integration := &WeaviateIntegration{}
	...
	field2 := fields[1]
	if field2.GetKey() != "api_key" {
		t.Errorf("Expected Field 2 Key to be 'api_key', got '%s'", field2.GetKey())
	}
