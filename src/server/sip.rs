    ...
    pub async fn prune_stale_missions(&self, age_threshold: chrono::Duration) -> Result<(), sqlx::Error> {
        ...
        loop {
            let res = async {
                ...
                sqlx::query("UPDATE agent_missions SET status = 'FAILED' WHERE (status = 'STUCK' OR ((status = 'PENDING' OR status = 'BURSTING' OR status = 'RUNNING') AND updated_at < $1)) AND tenant_id = $2")
