    ...
    pub async fn prune_stale_missions(&self, age_threshold: chrono::Duration) -> Result<(), sqlx::Error> {
        ...
        loop {
            let res = async {
                ...
                sqlx::query("UPDATE agent_missions SET updated_at = CURRENT_TIMESTAMP WHERE id IN (SELECT id FROM agent_missions WHERE status = 'PENDING' AND tenant_id = $1 ORDER BY updated_at ASC LIMIT 10) RETURNING id")
                    .bind(&self.org_id)
                    .execute(&mut *tx)
                    .await?;
