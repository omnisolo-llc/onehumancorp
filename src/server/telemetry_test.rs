...
#[test]
fn test_cloud_vs_standalone_privacy_audit() {
    // Phase 1 & 3: Contrast data handling in Cloud vs Standalone to ensure privacy-by-design
    // Cloud mode MUST redact PII. Standalone mode MUST NOT exfiltrate data.
    ...
    temp_env::with_vars(
        ...
        || {
            ...
            let redacted = ::server_telemetry::redact_interface_pii(payload);
            assert_eq!(redacted["user_data"]["credit_card"], "[REDACTED]");
            assert_eq!(redacted["user_data"]["tenant_id"], "[REDACTED]");
        },
    );

    // Standalone Mode Simulation (Local Sovereignty)
    temp_env::with_vars(
        [
            ("OHC_STANDALONE", Some("true")),
            ("STANDALONE_MODE", Some("true")),
        ...
        || {
            let config = ::server_config::load().unwrap();
            assert_eq!(config.telemetry_enabled, false, "Local sovereignty violation: non-consented telemetry enabled in standalone mode");
        },
    );
}
