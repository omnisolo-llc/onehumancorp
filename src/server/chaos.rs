...
mod tests {
    ...
    async fn test_sipdb_chaos_parity() {
        ...
        let delegate_res = async {
            ...
            sip_db
                .delegate_mission_with_tx(&mut tx, "test_mission", "PENDING", "data", true)
                .await
        }
        .await;
        assert!(
        ...
        let row: (String, Option<String>, chrono::DateTime<chrono::Utc>) =
            sqlx::query_as("SELECT id, mission_log, updated_at FROM test_parity WHERE id = '1'")
        ...
        assert_eq!(
            row.1, None,
            "NULL handling parity must be maintained between SQLite and Postgres"
        );
        ...
        if let Ok(database_url) = std::env::var("OHC_DATABASE_URL") {
            ...
            for i in 0..100 {
                ...
                handles.push(tokio::spawn(async move {
                    ...
                    sqlx::query(&format!(
                        "INSERT INTO {} (id, workspace_id, payload) VALUES ($1, $2, 'data')",
                        t_name
                    ))
            ...
            tracing::info!(
                "Cloud Mode Stress - p50: {}ms, p95: {}ms, p99: {}ms",
                p50,
                p95,
                p99
            );
    ...
    async fn test_cuj_stress_workspaces_standalone_mode() {
        ...
        tracing::info!(
            "Standalone Mode Stress - p50: {}ms, p95: {}ms, p99: {}ms",
            p50,
            p95,
            p99
        );
    ...
    async fn test_host_cpu_exhaustion_degradation() {
        ...
        let _state_manager =
            crate::orchestration::state::standalone::StandaloneStateManager::new(db, latency_mesh);
        ...
        let state_manager2 = crate::orchestration::state::standalone::StandaloneStateManager::new(
            db2,
            Arc::new(InstantMockMesh),
        );
        ...
        assert!(
            ...
            "Must degrade gracefully under CPU exhaustion without panic"
        );
    }
    ...
    async fn test_chaos_network_spike_degradation() {
        ...
        cache
            .lock()
            .await
            .insert("key1".to_string(), "cached_data".to_string());
        ...
        if result.is_err() {
            ...
            assert_eq!(
                read_data,
                Some("cached_data".to_string()),
                "Mobile/Thin Client read operation must show cached data on backend failure"
            );

            // Write operation fail-safe: queue locally
            local_queue.lock().await.push("write_payload".to_string());
            assert_eq!(
                local_queue.lock().await.len(),
                1,
                "Mobile/Thin Client write operation must queue locally on backend failure"
            );
        } else {
    ...
    async fn test_degradation_validation_mobile() {
        ...
        assert!(
            result.is_err(),
            "Mobile API read operations must fail-safe when backend latency spikes >2s (returning cached data)"
        );
    ...
    async fn test_mobile_thin_client_degradation_fallback() {
        ...
        cache
            .set(
                cache_key,
                "cached_dashboard_data".to_string(),
        ...
        let read_result = tokio::time::timeout(timeout_duration, async {
            tokio::time::sleep(std::time::Duration::from_millis(150)).await;
            let pending = Ok::<String, String>("".to_string());
            pending
        })
        .await;
        ...
        assert!(
            read_result.is_err(),
            "Read operation must timeout after latency spike"
        );
        ...
        assert_eq!(
            fallback_data,
            Some("cached_dashboard_data".to_string()),
            "Mobile client must return cached data on read failure"
        );
        ...
        assert!(
            write_result.is_err(),
            "Write operation must timeout after latency spike"
        );
        ...
        assert!(
            write_queued,
            "Write operation must queue locally when connection drops/spikes"
        );
    }
    ...
    async fn test_task_queue_overload_degradation() {
        ...
        assert!(
            timeouts > 0,
            "System should shed load gracefully and return backpressure/Err when task queue is overloaded"
        );
    }
    ...
    async fn test_cuj_stress_verification() {
        ...
        assert_eq!(
            count, 50,
            "All 50 missions should be written successfully despite database is locked errors."
        );
    }
    ...
    async fn test_sipdb_chaos_mesh() {
        ...
        assert!(
            result.is_ok(),
            "Daemon should not panic when reading corrupted offline memory files."
        );
    }
...
async fn test_ml_resilience_inference_timeout_with_db_lag() {
    ...
    assert!(
        ...
        "ML-Resilience: Inference call must timeout"
    );

    let db_update_future = async { Ok::<(), String>(()) };

    let fallback_result =
        tokio::time::timeout(std::time::Duration::from_millis(250), db_update_future).await;
    assert!(
        fallback_result.is_ok(),
        "ML-Resilience: DB fallback update must succeed despite DB lag"
    );
}
#[tokio::test]
async fn test_db_sync_lag() {
    ...
    assert_eq!(
        lag_count, 1,
        "Lagging read should eventually see the synchronized data"
    );
...
async fn test_network_packet_drop() {
    ...
    assert!(
        res.is_err(),
        "Packet drop simulation should result in a timeout and be caught by the circuit breaker/timeout logic."
    );
}
...
async fn test_ml_resilience_malformed_llm_response() {
    ...
    let malformed_json =
        r#"{ "tool_calls": [ { "name": "do_something", "arguments": { "key": "value" "#;
    ...
    assert!(
        ...
        "Agent runtime must gracefully handle malformed JSON LLM response"
    );
    ...
    assert!(
        arguments.is_none(),
        "Agent runtime must safely handle missing fields in LLM tool_calls"
    );
}
...
async fn test_ml_resilience_api_unavailable_paused_state() {
    ...
    sqlx::query(
        "INSERT INTO tenants (id, name, industry) VALUES ('tenant-pause', 'Store', 'Retail')",
    )
    .execute(&pool)
    .await
    .unwrap();
    ...
    let processed = crate::workers::department_workers::CustomerSuccessWorker::poll(&db)
        .await
        .unwrap();
    ...
    let status: String =
        sqlx::query_scalar("SELECT status FROM department_tasks WHERE id = 'task-pause'")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(
        status, "PAUSED",
        "When API is totally unavailable, the agent state must fallback to PAUSED"
    );

    // Check if fallback notification (shared_task) was created
    let shared_task_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM shared_tasks WHERE tenant_id = 'tenant-pause'")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert!(
        shared_task_count > 0,
        "Owner must be notified (shared task created) when system enters PAUSED state"
    );
}
...
async fn test_token_budget_server_side_enforcement() {
    ...
    assert_eq!(
        success_count, 25,
        "Exactly 25 requests (20 points * 25 = 500) should be allowed"
    );
    assert_eq!(
        failure_count, 25,
        "The remaining 25 requests must be rejected server-side"
    );
}
...
async fn test_sipdb_multi_tenancy_isolation() {
    ...
    let count_a: i64 =
        sqlx::query_scalar("SELECT count(*) FROM agent_missions WHERE tenant_id = 'tenant_a'")
            .fetch_one(&*pool_arc)
            .await
            .unwrap();

    let count_b: i64 =
        sqlx::query_scalar("SELECT count(*) FROM agent_missions WHERE tenant_id = 'tenant_b'")
            .fetch_one(&*pool_arc)
            .await
            .unwrap();
    ...
    assert_eq!(
        count_total, 50,
        "Total count should be exactly the sum without leakage"
    );
    ...
    if let Ok(database_url) = std::env::var("OHC_DATABASE_URL") {
        ...
        let count_query_a = format!(
            "SELECT count(*) FROM {} WHERE tenant_id = 'tenant_a'",
            table_name
        );
        let count_a_pg: i64 = sqlx::query_scalar(&count_query_a)
            .fetch_one(&*pg_pool_arc)
            .await
            .unwrap();

        let count_query_b = format!(
            "SELECT count(*) FROM {} WHERE tenant_id = 'tenant_b'",
            table_name
        );
        let count_b_pg: i64 = sqlx::query_scalar(&count_query_b)
            .fetch_one(&*pg_pool_arc)
            .await
            .unwrap();

        let count_query_total = format!("SELECT count(*) FROM {}", table_name);
        let count_total_pg: i64 = sqlx::query_scalar(&count_query_total)
            .fetch_one(&*pg_pool_arc)
            .await
            .unwrap();
        ...
        assert_eq!(
            count_a_pg, 25,
            "Postgres Tenant A should strictly have 25 records"
        );
        assert_eq!(
            count_b_pg, 25,
            "Postgres Tenant B should strictly have 25 records"
        );
        assert_eq!(
            count_total_pg, 50,
            "Postgres Total count should be exactly the sum without leakage"
        );
    }
}
...
mod additional_chaos_tests {
    ...
    async fn test_degradation_mobile_latency() {
        ...
        let response = mobile_handler(backend_latency, max_allowed)
            .await
            .into_response();
        assert_eq!(
            response
    ...
    async fn test_chaos_exhaust_cpu_memory() {
        ...
        for _ in 0..25 {
            let _ = throttler
                .check_and_consume_budget(tenant_id, 20)
                .await
                .unwrap();
        }
        ...
        assert!(
            !shed,
            "Must drop non-critical background jobs under extreme load"
        );
    }
}
...
mod parity_auditing_tests {
    ...
    async fn test_db_parity_null_timezones_isolation() {
        ...
        let sqlite_null: Option<String> =
            sqlx::query_scalar("SELECT data FROM parity_test WHERE id = 'null_test'")
                .fetch_one(&sqlite_pool)
                .await
                .unwrap();
        ...
        if let Some((pg_pool, table_name)) = &pg_pool_opt {
            ...
            let pg_null: Option<String> = sqlx::query_scalar(&format!(
                "SELECT data FROM {} WHERE id = 'null_test'",
                table_name
            ))
            .fetch_one(pg_pool)
            .await
...
mod mesh_chaos_tests {
    ...
    async fn test_chaos_team_mesh_redis_mailbox_corruption() {
        ...
        assert!(
            result.is_ok(),
            "Redis mailbox parsing failure must be handled gracefully"
        );
    }
    ...
    async fn test_chaos_team_mesh_agent_lock_race_conditions() {
        ...
        assert_eq!(
            success_count, 1,
            "Only one agent should successfully acquire the lock in a race condition"
        );
    }
    ...
    async fn test_chaos_team_mesh_pubsub_message_loss() {
        ...
        assert!(
            drops > 0,
            "Pub/Sub simulation should successfully drop messages"
        );
        assert!(
            successes > 0,
            "Pub/Sub simulation should allow some messages to pass"
        );
    }
}
...
mod stress_verification_tests {
    ...
    async fn test_stress_100_concurrent_workspaces_cloud() {
        ...
        println!(
            "Cloud Mode 100 Concurrency - Latency p50: {}ms, p95: {}ms, p99: {}ms",
            p50, p95, p99
        );
        ...
        assert!(
            p99 < 200,
            "p99 latency should remain reasonable under stress"
        );
    }
    ...
    async fn test_stress_10_concurrent_workspaces_standalone() {
        ...
        println!(
            "Standalone Mode 10 Concurrency - Latency p50: {}ms, p95: {}ms, p99: {}ms",
            p50, p95, p99
        );
    ...
    async fn test_sentry_agent_lock_race_conditions() {
        let _tracker = crate::telemetry::ChaosRecoveryTracker::new("Cloud");
        let temp_dir =
            std::env::temp_dir().join(format!("agent_lock_test_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&temp_dir).unwrap();
        ...
        assert!(
            ...
            "Race condition check completed safely"
        );
    ...
    async fn test_sentry_pubsub_message_loss() {
        ...
        assert!(
            result.is_err(),
            "Pub/Sub message loss should be handled gracefully"
        );
        ...
        assert!(
            ...
            "Pub/Sub latency/timeout should be handled gracefully"
        );
    }
}
