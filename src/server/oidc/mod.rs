...
pub async fn validate_oidc_token(token_str: &str, cfg: &OIDCConfig) -> Result<Claims, String> {
    ...
    let org_id = raw.get("organization_id").and_then(|v| v.as_str()).map(|s| s.to_string());
    if ::server_config::get().multitenant && (org_id.is_none() || org_id.as_deref().unwrap().trim().is_empty()) {
