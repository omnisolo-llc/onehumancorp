...
fn is_blocked_ip(ip: std::net::IpAddr, scheme: &str) -> bool {
    ...
    // "When explicitly whitelisting localhost for OAuth/OIDC connectivity, ensure both IPv4 (`127.0.0.1`) and IPv6 (`::1` via `Ipv6Addr::LOCALHOST`) are explicitly permitted when the scheme is HTTPS."
    if scheme == "https" && (ip == std::net::IpAddr::V4(std::net::Ipv4Addr::LOCALHOST) || ip == std::net::IpAddr::V6(std::net::Ipv6Addr::LOCALHOST)) {
        return false;
    }
