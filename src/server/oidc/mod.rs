...
async fn fetch_jwks(issuer_url: &str) -> Result<Vec<JWK>, String> {
    ...
    let jwks_client = reqwest::Client::builder()
        .resolve(
            ...
            std::net::SocketAddr::new(
                jwks_ip,
                if disc.jwks_uri.starts_with("https") {
                    443
                } else {
                    80
                },
...
pub async fn validate_oidc_token(token_str: &str, cfg: &OIDCConfig) -> Result<Claims, String> {
    ...
    let key = keys
        .iter()
        .find(|k| k.kid == kid)
        .ok_or_else(|| "no matching JWK found".to_string())?;

    let decoding_key =
        DecodingKey::from_rsa_components(&key.n, &key.e).map_err(|e| e.to_string())?;
    ...
    let token_data =
        decode::<serde_json::Value>(token_str, &decoding_key, &validation).map_err(|e| {
    ...
    Ok(Claims {
        sub: {
            ...
            if sub.trim().is_empty() {
                return Err("missing or empty sub in OIDC token".to_string());
            }
            sub
        },
        username: raw
        ...
        jti: {
            ...
            if jti.trim().is_empty() {
                return Err("missing or empty jti in OIDC token".to_string());
            }
            jti
        },
    })
