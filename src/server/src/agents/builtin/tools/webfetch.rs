...
impl ToolExecutor for WebFetchExecutor {
    async fn execute(
        ...
        let url = args["url"].as_str().ok_or_else(|| super::ToolError::LlmRecoverable("webfetch: url is required".to_string()))?;
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| super::ToolError::LlmRecoverable(format!("webfetch: GET {}: {}", url, e)))?;

        if !resp.status().is_success() {
            return Err(super::ToolError::LlmRecoverable(format!("webfetch: HTTP {}", resp.status())));
        }
        ...
        let body = resp
            .text()
            .await
            .map_err(|e| super::ToolError::LlmRecoverable(format!("webfetch: read body: {}", e)))?;
