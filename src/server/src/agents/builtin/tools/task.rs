...
impl ToolExecutor for TaskCreateExecutor {
    async fn execute(
        ...
        let title = args["title"]
            .as_str()
            .ok_or_else(|| super::ToolError::LlmRecoverable("task_create: title is required".to_string()))?;
...
impl ToolExecutor for TaskGetExecutor {
    async fn execute(
        ...
        let id = args["id"].as_str().ok_or_else(|| super::ToolError::LlmRecoverable("task_get: id is required".to_string()))?;
        let store = self.store.read().await;
        if let Some(task) = store.get(id) {
            Ok(serde_json::to_string_pretty(task).unwrap_or_default())
        } else {
            Err(super::ToolError::LlmRecoverable(format!("Task not found: {}", id)))
        }
    }
}
...
impl ToolExecutor for TaskUpdateExecutor {
    async fn execute(
        ...
        let id = args["id"].as_str().ok_or_else(|| super::ToolError::LlmRecoverable("task_update: id is required".to_string()))?;
