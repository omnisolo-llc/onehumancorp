...
impl MyAgentManagerService {
    ...
    async fn get_snapshot(&self) -> Result<DashboardSnapshot, Status> {
        ...
        let (agents_res, meetings_res, cost_auditor_res) = tokio::join!(
        ...
        let cost_auditor = cost_auditor_res.map_err(|e| Status::internal(format!("Failed to get cost auditor: {}", e)))?;
        let costs = Summary {
