...
impl ChatService {
    ...
    pub async fn update_inbox_config(
        ...
        sqlx::query_as(
            r#"
            UPDATE chat_inboxes
            SET working_hours_enabled = COALESCE($1, working_hours_enabled),
                out_of_office_message = COALESCE($2, out_of_office_message),
                greeting_enabled = COALESCE($3, greeting_enabled),
    ...
    pub async fn get_inbox(&self, tenant_id: Uuid, inbox_id: Uuid) -> Result<ChatInbox, sqlx::Error> {
        sqlx::query_as(
            r#"
            SELECT id, tenant_id, name, working_hours_enabled, out_of_office_message, greeting_enabled, greeting_message, created_at, updated_at
            FROM chat_inboxes
    ...
    pub async fn receive_message(
        ...
        let msg = self.send_message(tenant_id, conversation_id, sender_type, sender_id, content).await?;

        let inbox = self.get_inbox(tenant_id, inbox_id).await?;
        if is_off_hours && inbox.working_hours_enabled.unwrap_or(false) {
            if let Some(ooo_msg) = inbox.out_of_office_message {
                let bot_id = Uuid::new_v4(); // Mock bot ID
                let _ = self.send_message(tenant_id, conversation_id, "bot".to_string(), Some(bot_id), ooo_msg).await?;
            }
        }
