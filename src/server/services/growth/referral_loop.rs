...
pub struct ReferralLoopTracker {
...
impl ReferralLoopTracker {
    pub fn new() -> Self {
        let meter = global::meter("ohc.growth");
        let invites_sent_metric = meter.u64_counter("ohc.growth.referral_loop.invites_sent").build();
        let invites_accepted_metric = meter.u64_counter("ohc.growth.referral_loop.invites_accepted").build();
...
mod tests {
    ...
    #[test]
    fn test_referral_loop_tracker() {
        let tracker = ReferralLoopTracker::new();
        
        tracker.record_invite_sent("user1");
        tracker.record_invite_sent("user2");
        tracker.record_invite_accepted("invitee1");
        
        let referral_score = tracker.calculate_referral_score();
        assert_eq!(referral_score, 0.5);
    }
}
