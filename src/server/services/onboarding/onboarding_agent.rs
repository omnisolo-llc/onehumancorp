...
Your response:",
    ...
    pub async fn start_onboarding(&self, req: StartOnboardingRequest) -> Result<StartOnboardingResponse, String> {
        ...
        let sample_task_id = uuid::Uuid::new_v4().to_string();
        let sample_task_payload = serde_json::json!({
            "description": "Follow up with new leads",
            "feature_type": "onboarding_sample_task",
            "company_name": company_name
        });
        if let Err(e) = sqlx::query(
            "INSERT INTO agent_feed_items (id, tenant_id, event_source, context_payload, proposed_action, lifecycle_state) VALUES ($1, $2, $3, $4, $5, 'PENDING_APPROVAL')"
        ...
        .bind(&org_id)
        .bind("system")
        .bind(&sample_task_payload)
        .bind(serde_json::json!({"action_type": "follow_up"}))
        .execute(&self.db.pool)
        .await {
            tracing::error!("Failed to create sample task feed item: {}", e);
        }
