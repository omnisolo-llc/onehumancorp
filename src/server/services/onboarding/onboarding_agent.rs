...
impl OnboardingAgent {
    ...
    pub async fn start_onboarding(&self, req: StartOnboardingRequest) -> Result<StartOnboardingResponse, String> {
        ...
        let publish_events_future = tokio::task::spawn(async move {
            ...
            if let Err(e) = sqlx::query("INSERT INTO ohc_job_queue (id, tenant_id, parent_task_id, payload, status, next_retry_at, created_at, updated_at, job_type) VALUES ($1, $2, NULL, $3::jsonb, 'PENDING', $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'default')")
