...
mod tests {
    ...
    async fn test_sync_mcp_deltas_empty() {
        ...
        req.metadata_mut().insert("x-spiffe-id", "spiffe://ohc.local/org/org1/agent/agent1".parse().unwrap());
        req.extensions_mut().insert(::server_auth::AuthInfo { spiffe_id: "spiffe://ohc.local/org/org1/agent/agent1".to_string(), org_id: "org1".to_string(), agent_id: "agent1".to_string() });
