...
impl TelemetrySyncDaemon {
    ...
    async fn sync_metrics(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self.mode == perf::CoordinatorMode::Parallel {
            ...
            let extracted_data: Vec<(i32, String, String, f64, String, DateTime<Utc>)> = rows
                .into_iter()
                .map(|row| {
        ...
        let res = client
            .post(format!("{}/api/v1/telemetry/sync", self.cloud_url))
            .json(&batch)
            .send()
            .await;
...
mod tests {
    ...
    async fn bench_telemetry_sync_parallel() {
        ...
        let daemon = TelemetrySyncDaemon::with_mode(
            pool.clone(),
            mock_url.clone(),
        ...
        let par_daemon = TelemetrySyncDaemon::with_mode(
            pool.clone(),
            mock_url.clone(),
        ...
        tracing::debug!("Sequential Sync Duration: {:?}", seq_duration);
        tracing::debug!("Parallel Sync Duration: {:?}", par_duration);
