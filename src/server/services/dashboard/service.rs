...
impl DashboardService for MyDashboardService {
    async fn get_dashboard(
        ...
        let products = products_res.unwrap().map_err(|e| Status::internal(e.to_string()))?;
        let orders = orders_res.unwrap().map_err(|e| Status::internal(e.to_string()))?;
        let org = org_res.unwrap().map_err(|e| Status::internal(e.to_string()))?;
