...
impl DashboardService for MyDashboardService {
    async fn get_dashboard(
        ...
        let (agents_res, meetings_res, cost_res, products_res, orders_res, org_res) = tokio::join!(
            tokio::spawn(async move { self1.fetch_agents(org_id_agents, mobile_optimized).await }),
            tokio::spawn(async move { self2.fetch_meetings().await }),
            tokio::spawn(async move { self3.fetch_cost_summary(org_id4).await }),
            tokio::spawn(async move { self4.fetch_products(org_id1, mobile_optimized).await }),
            tokio::spawn(async move { self5.fetch_orders(org_id2, mobile_optimized).await }),
            tokio::spawn(async move { self6.fetch_org(org_id3, mobile_optimized).await })
        );
