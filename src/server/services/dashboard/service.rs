    ...
    async fn get_dashboard(
        ...
        let (agents_res, meetings_res, cost_res, products_res, orders_res, org_res) = tokio::join!(
            ...
            tokio::spawn(async move {
                ...
                match &db1.store {
                    crate::db::DbStore::Postgres => {
                        let mut tx = match db1.pool.begin().await {
                            Ok(tx) => tx,
                            Err(_) => return Ok::<_, String>(results),
                        };
                        if let Err(_) = ::server_common::auth_utils::set_org_context(&mut *tx, &org_id).await {
                            return Ok::<_, String>(results);
                        }
                        if let Ok(rows) = sqlx::query(q).bind(&org_id).fetch_all(&mut *tx).await {
                            for r in rows {
            ...
            tokio::spawn(async move {
                ...
                match &db2.store {
                    crate::db::DbStore::Postgres => {
                        let mut tx = match db2.pool.begin().await {
                            Ok(tx) => tx,
                            Err(_) => return Ok::<_, String>(results),
                        };
                        if let Err(_) = ::server_common::auth_utils::set_org_context(&mut *tx, &org_id).await {
            ...
            tokio::spawn(async move {
                ...
                match &db3.store {
                    crate::db::DbStore::Postgres => {
                        let mut tx = match db3.pool.begin().await {
                            Ok(tx) => tx,
                            Err(_) => return Ok::<_, String>(org),
                        };
                        if let Err(_) = ::server_common::auth_utils::set_org_context(&mut *tx, &org_id).await {
                            return Ok::<_, String>(org);
                        }
