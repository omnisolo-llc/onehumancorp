...
impl OrgService for MyOrgService {
    ...
    async fn get_analytics(
        ...
        let spiffe_id_str = ::server_auth::extract_spiffe_id_from_metadata(_request.metadata()).unwrap_or_default();
        let org_id = ::server_auth::parse_spiffe_id(&spiffe_id_str).map(|(id, _)| id).unwrap_or_else(|_| "default".to_string());
