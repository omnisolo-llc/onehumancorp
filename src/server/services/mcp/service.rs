...
mod tests {
    ...
    async fn test_sync_missions_authenticated() {
        let registry = Arc::new(IntegrationsRegistry::new());
        let pool = sqlx::postgres::PgPoolOptions::new()
            .after_release(|conn, _meta| { Box::pin(async move { use sqlx::Executor; let _ = conn.execute("RESET app.current_tenant").await; Ok(true) }) }).before_acquire(|conn, _meta| { Box::pin(async move { use sqlx::Executor; conn.execute("SELECT set_config('app.current_tenant', 'org-1', false)").await?; Ok(true) }) })
            .connect_lazy("postgres://postgres:postgres@localhost:5432/test")
            .unwrap();
        if !matches!(tokio::time::timeout(std::time::Duration::from_millis(500), sqlx::query("SELECT 1").execute(&pool)).await, Ok(Ok(_))) { return; }
        let (tx, _rx) = tokio::sync::mpsc::channel(1);
