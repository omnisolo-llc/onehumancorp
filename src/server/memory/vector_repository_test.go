...
func TestVectorRepository(t *testing.T) {
	...
	if err := repo.PruneStale(ctx, "org-1", oldTime); err != nil {
...
func TestVectorRepository_GetRecentMemories(t *testing.T) {
	provider := db.NewTestProvider(t)
	repo := NewVectorRepository(provider)
	ctx := context.Background()
	...
	results, err := repo.GetRecentMemories(ctx, "org-recent", time.Now().Add(-1*time.Hour))
	if err != nil {
		t.Errorf("GetRecentMemories failed: %v", err)
	}
	if len(results) != 1 {
		t.Errorf("Expected 1 result, got %d", len(results))
	} else if results[0].ID != "recent-1" {
		t.Errorf("Expected ID recent-1, got %s", results[0].ID)
	}
}
