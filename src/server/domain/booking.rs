...
pub async fn handle_autonomous_quote_action(
    ...
    if !proposed_slot_id.is_empty() {
        ...
        let existing: Result<(i64,), _> =
            sqlx::query_as("SELECT count(*) FROM booking_slots WHERE id = $1")
    ...
    let deposit_amount_cents = payload
    ...
    let total_amount_cents = payload
        ...
        .and_then(|v| v.as_i64())
        .unwrap_or(deposit_amount_cents);
    let customer_id = payload
        .get("customer_id")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
    let mut drafted_message = payload
...
To secure your booking, please pay the deposit here: {}",
    ...
    let payment_link_opt = if stripe_payment_link.is_empty() {
        ...
        Some(stripe_payment_link.clone())
    };
