...
pub fn parse_blueprint(data: &[u8], _is_yaml: bool) -> Result<SkillBlueprint, String> {
    let bp: SkillBlueprint = serde_json::from_slice(data).map_err(|e| format!("failed to unmarshal blueprint: {}", e))?;
...
mod tests {
    ...
    fn test_blueprint_validation_missing_fields() {
        let tests = vec![
            ...
            (
                ...
                r#"{
                    "domain": "Test",
                    "roles": [
                        { "id": "a", "context": "context 1" },
                        { "id": "a", "context": "context 2" }
                    ]
                }"#,
                "duplicate role id: a",
            ),
            (
                ...
                r#"{
                    "domain": "Test",
                    "roles": [
                        { "id": "a", "context": "context 1", "reports_to": "b" }
                    ]
                }"#,
        ...
        for (name, json_data, expected_err) in tests {
            let res = parse_blueprint(json_data.as_bytes(), false);
    ...
    fn test_blueprint_dag_cycle_detection() {
        let json_data = r#"{
            ...
            "roles": [
                { "id": "a", "context": "context a", "reports_to": "b" },
                { "id": "b", "context": "context b", "reports_to": "c" },
                { "id": "c", "context": "context c", "reports_to": "a" }
            ]
        }"#;
