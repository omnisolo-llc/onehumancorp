...
impl TrustManager {
    ...
    ) -> Result<TrustAgreement, serde_json::Error> {
...
mod tests {
    ...
    fn test_trust_manager_parse_jwks() {
        ...
        // Valid JSON
        let res = tm.parse_jwks(
            "globex.com",
            r#"{"keys": [{"kty": "RSA"}]}"#,
            vec!["Sales Agent".to_string()],
        );
        assert!(res.is_ok());
        let got = res.unwrap();
    ...
    fn test_egress_filter_scan() {
        ...
        // Not blocked
        let got = ef.scan(
            "Hello, we would like to purchase 100 server racks.",
            &keywords,
        );
        assert!(!got.blocked);
        ...
        // Blocked (exact)
        let got = ef.scan(
            "We can offer a discount based on Internal Project X budget.",
            &keywords,
        );
        assert!(got.blocked);
