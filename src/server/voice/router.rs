...
>>>>>>> 6a45afe2 (Temp commit for rebase)
        );
    ...
    async fn test_voice_context_router_order_food_flow() {
        ...
        let planner = Arc::new(ScriptedVoiceTurnPlanner {
            plans: tokio::sync::Mutex::new(vec![
                VoiceTurnPlan {
                    intent_type: Some("ORDER_FOOD".to_string()),
                    ai_response: "I can help with that.".to_string(), // This should be overwritten
                    sms_body: None, // This should be overwritten
                },
            ]),
        });
        ...
        let response1 = router.process_user_input(&session_id, "I want to order food", "+0987654321").await;
        assert!(response1.contains("online menu right now so you can place your order"));
