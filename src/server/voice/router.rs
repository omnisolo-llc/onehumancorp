...
impl VoiceTurnPlanner for LlmVoiceTurnPlanner {
    async fn plan_turn(&self, session_id: &str, user_text: &str) -> Result<VoiceTurnPlan, String> {
        ...
        let raw: String = match std::env::var("OHC_VOICE_LLM_PROVIDER")
            .or_else(|_| std::env::var("OHC_LLM_PROVIDER"))
            .as_deref()
        {
            Ok("minimax") => {
                let api_key = std::env::var("MINIMAX_API_KEY").unwrap_or_default();
                ::minimax::MinimaxClient::new(api_key).reason(&prompt).await.unwrap_or_else(|_| "".to_string())
            }
            _ => ::minimax::LocalLLMClient::new().reason(&prompt).await.unwrap_or_else(|_| "".to_string()),
        };
