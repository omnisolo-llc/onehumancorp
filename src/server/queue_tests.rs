...
async fn test_in_mem_job_queue_worker_pool() {
    ...
    // Push jobs
    for i in 0..5 {
        queue.push("test_topic", format!("payload_{}", i).into_bytes()).await.unwrap();
    }
    ...
    // We can't easily assert on the stdout inside the test, but we can verify it doesn't crash
    // and shutdown cleanly.
    assert!(true);
}

#[tokio::test]
async fn test_task_queue_service_push_and_claim() {
