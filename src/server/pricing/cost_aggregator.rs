        ...
        if let Some(date) = row.date {
            ...
            if let Some(daily) = trends.get_mut(&date_str) {
                ...
                match row.metric_name.as_str() {
                    ...
                    "ohc_bandwidth_savings_total_cents" => daily.bandwidth_savings += val,
                    _ => {}
                }
...
pub async fn aggregate_daily_costs(pool: &PgPool, tenant_id: &str) -> Vec<DailyCost> {
    let raw_rows = sqlx::query(
        ...
        WHERE json_extract_path_text(labels_json::json, 'tenant_id') = $1
          AND metric_name IN ('ohc_mission_cost_cents', 'ohc_storage_rw_cost', 'ohc_network_cost_cents', 'ohc_compute_cost_cents', 'ohc_bandwidth_savings_total_cents')
...
mod tests {
    ...
    fn test_process_with_data() {
        ...
        let rows = vec![
            ...
            TelemetryRow {
                date: Some(today),
                metric_name: "ohc_bandwidth_savings_total_cents".to_string(),
                total: Some(50.0),
            },
        ];
        let res = process_telemetry_rows(rows);
        ...
        assert_eq!(today_data.bandwidth_savings, 50);
    }
