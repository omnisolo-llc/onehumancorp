...
impl Drop for BudgetReservation {
    fn drop(&mut self) {
        if !self.resolved {
            ...
            st.total_allocated -= self.amount;
        }
    }
}

impl BudgetReservation {
    pub fn settle(mut self, actual_amount_cents: i64) {
        self.resolved = true;
        let mut st = self.state.lock().unwrap();
        // Release the unspent portion
        if self.amount >= actual_amount_cents {
            st.total_allocated -= self.amount - actual_amount_cents;
        } else {
            // If they spent more than reserved (should be rare), adjust
            st.total_allocated += actual_amount_cents - self.amount;
        }
        st.settled += actual_amount_cents;

        // Telemetry must be preserved on settlement
