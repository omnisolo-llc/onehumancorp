...
impl BudgetManager {
    ...
    pub fn record_spend_cents(&self, amount_cents: i64) -> Result<bool, String> {
        ...
        if let Ok(Some(mut reservation)) = self.reserve_cents(amount_cents) {
    ...
    pub fn get_remaining(&self) -> f64 {
        let current = self.state.lock().map(|s| s.total_allocated).unwrap_or(0);
    ...
    pub fn is_projected_cost_over_threshold(&self, projected_cost_cents: i64) -> bool {
        if self.total_limit_cents <= 0 {
            return projected_cost_cents > 0
                || self.state.lock().map(|s| s.total_allocated).unwrap_or(0) > 0;
        }
        ...
        projected_cost_cents >= limit_threshold_cents
            || self.state.lock().map(|s| s.total_allocated).unwrap_or(0) >= limit_threshold_cents
    }
...
mod tests {
    ...
    #[test]
    fn test_budget_reservation_full_settlement() {
        let manager = BudgetManager::new(100.0);

        let mut reservation = manager.reserve_cents(2000).unwrap().unwrap();
        assert_eq!(manager.get_remaining_cents(), 8000);

        reservation.settle(2000);
        assert_eq!(manager.get_remaining_cents(), 8000);

        let state = manager.state.lock().unwrap();
        assert_eq!(state.total_allocated, 2000);
        assert_eq!(state.settled, 2000);
    }

    #[test]
    fn test_budget_reservation_partial_settlement() {
        let manager = BudgetManager::new(100.0);

        let mut reservation = manager.reserve_cents(5000).unwrap().unwrap();
        assert_eq!(manager.get_remaining_cents(), 5000);

        reservation.settle(3000); // Only used 3000 of the 5000

        assert_eq!(manager.get_remaining_cents(), 7000);
        let state = manager.state.lock().unwrap();
        assert_eq!(state.total_allocated, 3000);
        assert_eq!(state.settled, 3000);
    }

    #[test]
    fn test_budget_reservation_drop_release() {
        let manager = BudgetManager::new(100.0);
