...
impl BudgetReservation {
    pub fn settle(mut self, final_amount_cents: i64) -> Result<(), String> {
        ...
        if final_amount_cents > self.amount_cents {
            return Err("settlement amount exceeds reserved amount".to_string());
        }

        {
            let mut state = self.state.lock().unwrap();
            state.settled_cents += final_amount_cents;
            let unused = self.amount_cents - final_amount_cents;
            state.total_allocated_cents -= unused;
        }
...
impl BudgetManager {
    ...
    pub fn reserve(&self, amount_cents: i64) -> Result<BudgetReservation, String> {
        ...
        let mut state = self.state.lock().unwrap();
        if amount_cents > 0 {
            let next_total = state.total_allocated_cents.checked_add(amount_cents);
            if let Some(next) = next_total {
                if next <= self.total_limit_cents {
                    state.total_allocated_cents = next;
                } else {
                    return Err("insufficient budget limit".to_string());
                }
            } else {
                return Err("insufficient budget limit".to_string());
            }
    ...
    pub fn record_spend_cents(&self, amount_cents: i64) -> Result<bool, String> {
        ...
        let admitted = {
            ...
            if let Some(next) = next_total {
                if next <= self.total_limit_cents {
                    state.total_allocated_cents = next;
                    state.settled_cents += amount_cents;
                    true
                } else {
                    false
                }
...
mod tests {
    ...
    #[test]
    fn test_budget_reservation_atomic_settle() {
        ...
        assert_eq!(state.total_allocated_cents, 5000);
        assert_eq!(state.settled_cents, 0);
        drop(state);

        res.settle(3000).unwrap();
        // The remaining 2000 cents should be released.
        assert_eq!(manager.get_remaining_cents(), 7000);

        let state = manager.state.lock().unwrap();
        assert_eq!(state.total_allocated_cents, 3000);
        assert_eq!(state.settled_cents, 3000);
    }

    #[test]
    fn test_budget_reservation_drop_releases() {
        let manager = BudgetManager::new(100.0);
        {
            let _res = manager.reserve(5000).unwrap();
            assert_eq!(manager.get_remaining_cents(), 5000);
        }
        // Should be released on drop
