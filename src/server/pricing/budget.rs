...
impl BudgetReservation {
    pub fn settle(mut self, actual_amount: i64) {
        if !self.settled {
            ...
            state.total_allocated = state.total_allocated.saturating_sub(self.amount);
            state.settled = state.settled.saturating_add(actual_amount);
            self.settled = true;
        }
    }
}
...
impl BudgetManager {
    ...
    pub fn reserve(&self, amount_cents: i64) -> Option<BudgetReservation> {
        ...
        if amount_cents == 0 {
            ...
            if current <= self.total_limit_cents {
