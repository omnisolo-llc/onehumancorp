...
impl Drop for BudgetReservation {
    fn drop(&mut self) {
        if !self.is_settled {
            let mut st = self.state.lock().unwrap();
            st.total_allocated = st.total_allocated.saturating_sub(self.amount_cents);
        }
    }
}
...
mod reservation_tests {
    ...
    fn test_budget_reservation_drop() {
        let manager = BudgetManager::new(10.0); // $10 limit
        assert_eq!(manager.get_remaining_cents(), 1000);

        {
            let _res = manager.reserve(200).unwrap();
            assert_eq!(manager.get_remaining_cents(), 800);
        } // drop without settle
        ...
        assert_eq!(st.total_allocated, 0);
    ...
    fn test_budget_reservation_settle() {
        ...
        let res = manager.reserve(300).unwrap();
        assert_eq!(manager.get_remaining_cents(), 700);

        res.settle();

        // Total allocated remains since it's settled (consumed)
        assert_eq!(manager.get_remaining_cents(), 700);

        let st = manager.state.lock().unwrap();
        assert_eq!(st.total_allocated, 300);
        assert_eq!(st.settled, 300);
    }

    #[test]
    fn test_budget_reservation_drop_after_settle() {
        let manager = BudgetManager::new(10.0);

        {
            ...
            res.settle();
        } // Drop should do nothing because it's settled

        assert_eq!(manager.get_remaining_cents(), 600);

        let st = manager.state.lock().unwrap();
        assert_eq!(st.total_allocated, 400);
        assert_eq!(st.settled, 400);
    }

    #[test]
    fn test_budget_reservation_telemetry_no_deadlock() {
        let store = std::sync::Arc::new(::server_harness::telemetry::ViolationStore::new(None));
        let manager = BudgetManager::new(10.0).with_telemetry("tenant-1".to_string(), store);

        let res = manager.reserve(100).unwrap();
        res.settle(); // If locking was wrong, this could deadlock during telemetry reporting

        assert_eq!(manager.get_remaining_cents(), 900);
    }
}
