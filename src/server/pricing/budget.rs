...
impl BudgetManager {
    ...
    pub fn reserve(&self, amount_cents: i64) -> Result<BudgetReservation, String> {
        ...
        if let Some(next_allocated) = state.total_allocated_cents.checked_add(amount_cents) {
            if next_allocated <= self.total_limit_cents {
                state.total_allocated_cents = next_allocated;
...
impl BudgetReservation {
    pub fn settle(&mut self) {
        ...
        let mut state = self.state.lock().unwrap();
        state.settled_cents += self.amount_cents;
        drop(state); // Drop lock before side-effects
...
impl BudgetManager {
    pub fn get_remaining(&self) -> f64 {
        let allocated = self.state.lock().unwrap().total_allocated_cents;
        (self.total_limit_cents - allocated) as f64 / 100.0
    }

    pub fn get_remaining_cents(&self) -> i64 {
        let allocated = self.state.lock().unwrap().total_allocated_cents;
        self.total_limit_cents - allocated
    }
    ...
    pub fn check_alert_threshold(&self) -> bool {
        ...
        let allocated = self.state.lock().unwrap().total_allocated_cents;
        let usage_percent = (allocated as f64 / self.total_limit_cents as f64) * 100.0;
    ...
    pub fn is_projected_cost_over_threshold(&self, projected_cost_cents: i64) -> bool {
        let allocated = self.state.lock().unwrap().total_allocated_cents;
        if self.total_limit_cents <= 0 {
            return projected_cost_cents > 0 || allocated > 0;
        }
        let limit_threshold_cents = ((self.total_limit_cents as f64)
            * (self.alert_threshold_percent / 100.0))
            .round() as i64;
        projected_cost_cents >= limit_threshold_cents || allocated >= limit_threshold_cents
    }
    ...
    pub fn check_alert_threshold_cents(&self, total_limit_cents: i64) -> bool {
        ...
        allocated >= limit_threshold_cents
    }
    ...
    pub fn is_spend_rate_too_high(
        ...
        allocated as f64 > expected_spend * 1.5 // 50% higher than expected rate
    }
}
...
mod tests {
    ...
    #[test]
    fn test_budget_reservation_flow() {
        let manager = BudgetManager::new(100.0);

        // Reserve funds
        let mut reservation = manager.reserve(2000).unwrap(); // $20
        assert_eq!(manager.get_remaining_cents(), 8000);

        // Release reservation without settling
        reservation.release();
        assert_eq!(manager.get_remaining_cents(), 10000);

        // Reserve and settle
        let mut reservation2 = manager.reserve(3000).unwrap(); // $30
        assert_eq!(manager.get_remaining_cents(), 7000);
        reservation2.settle();
        assert_eq!(manager.get_remaining_cents(), 7000); // Settled funds remain deducted

        // Release should be a no-op if already settled
    ...
    fn test_budget_reservation_drop() {
        let manager = BudgetManager::new(100.0);
        {
            let _reservation = manager.reserve(4000).unwrap(); // $40
            assert_eq!(manager.get_remaining_cents(), 6000);
            // _reservation dropped here, should release funds
        }
        assert_eq!(manager.get_remaining_cents(), 10000);
    }
}
