...
pub struct BudgetReservation {
    ...
    amount_cents: i64,
    is_settled_or_released: bool,
}

impl BudgetReservation {
    pub fn settle(mut self) {
        self.settle_partial(self.amount_cents)
    }

    pub fn settle_partial(&mut self, actual_amount_cents: i64) {
        if !self.is_settled_or_released {
            let actual_amount = if actual_amount_cents > self.amount_cents {
                self.amount_cents // Cannot settle more than reserved
            } else {
                actual_amount_cents
            };

            let unused_amount = self.amount_cents.saturating_sub(actual_amount);

            {
                let mut guard = self.state.lock().unwrap();
                guard.total_allocated = guard.total_allocated.saturating_sub(unused_amount);
                guard.settled = guard.settled.saturating_add(actual_amount);
            }
            ...
            self.is_settled_or_released = true;
        }
    }
}
...
impl Drop for BudgetReservation {
    fn drop(&mut self) {
        if !self.is_settled_or_released {
            let mut guard = self.state.lock().unwrap();
            guard.total_allocated = guard.total_allocated.saturating_sub(self.amount_cents);
        }
    }
}
...
impl BudgetManager {
    ...
    pub fn record_spend_cents(&self, amount_cents: i64) -> Result<bool, String> {
        ...
        {
            let mut guard = self.state.lock().unwrap();
            let next_allocated = guard.total_allocated.checked_add(amount_cents);
            if let Some(next) = next_allocated {
                if next <= self.total_limit_cents {
                    guard.total_allocated = next;
                    guard.settled = guard.settled.saturating_add(amount_cents);
                } else {
                    return Ok(false);
                }
            } else {
    ...
    pub fn reserve_cents(&self, amount_cents: i64) -> Result<BudgetReservation, String> {
...

        {
            ...
            if let Some(next) = next_allocated {
                ...
                return Err("amount overflow".to_string());
            }
        }

        if let (Some(_store), Some(tid)) = (&self.telemetry_store, &self.tenant_id) {
            tracing::info!(
                "💰 Miser telemetry: Recording budget reservation for tenant {}",
                tid
            ); // pii-safe
        }
    ...
    pub fn release_cents(&self, amount_cents: i64) -> Result<(), String> {
        ...
        {
            let mut guard = self.state.lock().unwrap();
            guard.total_allocated = guard.total_allocated.saturating_sub(amount_cents);
        }
