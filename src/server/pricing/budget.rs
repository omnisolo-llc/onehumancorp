...
impl BudgetManager {
    ...
    pub fn reserve(&self, amount_cents: i64) -> Result<Option<BudgetReservation>, String> {
        ...
        let mut state = self
            .state
            .lock()
            .map_err(|_| "failed to lock budget state".to_string())?;
...
mod tests {
    ...
    fn test_reserve_and_settle() {
        ...
        let reservation = manager.reserve(2000).unwrap().unwrap();
        assert_eq!(manager.get_remaining(), 80.0);

        reservation.settle();
        assert_eq!(manager.get_remaining(), 80.0);
        assert_eq!(manager.state.lock().unwrap().settled, 2000);
    }
    ...
    fn test_reserve_and_release() {
...

        {
            ...
            assert_eq!(manager.get_remaining(), 80.0);
            assert_eq!(manager.state.lock().unwrap().total_allocated, 2000);
        }

        // Dropping reservation should release funds
        assert_eq!(manager.get_remaining(), 100.0);
