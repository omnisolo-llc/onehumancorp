pub struct BudgetManagerState {
    pub total_allocated: i64,
    pub settled: i64,
}

pub struct BudgetReservation {
    state: std::sync::Arc<std::sync::Mutex<BudgetManagerState>>,
    amount_cents: i64,
...
impl BudgetManager {
    pub fn new(limit: f64) -> Self {
        ...
        BudgetManager {
            total_limit: limit,
            state: std::sync::Arc::new(std::sync::Mutex::new(BudgetManagerState {
                total_allocated: 0,
                settled: 0,
            })),
            total_limit_cents,
    ...
    pub fn reserve(&self, amount_cents: i64) -> Result<BudgetReservation, String> {
        ...
        if amount_cents == 0 {
            let remaining = self.get_remaining_cents();
            if remaining < 0 {
        ...
        {
            let mut state = self.state.lock().unwrap();
            let new_allocated = match state.total_allocated.checked_add(amount_cents) {
                Some(val) => val,
                None => return Err("spend amount overflow".to_string()),
            };

            if new_allocated > self.total_limit_cents {
                return Err("budget exhausted".to_string());
            }
            state.total_allocated = new_allocated;
        }
    ...
    pub fn record_spend_cents(&self, amount_cents: i64) -> Result<bool, String> {
        match self.reserve(amount_cents) {
            ...
            Err(e) if e == "budget exhausted" || e == "spend amount overflow" => Ok(false),
            Err(e) => Err(e),
        }
    }
