...
impl BudgetReservation {
    pub fn settle(mut self, final_amount_cents: i64) -> Result<(), String> {
        if final_amount_cents < 0 {
            return Err("final amount cannot be negative".to_string());
        }
        if final_amount_cents > self.amount_cents {
            return Err("cannot settle more than reserved amount".to_string());
        }

        let unused_cents = self.amount_cents - final_amount_cents;
        self.settled = true;

        {
            let mut state = self.state.lock().unwrap();
            state.total_allocated -= unused_cents;
            state.settled += final_amount_cents;
        }
...
impl BudgetManager {
    ...
    pub fn reserve_cents(&self, amount_cents: i64) -> Result<Option<BudgetReservation>, String> {
        ...
        state.total_allocated += amount_cents;
        drop(state);
    ...
    pub fn get_remaining(&self) -> f64 {
        ...
        (self.total_limit_cents - state.total_allocated) as f64 / 100.0
    }

    pub fn get_remaining_cents(&self) -> i64 {
        let state = self.state.lock().unwrap();
        self.total_limit_cents - state.total_allocated
    }
    ...
    pub fn check_alert_threshold(&self) -> bool {
        ...
        let state = self.state.lock().unwrap();
        let usage_percent = (state.total_allocated as f64 / self.total_limit_cents as f64) * 100.0;
    ...
    pub fn is_projected_cost_over_threshold(&self, projected_cost_cents: i64) -> bool {
        if self.total_limit_cents <= 0 {
            let state = self.state.lock().unwrap();
            return projected_cost_cents > 0 || state.total_allocated > 0;
        }
        ...
        projected_cost_cents >= limit_threshold_cents
            || state.total_allocated >= limit_threshold_cents
    }
    ...
    pub fn is_spend_rate_too_high(
        ...
        state.total_allocated as f64 > expected_spend * 1.5 // 50% higher than expected rate
    }
}
...
mod tests {
    ...
    fn test_reserve_and_settle() {
        ...
        let reservation = manager.reserve_cents(1000).unwrap().unwrap();
        assert_eq!(manager.get_remaining_cents(), 9000);

        // state total_allocated is 1000, settled is 0
        {
            let state = manager.state.lock().unwrap();
            assert_eq!(state.total_allocated, 1000);
            assert_eq!(state.settled, 0);
        }

        reservation.settle(1000).unwrap();

        // total_allocated should still be 1000, settled is 1000
        {
            let state = manager.state.lock().unwrap();
            assert_eq!(state.total_allocated, 1000);
            assert_eq!(state.settled, 1000);
        }
        assert_eq!(manager.get_remaining_cents(), 9000);
    }

    #[test]
    fn test_reserve_and_partial_settle() {
        let manager = BudgetManager::new(100.0);
        assert_eq!(manager.get_remaining_cents(), 10000);
        ...
        reservation.settle(600).unwrap();

        // unused 400 is released, total_allocated = 600, settled = 600
        {
            let state = manager.state.lock().unwrap();
            assert_eq!(state.total_allocated, 600);
            assert_eq!(state.settled, 600);
        }
        assert_eq!(manager.get_remaining_cents(), 9400);
    }
    ...
    fn test_reserve_and_drop() {
        ...
        {
            let _reservation = manager.reserve_cents(1000).unwrap().unwrap();
            assert_eq!(manager.get_remaining_cents(), 9000);
        } // guard is dropped here without settling

        // all 1000 should be released back
        {
            let state = manager.state.lock().unwrap();
    ...
    #[test]
    fn test_reserve_and_over_settle() {
        let manager = BudgetManager::new(100.0);
        let reservation = manager.reserve_cents(1000).unwrap().unwrap();

        let err = reservation.settle(1500).unwrap_err();
        assert_eq!(err, "cannot settle more than reserved amount");
    }
}
