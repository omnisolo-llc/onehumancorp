...
impl BudgetManager {
    ...
    pub fn settle(&self, reserved_amount_cents: i64, actual_amount_cents: i64) -> Result<(), String> {
        ...
        let diff = actual_amount_cents - reserved_amount_cents;
        if diff != 0 {
            self.allocated.fetch_add(diff, Ordering::SeqCst);
        }
    ...
    pub fn release(&self, reserved_amount_cents: i64) -> Result<(), String> {
    ...
    pub fn get_remaining(&self) -> f64 {
        ...
        (self.total_limit_cents - alloc) as f64 / 100.0
    }

    pub fn get_remaining_cents(&self) -> i64 {
        let alloc = self.allocated.load(Ordering::SeqCst);
        self.total_limit_cents - alloc
    }
    ...
    pub fn is_projected_cost_over_threshold(&self, projected_cost_cents: i64) -> bool {
        if self.total_limit_cents <= 0 {
            return projected_cost_cents > 0 || self.allocated.load(Ordering::SeqCst) > 0;
        }
        ...
        projected_cost_cents >= limit_threshold_cents
            || self.allocated.load(Ordering::SeqCst) >= limit_threshold_cents
    }
    ...
    pub fn is_spend_rate_too_high(
        ...
        alloc as f64 > expected_spend * 1.5 // 50% higher than expected rate
    }
}
...
mod tests {
    ...
    fn concurrent_reservation_never_exceeds_limit() {
        ...
        let workers: Vec<_> = (0..32)
            .map(|_| {
                let manager = manager.clone();
                std::thread::spawn(move || manager.reserve(10).unwrap())
            })
            .collect();
        let admitted = workers
            .into_iter()
    ...
    #[test]
    fn reserve_settle_release_flow() {
        let manager = BudgetManager::new(100.0);

        // Reserve $50
        assert!(manager.reserve(5000).unwrap());
        assert_eq!(manager.get_remaining_cents(), 5000);
        assert_eq!(manager.allocated.load(Ordering::SeqCst), 5000);

        // Settle $40 from the $50 reservation
        manager.settle(5000, 4000).unwrap();
        assert_eq!(manager.current.load(Ordering::SeqCst), 4000);
        assert_eq!(manager.allocated.load(Ordering::SeqCst), 4000);
        assert_eq!(manager.get_remaining_cents(), 6000); // 100 - 40 = 60

        // Reserve another $50
        assert!(manager.reserve(5000).unwrap());
        assert_eq!(manager.get_remaining_cents(), 1000); // 60 - 50 = 10

        // Release the reservation
        manager.release(5000).unwrap();
        assert_eq!(manager.get_remaining_cents(), 6000);
        assert_eq!(manager.allocated.load(Ordering::SeqCst), 4000);
    }

    #[test]
    fn settle_overage_is_accurately_recorded() {
        let manager = BudgetManager::new(100.0);

        // Reserve $50
        assert!(manager.reserve(5000).unwrap());

        // Settle $60 from the $50 reservation (overage)
        manager.settle(5000, 6000).unwrap();

        // It should accurately record $60 spent and allocated, reducing remaining to $40
        assert_eq!(manager.current.load(Ordering::SeqCst), 6000);
        assert_eq!(manager.allocated.load(Ordering::SeqCst), 6000);
        assert_eq!(manager.get_remaining_cents(), 4000);

        // Reserve another $40
        assert!(manager.reserve(4000).unwrap());

        // Settle $50 from the $40 reservation (pushes past total limit)
        manager.settle(4000, 5000).unwrap();

        // Overages should be preserved!
        assert_eq!(manager.current.load(Ordering::SeqCst), 11000);
        assert_eq!(manager.allocated.load(Ordering::SeqCst), 11000);
