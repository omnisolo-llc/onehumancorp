...
pub struct BudgetManager {
    ...
    pub state: std::sync::Arc<std::sync::Mutex<BudgetState>>,
...
impl BudgetReservation {
    pub fn settle(mut self, actual_amount_cents: i64) -> Result<(), String> {
        if actual_amount_cents < 0 {
            return Err("settle amount cannot be negative".to_string());
        }
        if actual_amount_cents > self.amount_cents {
            return Err("settle amount exceeds reservation".to_string());
        }

        let mut state = self.state.lock().unwrap();
        state.settled += actual_amount_cents;
        // Release the unused portion
        state.total_allocated -= self.amount_cents - actual_amount_cents;
        self.settled = true;
        drop(state);
...
mod tests {
    ...
    fn test_budget_reserve_success() {
        let manager = BudgetManager::new(100.0);
        let res = manager.reserve(5000).unwrap();
        ...
        res.settle(4000).unwrap();

        // remaining should be 6000 (10000 - 4000)
        assert_eq!(manager.get_remaining_cents(), 6000);
        let state = manager.state.lock().unwrap();
        assert_eq!(state.total_allocated, 4000);
        assert_eq!(state.settled, 4000);
    }
    ...
    fn test_budget_reservation_drop() {
        let manager = BudgetManager::new(100.0);
        {
            let _res = manager.reserve(3000).unwrap();
            assert_eq!(manager.get_remaining_cents(), 7000);
            assert_eq!(manager.state.lock().unwrap().total_allocated, 3000);
        } // _res goes out of scope here and should be dropped without being settled

        // remaining should be back to 10000
        assert_eq!(manager.get_remaining_cents(), 10000);
        let state = manager.state.lock().unwrap();
