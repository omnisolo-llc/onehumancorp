...
async fn geo_score(
    ...
    let typed_args: ohc_builtin_agent::tools::generative_visibility::GenerativeVisibilityArgs = serde_json::from_value(args).map_err(|_| axum::http::StatusCode::BAD_REQUEST)?;
    let res_str = executor.execute_typed(typed_args).await.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
