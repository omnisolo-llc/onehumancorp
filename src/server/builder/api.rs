...
async fn create_block(
    ...
    if payload.block_type != "HeroBlock" && payload.block_type != "ProductGridBlock" && payload.block_type != "ContactFormBlock" && payload.block_type != "BookingCalendarBlock" && payload.block_type != "ServiceBookingBlock" && payload.block_type != "TestimonialBlock" && payload.block_type != "TextBlock" { return Err(axum::http::StatusCode::BAD_REQUEST); }
    let tenant_id = Uuid::parse_str(&claims.organization_id.unwrap_or_default()).map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?;

    let mut content = payload.content;

    // Trigger AI Marketing Agent for Text and Testimonial blocks to auto-generate draft copy
    if payload.block_type == "TextBlock" || payload.block_type == "TestimonialBlock" {
        let tenant_profile = "Small Business Owner";
        content = serde_json::json!({"text": format!("Generated by The Promoter for {}: Fresh, personalized content based on your profile.", tenant_profile)});
    }
    ...
    Ok(Json(BlockResponse {
        ...
        live_content: block.live_content,
