...
func TestMemoryLock_FailuresAndPaths(t *testing.T) {
	...
	os.Mkdir(path, 0700)
	os.WriteFile(filepath.Join(path, "info"), []byte("invalid_format"), 0600)
...
func TestMemoryLock_ExpiredLockOverwrite(t *testing.T) {
	...
	os.Mkdir(path, 0700)
	os.WriteFile(filepath.Join(path, "info"), []byte(expiry+",old_token"), 0600)
...
func TestMemoryLock_CoveragePaths(t *testing.T) {
	...
	// Force os.WriteFile to fail.
	// We can do this by creating a directory where the info file should be.
	os.MkdirAll(filepath.Join(path, "info"), 0755)

	locked, err := lock1.Lock(ctx, key, 1*time.Second)
	if locked {
		t.Fatalf("Expected lock to fail when info path is a directory")
	}
