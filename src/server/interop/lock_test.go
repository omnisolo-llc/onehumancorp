...
func TestMemoryLock_CoveragePaths(t *testing.T) {
	...
	// Actually, if we create a normal file at path, Mkdir fails with Not a directory or File exists.
	// We will create a file so it fails with IsExist but reading meta.txt from it fails because it's not a dir
	os.WriteFile(path, []byte("not_a_dir"), 0666)

	locked, err := lock1.Lock(ctx, key, 1*time.Second)
	if locked {
		t.Fatalf("Expected lock to fail when path is a file")
	}
