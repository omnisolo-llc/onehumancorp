...
impl InteropProtocol {
    ...
    pub async fn resume_mission(&self, mission_id: &str, tenant_id: &str, state_payload: Vec<u8>, target_mode: i32) -> Result<(), String> {
        // Handoff uses the same mechanism to synchronize state
        self.handoff(mission_id, tenant_id, state_payload, target_mode).await
    }
...
mod tests {
    ...
    async fn test_interop_handoff_memory() {
        ...
        protocol.handoff("mission_1", "tenant_1", vec![1, 2, 3], 0).await.unwrap();
    ...
    async fn test_interop_resume_mission() {
        ...
        protocol.resume_mission("mission_resume_1", "tenant_1", vec![1, 2, 3], 0).await.unwrap();
        sleep(Duration::from_millis(100)).await;
    ...
    async fn test_interop_handoff_idempotency_simulation() {
        ...
        let protocol2 = InteropProtocol::new(bus.clone(), lock.clone(), "node2".to_string(), 0);
        protocol2.handoff("mission_1", "tenant_1", vec![1, 2, 3], 0).await.unwrap();
    ...
    async fn test_interop_listen_for_state_handoff() {
        ...
        protocol.handoff("mission_2", "tenant_2", vec![1, 2, 3], 0).await.unwrap();
        sleep(Duration::from_millis(50)).await;
        assert!(received.load(Ordering::SeqCst));
    }
    ...
    async fn test_interop_handoff_lock_deadlock_prevention() {
        ...
        let result = protocol1.handoff("mission_locked", "tenant_1", vec![1, 2, 3], 0).await;
    ...
    async fn test_interop_handoff_retry_success() {
        ...
        let result = protocol.handoff("mission_retry_1", "tenant_1", vec![1, 2, 3], 0).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_interop_handoff_retry_failure() {
        ...
        let result = protocol.handoff("mission_retry_2", "tenant_1", vec![1, 2, 3], 0).await;
        assert!(result.is_err());
    ...
    async fn test_interop_handoff_success() {
        ...
        let result = protocol.handoff("m1", "t1", vec![], 0).await;
        assert!(result.is_ok());
    ...
    #[tokio::test]
    async fn test_interop_handoff_idempotency() {
        let bus = Arc::new(MemoryBus::new());
        let lock = Arc::new(MemoryBus::new());
        let protocol = InteropProtocol::new(bus.clone(), lock.clone(), "node1".to_string(), 1);

        // Pre-acquire the idempotency lock to simulate it already being processed
        let mission_id = "m_idem";
        let idempotency_lock_resource = format!("handoff:processed:{}", mission_id);

        lock.acquire_lock(&idempotency_lock_resource, "someone_else", 3600).await.unwrap();
        ...
        // Calling handoff should return Ok(()) without actually sending a message
        let result = protocol.handoff(mission_id, "t1", vec![], 2).await;
        assert!(result.is_ok());
