...
impl InteropProtocol {
    ...
    pub async fn handoff(&self, mission_id: &str, tenant_id: &str, state_payload: Vec<u8>, target_mode: i32) -> Result<(), String> {
    ...
    pub async fn resume_mission(&self, mission_id: &str, tenant_id: &str, state_payload: Vec<u8>, _source_mode: i32) -> Result<(), String> {
        // Handoff uses the same mechanism to synchronize state
        self.handoff(mission_id, tenant_id, state_payload, self.current_mode).await
    }
...
mod tests {
    ...
    async fn test_interop_handoff_memory() {
        ...
        let lock = bus.clone();
        let protocol = InteropProtocol::new(bus.clone(), lock, "node1".to_string(), 1);
        ...
        protocol.handoff("mission_1", "tenant_1", vec![1, 2, 3], 2).await.unwrap();
    ...
    async fn test_interop_health_memory() {
        ...
        let protocol1 = InteropProtocol::new(bus.clone(), lock.clone(), "node1".to_string(), 1);
        let protocol2 = InteropProtocol::new(bus.clone(), lock.clone(), "node2".to_string(), 1);
    ...
    async fn test_interop_job_dispatch() {
        ...
        let protocol_server = InteropProtocol::new(bus.clone(), lock.clone(), "server".to_string(), 1);
        let protocol_agent = InteropProtocol::new(bus.clone(), lock.clone(), "agent".to_string(), 1);
    ...
    async fn test_interop_resume_mission() {
        ...
        protocol.resume_mission("mission_resume_1", "tenant_1", vec![1, 2, 3], 1).await.unwrap();
        sleep(Duration::from_millis(100)).await;
    ...
    async fn test_interop_handoff_idempotency_simulation() {
        let bus = Arc::new(MemoryBus::new());
        let lock = bus.clone();
        let protocol = InteropProtocol::new(bus.clone(), lock.clone(), "node1".to_string(), 1);
        ...
        let protocol2 = InteropProtocol::new(bus.clone(), lock.clone(), "node2".to_string(), 1);
        protocol2.handoff("mission_1", "tenant_1", vec![1, 2, 3], 2).await.unwrap();
    ...
    async fn test_interop_listen_for_state_handoff() {
        ...
        protocol.handoff("mission_2", "tenant_2", vec![1, 2, 3], 2).await.unwrap();
        sleep(Duration::from_millis(50)).await;
        assert!(received.load(Ordering::SeqCst));
    }
    ...
    async fn test_interop_listen_for_pings() {
        ...
        let protocol_listener = InteropProtocol::new(bus.clone(), lock.clone(), "listener_node".to_string(), 1);
    ...
    async fn test_interop_handoff_lock_deadlock_prevention() {
        ...
        let result = protocol1.handoff("mission_locked", "tenant_1", vec![1, 2, 3], 2).await;
    ...
    async fn test_interop_dispatch_job_retry_success() {
        ...
        let lock = Arc::new(MemoryBus::new());
        let protocol = InteropProtocol::new(bus, lock, "server".to_string(), 1);
    ...
    async fn test_interop_handoff_retry_success() {
        ...
        let lock = Arc::new(MemoryBus::new());
        let protocol = InteropProtocol::new(bus, lock, "node1".to_string(), 1);

        let result = protocol.handoff("mission_retry_1", "tenant_1", vec![1, 2, 3], 2).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_interop_handoff_retry_failure() {
        ...
        let result = protocol.handoff("mission_retry_2", "tenant_1", vec![1, 2, 3], 2).await;
        assert!(result.is_err());
    ...
    async fn test_interop_job_status_reporting_retry_success() {
        ...
        let protocol = InteropProtocol::new(bus, lock, "agent".to_string(), 1);
    ...
    async fn test_interop_health_timeout() {
        let bus = Arc::new(MemoryBus::new());
        let lock = bus.clone();
        let protocol = InteropProtocol::new(bus.clone(), lock.clone(), "node_timeout".to_string(), 1);
    ...
    async fn test_interop_handoff_success() {
        ...
        let result = protocol.handoff("m1", "t1", vec![], 2).await;
        assert!(result.is_ok());
