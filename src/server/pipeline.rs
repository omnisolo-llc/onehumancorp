...
impl Orchestrator {
    ...
    pub async fn handle_test_results(&self, msg: Message) -> Result<(), String> {
        ...
        if msg.r#type == "TestsPassed" {
            ...
            self.hub.clone().publish(approval_msg).await.map_err(|e| e.to_string())?;
            ...
            self.hub.clone().publish(fail_msg).await.map_err(|e| e.to_string())?;
        }
    ...
    pub async fn reject_staging(&self, branch: &str, reason: &str) -> Result<(), String> {
        ...
        self.hub.clone().publish(reject_msg).await.map_err(|e| e.to_string())?;
    ...
    pub async fn approve_for_production(&self, branch: &str) -> Result<(), String> {
        ...
        self.hub.clone().publish(merge_msg).await.map_err(|e| e.to_string())?;
