...
resolve_capture_script() {
  ...
  for candidate in \
    ...
    "${runfiles_base}/${TEST_WORKSPACE:-mono}/src/app/e2e/capture_screenshots.mjs" \
    "${runfiles_base}/_main/src/app/e2e/capture_screenshots.mjs" \
    "${runfiles_base}/__main__/src/app/e2e/capture_screenshots.mjs"; do
...
if [[ -z "${web_artifacts}" && -n "${runfiles_root}" ]]; then
  web_artifacts="$(find_first_dir \
    "${runfiles_root}/${TEST_WORKSPACE:-mono}/src/app/app_web.web_build_artifacts" \
    "${runfiles_root}/${TEST_WORKSPACE:-mono}/src/app/app.web_build_artifacts" \
    "${runfiles_root}/${TEST_WORKSPACE:-mono}/src/app/app_web_build_artifacts" \
    "${runfiles_root}/${TEST_WORKSPACE:-mono}/src/app/app.web_build_artifacts" \
    "${runfiles_root}/_main/src/app/app_web.web_build_artifacts" \
    "${runfiles_root}/_main/src/app/app.web_build_artifacts" \
    "${runfiles_root}/_main/src/app/app_web_build_artifacts" \
    "${runfiles_root}/__main__/src/app/app_web.web_build_artifacts" \
    "${runfiles_root}/__main__/src/app/app.web_build_artifacts" \
    "${runfiles_root}/__main__/src/app/app_web_build_artifacts" || true)"
...
if [[ -z "${web_artifacts}" ]]; then
  web_artifacts="$(find_first_dir \
    "${workspace_root}/bazel-bin/src/app/app.web_build_artifacts" \
    "${workspace_root}/bazel-bin/src/app/app_web.web_build_artifacts" || true)"
fi
...
if [[ -z "${playwright_package_dir}" ]]; then
  playwright_package_dir="$(find_first_dir \
    ...
    "${workspace_root}/bazel-bin/src/app/capture_screenshots.runfiles/${TEST_WORKSPACE:-mono}/node_modules/@playwright/test" \
    "${workspace_root}/bazel-bin/src/app/capture_screenshots.runfiles/_main/node_modules/@playwright/test" || true)"
fi

if [[ -z "${web_artifacts}" ]]; then
  echo "ERROR: could not locate Bazel-built Flutter web artifacts in runfiles." >&2
  echo "Ensure this helper is launched via 'bazelisk run //src/app:capture_screenshots'." >&2
  exit 1
fi
