...
#[cfg(test)]
use crate::app;
#[allow(unused_imports)] use slint::ComponentHandle;
use slint::Model;
    ...
    fn test_login_forgot_password_trigger() { let ui = create_login(); ui.invoke_open_settings(); }
    ...
    fn test_login_view_password() { let ui = create_login(); ui.set_is_sign_up(true); assert!(ui.get_is_sign_up()); }
    ...
    fn test_wizard_template_selection() { let ui = create_wizard(); ui.set_business_type("Dark".into()); assert_eq!(ui.get_business_type(), "Dark"); }
    fn test_wizard_domain_choice() { let ui = create_wizard(); ui.set_company_name("custom".into()); assert_eq!(ui.get_company_name(), "custom"); }
    ...
    fn test_wizard_back_button() { let ui = create_wizard(); ui.set_step(2); ui.set_step(1); assert_eq!(ui.get_step(), 1); }
    fn test_wizard_next_button() { let ui = create_wizard(); ui.set_step(1); ui.set_step(2); assert_eq!(ui.get_step(), 2); }
    fn test_wizard_skip_button() { let ui = create_wizard(); ui.set_step(3); }
    fn test_wizard_instant_preview() { let ui = create_wizard(); ui.set_step(4); }
    fn test_wizard_launch() { let ui = create_wizard(); ui.set_step(5); }
    ...
    fn test_template_minimal() { let ui = create_wizard(); ui.set_business_type("Minimal".into()); }
    fn test_template_bold() { let ui = create_wizard(); ui.set_business_type("Bold".into()); }
    fn test_template_classic() { let ui = create_wizard(); ui.set_business_type("Classic".into()); }
    fn test_template_tech() { let ui = create_wizard(); ui.set_business_type("Tech".into()); }
    fn test_template_creative() { let ui = create_wizard(); ui.set_business_type("Creative".into()); }
    ...
    fn test_wizard_name_error() { let ui = create_wizard(); ui.set_product_name("Required".into()); assert_eq!(ui.get_product_name(), "Required"); }
    fn test_wizard_email_error() { let ui = create_wizard(); ui.set_admin_email("Invalid".into()); assert_eq!(ui.get_admin_email(), "Invalid"); }
    ...
    fn test_dash_user_name() { let ui = create_dashboard(); ui.set_show_milestone(true); assert!(ui.get_show_milestone()); }
    ...
    fn test_dash_open_billing() { let ui = create_dashboard(); ui.invoke_dismiss_milestone(); }
    fn test_dash_open_settings() { let ui = create_dashboard(); ui.invoke_dismiss_milestone(); }

    fn test_dash_data_1() { let ui = create_dashboard(); ui.set_todays_sales("+10%".into()); }
    fn test_dash_data_2() { let ui = create_dashboard(); ui.set_todays_sales("-2%".into()); }
    fn test_dash_data_3() { let ui = create_dashboard(); ui.set_new_orders_count(100); }
    fn test_dash_data_4() { let ui = create_dashboard(); ui.set_new_orders_count(5); }
    fn test_dash_data_5() { let ui = create_dashboard(); ui.set_todays_sales("$50".into()); }
    fn test_dash_data_6() { let ui = create_dashboard(); ui.set_todays_sales("Widget A".into()); }
    fn test_dash_data_7() { let ui = create_dashboard(); ui.set_todays_sales("Healthy".into()); }
    fn test_dash_data_8() { let ui = create_dashboard(); ui.set_todays_sales("2m ago".into()); }
    fn test_dash_data_9() { let ui = create_dashboard(); ui.set_new_orders_count(3); }
    fn test_dash_data_10() { let ui = create_dashboard(); ui.set_show_milestone(true); }
    ...
    fn test_build_price() { let ui = create_builder(); ui.set_product_name("1.99".into()); assert_eq!(ui.get_product_name(), "1.99"); }
    fn test_build_publish() { let ui = create_builder(); ui.set_step(1); }
    ...
    fn test_build_preview_toggle() { let ui = create_builder(); ui.set_product_name("mobile".into()); assert_eq!(ui.get_product_name(), "mobile"); }
    fn test_build_color_picker() { let ui = create_builder(); ui.set_product_name("#FF0000".into()); }
    fn test_build_font_selection() { let ui = create_builder(); ui.set_product_name("Serif".into()); }
    fn test_build_upload_image() { let ui = create_builder(); ui.set_step(2); }
    fn test_build_delete_image() { let ui = create_builder(); ui.set_step(3); }
    // ─────────────────────────────────────────────────────────────────────────
