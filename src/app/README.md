...
bazelisk test //src/app:flutter_unit_tests

# One specific test file (per-file Bazel target)
bazelisk test //src/app/lib/services:local_manager_service_test

# Per-package coverage checks (>= 90% each)
bazelisk test //src/app:flutter_coverage_tests
```
...
```bash
# Layout validation
bazelisk test //src/app:app_layout_test

# Desktop button-click regression coverage
bazelisk test //src/app:app_desktop_e2e_test

# Web Playwright end-to-end validation
bazelisk test //src/app:app_web_e2e_test
```
...
The app communicates with the OHC backend (`src/core` Rust library exposed
through the Go dashboard server).  All business logic — agent scheduling,
meeting rooms, chat integration — lives in `src/core` and is shared between:

- This Flutter app (via the REST/gRPC API)
- The Tauri desktop app (`src/desktop`)
- The web frontend (`src/frontend`)
