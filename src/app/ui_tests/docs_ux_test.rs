...
fn test_e2e_help_center_navigation_flow() {
    ...
    assert_eq!(
        ui.get_test_title(),
        slint::SharedString::from("Help Center")
    );
}
...
fn test_e2e_api_docs_navigation_flow() {
    ...
    assert_eq!(
        ui.get_test_title(),
        slint::SharedString::from("Connect Custom Software")
    );
    ...
    assert!(
        *endpoint_tested.borrow(),
        "Endpoint execution callback must be triggered"
    );
}
...
fn test_e2e_release_notes_navigation_flow() {
    ...
    assert_eq!(
        ui.get_current_version(),
        slint::SharedString::from("v0.3.4")
    );
}
...
fn test_e2e_ai_help_chat_navigation_flow() {
    ...
    assert_eq!(
        ui.get_test_title(),
        slint::SharedString::from("AI Help Assistant")
    );
}
