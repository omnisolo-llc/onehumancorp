...
fn create() -> app::Landing {
    crate::ui_tests::init();
    app::Landing::new().unwrap()
}

// --- Specialized / Flow Tests ---

#[test]
fn landing_flow_variant_toggle() {
    let ui = create();
...
#[test]
fn landing_flow_start_setup_callback() {
    let ui = create();
    let called = std::rc::Rc::new(std::cell::RefCell::new(false));
    let c = called.clone();
    ui.on_start_business_setup(move || {
        *c.borrow_mut() = true;
    });
...
#[test]
fn landing_flow_continue_callback() {
    let ui = create();
    let called = std::rc::Rc::new(std::cell::RefCell::new(false));
    let c = called.clone();
    ui.on_continue_to_dashboard(move || {
        *c.borrow_mut() = true;
    });
...
#[test]
fn landing_flow_download_callback() {
    let ui = create();
    let called = std::rc::Rc::new(std::cell::RefCell::new(String::new()));
    let c = called.clone();
    ui.on_download(move |os| {
        *c.borrow_mut() = os.to_string();
    });
