...
#[test]
fn channels_massive_list() {
    let ui = create();
    let v: Vec<app::UiChatChannel> = (0..500)
        .map(|i| app::UiChatChannel {
            id: i.to_string().into(),
...
#[test]
fn channels_flow_add_callback() {
    let ui = create();
    let called = std::rc::Rc::new(std::cell::RefCell::new(false));
    let c = called.clone();
    ui.on_add_channel(move || {
        *c.borrow_mut() = true;
    });
