...
fn create() -> app::Pipelines {
    crate::ui_tests::init();
    app::Pipelines::new().unwrap()
}

// --- Hacking / Corner Cases ---

#[test]
fn pipelines_xss_name() {
    let ui = create();
    let xss = "<script>alert('pipeline')</script>";
    let pipelines = slint::VecModel::from(vec![app::UiPipeline {
        id: "1".into(),
        name: xss.into(),
        branch: "main".into(),
        status: "Running".into(),
...
#[test]
fn pipelines_flow_promote_callback() {
    let ui = create();
    let called = std::rc::Rc::new(std::cell::RefCell::new(String::new()));
    let c = called.clone();
    ui.on_promote_pipeline(move |id| {
...
#[test]
fn pipelines_flow_refresh_callback() {
    let ui = create();
