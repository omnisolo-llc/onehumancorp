...
fn login_handles_advanced_options_to_app_settings() {
    ...
    assert!(
        *invoked.borrow(),
        "The Advanced Options button should invoke open_settings"
    );
}
...
fn grandmother_test_login_advanced_options_callback_preservation() {
    ...
    assert_eq!(
        *counter.borrow(),
        2,
        "Callback for Advanced Options should remain intact and fire multiple times."
    );
}
...
fn login_handles_oauth() {
    ...
    assert!(
        *invoked.borrow(),
        "The Continue with Google/Apple button should invoke oauth_login"
    );
}
...
fn login_handles_start_setup_wizard_programmatically() {
    ...
    assert!(
        *invoked.borrow(),
        "start_setup_wizard should still be callable programmatically"
    );
}
