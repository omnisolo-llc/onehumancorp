...
fn create() -> app::Handoffs {
    crate::ui_tests::init();
    app::Handoffs::new().unwrap()
}

// --- Hacking / Corner Cases ---

#[test]
fn handoffs_xss_intent() {
    let ui = create();
    let xss = "<script>alert('handoff')</script>";
    let handoffs = slint::VecModel::from(vec![app::UiHandoff {
        id: "1".into(),
        intent: xss.into(),
...
#[test]
fn handoffs_massive_list() {
    let ui = create();
    let v: Vec<app::UiHandoff> = (0..300)
        .map(|i| app::UiHandoff {
            id: format!("h-{}", i).into(),
...
#[test]
fn handoffs_flow_resolve_callback() {
    let ui = create();
