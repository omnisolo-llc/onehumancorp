...
#[test]
fn meetings_xss_name() {
    let ui = create();
    let xss = "<script>alert('meeting')</script>";
    let meetings = slint::VecModel::from(vec![app::UiMeeting {
        name: xss.into(),
        status: "Active".into(),
        participants: 1,
        is_active: true,
...
#[test]
fn meetings_negative_participants() {
    let ui = create();
    let meetings = slint::VecModel::from(vec![app::UiMeeting {
...
#[test]
fn meetings_flow_mass_list() {
    let ui = create();
    let v: Vec<app::UiMeeting> = (0..100)
        .map(|i| app::UiMeeting {
...
#[test]
fn meetings_flow_new_room_callback() {
    let ui = create();
