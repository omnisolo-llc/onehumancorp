...
#[test]
fn tasklist_xss_title() {
    let ui = create();
    let xss = "<script>alert('task')</script>";
    let tasks = slint::VecModel::from(vec![app::UiTask {
        title: xss.into(),
        status: "Todo".into(),
        agent_id: "a1".into(),
        dependencies: "".into(),
...
#[test]
fn tasklist_massive_list() {
    let ui = create();
    let v: Vec<app::UiTask> = (0..500)
        .map(|i| app::UiTask {
...
#[test]
fn tasklist_flow_refresh_callback() {
    let ui = create();
