        ...
        // Handle download invoice
            ...
            if let Ok(dashboard) = app::Dashboard::new() {
                ...
                let all_articles = vec![
                    app::HelpArticle { category: "Getting Started".into(), title: "Welcome to OneHumanCorp!".into(), description: "We're so glad you're here! Our mission is to help you launch your business in under 10 minutes. Use the Setup Wizard to get started with no technical skills needed.".into() },
                    app::HelpArticle { category: "Getting Started".into(), title: "Guided Setup vs Instant Build".into(), description: "Choose 'Guided Setup' to walk through every detail, or 'Instant Build' to let our AI design your entire storefront from a short description of your business.".into() },
                    app::HelpArticle { category: "My Store".into(), title: "Adding Your Products".into(), description: "Go to 'Add' in the bottom menu to list what you sell. You can add photos, set prices, and even let AI write your product descriptions for you.".into() },
                    app::HelpArticle { category: "My Store".into(), title: "Managing Your Inventory".into(), description: "Keep track of what's in stock. You can set inventory limits for physical products like Maya's cakes or Priya's boutique items.".into() },
                    app::HelpArticle { category: "Payments".into(), title: "How to Get Paid".into(), description: "Connect your bank account via Stripe to accept credit cards, Apple Pay, and Google Pay. You can take payments online or in-person using your phone.".into() },
                    app::HelpArticle { category: "Payments".into(), title: "Deposits & Custom Quotes".into(), description: "If you're a service provider like Carlos the handyman, you can ask for a deposit or send a custom quote before the job starts.".into() },
                    app::HelpArticle { category: "AI Helpers".into(), title: "Meet Your AI Team".into(), description: "Your AI Helpers (The Manager, The Promoter, The Ambassador) work in the background 24/7. They handle customer questions, post to social media, and track your sales.".into() },
                    app::HelpArticle { category: "AI Helpers".into(), title: "Training Your Helpers".into(), description: "You can adjust how your helpers talk to customers. Want them to be 'Friendly' like Maya or 'Professional' like Priya? Just head to Helper Settings.".into() },
                    app::HelpArticle { category: "Marketing".into(), title: "Promoting Your Business".into(), description: "Use the 'Grow Business' tool to run email campaigns or connect your Instagram. Your helpers can even draft posts for you to approve.".into() },
                    app::HelpArticle { category: "Marketing".into(), title: "Sharing Your Store".into(), description: "Click 'Share Store' to get a beautiful link for your TikTok bio or to send to friends via WhatsApp.".into() },
                    app::HelpArticle { category: "Account & Billing".into(), title: "Simple, Transparent Pricing".into(), description: "We offer a 'Free' tier to get you started. As you grow, you can upgrade to 'Pro' for more AI power and a custom domain (like www.yourname.com).".into() },
                ];
                ...
                ai_chat_ui.on_send_message({
                    ...
                    move || {
                        if let Some(ui) = chat_handle.upgrade() {
                            ...
                            // Simulating a realistic backend response with context-aware responses
                            let input_lower = input.to_lowercase();
                            let (response_text, article_link) = if input_lower.contains("product") || input_lower.contains("sell") {
                                ("To add a product, click 'Add' in the bottom menu. You can set the name, price, and description there.".to_string(), "Adding Your Products")
                            } else if input_lower.contains("pay") || input_lower.contains("stripe") || input_lower.contains("bank") {
                                ("You can set up payments via Stripe in the Payments section to start accepting credit cards and Apple Pay.".to_string(), "How to Get Paid")
                            } else if input_lower.contains("helper") || input_lower.contains("ai") || input_lower.contains("agent") {
                                ("Your AI Helpers are here to do the heavy lifting. You can manage them in the Helpers section.".to_string(), "Meet Your AI Team")
                            } else if input_lower.contains("instagram") || input_lower.contains("marketing") || input_lower.contains("grow") {
                                ("Use the 'Grow Business' tool to connect Instagram and run marketing campaigns.".to_string(), "Promoting Your Business")
                            } else {
                                (format!("I found some information about '{}'. You can read more in our Help Center.", input), "Welcome to OneHumanCorp!")
                            };

                            msgs.push(app::ChatMessage {
                                sender: "AI".into(),
                                text: response_text.into(),
                                article_link: article_link.into(),
                            });
        ...
        tokio::spawn(async move {
            if let Ok(mut client) = OrgServiceClient::connect(std::env::var("OHC_HUB_URL").unwrap_or_else(|_| "http://127.0.0.1:18789".to_string())).await {
                ...
                if let Ok(resp) = resp {
                    ...
                    let total_helpers = analytics.total_agents;
                    slint::invoke_from_event_loop(move || {
...
#[test]
    ...
    fn test_e2e_soft_paywall_flow() {
        ...
        // Test helpers limit soft paywall
        let helpers_ui = app::Helpers::new().unwrap();
        let helpers_ui_handle = helpers_ui.as_weak();
        helpers_ui.on_hire_helper(move || {
...
mod remaining_e2e_tests {
    ...
    fn test_e2e_progressive_disclosure_flow() {
        ...
        let helper_config = app::HelperConfig::new().unwrap();
        helper_config.set_is_advanced(false);
        let ac_weak = helper_config.as_weak();
        add_advanced_listener(Box::new(move |val| {
        ...
        assert_eq!(helper_config.get_is_advanced(), true);
        ...
        assert_eq!(setup_wizard.get_is_advanced(), false);
        assert_eq!(helper_config.get_is_advanced(), false);
    }
