...
#[cfg(not(target_arch = "wasm32"))]
thread_local! {
    pub static GLOBAL_SOCIAL_POSTING: std::cell::RefCell<Option<slint::Weak<app::SocialPosting>>> = std::cell::RefCell::new(None);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    social_posting_ui.on_connect_instagram({
        let handle = social_posting_handle.clone();
        move || {
    ...
    social_posting_ui.on_generate_post({
        ...
        move || {
            if let Some(ui) = handle.upgrade() {
                ui.set_post_content("Check out our new products! 🚀 #newarrival".into());
            }
        }
    });
    ...
    grow_business_ui.on_execute({
        move |strategy, _kpi| {
            if strategy == "Run your first email campaign" {
                ...
                GLOBAL_SOCIAL_POSTING.with(|global| {
                    if let Some(weak) = global.borrow().as_ref() {
...
mod docs_tests {
    ...
    fn test_e2e_tooltip_flow() {
        ...
        assert_eq!(
            tr.get_active_text(),
            "Ask the AI assistant for help or to perform tasks."
        );
