...
thread_local! {
    ...
    static GLOBAL_POS: RefCell<Option<slint::Weak<app::Pos>>> = RefCell::new(None);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    if let Ok(dashboard) = app::Dashboard::new() {
                        ...
                        dashboard
                            ...
                            .on_request_tooltip_text(|id| {
                                static TOOLTIPS: std::sync::OnceLock<
                                    std::collections::HashMap<String, String>,
                                > = std::sync::OnceLock::new();
                                let tooltips = TOOLTIPS.get_or_init(|| {
                                    serde_json::from_str(include_str!("tooltips.json"))
                                        .unwrap_or_default()
                                });
                                tooltips
                                    .get(id.as_str())
    ...
    integrations_ui.on_configure_integration(|id| {
        ...
        tokio::spawn(async move {
            println!("Configure integration requested for: {}", id_clone);
        });
    });
    integrations_ui.on_invoke_tool(|id| {
        let id_clone = id.to_string();
        tokio::spawn(async move {
            println!("Invoke tool requested for: {}", id_clone);
        });
    });
    ...
    my_plan_ui.on_download_invoice(move || {
        // Handle download invoice
            ...
            if let Ok(dashboard) = app::Dashboard::new() {
                ...
                let pos_ui = app::Pos::new().unwrap();
                GLOBAL_POS.with(|g| *g.borrow_mut() = Some(pos_ui.as_weak()));

                let pos_handle = pos_ui.as_weak();
                pos_ui.on_close({
                    let pos_h = pos_handle.clone();
                    move || {
                        if let Some(ui) = pos_h.upgrade() {
                            ui.hide().unwrap();
                        }
                    }
                });

                pos_ui.on_add_custom_amount({
                    let pos_h = pos_handle.clone();
                    move |amount| {
                        if let Some(ui) = pos_h.upgrade() {
                            let current = ui.get_cart_total().parse::<f64>().unwrap_or(0.0);
                            let added = amount.parse::<f64>().unwrap_or(0.0);
                            ui.set_cart_total(format!("{:.2}", current + added).into());
                        }
                    }
                });

                pos_ui.on_tap_to_pay({
                    let pos_h = pos_handle.clone();
                    move || {
                        if let Some(ui) = pos_h.upgrade() {
                            ui.set_step(1); // Move to processing

                            let ui_weak = pos_h.clone();
                            let payment_amount = ui.get_cart_total().to_string();
                            ...
                            tokio::spawn(async move {
                                ...
                                // Emit OfflinePaymentCompleted to KAIROS mesh
                                if let Ok(mut client) = HubServiceClient::connect(
                                    std::env::var("OHC_HUB_URL")
                                        .unwrap_or_else(|_| "http://127.0.0.1:18789".to_string()),
                                )
                                .await
                                {
                                    let mut req = tonic::Request::new(ohc::orchestration::PublishTeammateMeshEventRequest {
                                        ...
                                        event: Some(ohc::orchestration::TeammateMeshEvent {
                                            agent_id: "pos_terminal".into(),
                                            action: "OfflinePaymentCompleted".into(),
                                            status: "success".into(),
                                            payload: format!("{{\"amount\": \"{}\"}}", payment_amount).into_bytes(),
                                            msg_id: format!("msg-{}", chrono::Utc::now().timestamp_millis()),
                                        }),
                                    });
                                    if let Ok(token) = std::env::var("OHC_TOKEN") {
                                        req.metadata_mut().insert("authorization", format!("Bearer {}", token).parse().unwrap());
                ...
                pos_ui.on_cancel_payment({
                    let pos_h = pos_handle.clone();
                    move || {
                        if let Some(ui) = pos_h.upgrade() {
                            ui.set_step(0); // Move back to cart
                        }
                    }
                });
                ...
                let pos_opened_called = std::rc::Rc::new(std::cell::RefCell::new(false));
                let pos_opened_called_clone = pos_opened_called.clone();
                dashboard.on_action_open_pos(move || {
                    *pos_opened_called_clone.borrow_mut() = true;
                    // Logic to open POS window goes here (in real app)
                    GLOBAL_POS.with(|global| {
                        if let Some(weak) = global.borrow().as_ref() {
                            if let Some(ui) = weak.upgrade() {
                                ui.show().unwrap();
                            }
    ...
    setup_wizard_ui.on_copy_link(|link| {
        CLIPBOARD.with(|cb| {
            if let Some(ctx) = cb.borrow_mut().as_mut() {
                ...
                println!(
                    "Clipboard context not available, fallback to console: {}",
                    link
                );
            }
        });
...
mod docs_tests {
        ...
        dashboard_ui
            ...
            .on_request_tooltip_text(|id| {
                static TOOLTIPS: std::sync::OnceLock<std::collections::HashMap<String, String>> =
                    std::sync::OnceLock::new();
                let tooltips = TOOLTIPS.get_or_init(|| {
                    serde_json::from_str(include_str!("tooltips.json")).unwrap_or_default()
                });
                tooltips
                    .get(id.as_str())
                    .cloned()
    ...
    fn test_e2e_agent_hire_flow() {
        ...
        assert!(
            *agent_hire_opened.borrow(),
            "Agent Hire should be opened from Agents screen"
        );
...
mod remaining_e2e_tests {
    ...
    fn test_e2e_free_tier_limits_flow() {
        ...
        dashboard_ui.on_action_add_product(move || {
            if let Some(ui) = dashboard_handle_add_product.upgrade() {
                ui.set_upgrade_prompt_message(
                    "You've reached your free tier limit of 10 products. Upgrade to add more!"
                        .into(),
                );
                ui.set_show_upgrade_prompt(true);
            }
        ...
        agents_ui.on_hire_agent(move || {
            if let Some(ui) = agents_ui_handle.upgrade() {
                ui.set_upgrade_prompt_message(
                    "You've reached your free tier limit of 1 agent. Upgrade to unlock more power!"
                        .into(),
                );
                ui.set_show_upgrade_prompt(true);
            }
...
mod e2e_login_to_dashboard_tests {
    ...
    fn test_e2e_login_flow_and_dashboard_simplification() {
        ...
        assert_eq!(
            ...
            1,
            "Needs Your Approval section should contain items"
        );
    }
