    ...
    my_plan_ui.on_download_invoice(move || {
        // Handle download invoice
            ...
            if let Ok(dashboard) = app::Dashboard::new() {
                ...
                help_center_ui.global::<app::TooltipRegistry>().on_request_tooltip_text(|id| {
                    let tooltips: std::collections::HashMap<String, String> = serde_json::from_str(include_str!("tooltips.json")).unwrap_or_default();
                    tooltips.get(id.as_str()).cloned().unwrap_or_default().into()
                });
    ...
    setup_wizard_ui.on_launch({
        ...
        move |business_type, company_name, company_description, payment_pref, admin_email, _website_template, _product_name, _product_price, _domain_choice| {
            ...
            let state = std::collections::HashMap::from([
                ...
                ("website_template".to_string(), _website_template.to_string()),
                ("product_name".to_string(), _product_name.to_string()),
                ("product_price".to_string(), _product_price.to_string()),
                ("domain_choice".to_string(), _domain_choice.to_string()),
            ...
            let req_website_template = _website_template.to_string();
            let req_first_product_name = _product_name.to_string();
            let req_first_product_price = _product_price.to_string();
            let req_domain_choice = _domain_choice.to_string();
