...
impl TaskWorker {
            ...
            let handle = tokio::spawn(async move {
                loop {
                    ...
                    if let Some(issue) = issue {
                        ...
                        Self::process_issue_internal(issue, plane_client.clone(), hub.clone())
                            .await;
                    } else {
    ...
    async fn process_issue_internal(issue: Issue, plane_client: Arc<PlaneClient>, hub: Arc<Hub>) {
        debug!(
            "agent task worker: processing issue: {}, title: {}",
            issue.id, issue.name
        );

        if let Err(e) = plane_client
            .update_issue_status(&issue.id, "in_progress")
            .await
        {
        ...
        for a in agents {
            if a.status == "ACTIVE" || a.status == "WAITING_FOR_TOOLS" {
                ...
                debug!(
                    "agent task worker: issue marked in_progress, delegating to agent: {}",
                    a.id
                );

                if a.provider_type == "builtin" || a.provider_type.is_empty() {
                    debug!(
                        "agent task worker: dispatching to builtin Rust agent via gRPC: {}",
                        a.id
                    );
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::dispatch_to_builtin_agent(
                            &payload_str,
                            &format!("plane issue {}", issue_id),
                            &role,
                        )
                        .await
                        {
                            debug!(
                                "builtin agent dispatch error: {}, agent_id: {}",
                                e, agent_id
                            );
                        }
                    });
    ...
    async fn dispatch_to_builtin_agent(
        payload: &str,
        description: &str,
        role: &str,
    ) -> Result<(), String> {
        let address =
            std::env::var("OHC_AGENT_ADDRESS").unwrap_or_else(|_| "127.0.0.1:50051".to_string());

        let mut attempt = 0;
        let max_attempts = 3;

        // ML-Resilience: Circuit Breaker with Half-Open state
        static CONSECUTIVE_FAILURES: std::sync::atomic::AtomicUsize =
            std::sync::atomic::AtomicUsize::new(0);
        static LAST_FAILURE_TIME: std::sync::atomic::AtomicU64 =
        ...
        loop {
            ...
            match result {
                Ok(Ok(last_content)) => {
                    debug!(
                        "builtin agent task completed: {}, result_len: {}",
                        description,
                        last_content.len()
                    );
                ...
                Ok(Err(e)) => {
                    ...
                    if e.contains("exceeded")
                        || e.contains("connect to builtin")
                ...
                Err(_) => {
                    error!(
                        "builtin agent task timed out (ML-Resilience 60s) on attempt {}",
                        attempt
                    );
                    CONSECUTIVE_FAILURES.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                    LAST_FAILURE_TIME.store(
                    ...
                    if attempt >= max_attempts {
                        return Err(
                            "Timeout executing agent job (ML-Resilience 60s boundary)".to_string()
                        );
                    }
                }
            }
