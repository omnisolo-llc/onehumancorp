...
pub struct DomainExpert {
    pub role: String,
    pub llm: Arc<dyn ExpertTeamLlmClient>,
}
        ...
        for (output_res, skills) in results {
            match output_res {
                Ok(output) => {
                    // Merge skills back
                    for skill in skills {
                        trace.record_skill(&skill.to_string());
                    }
...
impl QualityGates {
    ...
    /// Ensures there are enough experts and basic context is present.
    pub fn pre_flight(manager: &ExpertTeamManager, task: &str) -> Result<(), String> {
        if manager.domain_experts.len() < 6 {
            return Err("Pre-flight Gate Failed: At least 6 domain experts are required for the team.".to_string());
        }
    ...
    pub fn pre_merge(summaries: &[String]) -> Result<(), String> {
        ...
        // Check for 75% similarity deduplication
        for i in 0..summaries.len() {
            let words_i: std::collections::HashSet<&str> = summaries[i].split_whitespace().collect();
            for j in (i + 1)..summaries.len() {
                let words_j: std::collections::HashSet<&str> = summaries[j].split_whitespace().collect();
                let intersection: std::collections::HashSet<_> = words_i.intersection(&words_j).cloned().collect();
                let union: std::collections::HashSet<_> = words_i.union(&words_j).cloned().collect();
        ...
        // Check for 8-chapter completeness
        let combined = summaries.join("\n");
        for chapter in 1..=8 {
            let chapter_str = format!("Chapter {}", chapter);
            if !combined.contains(&chapter_str) {
                return Err(format!("Pre-merge Gate Failed: Outputs do not meet the minimum completeness criteria (missing {}).", chapter_str));
            }
        }
    ...
    pub fn pre_deliver(final_output: &str, trace: &SkillTrace) -> Result<(), String> {
        ...
        // Enforce word count: >=20,000 words
...
mod tests {
    ...
    async fn test_expert_team_successful_execution() {
        ...
        let experts = vec![
            DomainExpert { role: "Industry Researcher".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Chapter 1 Chapter 2 {} uniqueA", long_text) }) },
            DomainExpert { role: "Financial Analyst".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Chapter 3 Chapter 4 {} uniqueB", long_text) }) },
            DomainExpert { role: "Strategic Analyst".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Chapter 5 Chapter 6 {} uniqueC", long_text) }) },
            DomainExpert { role: "Operations Expert".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Chapter 7 Chapter 8 {} uniqueD", long_text) }) },
            DomainExpert { role: "Marketing Expert".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Some marketing info {} uniqueE", long_text) }) },
            DomainExpert { role: "Legal Expert".to_string(), llm: Arc::new(MockExpertLlm { role_resp: format!("Some legal info {} uniqueF", long_text) }) },
        ];
    ...
    fn test_pre_flight_failure_not_enough_experts() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().contains("At least 6 domain experts are required"));
    }

    #[test]
    fn test_pre_merge_failure_high_similarity() {
        let mut words1 = String::new();
        let mut words2 = String::new();
        for i in 0..100 {
            words1.push_str(&format!("unique_word_{} ", i));
            words2.push_str(&format!("unique_word_{} ", i));
        }
        words1.push_str(" extra_a ");
        words2.push_str(" extra_b extra_c extra_d ");
    ...
    fn test_pre_deliver_failure_missing_chart() {
        let final_output = format!("This output is quite long so it passes the word count check. It is very detailed and thorough, however it is missing something important. {}", "word ".repeat(20000));
        let mut trace = SkillTrace::new();
    ...
    fn test_pre_deliver_failure_missing_skill_trace() {
        let final_output = format!("This output is quite long so it passes the word count check. Chart: Provided. Analysis: Provided. {}", "word ".repeat(20000));
