...
impl QualityGates {
    ...
    pub fn pre_flight(manager: &ExpertTeamManager, task: &str) -> Result<(), String> {
        if manager.domain_experts.len() < 2 {
            return Err("Pre-flight Gate Failed: At least 2 domain experts are required for the team.".to_string());
        }
    ...
    pub fn pre_deliver(final_output: &str, trace: &SkillTrace) -> Result<(), String> {
        ...
        // Enforce word count (Simulated with a smaller threshold for practicality)
        let word_count = final_output.split_whitespace().count();
        if word_count < 20 {
            return Err(format!("Pre-deliver Gate Failed: Final output is too short ({} words). Required >= 20 words for delivery.", word_count));
        }
...
mod tests {
    ...
    fn test_pre_flight_failure_not_enough_experts() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().contains("At least 2 domain experts are required"));
    }
