...
impl QualityGates {
    ...
    /// Mechanics: Tencent Workbuddy (Expert Team) Feature
    /// "Pre-flight (6 agent initialization)" -> Lead + 5 Domain Experts
    pub fn pre_flight(manager: &ExpertTeamManager, task: &str) -> Result<(), String> {
        // We enforce exactly 5 domain experts (plus the 1 lead agent, meaning 6 total agents initialization)
        if manager.domain_experts.len() != 5 {
            return Err(format!("Pre-flight Gate Failed: Exactly 5 domain experts (for a 6-agent team including the lead) are required. Found {}.", manager.domain_experts.len()));
        }
    ...
    pub fn pre_merge(summaries: &[String]) -> Result<(), String> {
        ...
        // Check for similarity (75% similarity deduplication). For now, we simulate this strictly
        // by looking for identical summaries, or significantly overlapping content.
        ...
        // We require 8 distinct chapters, denoted by "Chapter 1", "Chapter 2", etc.
        for chapter_num in 1..=8 {
            let chapter_marker = format!("Chapter {}", chapter_num);
            if !combined.contains(&chapter_marker) {
                return Err(format!("Pre-merge Gate Failed: Outputs do not meet the minimum completeness criteria. Missing '{}'.", chapter_marker));
            }
        }
    ...
    pub fn pre_deliver(final_output: &str, trace: &SkillTrace) -> Result<(), String> {
        ...
        // Enforce exact word count >= 20,000 words for true executability
        let word_count = final_output.split_whitespace().count();
        if word_count < 20_000 {
            return Err(format!("Pre-deliver Gate Failed: Final output is too short ({} words). Required >= 20,000 words for delivery.", word_count));
        }
...
mod tests {
    ...
    async fn test_expert_team_successful_execution() {
        let experts = vec![
            DomainExpert { role: "Industry Researcher".to_string(), llm: Arc::new(MockExpertLlm { role_resp: "Chapter 1 Chapter 2".to_string() }) },
            DomainExpert { role: "Financial Analyst".to_string(), llm: Arc::new(MockExpertLlm { role_resp: "Chapter 3 Chapter 4".to_string() }) },
            DomainExpert { role: "Strategic Analyst".to_string(), llm: Arc::new(MockExpertLlm { role_resp: "Chapter 5".to_string() }) },
            DomainExpert { role: "Process Supervisor".to_string(), llm: Arc::new(MockExpertLlm { role_resp: "Chapter 6".to_string() }) },
            DomainExpert { role: "Quality Auditor".to_string(), llm: Arc::new(MockExpertLlm { role_resp: "Chapter 7 Chapter 8".to_string() }) },
        ];
        ...
        // Need to hit the 20,000 word limit
        let padding = " word ".repeat(20_000);
        final_output.push_str(&padding);
    ...
    fn test_pre_flight_failure_not_enough_experts() {
        ...
        assert!(res.is_err());
        assert!(res.unwrap_err().contains("Exactly 5 domain experts"));
    }
    ...
    fn test_pre_deliver_failure_missing_chart() {
        let mut final_output = "This output is quite long so it passes the word count check. It is very detailed and thorough, however it is missing something important.".to_string();
        final_output.push_str(&" word ".repeat(20_000));
        let mut trace = SkillTrace::new();
