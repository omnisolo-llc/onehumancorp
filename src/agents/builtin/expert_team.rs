        ...
        for (output_res, skills) in results {
            match output_res {
                Ok(output) => {
                    ...
                    let condensed = if output_str.len() > max_chars {
                        let mut iter = output_str.char_indices();
                        let max_byte_index =
                            iter.nth(max_chars).map_or(output_str.len(), |(i, _)| i);
                        format!("{}... [Condensed for Harness: 1k-2k tokens limit reached]", &output_str[..max_byte_index])
                    } else {
