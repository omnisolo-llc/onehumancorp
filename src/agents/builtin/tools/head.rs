...
mod tests {
    ...
    async fn test_head_path_traversal() {
        ...
        let executor = HeadExecutor { working_dir: None }; let executor = HeadExecutor { working_dir: None }; let args = json!({ "path": "../../../etc/passwd" });
    ...
    async fn test_head_jit_limit() {
        ...
        let dir = tempfile::tempdir().unwrap(); let file_path = dir.path().join("test.txt"); tokio::fs::write(&file_path, "line1\n").await.unwrap(); let executor = HeadExecutor { working_dir: Some(dir.path().to_path_buf()) }; let args = json!({ "path": "test.txt", "lines": 2000 });
        let result = executor.execute(args).await;
        assert!(result.is_err());
        if let Err(ToolError::LlmRecoverable(msg)) = result {
