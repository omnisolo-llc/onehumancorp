...
impl SandboxedCommandRunner {
    ...
    fn container_args(
        ...
        match runtime {
            "ssh" => {
                let target = std::env::var("OMNISOLO_AGENT_SSH_TARGET")
            ...
            _ => {
                ...
                let mut docker_args = vec![
                    ...
                    "--rm".to_string(),
                    "--network".to_string(),
                    std::env::var("OMNISOLO_AGENT_CONTAINER_NETWORK")
    ...
    async fn run(
        ...
        if Self::should_use_container_backend(&backend_env, &execution_mode) {
            ...
            let runtime_opt = RUNTIME
                .get_or_init(|| {
                    let current_backend = std::env::var("OMNISOLO_AGENT_COMMAND_BACKEND")
                        .unwrap_or_default()
                        .to_lowercase();
