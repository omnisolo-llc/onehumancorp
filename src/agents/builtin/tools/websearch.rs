...
impl ToolExecutor for WebSearchExecutor {
    async fn execute(
        ...
        let query = args["query"]
            .as_str()
            .ok_or_else(|| crate::types::ToolError::LlmRecoverable("websearch: query is required".to_string()))?;
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::types::ToolError::LlmRecoverable(format!("websearch: {}", e)))?;
