...
impl ToolExecutor for TopicWriteExecutor {
    async fn execute(&self, args: Value) -> Result<String, ToolError> {
        ...
        self.accessor
            ...
            .await
            .map(|_| format!("Successfully wrote topic: {}", topic_name))
            .map_err(|e| ToolError::LlmRecoverable(e.to_string()))
    }
}
...
pub fn topic_write_tool(accessor: Arc<dyn MemoryAccessor>) -> Tool {
    Tool {
        ...
        description: "Create or update a detailed topic file in long-term memory for future retrieval.".to_string(),
        is_read_only: false,
        parameters: json!({
            "type": "object",
            "properties": {
                "topic_name": {
                    "type": "string",
                    "description": "The exact name of the topic to create or update."
                },
                "content": {
                    "type": "string",
