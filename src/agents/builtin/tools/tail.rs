...
impl ToolExecutor for TailExecutor {
    async fn execute(&self, args: Value) -> Result<String, ToolError> {
        let path = args["path"]
            .as_str()
            .ok_or_else(|| ToolError::LlmRecoverable("tail: path is required".to_string()))?;

        // Basic path sanitization: disallow relative path traversal
        if path.contains("..") {
            return Err(ToolError::LlmRecoverable(
                "tail: path traversal via '..' is not allowed".to_string(),
            ));
        }
        ...
        while current_pos > 0 && num_lines_found <= lines_to_read {
            ...
            file.seek(std::io::SeekFrom::Start(current_pos as u64))
                .await
                .map_err(|e| ToolError::LlmRecoverable(e.to_string()))?;
            let bytes_read = file
                .read(&mut buffer[0..read_size as usize])
                .await
                .map_err(|e| ToolError::LlmRecoverable(e.to_string()))?;
            ...
            for (i, &b) in chunk.iter().enumerate().rev() {
                if b == b'\n' {
                    ...
                    if num_lines_found == lines_to_read {
                        ...
                        file.seek(std::io::SeekFrom::Start(final_start))
                            .await
                            .map_err(|e| ToolError::LlmRecoverable(e.to_string()))?;
