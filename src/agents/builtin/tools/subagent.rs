...
impl ToolExecutor for SubagentExecutor {
    async fn execute(&self, args: Value) -> Result<String, ToolError> {
        ...
        if mode == "worktree" {
            ...
            let mut req = SubAgentRequest::default();
            req.task = task.to_string();
            if let Some(tid) = &self.source_thread_id {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_subagent_fork_mode_no_grpc_server() {
        // In unit test environment, we don't have a grpc server running,
        // so we expect a connection error, but we want to verify it goes down the fork path.
        let executor = SubagentExecutor { source_thread_id: Some("test_thread_123".to_string()) };
        let args = json!({
            "task": "do something",
            "mode": "fork"
        });
        ...
        match result {
            ...
            _ => panic!("Expected LlmRecoverable error for connection failure"),
        }
    }
}
