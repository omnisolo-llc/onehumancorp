...
impl SubagentExecutor {
    async fn summarize_output(
        ...
        if raw_output.len() <= TARGET_CHARS_MAX {
            if raw_output.len() > 1000 {
                let req = ohc_builtin_agent_core::types::ChatRequest {
                    ...
                    messages: vec![ohc_builtin_agent_core::types::Message::user(
                ...
                let resp = if let Some(l) = &self.llm {
                    l.chat(req).await?
                } else {
                    return Err("LLM client not available for condensation".into());
                };
        ...
        if final_text.len() > TARGET_CHARS_MAX {
            tracing::warn!(
                "Subagent condensation failed to reduce text below target size. Truncating."
            );
            final_text = format!(
            ...
            match res {
                Ok(inner) => {
                    if !inner.error.is_empty() {
                        ...
                        let summary =
                            self.summarize_output(&inner.result)
                                .await
                                .unwrap_or_else(|e| {
                                    format!(
                                        "Failed to summarize: {}
...
When finished or if you need to report progress, write your final summary to {}. To receive further instructions, read from {}.",
            ...
            tokio::spawn(async move {
                let output = runner_clone
                    .run(
                        "ohc_builtin_agent",
                        &["--task", &teammate_task, "--mailbox", &mailbox_dir_clone],
                        None,
                        envs,
                    )
                    .await;
...
Final Result: {}",
            ...
            let git_status = self
                .runner
                .run("git", &["status", "--porcelain"], None, vec![])
                .await;
            ...
            match add_worktree {
                ...
                Err(e) => {
                    return Err(ToolError::LlmRecoverable(format!(
                        "Command runner failed on git worktree: {}",
                        e
                    )));
                }
            }
            ...
            let output = self
                ...
                .run(
                    "ohc_builtin_agent",
                    &["--task", &task],
                    Some(std::path::Path::new(&worktree_dir)),
                    envs,
                )
                .await;
                    ...
                    if !inner.error.is_empty() {
                        ...
                        let summary =
                            self.summarize_output(&inner.result)
                                .await
                                .unwrap_or_else(|e| {
                                    format!("Failed to summarize: {}\n\n{}", e, inner.result)
                                });
                        Ok(format!(
                            "[Subagent (Worktree)] Completed task: {}. Summary: {}\nBranch: {}",
                            task, summary, branch_name
                        ))
                    }
                }
                Err(e) => Err(ToolError::LlmRecoverable(format!(
                    "Subagent failed in worktree: {}",
                    e
                ))),
            }
        } else {
...
pub fn subagent_tool(
    ...
    llm: Option<Arc<dyn ohc_builtin_agent_core::expert_team::ExpertTeamLlmClient>>,
) -> Tool {
    Tool {
...
mod tests {
    ...
    async fn test_subagent_fork_mode() {
        ...
        assert!(
            msg.contains("[Subagent (Fork)] Completed task: Do this fork task"),
            "msg was: {}",
            msg
        );
        assert!(
            msg.contains("I completed the fork task"),
            "Message should contain the agent output"
        );
    }
    ...
    async fn test_subagent_worktree_mode() {
        ...
        assert!(
            msg.contains("I completed the worktree task"),
            "Message should contain the agent output"
        );
    }
    ...
    async fn test_subagent_output_truncation() {
        ...
        let msg = result.unwrap();
        assert!(
            msg.contains("[Output truncated. Subagent failed to condense summary.]"),
            "Expected output to be truncated"
        );
        assert!(
            msg.len() < 9000,
            "Expected output length to be less than 9000 after truncation"
        );
    }
}
