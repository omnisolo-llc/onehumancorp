...
use omnisolo_builtin_agent_core::expert_team::{
...
impl PydanticToolExecutor<ExpertTeamArgs> for ExpertTeamExecutor {
    async fn execute_typed(&self, args: ExpertTeamArgs) -> Result<String, ToolError> {
        ...
        let expert_roles = args.expert_roles.unwrap_or_else(|| {
            vec![
        ...
        match manager
            .run_full_expert_workflow(task, &mut trace, bridge)
            .await
        {
            Ok(report) => Ok(report),
            Err(e) => Err(ToolError::LlmRecoverable(format!(
                "Expert Team Workflow failed at quality gate: {}",
                e
            ))),
        }
    }
        ...
        async fn chat(
            ...
            if !resps.is_empty() {
                ...
                Ok(ChatResponse {
                    message: Message {
                        role: Role::Assistant,
                        content: "Default expert response with more than 20000 words. "
                            .repeat(4000), // Simulate a long enough report to pass the 20000 words quality gate
                        tool_calls: vec![],
                        tool_results: vec![],
                        response_id: None,
