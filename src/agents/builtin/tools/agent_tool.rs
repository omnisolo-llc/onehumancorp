...
impl ToolExecutor for AgentExecutor {
    async fn execute(
        ...
        let prompt = args["prompt"]
            .as_str()
            .ok_or_else(|| crate::types::ToolError::LlmRecoverable("agent: prompt is required".to_string()))?;
...
impl ToolExecutor for TaskStopExecutor {
    async fn execute(
        ...
        let task_id = args["task_id"]
            .as_str()
            .ok_or_else(|| crate::types::ToolError::LlmRecoverable("taskstop: task_id is required".to_string()))?;
...
impl ToolExecutor for TaskStatusExecutor {
    async fn execute(
        ...
        let task_id = args["task_id"]
            .as_str()
            .ok_or_else(|| crate::types::ToolError::LlmRecoverable("taskstatus: task_id is required".to_string()))?;
        Ok(json!({
            "task_id": task_id,
            "status": "running",
