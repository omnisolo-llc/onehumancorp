...
impl ToolExecutor for OllamaExecutor {
    async fn execute(&self, args: Value) -> Result<String, ToolError> {
        let action = args["action"]
            .as_str()
            .ok_or_else(|| ToolError::LlmRecoverable("ollama: action is required".to_string()))?;
        ...
        match action {
            ...
            "pull_model" => {
                let model_name = args["model_name"].as_str().ok_or_else(|| {
                    ToolError::LlmRecoverable("ollama: model_name is required for pull".to_string())
                })?;
            ...
            "check_health" => {
                let model_name = args["model_name"].as_str().ok_or_else(|| {
                    ToolError::LlmRecoverable(
                        "ollama: model_name is required for health check".to_string(),
                    )
                })?;
