...
impl ToolExecutor for AiderRepoMapExecutor {
    async fn execute(&self, args: Value) -> Result<String, ToolError> {
        ...
        let map = tokio::task::spawn_blocking(move || AiderRepoMapExecutor::generate_map_recursive(abs_target.clone(), "".to_string()))
            .await
...
pub fn aider_repomap_tool(workspace_path: PathBuf) -> Tool {
    Tool {
        name: "AiderRepoMap".to_string(),
        ...
        execute: Arc::new(AiderRepoMapExecutor::new(workspace_path)),
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_aider_repomap_generation() {
        ...
        let executor = AiderRepoMapExecutor::new(root.to_path_buf());
