...
impl AgentServiceImpl {
    ...
    pub async fn run_ralph_loop(&self, req: RunTaskRequest) {
        ...
        let mut unarc_agent = Agent::new(llm, vec![]);
        let context_manager = unarc_agent.context_manager.clone();
        let tools = ohc_builtin_agent_tools::all_tools(todos, task_store, mailbox, None, None, context_manager);
        unarc_agent.tools = tools;
...
impl AgentService for AgentServiceImpl {
    ...
    async fn run_task(
        ...
        let all_tools = ohc_builtin_agent_tools::all_tools(todos, task_store, mailbox, None, accessor, context_manager);
    ...
    async fn dispatch_to_sub_agent(
        ...
        if sub_req.sub_agent_address.is_empty() {
            ...
            let tools = ohc_builtin_agent_tools::all_tools(todos, task_store, mailbox, working_dir, None, context_manager);
            agent.tools = tools;
