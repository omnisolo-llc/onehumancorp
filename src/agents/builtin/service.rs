...
impl AgentServiceImpl {
    ...
    pub async fn run_ralph_loop(&self, req: RunTaskRequest) {
        ...
        let tools = ohc_builtin_agent_tools::all_tools(todos.clone(), task_store.clone(), mailbox.clone(), None, None, run_cfg.thread_id.clone());
...
impl AgentService for AgentServiceImpl {
    ...
    async fn run_task(
        ...
        let all_tools = ohc_builtin_agent_tools::all_tools(todos.clone(), task_store.clone(), mailbox.clone(), None, accessor, run_cfg.thread_id.clone());
    ...
    async fn dispatch_to_sub_agent(
        ...
        if sub_req.sub_agent_address.is_empty() {
            ...
            approved_tool_calls: vec![],
                ...
                workspace_path: None,
                thread_id: Some(uuid::Uuid::new_v4().to_string()),
                source_thread_id: if sub_req.source_thread_id.is_empty() { None } else { Some(sub_req.source_thread_id.clone()) },
                resume_from_checkpoint_id: None,
            };
            ...
            let tools = ohc_builtin_agent_tools::all_tools(todos.clone(), task_store.clone(), mailbox.clone(), working_dir, None, run_cfg.thread_id.clone());
