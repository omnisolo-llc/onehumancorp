...
mod tests {
    ...
    async fn test_4_type_error_handling() {
        let e_transient = ToolError::Transient("timeout".to_string());
        let e_recoverable = ToolError::LlmRecoverable("missing arg".to_string());
        let e_user = ToolError::UserFixable("need input".to_string());
        let e_fatal = ToolError::Fatal("crash".to_string());
        let e_unexpected = ToolError::Unexpected("unknown".to_string());
    ...
    use super::*;
use ohc_builtin_agent_core::types::{ToolError, ChatRequest, Message, Role, ToolCall, ToolDefinition, ToolResult, Usage, ChatResponse};




use crate::agent::core::{build_hierarchical_system_prompt, load_cascading_agents_md, Agent};


use crate::agent::config::{AgentRunConfig, AgentProgress};
