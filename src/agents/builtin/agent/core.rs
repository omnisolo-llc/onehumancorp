...
impl Agent {
    ...
    {
        crate::agent::anthropic_loop::run_anthropic_dumb_loop(self, cfg, initial_message, session_tools, on_event).await
    }
    ...
    where
        F: FnMut(AgentEvent) + Send + Sync,
    {
        crate::agent::langgraph_loop::run_langgraph(self, cfg, initial_message, session_tools, initial_messages, on_event).await
    }
    ...
    where
        F: FnMut(AgentEvent) + Send + Sync,
    {
        crate::agent::plan_and_execute_loop::run_plan_and_execute(self, cfg, initial_message, session_tools, on_event).await
    }
    ...
    where
        F: FnMut(AgentEvent) + Send + Sync,
    {
        crate::agent::structured_loop::run_structured(self, cfg, initial_message, schema, on_event).await
    }
...
pub fn validate_schema(args: &serde_json::Value, schema: &serde_json::Value) -> Result<(), String> {
