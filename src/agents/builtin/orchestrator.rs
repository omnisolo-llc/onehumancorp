use std::sync::Arc;
use crate::agent::{Agent, AgentEvent, AgentRunConfig, load_cascading_agents_md};
...
impl<'a> AgentOrchestrator<'a> {
    ...
    {
        let mut self_with_memory = self.agent;
        let owned_agent;
        if let Some(ltm) = &cfg.long_term_memory {
            owned_agent = Agent {
                llm: self.agent.llm.clone(),
                tools: self.agent.tools.clone(),
                progress: self.agent.progress.clone(),
                memory_store: Some(ltm.clone()),
                checkpointer: self.agent.checkpointer.clone(),
                observation_store: self.agent.observation_store.clone(),
            };
        ...
        if final_cfg.enable_llmcompiler_plan_and_execute {
            return self.agent.run_plan_and_execute(&final_cfg, initial_message, &session_tools, on_event).await;
        }
        let mut session_tools = self.agent.tools.clone();
        ...
        if let (Some(checkpointer), Some(thread_id)) = (&self.agent.checkpointer, &final_cfg.thread_id) {
        ...
        let mut combined_system = crate::agent::build_hierarchical_system_prompt(&final_cfg, &session_tools);
        ...
        while turn_count < max_iterations {
            ...
            for t in &session_tools {
                ...
                {
                    req_tools.push(ohc_builtin_agent_core::types::ToolDefinition {
                        name: t.name.clone(),
            ...
            self.agent.progress.add_tokens(total_tokens);
            ...
            let orchestration_loop = OrchestrationLoop::new(self.agent, &final_cfg);
            let term_res = orchestration_loop.check_termination_conditions(
                turn_count,
                stop_reason,
            ...
            match term_res {
                ...
                Ok(Some(msg)) => {
                    if resp.message.tool_calls.is_empty() {
                        // Output Guardrail for completion
                        if let Some(guard_cfg) = &final_cfg.guardrails {
                            if let Err(e) = crate::guardrails::check_output(&msg, guard_cfg) {
                                on_event(AgentEvent::TaskError { error: e.clone() });
                                return Err(e.into());
                            }
                        }
                        ...
                        if final_cfg.enable_llm_judge {
                            ...
                            match self.agent.llm.chat(judge_req).await {
                                Ok(judge_resp) => {
                        ...
                        if !msg.is_empty() && msg != "I've reached my token budget for this task. Please upgrade your plan to unlock longer interactions!" {
                            on_event(AgentEvent::TaskComplete { content: msg.clone() });
                        }
                    }
                    return Ok(msg);
                ...
                Ok(None) => {
                    // Continue loop
                    // Re-evaluate budget nudge manually here if it was a continue
                    if stop_reason == "max_tokens" || stop_reason == "length" {
                        let decision = check_token_budget(&mut budget_tracker, final_cfg.max_task_tokens, global_turn_tokens);
...
{}", last_assistant_content))],
            ...
            for tc in &tool_calls {
                let is_read_only = self.agent.tools.iter().find(|t| t.name == tc.name).map(|t| t.is_read_only).unwrap_or(false);
                if is_read_only {
            ...
            for tc in &read_only_calls {
                ...
                let gating_res = crate::agent::Agent::check_tool_gating(tc, true, &final_cfg);
                ...
                read_only_futures.push(async move {
                    ...
                    loop {
                        match self.agent.execute_tool(&tc_clone, &session_tools_clone, &messages_clone).await {
                            Ok(r) => {
            ...
            for (tc, res) in ro_results {
                ...
                match res {
                    Ok(r) => {
                        ...
                        self.agent.progress.record_tool_use();
                        self.agent.observation_store.insert(tc.id.clone(), r.clone());
                        on_event(AgentEvent::ToolCall {
                            name: tc.name.clone(),
                            args_json: tc.arguments.to_string(),
                            result: r.clone(),
                        ...
                        if *count > final_cfg.max_retries {
                            if final_cfg.enable_time_travel_rewind && rewind_attempts_remaining > 0 && checkpoint_history.len() > 1 {
                                ...
                                if let Some(prev_id) = checkpoint_history.last().cloned() {
                                    ...
                                    if let Some(checkpointer) = &self.agent.checkpointer {
            ...
            for tc in &mutating_calls {
                ...
                if let Err(e) = crate::agent::Agent::check_tool_gating(&tc, false, &final_cfg) {
                    match e {
                ...
                loop {
                    match self.agent.execute_tool(&tc, &session_tools, &messages).await {
                        Ok(r) => {
                            tool_error_counts.remove(&tc.name);
                            self.agent.progress.record_tool_use();
                            self.agent.observation_store.insert(tc.id.clone(), r.clone());
            ...
            if final_cfg.enable_context_compaction && turn_input_tokens > final_cfg.compaction_threshold_tokens {
                ...
                if messages.len() > 5 {
                    ...
                    if middle_end > middle_start {
                        ...
                        match self.agent.llm.chat(summary_req).await {
                            Ok(summary_resp) => {
