...
mod tests {
    ...
    async fn test_tenant_memory() {
        ...
        let mock_provider = std::sync::Arc::new(MockEmbeddingProvider);
        let tenant_memory = TenantMemory::new("org1".to_string(), repo.clone(), mock_provider);

        tenant_memory.store("Test content", vec![]).await.unwrap();

        let results = tenant_memory.retrieve("Query", 10).await.unwrap();
...
impl EmbeddingProvider for OpenAIEmbeddingProvider {
    async fn generate_embedding(&self, text: &str) -> Result<Vec<f32>, String> {
        ...
        // Uses Minimax or OpenAI embeddings depending on what key is provided.
        // For OHC, we default to the Minimax compatible endpoint if MINIMAX_API_KEY is present
        let url = std::env::var("OHC_EMBEDDING_ENDPOINT")
            .unwrap_or_else(|_| "https://api.minimax.chat/v1/embeddings".to_string());
        ...
        let embedding = res_json["vectors"][0]
            .as_array()
            .ok_or("Failed to parse embeddings array")?
            .iter()
...
pub struct TenantMemory {
    tenant_id: String,
    repo: std::sync::Arc<VectorRepository>,
...
impl LongTermMemory for TenantMemory {
    async fn retrieve(&self, query: &str, limit: usize) -> Result<Vec<String>, String> {
        ...
        let records = self.repo.semantic_search(&self.tenant_id, &query_embedding, limit as i64).await?;
