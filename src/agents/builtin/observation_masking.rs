...
/// Applies JetBrains' Junie Observation Masking.
/// Hides the raw output of old tools from the prompt to save context,
/// but keeps the `tool_calls` themselves visible so the model remembers what it did.
/// Uses Recency-Aware Masking: Only masks if older than the threshold and exceeds the size limit.
pub fn apply_jetbrains_junie_observation_masking(messages: &mut Vec<Message>, cfg: &AgentRunConfig) {
