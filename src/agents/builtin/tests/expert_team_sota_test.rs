...
async fn test_expert_team_sota_quality_gates() {
    let experts = vec![
        DomainExpert { role: "Industry Researcher".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 1 Chapter 2 unique research".to_string() }) },
        DomainExpert { role: "Financial Analyst".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 3 Chapter 4 unique finance".to_string() }) },
        DomainExpert { role: "Strategic Analyst".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 5 Chapter 6 unique strategy".to_string() }) },
        DomainExpert { role: "Process Supervisor".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 7 unique process".to_string() }) },
        DomainExpert { role: "Quality Auditor".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 8 unique quality".to_string() }) },
    ];
    ...
    let result = manager.run_full_expert_workflow(task, &mut trace, lead_llm_short).await;
    ...
    let result = manager.run_full_expert_workflow(task, &mut trace, lead_llm_no_chart).await;
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Missing required chart/analysis/graph verification"));
    ...
    let lead_llm_ok = Arc::new(MockSotaLlm { role_resp: format!("Final report. Chart: Provided. word {}", word_padding) });

    let result = manager_dup.run_full_expert_workflow(task, &mut trace_dup, lead_llm_ok).await;
    assert!(result.is_err());
    ...
    let lead_llm_final = Arc::new(MockSotaLlm {
        role_resp: format!("Executive Summary: Market Trends. Chart: Included. Analysis: Comprehensive. word {}", word_padding),
    });

    let result = manager.run_full_expert_workflow(task, &mut trace_final, lead_llm_final).await;
    assert!(result.is_ok());
...
async fn test_expert_team_condensation_rule() {
    ...
    let experts = vec![
        DomainExpert { role: "Researcher".to_string(), llm: Arc::new(MockSotaLlm { role_resp: long_output.clone() }) },
        DomainExpert { role: "Analyst 1".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 3".to_string() }) },
        DomainExpert { role: "Analyst 2".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 5".to_string() }) },
        DomainExpert { role: "Analyst 3".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 7".to_string() }) },
        DomainExpert { role: "Analyst 4".to_string(), llm: Arc::new(MockSotaLlm { role_resp: "Chapter 8".to_string() }) },
    ];
    ...
    assert!(summaries[0].len() <= 6000 + " [Condensed for Harness: 1k-2k tokens limit reached]".len() + 10);
}
