    ...
    while attempt <= max_retries {
        ...
        if !msg.tool_calls.is_empty() {
            if let Some(call) = msg.tool_calls.iter().find(|t| t.name == "structured_output") {
                if let Some(data) = call.arguments.get("data") {
                    match serde_json::from_value::<T>(data.clone()) {
                        ...
                        Err(e) => {
                            last_error = format!("Failed to parse tool call arguments as valid JSON matching the schema. Error: {}. Please fix the JSON and retry calling the tool.", e);
                        }
                    }
                } else {
                    last_error = "Missing required 'data' parameter in tool call arguments. Please include the data matching the schema inside the 'data' property and retry calling the tool.".to_string();
                }
            } else {
                last_error = "No 'structured_output' tool call found.".to_string();
            }
        } else {
            ...
            match serde_json::from_str::<T>(json_str) {
                Ok(parsed) => return Ok(parsed),
                Err(e) => {
                    last_error = format!("Failed to parse output as valid JSON matching the schema. Error: {}. Please fix the JSON and return only the raw JSON without markdown formatting. Your raw text was: {}", e, completion);
                }
            }
        }

        // If we reach here, there was a parsing error. We retry if attempt < max_retries
        if attempt < max_retries {
            current_req.messages.push(msg.clone());
            current_req.messages.push(crate::types::Message::user(last_error.clone()));
        }

        attempt += 1;
    }

    Err(ToolError::Fatal(format!("Output parsing failed after {} retries. Last error: {}", max_retries, last_error)))
}
