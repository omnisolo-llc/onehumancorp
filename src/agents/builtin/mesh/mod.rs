...
pub trait TeammateMesh: Send + Sync {
    ...
    async fn subscribe_coordination(
        &self,
        handler: Box<dyn Fn(Message) + Send + Sync>,
    ) -> Result<Box<dyn Fn() + Send + Sync>, String>;
...
impl TeammateMesh for TeammateMeshClient {
    ...
    async fn subscribe_tasks(
        ...
        self.transport
            .subscribe("system:job_dispatch:mesh", handler)
            .await
    }
    ...
    async fn subscribe_coordination(
        ...
        self.transport
            .subscribe("system:coordination", handler)
            .await
    }
    ...
    async fn subscribe_state_handoff(
        ...
        self.transport
            .subscribe("system:state_handoff", handler)
            .await
    }
...
pub async fn create_teammate_mesh(
    redis_url: Option<&str>,
    is_cloud: bool,
) -> Result<Arc<dyn TeammateMesh>, String> {
    match crate::mesh::transport::create_transport(redis_url, is_cloud).await {
        Ok(transport) => Ok(Arc::new(TeammateMeshClient::new(transport))),
        Err(e) => Err(format!("Failed to create TeammateMesh: {}", e)),
    }
}
...
mod tests {
    ...
    async fn test_teammate_mesh_client() {
        ...
        let _task_cancel = mesh
            .subscribe_tasks(Box::new(move |msg| {
        ...
        let _coord_cancel = mesh
            .subscribe_coordination(Box::new(move |msg| {
        ...
        mesh.publish_coordination(b"coord_data".to_vec())
            .await
            .unwrap();
        ...
        assert!(
            tasks_received.load(Ordering::SeqCst),
            "Should receive task message"
        );
        assert!(
            coord_received.load(Ordering::SeqCst),
            "Should receive coordination message"
        );
    }
    ...
    async fn test_create_teammate_mesh_fallback() {
        ...
        let mesh = create_teammate_mesh(Some("redis://invalid-host:9999"), false)
            .await
            .unwrap();
        ...
        assert!(
            ...
            "Fallback InProcessTransport should successfully process messages"
        );
    }
    ...
    async fn test_mesh_acquire_lock() {
        ...
        let acquired_after_release = mesh
            .acquire_lock("test_resource", "agent_2", 10)
            .await
            .unwrap();
    ...
    async fn test_mesh_register_presence() {
        ...
        mesh.register_presence("agent_1", "online", 10)
            .await
            .unwrap();
    ...
    async fn test_mesh_state_handoff() {
        ...
        mesh.publish_state_handoff(b"state_data".to_vec())
            .await
            .unwrap();
        sleep(Duration::from_millis(50)).await;
