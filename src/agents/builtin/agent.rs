...
use crate::prompt_stack::HierarchicalPromptStack;
...
pub(crate) fn build_hierarchical_system_prompt(cfg: &AgentRunConfig, tools: &[crate::tools::Tool]) -> String {
    let stack = HierarchicalPromptStack::new(
        cfg.server_system_message.clone(),
        cfg.developer_instructions.clone(),
        cfg.user_instructions.clone(),
        tools,
        cfg.enable_lost_in_the_middle_prevention,
    );
    let wd = std::env::var("OHC_AGENT_WORKSPACE").unwrap_or_else(|_| ".".to_string());
    stack.build(Some(std::path::Path::new(&wd)))
}
