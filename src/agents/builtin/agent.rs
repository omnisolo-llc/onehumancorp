...
impl Default for AgentRunConfig {
    fn default() -> Self {
        Self {
            ...
            enable_sona_neural_patterns: true,
...
pub struct Agent {
    ...
    pub sona_manager: Option<Arc<crate::sona::SonaPatternManager>>,
...
impl Agent {
    ...
    pub fn with_sona_manager(mut self, manager: Arc<crate::sona::SonaPatternManager>) -> Self {
        self.sona_manager = Some(manager);
        self
    }
        ...
        for (i, phase) in phases.iter().enumerate() {
            ...
            let req = crate::types::ChatRequest {
                ...
                system: format!(
                    ...
                    cfg.server_system_message, phase_prompt
                ),
        ...
        F: FnMut(AgentEvent) + Send + Sync,
    {
        ...
        // Recall successful past trajectories and inject as hints.
        if final_cfg.enable_sona_neural_patterns {
            if let Some(sona) = &self.sona_manager {
                if let Ok(patterns) = sona.recall_pattern(initial_message) {
                    if !patterns.is_empty() {
                        let mut hints =
                            String::from("SONA Pattern Hints from previous successes:\n");
                        for p in patterns {
                            hints.push_str(&format!(
                                "- Intent: {}\n  Tool Sequence: {:?}\n",
                                p.inferred_intent, p.tool_sequence
                            ));
                        }
                        messages.push(Message::system(hints));
                    }
                }
            }
        }
        ...
        while turn_count < max_iterations {
            ...
            if tool_calls.is_empty() {
                ...
                if final_cfg.enable_llm_judge {
                    ...
                    let parser_client: std::sync::Arc<
                        dyn crate::output_parser::LlmClientForParser,
                    > = std::sync::Arc::new(ParserClientWrapper {
                        llm: self.llm.clone(),
                    });
                ...
                if let Some(guard_cfg) = &final_cfg.guardrails {
                    ...
                    if final_cfg.enable_sona_neural_patterns {
                        if let Some(sona) = &self.sona_manager {
                            let _ = sona.extract_and_store_pattern(initial_message, &messages);
                        }
                    }
                }
                on_event(AgentEvent::TaskComplete {
            ...
            for tc in &mutating_calls {
                if final_cfg.permission_architecture
                ...
                loop {
                    ...
                    match self
                        .execute_tool(&tc, &session_tools, &messages)
                        .instrument(tool_span)
                        .await
                    {
                        Ok(r) => {
...
mod tests {
    ...
    async fn test_langgraph_four_tier_errors() {
        ...
        assert!(res3
            .unwrap_err()
            .to_string()
            .contains("Transient error after retries"));
...
async fn test_time_travel_rewind_lightweight_chaining() {
    use crate::types::{ChatRequest, Message, Role, ToolCall, ToolError, Usage};
    use ohc_builtin_agent_tools::ToolExecutor;
