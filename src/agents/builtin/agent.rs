...
mod tests {
    ...
    async fn test_prompt_construction_lost_in_the_middle_prevention() {
        ...
        let _last_msg = req.messages.last().unwrap();

        assert_eq!(_last_msg.role, Role::User);
        assert!(_last_msg.content.contains("[System Reminder: Developer instructions here.]"));
        assert!(_last_msg.content.contains("[System Reminder to combat 'Lost in the Middle' effect: Remember your core objective: Super long user instructions that span many many words....]"));
