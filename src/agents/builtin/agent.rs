...
struct ParserLlmClientWrapper {
    llm: std::sync::Arc<dyn crate::llm::LlmClient>,
}

#[async_trait::async_trait]
impl crate::output_parser::LlmClientForParser for ParserLlmClientWrapper {
...
impl Agent {
        ...
        F: FnMut(AgentEvent) + Send + Sync,
    {
        ...
        let plan: Vec<serde_json::Value> = match crate::output_parser::parse_structured_output(&(std::sync::Arc::new(ParserLlmClientWrapper { llm: self.llm.clone() }) as std::sync::Arc<dyn crate::output_parser::LlmClientForParser>), plan_req.clone(), 3).await {
            Ok(p) => p,
            Err(e) => return Err(format!("Failed to generate a valid execution plan after retries. Last error: {}", e).into()),
        };
