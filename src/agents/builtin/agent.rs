        ...
        while turn_count < max_iterations {
            ...
            if tool_calls.is_empty() {
                ...
                if final_cfg.enable_visual_verification && !final_cfg.visual_verification_command.is_empty() {
                    let wd = final_cfg.workspace_path.clone().unwrap_or_else(|| ".".to_string());
                    verification_manager.add_visual(std::sync::Arc::new(crate::verification_loops::BashVisualVerifier {
                ...
                if let Err(e) = verification_manager.run_inferential_sensors(&last_assistant_content, "").await {
                    if !e.starts_with("FAIL:") {
                        let err = format!("LLM Judge error: {}", e);
                        on_event(AgentEvent::TaskError { error: err.clone() });
                        return Err(err.into());
                    }
                    let err_msg = format!("Your previous output was evaluated by an LLM-as-judge and rejected. Reason: {}. Please correct your work and use tools if necessary.", e);
    ...
    async fn test_llm_judge_rejects_and_approves() {
        let client = Arc::new(MockLlmClient {
            responses: tokio::sync::Mutex::new(vec![
                ...
                ChatResponse {
                    message: Message::assistant("FAIL: The answer is incomplete."),
                    usage: Usage::default(),
                    stop_reason: "stop".to_string(),
                        response_id: Some("mock-id".to_string()),
                },
