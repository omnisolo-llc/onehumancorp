                ...
                loop {
                    match self.execute_tool(&current_tc, &session_tools_clone, &[], cfg.max_retries).await {
                        ...
                        Err(crate::types::ToolError::LlmRecoverable(err_msg)) => {
                            ...
                            let msg_to_push = crate::types::Message::new_llm_recoverable_tool_message(
                                current_tc.id.clone(),
                                &current_tc.name,
                                &err_msg,
                            );
                ...
                match res {
                    ...
                    Err(ToolError::LlmRecoverable(err_msg)) => {
                        ...
                        *count += 1;
                        ...
                        let msg_to_push = Message::new_llm_recoverable_tool_message(
                            tc.id.clone(),
                            &tc.name,
                            &err_msg,
                        );
                        tool_results[idx] = msg_to_push.tool_results[0].clone();
