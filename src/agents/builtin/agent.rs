...
#[derive(Debug, Clone)]
/// Agent state enumeration: `AgentEvent`.
///
/// Defines the distinct categorical states or events associated with `AgentEvent`.
///
/// # Pattern Matching Requirements
/// Due to the critical nature of the control flow in the agent loop, all `match`
/// statements resolving `AgentEvent` must be exhaustive.
///
/// # Evolution
/// New variants may be added to `AgentEvent` in future releases to support
/// additional tool types or orchestration signals.
/// Enumeration defining the possible variants for `AgentEvent`.
/// Used within event-driven architectures to signify state changes or triggers.
pub enum AgentEvent {
...
#[derive(Debug, Clone)]
/// Agent core component: `AgentRunConfig`.
///
/// This struct is essential for the ReAct loop mechanics, specifically handling
/// the `AgentRunConfig` aspect of the agent's context.
///
/// # Invariants
/// The state encapsulated by `AgentRunConfig` must be carefully managed between
/// iterations to prevent context drift or memory leaks during long-running tasks.
///
/// # Configuration
/// Many fields within `AgentRunConfig` are derived directly from the overarching
/// `AgentRunConfig`, allowing operators to tune its behavior at runtime.
/// Data structure representing `AgentRunConfig`.
/// Contains configuration parameters to customize the behavior of the associated system.
/// It is recommended to use the `Default` trait implementation for baseline settings.
...
#[derive(Default)]
/// Agent core component: `AgentProgress`.
///
/// This struct is essential for the ReAct loop mechanics, specifically handling
/// the `AgentProgress` aspect of the agent's context.
///
/// # Invariants
/// The state encapsulated by `AgentProgress` must be carefully managed between
/// iterations to prevent context drift or memory leaks during long-running tasks.
///
/// # Configuration
/// Many fields within `AgentProgress` are derived directly from the overarching
/// `AgentRunConfig`, allowing operators to tune its behavior at runtime.
/// Data structure representing `AgentProgress`.
...
impl AgentProgress {
/// Agent operational primitive: `record_tool_use`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `record_tool_use` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `record_tool_use` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `record_tool_use` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `record_tool_use` operation.
...
/// Agent operational primitive: `add_tokens`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `add_tokens` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `add_tokens` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `add_tokens` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `add_tokens` operation.
/// A utility or core domain function designed to perform specific business logic.
    pub fn add_tokens(&self, n: i64) {
        self.token_count.fetch_add(n, Ordering::Relaxed);
    }

/// Agent operational primitive: `tool_use_count`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `tool_use_count` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `tool_use_count` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `tool_use_count` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `tool_use_count` operation.
/// A utility or core domain function designed to perform specific business logic.
    pub fn tool_use_count(&self) -> u64 {
        self.tool_use_count.load(Ordering::Relaxed)
    }

/// Agent operational primitive: `token_count`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `token_count` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `token_count` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `token_count` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `token_count` operation.
/// A utility or core domain function designed to perform specific business logic.
...
/// Agent core component: `Agent`.
///
/// This struct is essential for the ReAct loop mechanics, specifically handling
/// the `Agent` aspect of the agent's context.
///
/// # Invariants
/// The state encapsulated by `Agent` must be carefully managed between
/// iterations to prevent context drift or memory leaks during long-running tasks.
///
/// # Configuration
/// Many fields within `Agent` are derived directly from the overarching
/// `AgentRunConfig`, allowing operators to tune its behavior at runtime.
/// Data structure representing `Agent`.
/// This type is used internally for data modeling and state management.
pub struct Agent {
...
impl Agent {
/// Agent operational primitive: `add_tool`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `add_tool` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `add_tool` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `add_tool` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `add_tool` operation.
/// A utility or core domain function designed to perform specific business logic.
    pub fn add_tool(&mut self, tool: Tool) {
        self.tools.push(tool);
    }
/// Agent operational primitive: `new`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `new` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `new` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `new` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
...
/// Agent operational primitive: `with_memory_store`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `with_memory_store` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `with_memory_store` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `with_memory_store` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `with_memory_store` operation.
/// A utility or core domain function designed to perform specific business logic.
...
/// Agent operational primitive: `with_checkpointer`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `with_checkpointer` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `with_checkpointer` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `with_checkpointer` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
/// Executes the `with_checkpointer` operation.
/// A utility or core domain function designed to perform specific business logic.
...
/// Agent operational primitive: `query`.
///
/// Executes a vital step in the agent's cognitive or action pipeline. The
/// `query` function is optimized for token efficiency and strict error
/// boundary enforcement.
///
/// # Recovery Mechanisms
/// If `query` encounters an API timeout or a malformed LLM response, it
/// automatically engages the configured retry strategy with exponential backoff.
///
/// # Telemetry
/// Every invocation of `query` records detailed metrics, including token
/// consumption, latency, and tool invocation success rates.
