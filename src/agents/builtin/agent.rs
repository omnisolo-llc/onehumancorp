                ...
                for tc_val in read_only_calls {
                    ...
                    read_only_futures.push(async move {
                        ...
                        if let Some(tool) = tt_clone.iter().find(|t| t.name == name) {
                            ...
                            #[allow(unused)] let mut retry_count = 0;
                            #[allow(unused)] let max_retries = std::cmp::min(cfg_max_retries, 2); // Error Handling (Compounding Error Prevention): Stripe limits retries to exactly 2.
                            let final_res;
