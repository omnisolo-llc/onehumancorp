...
impl Agent {
        ...
        F: FnMut(AgentEvent) + Send + Sync,
    {
        ...
        let system_prompt = crate::prompt_builder::build_hierarchical_system_prompt(&cfg_arc, &session_tools_arc);
        ...
        F: FnMut(AgentEvent) + Send + Sync,
    {
        ...
        let mut combined_system = crate::prompt_builder::build_hierarchical_system_prompt(&final_cfg, &session_tools);
...
{}", last_assistant_content))],
    ...
    pub async fn execute_tool(
        ...
        // 10. Verification Loops Integration
        // We initialize the verification manager for post-tool checks.
        let verification_manager = crate::verification_loops::VerificationManager::new(self.llm.clone());
        ...
        // 10. Verification Loops Integration
        if let Err(guide_err) = verification_manager.run_computational_guide(&final_result) {
            return Err(ToolError::LlmRecoverable(guide_err));
        }

        if final_result.len() > 1000 {
            match verification_manager.run_inferential_sensor(&final_result, &tc.name).await {
                Ok(approved) => {
                    if !approved {
                        return Err(ToolError::LlmRecoverable("Inferential sensor rejected this output.".to_string()));
                    }
                }
                Err(_e) => {
