...
impl Agent {
        ...
        F: FnMut(AgentEvent) + Send + Sync,
    {
        ...
        let mut dynamic_checkpointer = self.checkpointer.clone();
        if dynamic_checkpointer.is_none() && cfg.enable_git_checkpointing {
            let wd = cfg
                .workspace_path
                .clone()
                .unwrap_or_else(|| ".".to_string());
            dynamic_checkpointer = Some(std::sync::Arc::new(
                crate::checkpointer::GitCheckpointer::new(std::path::PathBuf::from(wd)),
            ));
        }

        if cfg.long_term_memory.is_some()
            || (dynamic_checkpointer.is_some() && self.checkpointer.is_none())
        {
            owned_agent = Agent {
                llm: self.llm.clone(),
        ...
        if let (Some(checkpointer), Some(thread_id)) =
            (&self_with_memory.checkpointer, &final_cfg.thread_id)
        {
...
{}", last_assistant_content))],
            ...
            if final_cfg.enable_git_checkpointing && mutating_calls.is_empty() {
            ...
            if should_checkpoint {
                    ...
                    (&self_with_memory.checkpointer, &final_cfg.thread_id)
                {
                    ...
                    if let Err(e) = checkpointer.put_checkpoint(cp).await {
                        ...
                        let path_prefix = if final_cfg.enable_git_checkpointing {
                            "git"
                        } else {
                            "db"
                        };
                        on_event(AgentEvent::CheckpointSaved {
                            iteration,
                            path: format!("{}:{}", path_prefix, checkpoint_id),
                        });
                    }
                }
            }
...
mod tests {
    ...
    async fn test_langgraph_four_tier_errors() {
        ...
        let tool_recoverable = Tool {
            ...
            is_read_only: true,
            parameters: serde_json::json!({}),
            execute: Arc::new(LanggraphFourTierErrorToolExecutor {
                name: "llm_recoverable_tool".to_string(),
                call_count: tokio::sync::Mutex::new(0),
            }),
        };
