...
async fn test_agent_state_checkpointing_mechanic() {
    ...
    let agent1 =
        Agent::new(client1, vec![mutating_tool.clone()]).with_checkpointer(checkpointer.clone());
    let mut cfg = AgentRunConfig::default();
...
async fn test_git_checkpointer_integration() {
    ...
    let agent =
        Agent::new(client_with_tools, vec![mutating_tool]).with_checkpointer(checkpointer.clone());
