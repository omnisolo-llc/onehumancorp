...
mod tests {
    ...
    #[tokio::test]
    async fn test_scout_agent_flow_sqlite() {
    ...
    #[tokio::test]
    async fn test_scout_agent_flow_postgres() {
        ...
        let db = ScoutDb::new_pg(pool.clone());
        let bus = Arc::new(SubagentBus::new());
        let mut rx = bus.subscribe();
        ...
        assert_eq!(integration.name, "TestToolPG");
        assert_eq!(integration.tenant_id, "tenant-123");
        assert!(integration.integration_code.clone().unwrap_or_default().contains("TestToolPGClient"));
