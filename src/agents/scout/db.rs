...
impl ScoutDb {
    ...
    pub async fn get_all_integrations_by_tenant(&self, tenant_id: &str) -> Result<Vec<ToolIntegration>, sqlx::Error> {
        ...
        match self {
            Self::Postgres(pool) => {
                ...
                sqlx::query("SELECT set_config('app.current_tenant', $1, true)")
                    ...
                    .execute(&mut *tx)
                    .await?;
                let query = "SELECT id, tenant_id, name, description, api_url, integration_code, status, created_at FROM tool_integrations WHERE tenant_id = $1";
                let rows = sqlx::query(query).bind(tenant_id).fetch_all(&mut *tx).await?;
                tx.commit().await?;

                for row in rows {
                    let id_uuid: Uuid = row.try_get("id")?;
                    results.push(ToolIntegration {
                        id: id_uuid.to_string(),
                        tenant_id: row.try_get("tenant_id")?,
                        name: row.try_get("name")?,
                        description: row.try_get("description")?,
            ...
            Self::Sqlite(pool) => {
                let query = "SELECT id, tenant_id, name, description, api_url, integration_code, status, created_at FROM tool_integrations WHERE tenant_id = ?";
