...
func TestPIIRedactingHandler_WithGroup(t *testing.T) {
	...
	logger.Info("User info", slog.Group("user", slog.String("email", "test@example.com"), slog.String("phone", "555-123-4567")))

	output := buf.String()
	if strings.Contains(output, "test@example.com") {
		t.Errorf("Expected email to be redacted inside group, got: %s", output)
	}
	if !strings.Contains(output, "[REDACTED_EMAIL]") {
		t.Errorf("Expected [REDACTED_EMAIL] in output, got: %s", output)
	}
	if strings.Contains(output, "555-123-4567") {
		t.Errorf("Expected phone to be redacted inside group, got: %s", output)
	}
	if !strings.Contains(output, "[REDACTED_PHONE]") {
		t.Errorf("Expected [REDACTED_PHONE] in output, got: %s", output)
	}
