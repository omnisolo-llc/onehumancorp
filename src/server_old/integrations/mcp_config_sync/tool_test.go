...
func TestConfigTool_GetConfigTool(t *testing.T) {
	...
	// test invalid JSON
	_, err = mcpTool.Execute(context.Background(), "", map[string]interface{}{"stub": "invalid"})
	require.Error(t, err)

	// test missing key
	_, err = mcpTool.Execute(context.Background(), "", map[string]interface{}{"stub": "{}"})
	require.Error(t, err)
...
func TestConfigTool_SyncConfigToCloudTool(t *testing.T) {
	...
	// test invalid JSON
	_, err := mcpTool.Execute(ctx, "", map[string]interface{}{"stub": "invalid"})
	require.Error(t, err)
