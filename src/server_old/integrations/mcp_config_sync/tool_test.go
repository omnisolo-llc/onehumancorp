...
func TestConfigTool_GetConfigTool(t *testing.T) {
	...
	// test invalid JSON
	_, err = executeHelper(context.Background(), mcpTool, []byte(`invalid`))
	require.Error(t, err)

	// test missing key
	_, err = executeHelper(context.Background(), mcpTool, []byte(`{}`))
	require.Error(t, err)

	// test success
	res, err := executeHelper(context.Background(), mcpTool, []byte(`{"key":"my_key"}`))
	require.NoError(t, err)
	assert.Equal(t, "local_value", res)

    // test execution error (key not found)
    _, err = executeHelper(context.Background(), mcpTool, []byte(`{"key":"other_key"}`))
	require.Error(t, err)
}
...
func TestConfigTool_SyncConfigToCloudTool(t *testing.T) {
	...
	// test invalid JSON
	_, err := executeHelper(ctx, mcpTool, []byte(`invalid`))
	require.Error(t, err)

	// test missing fields
	_, err = executeHelper(ctx, mcpTool, []byte(`{"tenant_id":"org1"}`))
	require.Error(t, err)

	// test success
	res, err := executeHelper(ctx, mcpTool, []byte(`{"tenant_id":"org1","agent_id":"a1","key":"k1","value":"v1","metadata":{"k":"v"}}`))
	require.NoError(t, err)
	assert.Equal(t, "Successfully synced config to cloud", res)

    // test execution error (no auth)
    _, err = executeHelper(context.Background(), mcpTool, []byte(`{"tenant_id":"org1","agent_id":"a1","key":"k1","value":"v1","metadata":{"k":"v"}}`))
	require.Error(t, err)
}
