...
func TestConfigTool_GetConfigTool(t *testing.T) {
	...
	// test invalid JSON
	_, err = mcpTool.Execute(context.Background(), "", map[string]interface{}{"key": "invalid"})
	require.Error(t, err)

	// test missing key
	_, err = mcpTool.Execute(context.Background(), "", map[string]interface{}{"key": map[string]interface{}{}})
	require.Error(t, err)
...
func TestConfigTool_SyncConfigToCloudTool(t *testing.T) {
	...
	// test invalid JSON
	_, err := mcpTool.Execute(ctx, "", map[string]interface{}{})
	require.Error(t, err)

	// test missing fields
	_, err = mcpTool.Execute(ctx, "", map[string]interface{}{"tenant_id":""})
	require.Error(t, err)
