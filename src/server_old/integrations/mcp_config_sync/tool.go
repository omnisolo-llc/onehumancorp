...
func (t *ConfigTool) GetConfigTool() *configGetWrapper {
	return &configGetWrapper{t}
}
type configGetWrapper struct{ t *ConfigTool }
func (w *configGetWrapper) Definition() local.ToolDefinition {
	return local.ToolDefinition{
		...
		InputSchema: func() map[string]interface{} {
            ...
            json.Unmarshal([]byte(`{"type":"object","properties":{"key":{"type":"string","description":"The configuration key to read"}},"required":["key"]}`), &m)
            return m
		}(),
	}
}
func (w *configGetWrapper) Execute(ctx context.Context, workDir string, input_map map[string]interface{}) (string, error) {
			var input struct {
				Key string `json:"key"`
			}
			if err := json.Unmarshal(func() []byte { b, _ := json.Marshal(input_map); return b }(), &input); err != nil {
			...
			val, err := w.t.GetConfig(ctx, input.Key)
			if err != nil {
				return "", err
			}
			return val, nil
...
func (t *ConfigTool) SyncConfigToCloudTool() *configSyncWrapper {
	return &configSyncWrapper{t}
}
type configSyncWrapper struct{ t *ConfigTool }
func (w *configSyncWrapper) Definition() local.ToolDefinition {
	return local.ToolDefinition{
		...
		InputSchema: func() map[string]interface{} {
            var m map[string]interface{}
            json.Unmarshal([]byte(`{"type":"object","properties":{"tenant_id":{"type":"string"},"agent_id":{"type":"string"},"key":{"type":"string"},"value":{"type":"string"},"metadata":{"type":"object","additionalProperties":{"type":"string"}}},"required":["tenant_id","agent_id","key","value"]}`), &m)
            return m
		}(),
	}
}
func (w *configSyncWrapper) Execute(ctx context.Context, workDir string, input_map map[string]interface{}) (string, error) {
			...
			if err := w.t.SyncConfigToCloud(ctx, input); err != nil {
				return "", err
			}
