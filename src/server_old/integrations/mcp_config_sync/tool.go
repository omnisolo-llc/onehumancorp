...
func (t *getConfigTool) Execute(ctx context.Context, workDir string, input map[string]interface{}) (string, error) {
    keyInterface, ok := input["key"]
    if !ok {
        return "", fmt.Errorf("invalid arguments: key is required")
    }
    key, ok := keyInterface.(string)
    if !ok || key == "" {
        return "", fmt.Errorf("invalid arguments: key is required")
    }
	val, err := t.t.GetConfig(ctx, key)
	if err != nil {
		return "", err
	}
	return val, nil
...
func (t *syncConfigToCloudTool) Execute(ctx context.Context, workDir string, inputMap map[string]interface{}) (string, error) {
	...
	if err := t.t.SyncConfigToCloud(ctx, input); err != nil {
		return "", err
	}
...
func (t *ConfigTool) SyncConfigToCloudTool() local.Tool {
	return &syncConfigToCloudTool{t: t}
}
