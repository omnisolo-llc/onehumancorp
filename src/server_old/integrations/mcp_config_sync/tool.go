...
type structTool struct {
	...
	fn  func(context.Context, []byte) (string, error)
}

func (s structTool) Definition() local.ToolDefinition {
	return s.def
}

func (s structTool) Execute(ctx context.Context, _ string, input map[string]interface{}) (string, error) {
...
func (t *ConfigTool) GetConfigTool() local.Tool {
	...
	json.Unmarshal([]byte(`{"type":"object","properties":{"key":{"type":"string","description":"The configuration key to read"}},"required":["key"]}`), &schema)
	return structTool{
		def: local.ToolDefinition{
			Name:        "get_config",
		...
		fn: func(ctx context.Context, args []byte) (string, error) {
			var input struct {
				Key string `json:"key"`
			}
...
func (t *ConfigTool) SyncConfigToCloudTool() local.Tool {
	var schema map[string]interface{}
	json.Unmarshal([]byte(`{"type":"object","properties":{"tenant_id":{"type":"string"},"agent_id":{"type":"string"},"key":{"type":"string"},"value":{"type":"string"},"metadata":{"type":"object","additionalProperties":{"type":"string"}}},"required":["tenant_id","agent_id","key","value"]}`), &schema)
	return structTool{
		def: local.ToolDefinition{
