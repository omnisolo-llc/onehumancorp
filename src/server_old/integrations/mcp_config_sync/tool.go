...
type wrappedGetConfigTool struct {
	tool *ConfigTool
}

func (w *wrappedGetConfigTool) Definition() local.ToolDefinition {
...
func (w *wrappedGetConfigTool) Execute(ctx context.Context, workDir string, args map[string]interface{}) (string, error) {
	...
	val, err := w.tool.GetConfig(ctx, input.Key)
	if err != nil {
		return "", err
	}
	return val, nil
...
func (t *ConfigTool) GetConfigTool() local.Tool {
	return &wrappedGetConfigTool{tool: t}
}

// SyncConfigToCloudTool returns the MCP tool definition for syncing a config value to the cloud.
type wrappedSyncConfigToCloudTool struct {
	tool *ConfigTool
}

func (w *wrappedSyncConfigToCloudTool) Definition() local.ToolDefinition {
	return local.ToolDefinition{
...
func (w *wrappedSyncConfigToCloudTool) Execute(ctx context.Context, workDir string, args map[string]interface{}) (string, error) {
	argsJSON, _ := json.Marshal(args)
	...
	if err := w.tool.SyncConfigToCloud(ctx, input); err != nil {
		return "", err
	}
...
func (t *ConfigTool) SyncConfigToCloudTool() local.Tool {
	return &wrappedSyncConfigToCloudTool{tool: t}
}
