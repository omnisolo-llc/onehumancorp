...
func NewAutoDreamPipeline(pool db.Provider, redisClient rueidis.Client) *AutoDreamPipeline {
	...
	if key := os.Getenv("ANTHROPIC_API_KEY"); key != "" {
		llmClient = local.NewAnthropicClient(key, "claude-3-5-sonnet-20241022", "")
	} else if key := os.Getenv("OPENAI_API_KEY"); key != "" {
		llmClient = local.NewOpenAICompatClient("https://api.openai.com/v1/chat/completions", key, "gpt-4o")
	} else {
		llmClient = local.NewOllamaClient("http://localhost:11434/api/chat", "llama3.1")
	}
