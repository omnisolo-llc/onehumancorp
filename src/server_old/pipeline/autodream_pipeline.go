...
func NewAutoDreamPipeline(pool db.Provider, redisClient rueidis.Client) *AutoDreamPipeline {
	...
	if key := os.Getenv("ANTHROPIC_API_KEY"); key != "" {
		llmClient = local.NewAnthropicClient(key, "claude-3-haiku-20240307", "https://api.anthropic.com")
	} else if key := os.Getenv("OPENAI_API_KEY"); key != "" {
		llmClient = local.NewOpenAICompatClient("https://api.openai.com/v1", key, "gpt-4o-mini")
	} else {
		llmClient = local.NewOllamaClient("http://localhost:11434", "llama3")
	}
	...
	for _, c := range conflicts {
		...
		req := local.CompletionRequest{
			Messages: []local.ConversationMessage{
				{Role: "system", Content: []local.ContentPart{{Type: "text", Text: "You are an AI Memory Consolidator."}}},
