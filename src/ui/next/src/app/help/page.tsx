                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 flex-col">
                      {filteredArticles.filter(a => (a.category || "General") === category).map((article, idx) => (
                        <Link key={idx} href={article.link} className="block group">
                          <div className="bg-white/60 backdrop-blur-[40px] saturate-[200%] border border-white/40 p-6 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.08)] group-hover:bg-white/80 group-hover:shadow-[0_16px_48px_rgba(0,0,0,0.12)] group-hover:-translate-y-1 group-hover:border-white/60 transition-all duration-300 cursor-pointer h-full flex flex-col min-h-[120px] sm:min-h-[140px]">
