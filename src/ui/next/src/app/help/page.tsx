...
import { ArticleSections } from "../../components/help/ArticleSections";

export default function HelpCenterPage() {
  const [articles, setArticles] = useState<
    { category: string; title: string; desc: string; link: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const fetchUrl = isMobile
      ? "/api/v1/videos?mobile_optimized=true"
      : "/api/v1/videos";
    fetch(fetchUrl)
      .then((res) => {
    ...
    <div className="min-h-screen bg-[#F5F5F7] py-6 sm:py-12 px-4 sm:px-6 lg:px-8 font-inter">
      <div className="max-w-4xl mx-auto">
        <h1
          ...
          className="text-3xl sm:text-4xl font-extrabold font-outfit text-[#1D1D1F] mb-6 sm:mb-8 text-center tracking-tight"
        >
          In-App Help Center
        </h1>
        ...
        <div className="mb-8 sm:mb-10 w-full max-w-2xl mx-auto block">
          <div className="w-full relative block">
            <WithTooltip id="help-search-tooltip">
              ...
              />
              <svg
                className="w-6 h-6 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 px-8 backdrop-blur-[30px] saturate-[210%] bg-white/65 dark:bg-[#16161a]/70 border border-white/40 dark:border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] rounded-3xl min-h-[400px] w-full max-w-2xl mx-auto transition-all">
            <svg
              className="w-20 h-20 max-w-[80px] max-h-[80px] text-red-500 mb-6 opacity-80"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div className="flex flex-col items-center justify-center py-24 px-8 backdrop-blur-[30px] saturate-[210%] bg-white/65 dark:bg-[#16161a]/70 border border-white/40 dark:border-white/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] rounded-3xl min-h-[400px] w-full max-w-2xl mx-auto transition-all">
            <svg
              className="w-20 h-20 max-w-[80px] max-h-[80px] text-gray-400 mb-6 opacity-50"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-center text-gray-700 dark:text-gray-300 font-medium text-xl md:text-2xl mb-2">
              {searchQuery ? (
                ...
                <>No help articles available right now.</>
              )}
            </p>
            <p className="text-center text-gray-500 dark:text-gray-400 text-base md:text-lg mb-8 max-w-md">
            ...
            <WithTooltip
              id="ask-ai-tooltip"
              defaultText="Open AI Help Chat to get answers instantly."
            >
              <button
            ...
            {advancedArticles.length > 0 && (
              <div className="pt-8">
                <div className="border-t border-gray-200/50 pt-8 mt-4">
                  <WithTooltip
                    id="help-advanced-toggle-tooltip"
                    defaultText="Show advanced developer options"
                  >
                    <button
                      onClick={() => setIsAdvancedOpen(!isAdvancedOpen)}
                      className="flex items-center text-gray-500 hover:text-gray-900 transition-colors duration-200"
                    >
                      <span className="text-lg font-bold font-outfit mr-2">
                        Advanced
                      </span>
                      <svg
                        className={`w-5 h-5 transform transition-transform duration-200 ${isAdvancedOpen ? "rotate-180" : " "}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
