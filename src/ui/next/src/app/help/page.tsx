...
export default function HelpCenterPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7]/80 backdrop-blur-[30px] saturate-[210%] py-6 sm:py-12 px-4 sm:px-6 lg:px-8 font-inter">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 sm:mb-10 w-full sm:w-3/4 mx-auto">
          <input
            ...
            className="w-full p-4 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-[0_8px_32px_rgba(0,0,0,0.05)] text-gray-900 bg-white/60 backdrop-blur-[40px] saturate-[210%] border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.1)] hover:bg-white/75 min-h-[44px] text-base placeholder:text-gray-500 transition-all rounded-xl"
          />
        </div>

        {filteredArticles.length === 0 && filteredVideos.length === 0 ? (
          <div className="flex flex-col items-center justify-center bg-white/60 backdrop-blur-[40px] saturate-[210%] border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.1)] py-16 px-4 shadow-[0_4px_16px_rgba(0,0,0,0.02)] rounded-2xl min-h-[300px] w-full max-w-[400px] mx-auto">
          ...
          <div className="space-y-10 sm:space-y-12">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 flex-col">
                      {filteredArticles.filter(a => (a.category || "General") === category).map((article, idx) => (
                        <Link key={idx} href={article.link} className="block group">
                          <div className="backdrop-blur-xl bg-white/50 border border-white/20 p-5 sm:p-6 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.04)] group-hover:border-blue-300 group-hover:shadow-[0_0_15px_rgba(59,130,246,0.5)] group-hover:-translate-y-1 hover:bg-white/75 transition-all duration-300 cursor-pointer h-full flex flex-col min-h-[120px] sm:min-h-[140px]">
            ...
            <section className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-gray-200/50">
              <WithTooltip id="api-docs-tooltip" defaultText="Direct API access is only for custom integrations.">
                <div className="bg-yellow-50/50 backdrop-blur-[40px] saturate-[210%] border border-yellow-200/50 shadow-[0_8px_32px_rgba(0,0,0,0.1)] p-5 sm:p-6 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
