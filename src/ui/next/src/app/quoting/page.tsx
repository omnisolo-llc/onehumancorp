...
function MobileQuotingPageContent() {
    ...
    setQuotes([
      {
        ...
        requestText:
          "Hi Carlos, the pipe under my kitchen sink started leaking yesterday. It's a steady drip. Can you take a look?",
        status: "DRAFT",
        items: [
          {
            id: "item-1",
            description: "Callout Fee & Diagnostics",
            price: 75.0,
            quantity: 1,
            isOptional: false,
            selected: true,
          ...
          {
            id: "item-2",
            description: "Standard P-Trap Replacement",
            price: 120.0,
            quantity: 1,
            isOptional: false,
            selected: true,
          ...
          {
            id: "item-3",
            description: "Emergency Weekend Surcharge",
            price: 50.0,
            quantity: 1,
            isOptional: true,
            selected: false,
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/quotes/${activeQuoteId}/approve`, {
        method: "PATCH",
      ...
      if (updatedQuote.checkout_url) {
        // Redirect to the checkout URL or show it
        setTimeout(() => {
          if (confirm("Quote approved! Proceed to Stripe Payment Link?")) {
            window.location.href = updatedQuote.checkout_url;
          }
        }, 500);
      } else {
        alert("Quote approved, but no payment link was generated.");
      }
    } catch (error) {
      console.error(error);
        ...
        <section className="glassmorphism rounded-2xl p-5 shadow-sm border border-white/40 dark:border-white/10">
          <div className="flex items-start space-x-4">
            {activeQuote.customerPhotoUrl ? (
              <img
                src={activeQuote.customerPhotoUrl}
                alt={activeQuote.customerName}
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p
                          className={`font-medium ${item.selected ? "text-gray-900" : "text-gray-500"}`}
                        >
                          {item.description}
                        </p>
                        {item.isOptional && (
                          <span className="text-xs text-gray-400 font-medium bg-gray-100 px-2 py-0.5 rounded-full mt-1 inline-block">
                            Optional
                          </span>
                        )}
                      </div>
        ...
        <section className="glassmorphism rounded-xl p-4 border border-white/40 dark:border-white/10">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-xl font-bold text-gray-900">
              ${calculateTotal(activeQuote).toFixed(2)}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-500">
              Deposit required to book (50%)
            </span>
            <span className="text-gray-700 font-semibold">
              ${(calculateTotal(activeQuote) / 2).toFixed(2)}
            </span>
          </div>{" "}
        </section>
      </main>
