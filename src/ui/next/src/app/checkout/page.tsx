...
export default function CheckoutPage() {
  ...
  React.useEffect(() => {
    const fetchUpsell = async () => {
      ...
      try {
        // Read the real cart items, fallback if empty to unblock demo
        let cartItems = [];
        try {
            cartItems = JSON.parse(localStorage.getItem('ohc_cart') || '[]');
        } catch(e) {}

        const payload = cartItems.length > 0 ? cartItems : ['Product'];

        const response = await fetch('/api/v1/growth/upsell/generate', {
          method: 'POST',
        ...
        console.error("Failed to generate upsell", e);
      } finally {
  ...
  return (
    <div className="flex flex-col min-h-screen font-inter" style={{ backgroundColor: '#F5F5F7' }}>
      ...
      <main id="checkout-screen" className="p-6 md:p-8 flex-1 max-w-lg mx-auto w-full flex flex-col gap-6">
        ...
        {upsellOffer && !isCheckingUpsell && (
          <div className="p-5 flex flex-col gap-3 relative overflow-hidden" style={{ background: 'rgba(255, 255, 255, 0.75)', backdropFilter: 'blur(20px) saturate(200%)', border: '1px solid rgba(220, 252, 231, 0.8)', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)' }}>
            <div className="absolute top-0 right-0 w-24 h-24 bg-green-50 rounded-bl-full -z-10 opacity-70"></div>
            <div className="flex justify-between items-start">
              <div>
                <span className="inline-block px-2 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full mb-2 uppercase tracking-wide">Frequently Bought Together</span>
                <h3 className="text-lg font-bold text-gray-900 font-outfit">{upsellOffer.product_name}</h3>
                <p className="text-sm text-gray-600 leading-snug">{upsellOffer.description}</p>
              </div>
              <div className="text-right">
                 <div className="text-lg font-bold text-gray-900">${upsellOffer.discounted_price?.toFixed(2)}</div>
                 <div className="text-xs text-gray-400 line-through">${upsellOffer.original_price?.toFixed(2)}</div>
              </div>
            </div>
            <button className="w-full mt-2 px-4 py-2 bg-gray-900 text-white rounded-lg text-sm font-medium hover:bg-black transition-colors flex items-center justify-center gap-2">
               <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
               Add to Order ({upsellOffer.discount_percentage}% Off)
            </button>
          </div>
        )}
