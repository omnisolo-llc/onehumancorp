...
export default function CheckoutPage() {
  ...
  const handlePayment = async (isSub = false) => {
    ...
    const normalizeReferralLink = (rawLink: string) => {
      if (
        !rawLink ||
        rawLink.includes("ohc.store") ||
        rawLink.startsWith("ohc://")
      )
        return fallbackReferralLink();
      return rawLink;
    };
      ...
      >
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6" style={{ background: "rgba(255, 255, 255, 0.65)", backdropFilter: "blur(30px) saturate(210%)", borderRadius: "16px" }}>
              ...
              <WithTooltip id="checkout-plan-upgrade-tooltip" defaultText={"Click here to securely subscribe to the " + tier + " plan."}>
                <button
                  ...
                  className={"w-full min-h-[44px] min-w-[44px] px-4 py-3 text-white rounded-lg font-medium transition-colors shadow-sm " + (isProcessing ? 'bg-indigo-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700')}
                >
              ...
              <WithTooltip id="checkout-cancel-tooltip" defaultText="Go back to the previous screen without subscribing.">
                <button
                  ...
                  className="w-full min-h-[44px] min-w-[44px] px-4 py-3 bg-gray-200 text-gray-800 rounded-lg font-medium hover:bg-gray-300 transition-colors"
                >
                  Cancel
                </button>
              </WithTooltip>
        ...
        >
          ...
          <p className="text-sm text-gray-600">
            Enter your address to see if we can deliver to you via DoorDash
            Drive (flat fee).
          </p>
        ...
        <div
          className="p-6 shadow-sm flex flex-col gap-4"
          style={{
            background: "rgba(255, 255, 255, 0.65)",
            backdropFilter: "blur(30px) saturate(210%)",
            border: "1px solid rgba(255, 255, 255, 0.4)",
        ...
        >
          <p className="text-sm text-gray-600">
            100% money back guarantee. Secure SSL payments.
          </p>
          {deliveryFee !== null && (
          ...
          >
            <button
              ...
              disabled={isProcessing}
              className={`w-full min-h-[44px] min-w-[44px] px-4 py-3 text-white rounded-lg font-medium transition-colors shadow-sm ${isProcessing ? "bg-green-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}
            >
              {isProcessing
                ? "Processing..."
          ...
          <WithTooltip
            ...
            defaultText="Tap your card or phone on the reader to pay in person."
          >
            <button
              onClick={async () => {
                if (navigator.onLine) {
                  setCheckoutStatus(
                    "Stripe Terminal payment captured for $45.00.",
                  );
                  await handlePayment(false);
                } else {
                  // Dynamically import SyncManager to avoid SSR issues
                  const { SyncManager } =
                    await import("../../lib/sync/SyncManager");
              ...
              className="w-full min-h-[44px] min-w-[44px] px-4 py-3 bg-blue-500 text-white rounded-lg font-medium hover:bg-blue-600 transition-colors shadow-sm"
            >
          ...
          >
            <button
              ...
              className="w-full min-h-[44px] min-w-[44px] px-4 py-3 bg-[#009EE3] text-white rounded-lg font-medium hover:bg-[#007ebd] transition-colors shadow-sm flex items-center justify-center gap-2"
            >
          ...
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl relative overflow-hidden font-inter border border-green-100">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full min-h-[44px] min-w-[44px] px-4 py-3 text-indigo-600 bg-indigo-50 rounded-lg font-medium hover:bg-indigo-100 transition-colors"
              >
