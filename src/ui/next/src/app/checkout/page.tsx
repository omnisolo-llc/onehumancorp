        ...
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl relative overflow-hidden font-inter border border-green-100">
            ...
            <p className="text-gray-600 mb-4 text-sm leading-relaxed">
              Your order is confirmed. Share your purchase with friends to unlock <strong className="text-gray-900">$10 off your next order!</strong>
            </p>

            <div className="space-y-5">
              {/* Visual Share Card Preview */}
              <div className="w-full aspect-[1.91/1] rounded-xl shadow-lg flex flex-col justify-center items-center text-center p-6 overflow-hidden relative" style={{ background: 'linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%)', color: '#fff' }}>
                 <div className="absolute top-0 left-0 w-32 h-32 bg-white/10 rounded-full blur-2xl -translate-x-1/2 -translate-y-1/2"></div>
                 <div className="absolute bottom-0 right-0 w-32 h-32 bg-black/10 rounded-full blur-2xl translate-x-1/2 translate-y-1/2"></div>

                 <div className="z-10 flex flex-col items-center">
                    <div className="w-12 h-12 mb-3 bg-white/20 rounded-xl shadow-inner flex items-center justify-center backdrop-blur-sm border border-white/30">
                        <span className="text-2xl">🛍️</span>
                    </div>
                    <h3 className="text-2xl font-bold font-outfit mb-1 drop-shadow-sm">
                        I just bought something awesome!
                    </h3>
                    <p className="text-sm font-medium opacity-90 drop-shadow-sm">
                        Get 10% off your first order with my link.
                    </p>
                 </div>

                 <div className="absolute bottom-3 left-4 right-4 flex justify-between items-center opacity-80">
                     <span className="text-[10px] font-semibold tracking-wider uppercase">Powered by OHC</span>
                 </div>
              </div>

              <div className="grid grid-cols-2 gap-3 mb-2">
                <a
                  href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(`I just bought an amazing product! Use my link to get 10% off your first order: ${referralLink}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 bg-black text-white p-3 rounded-xl font-semibold text-sm shadow-sm hover:bg-gray-800 transition-all hover:-translate-y-0.5"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.008 5.94H5.078z"/></svg>
                  Share to X
                </a>
                <a
                  href={`https://wa.me/?text=${encodeURIComponent(`I just bought an amazing product! Use my link to get 10% off your first order: ${referralLink}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 bg-[#25D366] text-white p-3 rounded-xl font-semibold text-sm shadow-sm hover:bg-[#20bd5a] transition-all hover:-translate-y-0.5"
                >
              ...
              <div className="relative py-2">
                ...
                <div className="relative flex justify-center"><span className="bg-white px-2 text-xs text-gray-500 uppercase font-semibold tracking-wide">Or copy link</span></div>
              </div>
