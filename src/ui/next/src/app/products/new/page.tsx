    ...
    try {
      const response = await fetch("/api/product", {
        ...
        body: JSON.stringify({
          ...
          subscription_discount: productData.subscriptionDiscount
            ? parseInt(productData.subscriptionDiscount)
            : undefined,
        }),
      });
    ...
    return (
      <div className="p-4 max-w-[375px] mx-auto min-h-screen bg-gray-50 flex flex-col justify-center items-center font-inter">
        ...
        <p className="text-gray-600 mb-6 text-center">
          Your new product is now live on your storefront.
        </p>
        <Link
          href="/dashboard"
  ...
  return (
    <div className="p-4 max-w-[375px] mx-auto min-h-screen bg-gray-50 flex flex-col font-inter relative pb-20">
      <div className="flex items-center mb-6 border-b border-gray-200 pb-4">
        <Link href="/dashboard" className="text-blue-500 font-semibold mr-4">
          &lt; Back
        </Link>
      ...
      {!loading && !productData && (
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-gray-500 mt-4 text-center">
            Our AI Auto-Catalog Agent will instantly write the title,
            description, and suggest a price.
          </p>
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col gap-6 animate-fade-in-up">
          <div className="w-full flex flex-col gap-4">
            <div className="w-full aspect-square bg-gray-200 rounded-2xl overflow-hidden relative shadow-md">
              ...
              {/* Magic Enhance Overlay/Button */}
              {rawImage && !selectedImage && !enhancing && !variations && (
                <div className="absolute bottom-4 left-0 right-0 flex justify-center">
                  <button
                    onClick={handleEnhance}
                    className="px-6 py-3 flex items-center gap-2 rounded-full font-bold shadow-lg transition-transform hover:scale-105"
                    style={{
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 snap-x hide-scrollbar">
                  {variations.map((v, i) => (
                    <div
                      ...
                      className={`min-w-[80px] w-20 aspect-square rounded-xl overflow-hidden snap-center cursor-pointer border-2 transition-all ${selectedImage === v ? "border-[#0066FF] scale-105 shadow-md" : "border-transparent opacity-70 hover:opacity-100"}`}
                    >
                      <img
                        src={v}
                        alt={`Variation ${i}`}
