...
export default function FeedPage() {
  ...
  return (
    <AppShell title="Daily Work" subtitle="Your daily priorities, coordinated by your team.">
      <div className="w-full max-w-[375px] mx-auto md:max-w-none overflow-hidden px-4 space-y-4" data-testid="agent-feed">
                  ...
                  isDisputeResolution ? (
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full">
                      <button
                        ...
                        data-testid="feed-approve-btn"
                      >
                        {isProcessing ? 'Processing...' : item.proposed_action?.action_type === 'Draft Quote' ? 'Review Estimate' : item.proposed_action?.action_type === 'Draft Follow-up' ? 'Send Follow-up' : item.proposed_action?.action_type === 'Draft Booking' ? 'Approve & Confirm' : item.proposed_action?.action_type === 'Reassign Shift' ? 'Approve & Notify' : 'Send Invoice'}
                      </button>
                      <button
                        onClick={() => startEditing(item)}
                        disabled={isProcessing}
