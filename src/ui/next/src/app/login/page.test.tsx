...
describe("login page", () => {
  ...
  it("submits controlled credentials and navigates only after success", async () => {
    (global.fetch as Mock).mockImplementation(async (input: any) => String(input).includes("public-settings")
  ...
  it("supports keyboard submission and omits an empty organization", async () => {
    ...
    const loginCall = (global.fetch as Mock).mock.calls.find(([input]) => String(input).includes("/auth/login"));
  ...
  it("disables duplicate submissions while pending", async () => {
    ...
    (global.fetch as Mock).mockImplementation((input: any) => String(input).includes("public-settings")
    ...
    expect((global.fetch as Mock).mock.calls.filter(([input]) => String(input).includes("/auth/login"))).toHaveLength(1);
