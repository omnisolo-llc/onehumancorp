...
describe("login page", () => {
  ...
  it("supports keyboard submission and omits an empty organization", async () => {
    (global.fetch as any).mockResolvedValueOnce(Response.json({ user: {}, next: "/dashboard" }));
    render(<Login />);
    const user = userEvent.setup();
    ...
    expect(JSON.parse(String((global.fetch as any).mock.calls[0][1]?.body))).toEqual({
  ...
  it("disables duplicate submissions while pending", async () => {
    ...
    (global.fetch as any).mockImplementationOnce(() => new Promise<Response>((done) => { resolve = done; }));
    render(<Login />);
    const user = userEvent.setup();
    await user.type(screen.getByLabelText(/email or username/i), "Alice");
    await user.type(screen.getByLabelText(/^password$/i), "correct horse");
