...
export default function AgentMarketplacePage() {
  ...
  const fetchAgents = async (searchQuery: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/marketplace?q=${encodeURIComponent(searchQuery)}`);
      if (!res.ok) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 relative">
          <input
            ...
            className="w-full p-4 pl-12 text-lg border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-white/70 backdrop-blur-lg"
          />
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {agents.map((agent) => (
              <div
                key={agent.id}
                className="bg-white/80 backdrop-blur-xl p-6 rounded-2xl shadow-sm hover:shadow-md border border-gray-100 transition-all flex flex-col"
              >
