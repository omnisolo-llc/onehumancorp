    ...
    <div className="flex flex-col min-h-screen font-inter" style={{ backgroundColor: '#16161A' }}>
      ...
      <main className="p-6 md:p-8 flex-1 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section id="kairos-brain" className="lg:col-span-2 space-y-6">
            ...
            {/* Daily Briefing (Draft-for-Review) */}
            <div className="ohc-hybrid-panel shadow-lg flex flex-col gap-4 max-w-[375px] mx-auto lg:max-w-none border-blue-500/30 bg-blue-900/10 backdrop-blur-[20px] bg-white/5 border-white/40">
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold font-outfit text-[#F5F5F7]">Daily Briefing</h2>
                    <span className="text-xs font-bold px-2 py-1 bg-blue-100 text-blue-700 rounded-md">THE ADVISOR</span>
                </div>
                <p className="text-sm text-gray-400 leading-relaxed">
                    Pending drafts requiring your approval before execution.
                </p>
                <div className="space-y-4">
                  {loadingApprovals ? (
                    ...
                    pendingApprovals.map(draft => (
                      <div key={draft.id} className="p-4 bg-white/10 rounded-xl border border-gray-700 shadow-sm flex flex-col gap-3">
                          <div className="flex justify-between items-start">
                              <div>
                                  <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{draft.department.replace('_', ' ')}</span>
                                  <h3 className="text-sm font-semibold text-gray-200 mt-1">{draft.description.split('|')[0].trim()}</h3>
                              </div>
                              <span className="px-2 py-0.5 bg-yellow-900/30 text-yellow-500 border border-yellow-700/50 rounded text-[10px] font-bold uppercase tracking-widest">
                                  Review
                              </span>
                          </div>

                          {draft.payload?.original_message && (
                              <div className="bg-gray-900/50 p-2 rounded text-xs text-gray-400 border border-gray-800">
                                  <span className="font-semibold block text-gray-500 mb-1">Context:</span>
                                  "{draft.payload.original_message}"
                              </div>
                          )}

                          {draft.payload?.generated_response && (
                              <div className="bg-blue-900/20 p-3 rounded text-sm text-blue-200 border border-blue-800/50 relative italic">
                                  "{draft.payload.generated_response}"
                              </div>
                          )}
                          ...
                          <div className="flex gap-3 mt-2">
                              <button
                                ...
                                className="flex-1 py-2 px-4 rounded-lg font-semibold text-sm bg-gray-800 text-gray-300 hover:bg-gray-700 border border-gray-700 transition-colors"
                              >
                                  Reject / Edit
                              </button>
                              <button
                                onClick={() => handleApprove(draft.id)}
                                className="flex-1 py-2 px-4 rounded-lg font-bold text-sm bg-blue-600 text-white hover:bg-blue-500 shadow-md transition-colors"
                              >
                                  Approve
                              </button>
                          </div>
