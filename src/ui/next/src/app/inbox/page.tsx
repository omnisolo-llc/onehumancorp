                  ...
                  {(selected as any).checkout_link && (
                    <div className="mt-3 flex items-center bg-white/60 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-lg p-3 backdrop-filter backdrop-blur-md">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-gray-900 dark:text-white m-0">Product: {(selected as any).proposed_product_id || "Checkout Link"}</p>
                        <a href={(selected as any).checkout_link} target="_blank" rel="noreferrer" className="text-xs text-blue-600 dark:text-blue-400 truncate block">{(selected as any).checkout_link}</a>
                      </div>
                    </div>
                  )}
                </div>
