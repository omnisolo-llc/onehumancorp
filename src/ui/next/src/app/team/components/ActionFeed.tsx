...
export default function ActionFeed({
  ...
  return (
    <div className="flex flex-col gap-4 relative">
      {feedItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 text-center px-8 bg-white/40 rounded-2xl border border-white/40 backdrop-blur-[10px]">
          ...
          <p className="text-sm text-gray-500">
            No pending or recent actions.
          </p>
        </div>
      ) : (
        feedItems.map((req) => {
          const { desc, payload } = extractPayload(req.description);
          const isPending = req.status === "PendingApproval" || req.status === "PENDING_APPROVAL";
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-3 justify-between">
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                      req.action_risk?.toLowerCase() === "high" || req.action_risk?.toLowerCase() === "draftforreview" || req.action_risk?.toLowerCase() === "draft_for_review"
                        ? "bg-orange-100 text-orange-700"
                        : "bg-blue-100 text-blue-700"
                    }`}
                  >
                    {req.action_risk ? req.action_risk.replace(/([A-Z])/g, " $1").trim() : "Unknown"} Risk
                  </span>
                ...
                {!isPending && (
                  <span className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wider ${isApproved ? 'text-emerald-700 bg-emerald-50' : 'text-rose-700 bg-rose-50'}`}>
              ...
              {/* Special Payloads Based on feature_type */}
              {req.payload?.feature_type === "ambassador_reply" && (
                <div className="mb-4 p-4 rounded-xl bg-blue-50 border border-blue-100 flex flex-col gap-3">
              ...
              {req.payload?.feature_type === "legal_compliance" && (
                <div className="mb-4 p-4 rounded-xl bg-orange-50 border border-orange-100 flex flex-col gap-3">
              ...
              {req.payload?.feature_type === "global_localization" && (
                <div className="mb-4 p-4 rounded-xl bg-indigo-50 border border-indigo-100 flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="bg-indigo-100 p-2 rounded border border-indigo-200">
                      <span className="text-indigo-400 block mb-1">Translated (ES)</span>
                      <div className="font-medium">Pastel Vegano<br />€23.50</div>
                    </div>
                  </div>
                </div>
              )}

              {req.payload?.feature_type === "smart_search_setup" && (
                <div className="mb-4 p-4 rounded-xl bg-emerald-50 border border-emerald-100 flex flex-col gap-3">
                  ...
                  <div className="text-xs text-emerald-700">
                    Updating your store's information so it can be easily found by AI search tools like ChatGPT.
                  </div>
              ...
              {req.payload?.feature_type === "social_calendar" && (
                <div className="mb-4 p-4 rounded-xl bg-purple-50 border border-purple-100 flex flex-col gap-3">
                  ...
                  <div className="text-xs text-purple-700">
                    The Generative Promoter has created a week of content based on your new product.
                  </div>
                  <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
                    {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day, idx) => (
                      <div key={day} className="flex-shrink-0 w-24 bg-white rounded-lg border border-purple-100 p-2 shadow-sm">
                        <div className="text-[10px] font-bold text-gray-400 uppercase mb-1">{day}</div>
                        ...
                        <div className="text-[8px] text-gray-500 leading-tight line-clamp-2">
                          {["New flavor drop!", "Behind the scenes", "Customer favorite", "Special discount", "Weekend vibes", "Shop local", "Sunday showcase"][idx]}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {req.payload?.feature_type === "abandoned_cart" && (
                <div className="mb-4 p-4 rounded-xl bg-rose-50 border border-rose-100 flex flex-col gap-3">
                  ...
                  <div className="bg-white p-3 rounded-lg border border-rose-100 relative">
                    <div className="text-[10px] uppercase font-bold text-gray-400 mb-1 absolute top-2 right-2">AI Draft</div>
                    <p className="text-xs text-gray-700 italic">
                      "Hi Sarah, noticed you left the Vegan Chocolate Cake in your cart! Here's 10% off to sweeten the deal if you finish your order today."
                    </p>
                  </div>
                  <div className="flex gap-2 mt-1">
                    <span className="text-[10px] bg-rose-100 text-rose-700 px-2 py-1 rounded font-medium">Margin Safe: 10% Discount</span>
                    <span className="text-[10px] bg-gray-100 text-gray-600 px-2 py-1 rounded font-medium">SMS</span>
                  </div>
                </div>
              )}
      ...
      {/* Review Modal for Pending Actions */}
      {selectedReview && (
        <div className="fixed inset-0 bg-black/40 z-[100] flex flex-col justify-end">
          <div
            className="bg-white rounded-t-3xl p-6 shadow-2xl transition-transform duration-300 w-full max-w-[375px] mx-auto"
