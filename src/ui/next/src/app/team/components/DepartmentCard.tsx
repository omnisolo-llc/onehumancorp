...
export default function DepartmentCard({ name, pendingCount, onClick }: Props) {
  return (
    <button
      onClick={onClick}
      className="w-full min-h-[44px] text-left bg-white/65 backdrop-blur-[30px] saturate-[210%] border border-white/40 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all active:scale-[0.98] flex items-center justify-between group mb-4"
    >
