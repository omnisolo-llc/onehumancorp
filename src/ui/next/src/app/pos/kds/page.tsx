  ...
  return (
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="flex flex-col items-center justify-center min-h-screen bg-gray-50 font-inter py-10">
      <div className="w-[375px] h-[812px] bg-gradient-to-br from-white/40 to-white/10 backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col relative border-x border-gray-200">
        ...
        {/* Header */}
        <div className="pt-12 pb-4 px-6 bg-white/65 backdrop-blur-[30px] saturate-[210%] border-b border-white/40 sticky top-0 z-10 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold font-outfit text-gray-900">{texts.kds}</h1>
            {isOffline && <span className="text-[#DE1B1B] font-bold text-sm bg-red-100 px-2 py-1 rounded-md">{texts.offline}</span>}
          </div>
          <button
            onClick={toggleLanguage}
            className="text-[#0071E3] font-bold px-3 min-w-[44px] min-h-[44px] flex items-center justify-center bg-blue-50 rounded-lg hover:bg-blue-100 transition"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-20">
          ...
          <h2 className="text-lg font-bold font-outfit text-gray-800 mb-3">{texts.orders}</h2>
          <div className="flex flex-col gap-4 mb-6">
            {orders.map(order => (
              <div key={order.id} className="bg-white/65 backdrop-blur-[30px] saturate-[210%] rounded-[16px] p-4 shadow-sm border border-white/40">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-lg text-gray-900">#{order.id} - {order.customer_name}</h3>
                  <span className={`px-2 py-1 rounded-[8px] text-xs font-bold ${
                    order.status === 'Ready' ? 'bg-green-100 text-[#34C759]' :
                    order.status === 'Preparing' ? 'bg-yellow-100 text-[#FF9500]' :
                ...
                <div className="grid grid-cols-2 gap-2">
                   {order.status === 'Received' && (
                      <button
                        ...
                        className="col-span-2 w-full py-4 min-h-[44px] bg-[#FF9500] text-white font-bold text-lg rounded-[8px] shadow active:scale-95 transition"
                   ...
                   {order.status === 'Preparing' && (
                      <button
                        ...
                        className="col-span-2 w-full py-4 min-h-[44px] bg-[#34C759] text-white font-bold text-lg rounded-[8px] shadow active:scale-95 transition"
                   ...
                   {order.status === 'Ready' && (
                      <button
                         className="col-span-2 w-full py-4 min-h-[44px] bg-gray-300 text-gray-600 font-bold text-lg rounded-[8px]"
                         disabled
                      >
                         {texts.ready}
                      </button>
          ...
          <h2 className="text-lg font-bold font-outfit text-gray-800 mb-3">{texts.inventory}</h2>
          <div className="flex flex-col gap-3">
             {inventory.map(item => (
                <div key={item.id} className="bg-white/65 backdrop-blur-[30px] saturate-[210%] rounded-[16px] p-4 shadow-sm border border-white/40 flex justify-between items-center">
                   ...
                   <button
                     ...
                     className={`px-6 py-3 min-h-[44px] rounded-[8px] font-bold shadow active:scale-95 transition ${item.is_sold_out ? 'bg-[#FF3B30] text-white' : 'bg-green-100 text-[#34C759]'}`}
