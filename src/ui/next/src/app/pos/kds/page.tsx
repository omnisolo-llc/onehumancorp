...
export default function KDSPage() {
      ...
      if (events.length > 0 && navigator.onLine) {
        ...
        try {
          const orderEvents = events.filter(
            (e: any) => e.type === "UPDATE_ORDER_STATUS",
          );
          const inventoryEvents = events.filter(
            (e: any) => e.type === "TOGGLE_SOLD_OUT",
          );
  ...
  const handleToggleSoldOut = (itemId: string, isSoldOut: boolean) => {
    // Optimistic UI Update
    setInventory((prev) =>
      prev.map((i) => (i.id === itemId ? { ...i, is_sold_out: isSoldOut } : i)),
    );
  ...
  return (
    <div
      ...
      className="flex flex-col items-center justify-center min-h-screen bg-gray-50 font-inter py-10"
    >
      <div className="w-[375px] h-[812px] bg-gradient-to-br from-white/40 to-white/10 backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col relative border-x border-gray-200">
        {/* Header */}
        <div className="pt-12 pb-4 px-6 bg-[rgba(255,255,255,0.65)] backdrop-blur-[30px] saturate-[210%] border-b border-[rgba(255,255,255,0.4)] sticky top-0 z-10 flex justify-between items-center">
          <div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 pb-20">
          ...
          <div className="flex flex-col gap-4 mb-6">
            {orders.map((order) => (
              <div
                key={order.id}
                className="bg-[rgba(255,255,255,0.65)] backdrop-blur-[30px] saturate-[210%] rounded-[16px] p-4 shadow-sm border border-[rgba(255,255,255,0.4)]"
              >
          ...
          <div className="flex flex-col gap-3">
            {inventory.map((item) => (
              <div
                key={item.id}
                className="bg-[rgba(255,255,255,0.65)] backdrop-blur-[30px] saturate-[210%] rounded-[16px] p-4 shadow-sm border border-[rgba(255,255,255,0.4)] flex justify-between items-center"
              >
