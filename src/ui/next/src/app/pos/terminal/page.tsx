...
export default function TerminalPage() {
  ...
  useEffect(() => {
    // E2E helper bypass
    if (typeof window !== "undefined" && localStorage.getItem("e2e_active_staff")) {
      setActiveStaff(JSON.parse(localStorage.getItem("e2e_active_staff") || "{}"));
    }
  }, []);
  ...
  useEffect(() => {
    const syncInterval = setInterval(async () => {
      ...
      if (events.length > 0 && navigator.onLine) {
        setSyncing(true);
        try {
          const timecardEvents = events.filter((e: any) => e.event_type === 'CLOCK_IN' || e.event_type === 'CLOCK_OUT');
          if (timecardEvents.length > 0) {
            await fetch('/api/staff/timecard', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ events: timecardEvents })
            });
          }

          const transactionEvents = events.filter((e: any) => e.event_type === 'OFFLINE_TRANSACTION');
          if (transactionEvents.length > 0) {
             await fetch('/api/pos/orders', {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
               body: JSON.stringify({ events: transactionEvents })
             });
          }
    ...
    setPin(prevPin => {
      ...
      if (newPin.length <= 4) {
        ...
        if (newPin.length === 4) {
          ...
          const found = staff.find((s: any) => String(s.pin_hash) === newPin);
  ...
  const handleNewOrder = () => {
    ...
    if (converted.isOffline || !navigator.onLine) {
      setOfflineConversion(true);
      setTimeout(() => setOfflineConversion(false), 3000);

      const transactionId = crypto.randomUUID();
      OfflineStore.addEvent({
        event_type: 'OFFLINE_TRANSACTION',
        transaction_id: transactionId,
        product_id: 'prod-offline-1', // Mock product id
        quantity_deducted: 1,
        amount: basePrice,
        currency: 'USD',
        timestamp: new Date().toISOString(),
      ...
      alert(`${t('Offline Cash Sale Logged')}: ${converted.amount / 100} ${currency}`);
    } else {
      setCheckoutAmount(basePrice);
    }
  };
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 bg-gray-50">
           ...
           {checkoutAmount ? (
             <div className="mt-8">
               ...
               <StripeTerminalClient amount={checkoutAmount} />
             </div>
           ) : (
             <>
