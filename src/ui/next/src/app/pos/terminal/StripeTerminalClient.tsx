      ...
      {!selectedMethod ? (
        <div className="flex flex-col space-y-3 slide-in-from-bottom animate-in duration-300">
           ...
           <button
             ...
             className="w-full bg-gradient-to-b from-[#000000] to-[#333333] text-white px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-xl shadow-gray-500/20 active:scale-[0.98] transition-all flex items-center justify-center space-x-3"
           >
           ...
           <button
             ...
             className="w-full bg-white/80 text-[#0066FF] border border-[#0066FF]/30 px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-sm active:scale-[0.98] transition-all flex items-center justify-center space-x-3"
           >
           ...
           <button
             ...
             className="w-full bg-gradient-to-b from-[#34C759] to-[#28A745] text-white px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-xl shadow-green-500/20 active:scale-[0.98] transition-all flex items-center justify-center space-x-3"
           >
        ...
        <>
          ...
          <p className={`text-sm mb-6 font-medium p-3 glass-control border ${status?.toLowerCase()?.includes('fail') || status?.toLowerCase()?.includes('error') || status?.toLowerCase()?.includes('sold out') ? 'bg-red-50/80 backdrop-blur-[30px] saturate-[210%] text-red-800 border-red-200' : 'text-gray-600 border-transparent'}`}>Status: {status}</p>
               ...
               <div className="bg-[rgba(255,255,255,0.65)] backdrop-blur-[40px] saturate-[200%] border border-[rgba(255,255,255,0.4)] rounded-3xl p-6 shadow-2xl max-w-sm w-full text-center">
                 ...
                 <ul className="space-y-2 mb-6">
                   {pendingReconciliation.map((pr, idx) => (
                     <li key={idx} className="text-xs text-gray-800 bg-gray-100/50 p-3 glass-control flex justify-between border border-gray-200">
                 ...
                 <div className="flex flex-col gap-3">
                   <button onClick={() => setPendingReconciliation([])} className="w-full bg-red-100 hover:bg-red-200 text-red-800 font-bold py-3 px-4 glass-control transition-colors active:scale-[0.98] border border-red-200 text-sm">
                     Option A: Refund in-store customer
                   </button>
                   <button onClick={() => setPendingReconciliation([])} className="w-full bg-blue-100 hover:bg-blue-200 text-blue-800 font-bold py-3 px-4 glass-control transition-colors active:scale-[0.98] border border-blue-200 text-sm">
                     Option B: Cancel & refund online order
                   </button>
                   <button onClick={() => setPendingReconciliation([])} className="w-full mt-2 text-gray-500 font-bold py-2 px-4 glass-control hover:bg-gray-100 transition-colors active:scale-[0.98] text-sm">
                     Decide Later
                   </button>
                 </div>
               </div>
          ...
          {selectedMethod === 'tap' && !connectedReader && (
            <div className="mb-4">
              <button onClick={discoverReaders} disabled={typeof window !== 'undefined' && !navigator.onLine} className={`w-full bg-[#0066FF] text-white px-4 py-3 min-h-[44px] glass-control font-bold shadow-md shadow-blue-500/20 active:scale-[0.98] transition-colors ${(typeof window !== 'undefined' && !navigator.onLine) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`}>
                Discover Readers
              </button>
              <ul className="mt-4 space-y-2">
                {discoveredReaders.map(reader => (
                  <li key={reader.id} className="flex justify-between items-center p-4 border border-[rgba(255,255,255,0.4)] translucent-glass-light rounded-2xl bg-[rgba(255,255,255,0.65)] backdrop-blur-[30px] saturate-[210%] shadow-sm transition-all hover:bg-white/80">
                    <span className="font-medium text-gray-800 text-sm">{reader.label || reader.id}</span>
                    <button onClick={() => connectReader(reader)} className="bg-[#34C759] text-white px-5 py-2 min-h-[44px] min-w-[44px] glass-control text-sm font-bold shadow-sm shadow-green-500/20 hover:bg-green-600 transition-colors active:scale-[0.98]">
                      Connect
                    </button>
                  </li>
                ))}
          ...
          {selectedMethod === 'tap' && connectedReader && (
            <div className="mt-4">
              ...
              }} id="tap-to-pay-btn" disabled={reserving || (typeof window !== 'undefined' && !navigator.onLine)} className={`w-full bg-gradient-to-b from-[#0066FF] to-[#0052CC] text-white px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-xl shadow-blue-500/30 transition-all ${reserving || (typeof window !== 'undefined' && !navigator.onLine) ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-blue-500/40 hover:scale-[1.02] active:scale-[0.98]'}`}>
          ...
          {selectedMethod === 'cash' && (
            <div className="mt-4">
               ...
               }} disabled={reserving} className={`w-full bg-gradient-to-b from-[#FF9500] to-[#E58600] text-white px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-xl shadow-orange-500/30 transition-all backdrop-blur-[30px] saturate-[210%] border border-white/20 ${reserving ? 'opacity-50' : 'hover:shadow-orange-500/40 hover:scale-[1.02] active:scale-[0.98]'}`}>
          ...
          {selectedMethod === 'link' && (
            <div className="mt-4">
               ...
               }} disabled={reserving || (typeof window !== 'undefined' && !navigator.onLine)} className={`w-full bg-[#0066FF] text-white px-6 py-4 min-h-[56px] translucent-glass-light rounded-2xl font-bold text-lg shadow-xl shadow-blue-500/30 transition-all ${reserving || (typeof window !== 'undefined' && !navigator.onLine) ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:scale-[0.98]'}`}>
