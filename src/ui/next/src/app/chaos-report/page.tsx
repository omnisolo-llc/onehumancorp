          ...
          <div className="h-64 flex items-end gap-3 p-4 rounded-xl relative" style={{ background: isDarkMode ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.03)' }}>
            ...
            {data?.latencyHistograms?.map((val: number, i: number) => (
              <div key={i} className="flex-1 flex flex-col items-center justify-end group">
                ...
                >
                  <div className={`absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity text-xs font-bold px-2 py-1 rounded ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white backdrop-blur-[30px] backdrop-saturate-[2.1] shadow-sm text-gray-900'}`}>
                    {val}ms
                  </div>
                </div>
