...
function page(relativePath: string): string {
  return readFileSync(join(WORKSPACE_ROOT, "src/app", relativePath), "utf8");
}
...
describe("page rendering resilience", () => {
  ...
  it("keeps handled optional-help failures out of the error console", () => {
    ...
    expect(readFileSync(join(WORKSPACE_ROOT, "src/components/TooltipRegistry.tsx"), "utf8"))
  ...
  it("uses App Router metadata primitives and deterministic render values", () => {
    ...
    expect(readFileSync(join(WORKSPACE_ROOT, "src/components/VoiceAssistant.tsx"), "utf8"))
