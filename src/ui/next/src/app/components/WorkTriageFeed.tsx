...
export function WorkTriageFeed({ items, loading, error, onDecision }: { items: TriageItem[], loading: boolean, error: string, onDecision: (id: string, approved: boolean) => void }) {
  if (error) {
    return (
  ...
  if (!loading && items.length === 0) {
    return (
      <div className="glassmorphism flex flex-col items-center justify-center p-12 text-center border border-white/40 dark:border-white/10 shadow-sm" data-testid="triage-feed-empty">
  ...
  return (
    <div className="w-full max-w-[375px] sm:max-w-full mx-auto overflow-hidden" data-testid="work-triage-feed">
      ...
      {items.filter(item => item.source !== "Proactive Context Agent").map((item) => (
        <div key={item.id} className="mb-6 p-6 rounded-[16px] glassmorphism border border-white/40 dark:border-white/10 shadow-sm overflow-hidden" data-testid={`triage-card-${item.id}`}>
          <div className="flex justify-between items-start mb-3">
            <div>
          ...
          <div className="flex flex-col gap-3 mt-2 w-full">
            <button
              onClick={() => onDecision(item.id, true)}
              className="w-full px-6 py-2.5 min-h-[44px] min-w-[44px] rounded-[16px] bg-[#0066FF] hover:bg-[#0052CC] text-white font-medium shadow-sm transition-colors flex items-center justify-center cursor-pointer"
              data-testid={`triage-approve-${item.id}`}
            >
              ✨ Approve & Execute
            </button>
            <button
              onClick={() => onDecision(item.id, false)}
              className="w-full px-6 py-2.5 min-h-[44px] min-w-[44px] rounded-[16px] bg-white/50 dark:bg-black/30 border border-gray-200 dark:border-gray-700/50 hover:bg-white/80 dark:hover:bg-black/50 text-[#1D1D1F] dark:text-[#F5F5F7] font-medium transition-colors flex items-center justify-center cursor-pointer"
              data-testid={`triage-dismiss-${item.id}`}
            >
              Dismiss
            </button>
