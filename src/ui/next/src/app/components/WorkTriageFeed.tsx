    ...
    >
      {items
        ...
        .map((item) => (
          <div
            key={item.id}
            className="mb-6 p-6 glassmorphism bg-[rgba(255,255,255,0.65)] dark:bg-[rgba(22,22,26,0.7)] backdrop-blur-[30px] saturate-[210%] border border-[rgba(255,255,255,0.4)] dark:border-[rgba(255,255,255,0.1)] rounded-[16px] shadow-lg relative overflow-hidden"
          ...
          >
            ...
            <div className="flex flex-col gap-3 mt-2 w-full">
              <button
                ...
                className="w-full px-6 py-2.5 min-h-[44px] min-w-[44px] bg-[#FF9500] hover:bg-orange-600 text-white font-medium shadow-sm rounded-[8px] transition-all active:scale-[0.98] flex items-center justify-center cursor-pointer"
              ...
              <button
                ...
                className="w-full px-6 py-2.5 min-h-[44px] min-w-[44px] bg-white/50 dark:bg-black/30 border border-orange-200 dark:border-orange-900/30 hover:bg-white/80 dark:hover:bg-black/50 text-orange-900 dark:text-orange-100 font-medium shadow-sm rounded-[8px] transition-all active:scale-[0.98] flex items-center justify-center cursor-pointer glassmorphism"
      ...
      {items
        ...
        .map((item) => (
          <div
            key={item.id}
            className="mb-6 p-6 glassmorphism bg-[rgba(255,255,255,0.65)] dark:bg-[rgba(22,22,26,0.7)] backdrop-blur-[30px] saturate-[210%] border border-[rgba(255,255,255,0.4)] dark:border-[rgba(255,255,255,0.1)] rounded-[16px] shadow-sm overflow-hidden flex flex-col gap-4"
            data-testid={`triage-card-${item.id}`}
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full pt-2">
              <button
                onClick={() => onDecision(item.id, true)}
                className="w-full flex-1 px-6 py-2.5 min-h-[44px] min-w-[44px] bg-[#0066FF] hover:bg-[#0052CC] text-white font-medium shadow-md rounded-[8px] transition-all active:scale-[0.98] flex items-center justify-center cursor-pointer"
                data-testid={`triage-approve-${item.id}`}
              >
                ✨ Approve & Send
              </button>
              <button
                onClick={() => onDecision(item.id, false)}
                className="w-full flex-1 px-6 py-2.5 min-h-[44px] min-w-[44px] bg-white/50 dark:bg-black/20 border border-gray-200 dark:border-white/10 hover:bg-white/80 dark:hover:bg-black/40 text-[#1D1D1F] dark:text-[#F5F5F7] font-medium shadow-sm rounded-[8px] transition-all active:scale-[0.98] flex items-center justify-center cursor-pointer glassmorphism"
                data-testid={`triage-dismiss-${item.id}`}
              >
                Dismiss
              </button>
