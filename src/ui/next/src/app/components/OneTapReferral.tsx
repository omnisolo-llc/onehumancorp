...
export function OneTapReferral({ tenantId, source, style }: { tenantId: string, source: string, style?: React.CSSProperties }) {
  ...
  return (
    <div className="one-tap-referral p-4 bg-indigo-50/50 backdrop-blur-[30px] saturate-[210%] border border-indigo-100 rounded-xl shadow-sm text-center">
      ...
      <p className="text-xs text-indigo-700 mb-3">Invite a friend to OmniSolo and you both get rewarded!</p>
      <div className="flex gap-2 justify-center">
         ...
         <a href={`https://wa.me/?text=${encodeURIComponent(`Start your business on OmniSolo! Use my link: ${referralLink}`)}`} target="_blank" rel="noopener noreferrer" className="flex-1 py-2 px-3 rounded-lg text-sm font-semibold bg-[#25D366] text-white hover:bg-[#20bd5a] transition-all flex items-center justify-center gap-1">
            WhatsApp
         </a>
      </div>
    </div>
