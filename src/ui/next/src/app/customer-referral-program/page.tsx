...
<div style="font-family: sans-serif; text-align: center; font-size: 12px; margin-top: 8px;"><a href="${cloudUrl(`/api/v1/growth/referrals/click?target=/onboarding&ref=${encodeURIComponent(tenant)}`)}" target="_blank" style="color: #6b7280; text-decoration: none; font-weight: 600;">⚡ Powered by OmniSolo</a></div>`);
  ...
  const referralUrl = cloudUrl(`/ref/${encodeURIComponent(tenant.slice(0, 6))}`);
  ...
  const handleShareReferral = (network: 'WhatsApp' | 'X' | 'Facebook') => {
    const message = `Give ${giveAmount}, get ${getAmount} with my referral link: ${referralUrl}`;
    ...
    setShareStatus(`Opened ${network} share`);
  };
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-4 py-3 bg-white/50 dark:bg-black/20 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all dark:text-white"
                    />
                  </div>
                </div>
            ...
            <div className="glassmorphism p-6 md:p-8 rounded-[24px] border border-white/40 dark:border-white/10 shadow-xl bg-white/40 backdrop-blur-[30px] saturate-[210%] relative overflow-hidden group">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 text-center relative">
                ...
                <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 p-2 rounded-lg border border-gray-200 dark:border-gray-700 mb-4">
                  ...
                  <button type="button" onClick={handleCopyReferral} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-md transition-colors">
                    {referralCopied ? 'Copied!' : 'Copy'}
                  </button>
                </div>

                <div className="flex items-center justify-center gap-3 mb-6">
                  <button type="button" aria-label="Share referral on WhatsApp" onClick={() => handleShareReferral('WhatsApp')} className="w-10 h-10 rounded-full bg-[#25D366] text-white flex items-center justify-center hover:opacity-90 transition-opacity">
                  ...
                  <button type="button" aria-label="Share referral on X" onClick={() => handleShareReferral('X')} className="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center hover:opacity-90 transition-opacity">
                     𝕏
                  </button>
                  <button type="button" aria-label="Share referral on Facebook" onClick={() => handleShareReferral('Facebook')} className="w-10 h-10 rounded-full bg-[#0066FF] text-white flex items-center justify-center hover:opacity-90 transition-opacity">
                     <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  </button>
                </div>
                {shareStatus && <p role="status" className="text-xs text-gray-500 dark:text-gray-400">{shareStatus}</p>}
              </div>
            </div>
          </div>
        </div>
