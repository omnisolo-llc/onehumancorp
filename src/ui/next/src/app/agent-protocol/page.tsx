...
export default function AgentProtocolPage() {
  ...
  const [activeTab, setActiveTab] = useState<'steps' | 'artifacts'>('steps');
  ...
  const fetchArtifacts = async (taskId: string) => {
    try {
      const res = await fetch(`/api/agents/protocol?method=ap_list_task_artifacts&task_id=${taskId}`);
          ...
          {!selectedTaskId ? (
            ...
            <>
              <div className="flex border-b border-gray-200 mb-4">
                <button
                  className={`px-4 py-2 font-medium ${activeTab === 'steps' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  onClick={() => setActiveTab('steps')}
                >
                  Steps
                </button>
                <button
                  className={`px-4 py-2 font-medium ${activeTab === 'artifacts' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              ...
              {activeTab === 'artifacts' && (
                <ul className="space-y-2">
                  {artifacts.map((artifact) => (
                    <li key={artifact.artifact_id} className="p-3 border rounded-md border-gray-200">
                      <div className="flex items-center">
                        <span className="text-blue-500 mr-2">📄</span>
                        <span className="font-medium">{artifact.file_name}</span>
                      </div>
                      <div className="text-xs text-gray-500 mt-1">{artifact.relative_path || 'No path specified'}</div>
                    </li>
                  ))}
                  {artifacts.length === 0 && <div className="text-gray-500 text-sm italic">No artifacts found for this task.</div>}
                </ul>
              )}
            </>
          )}
