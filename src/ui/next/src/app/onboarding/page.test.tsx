...
describe('OnboardingWizard', () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn((url: string | URL | Request) => {
      ...
      if (urlStr.includes('/api/onboarding/state')) {
        return Promise.resolve({
          ok: true,
          json: async () => ({}) // Empty state initially
        });
      }
      return Promise.resolve({
        ok: true,
  ...
  it('Handles multi-step successful onboarding flow', async () => {
    ...
    // We need to implement a more complex fetch mock for this test because of the multiple endpoints
    global.fetch = vi.fn((url: string | URL | Request) => {
      const urlStr = url.toString();
      if (urlStr.includes('/api/onboarding/state')) {
        return Promise.resolve({
      ...
      if (urlStr.includes('/api/onboarding/intake')) {
        return Promise.resolve({
          ok: true,
          json: async () => ({
      ...
      if (urlStr.includes('/api/onboarding/start')) {
        return Promise.resolve({
          ok: true,
