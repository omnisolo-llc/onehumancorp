...
describe('OnboardingWizard', () => {
  ...
  it('Handles multi-step successful onboarding flow', async () => {
    ...
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({}) }) // /state mount
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ message: "Success!" }) }) // start
      .mockResolvedValue({ ok: true, json: async () => ({}) }); // subsequent
    act(() => { render(<OnboardingWizard />); });
  ...
  it('Step 1: Handles intake API failure', async () => {
    ...
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({}) }) // /state mount
      .mockResolvedValueOnce({ ok: false }) // intake failure
  ...
  it('Step 3: Handles start API failure and returns to Step 3', async () => {
    ...
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({}) }) // /state mount
      .mockResolvedValueOnce({ ok: false }) // start failure
      .mockResolvedValue({ ok: true, json: async () => ({}) });
