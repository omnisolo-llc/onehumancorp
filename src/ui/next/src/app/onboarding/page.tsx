...
export default function OnboardingWizard() {
  ...
  const handleVisionUpload = async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/agents/vision-onboarding", { method: "POST" });
      const data = await res.json();
      setWhatYouSell(data.extracted_text);
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
              ...
              {chatStep === 1 && (
                <div className="flex flex-col justify-center items-center gap-4 flex-1 animate-fade-in">
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <button onClick={handleVisionUpload} disabled={isLoading} className="w-full bg-gray-100 text-[#0066FF] p-4 rounded-[8px] font-bold border-2 border-dashed border-[#0066FF] mt-4 hover:bg-blue-50 transition-colors flex justify-center items-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                      Upload Menu Photo (Vision Agent)
                    </button>
