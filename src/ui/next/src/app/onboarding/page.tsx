...
export default function OnboardingWizard() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(syncState, 1000); // debounce 1s

    window.addEventListener('beforeunload', syncState);

    return () => {
      clearTimeout(timer);
      window.removeEventListener('beforeunload', syncState);
    };
  }, [
          ...
          {error && (
            <div className="mb-4 bg-[#FF3B30]/10 border border-[#FF3B30]/30 text-[#FF3B30] p-3 rounded-[8px] text-sm">
              {error}
                    ...
                    {step === 1 && <Step1Intake handleIntake={handleIntake} />}
          {step === 2 && <Step2Review validationError={validationError} setValidationError={setValidationError} />}
          {step === 3 && <Step3Style handleStartOnboarding={handleStartOnboarding} />}
          {step === 4 && <Step4Loading />}
          {step === 5 && <Step5Success />}
...
function Step1Intake({ handleIntake }: { handleIntake: () => void }) {
  const {
    chatStep, setChatStep, businessName, setBusinessName,
    whatYouSell, setWhatYouSell, location, setLocation, isLoading
  } = useOnboardingStore();
        ...
        <div className="flex flex-col flex-1 animate-fade-in">
          ...
          <div className="mt-auto pt-6">
            <button
              ...
              className="w-full bg-[#0066FF] text-white p-4 rounded-[8px] font-bold shadow-md hover:bg-[#0052cc] active:scale-[0.98] transition-all duration-250 ease-[cubic-bezier(0.4,0,0.2,1)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {isLoading ? (
...
function Step2Review({ validationError, setValidationError }: { validationError: string, setValidationError: (e: string) => void }) {
  const {
    setStep, businessName, setBusinessName, businessType, setBusinessType,
    categories, setCategories, firstProductName, setFirstProductName,
    firstProductPrice, setFirstProductPrice
  } = useOnboardingStore();
...
function Step3Style({ handleStartOnboarding }: { handleStartOnboarding: () => void }) {
  const {
    setStep, websiteTemplate, setWebsiteTemplate, aiAgents, setAiAgents,
    aiAutoRespond, setAiAutoRespond, isLoading
  } = useOnboardingStore();
