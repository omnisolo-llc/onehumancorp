...
export default function OnboardingWizard() {
  ...
  const handleZeroClickBuild = async () => {
    if (!bio.trim()) {
    ...
    try {
      ...
      const res = await fetchWithRetry(`/api/onboarding/generate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-Tenant-ID": tenantId,
      ...
      if (!res.ok) {
        throw new Error("Failed to generate zero-click onboarding");
      }
      ...
      updateState({ step: 5, startResult: result });
      syncStateToBackend({ step: 5 });
    ...
    <div className="setup-page min-h-screen w-full bg-[#F5F5F7] dark:bg-[#16161a] flex items-center justify-center sm:p-4 font-inter overflow-x-hidden">
      ...
      >
        ...
        <div className="p-6 flex-1 flex flex-col overflow-y-auto custom-scrollbar">
          ...
          {step === -3 && (
            <div className="flex flex-col flex-1 animate-fade-in w-full h-full max-h-full items-center justify-center">
              <button
                onClick={() => {
                  updateState({ step: 0 });
                  syncStateToBackend({ step: 0 });
                }}
                className="absolute top-4 left-4 text-[#0066FF] text-sm font-semibold flex items-center gap-1 min-h-[44px] min-w-[44px] p-2"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              <div className="w-full max-w-sm flex flex-col items-center">
                <h2 className="text-3xl font-bold font-outfit text-[#1D1D1F] dark:text-[#F5F5F7] mb-4 text-center">
                  Zero-Click Build
                </h2>
                <p className="text-gray-500 dark:text-[#A1A1A6] text-sm text-center mb-8 leading-relaxed">
                  Tell us what you do, and we'll build your business engine.
                </p>

                <div className="w-full relative">
                  <textarea
                    autoFocus
                    placeholder="e.g., I'm a baker specializing in custom vegan cakes in Austin..."
                    className="w-full min-h-[120px] p-4 pr-12 text-lg resize-none rounded-[16px] bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-[30px] backdrop-saturate-[210%] border border-white/40 dark:border-white/10 text-[#1D1D1F] dark:text-[#F5F5F7] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0066FF]"
                    value={bio}
                    onChange={(e) => updateState({ bio: e.target.value })}
                  />
                  <button
                    className="absolute bottom-4 right-4 text-[#0066FF] p-2 hover:bg-[#0066FF]/10 rounded-full transition-colors"
                    title="Voice Dictation (Simulated)"
                  >
                    <svg
                      className="w-6 h-6"
                      fill="none"
                ...
                <button
                  onClick={handleZeroClickBuild}
                  disabled={!bio.trim() || isLoading}
                  className="w-full mt-6 bg-[#0066FF] text-white p-4 font-bold shadow-[0_4px_14px_0_rgba(0,102,255,0.39)] hover:bg-[#005bb5] transition-all duration-[250ms] ease-[cubic-bezier(0.4,0,0.2,1)] disabled:opacity-50 disabled:cursor-not-allowed rounded-[16px]"
                >
                  Build My Business
                </button>
                {error && (
          ...
          {step === -2 && (
            <div className="flex flex-col flex-1 animate-fade-in w-full h-full max-h-full">
              ...
              <div className="flex flex-col flex-1 gap-4 overflow-hidden w-full max-w-full">
                <div
                  id="chat-messages"
                  className="bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-[30px] backdrop-saturate-[210%] border border-white/40 dark:border-white/10 rounded-[16px] flex-1 overflow-y-auto p-4 text-[#1D1D1F] dark:text-[#F5F5F7] text-left space-y-4"
                >
          ...
          {step === 4 && (
            <div
              aria-live="polite"
              className="flex flex-col flex-1 justify-center items-center text-center animate-fade-in bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-[30px] backdrop-saturate-[210%] border border-white/40 dark:border-white/10 rounded-[16px] shadow-2xl p-8"
            >
