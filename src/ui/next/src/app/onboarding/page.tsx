...
export default function OnboardingWizard() {
  ...
  const handleIntake = async () => {
    ...
    try {
      ...
      // Call start immediately after intake for 3-step flow
  ...
  const handleStartOnboarding = async (intakeData: any) => {
    ...
    // Default or inferred values since we removed the manual review steps
    const inferredBusinessType = intakeData.business_type || 'Online Store';
    const inferredBusinessName = intakeData.business_name || 'My Business';
    const inferredFirstProductName = intakeData.initial_products?.[0]?.name || 'First Product';
    const inferredFirstProductPrice = intakeData.initial_products?.[0]?.price || '10.00';
    const inferredCategories = intakeData.categories || ['physical'];
    ...
    try {
      ...
      const startRes = await fetch('/api/onboarding/start', {
        ...
        body: JSON.stringify({
          ...
          selling_categories: inferredCategories,
          payment_pref: 'online',
          admin_email: 'admin@ohc.app',
          admin_name: 'Admin',
          admin_password: 'password123',
          website_template: 'Modern', // default
          first_product_name: inferredFirstProductName,
          first_product_price: inferredFirstProductPrice,
      ...
      setStep(1); // Go back to input screen on error
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {step === 1 && (
            <div className="flex flex-col flex-1 justify-center animate-fade-in">
              ...
              <p className="text-gray-500 dark:text-[#A1A1A6] text-sm mb-8">
                Describe what you sell or do in one sentence. Our AI will set up your store automatically.
              </p>
