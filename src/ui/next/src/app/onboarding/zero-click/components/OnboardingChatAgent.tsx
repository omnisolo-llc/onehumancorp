...
export function OnboardingChatAgent({ onComplete }: OnboardingChatAgentProps) {
  ...
  return (
    <div className="flex flex-col min-h-[50vh] glassmorphism overflow-hidden shadow-xl w-full max-w-2xl mx-auto">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[300px] max-h-[500px]">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="glassmorphism rounded-bl-sm px-4 py-3 flex gap-1 items-center">
      ...
      {isProvisioning && (
        <div className="absolute inset-0 z-10 glassmorphism flex flex-col items-center justify-center rounded-[16px]">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {predefinedChips.map((chip, idx) => (
              <button
                ...
                className="text-xs font-medium min-h-[44px] px-4 py-2 flex items-center justify-center glass-control rounded-full hover:border-gray-400 dark:hover:border-gray-500 text-[#1D1D1F] dark:text-[#F5F5F7] transition-colors"
              >
                {chip}
              </button>
            ))}
