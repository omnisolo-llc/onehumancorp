...
interface IntakeData {
  ...
  categories?: string[];
  initial_products?: Array<{ name: string; price: string }>;
}

interface Step4Props {
  intakeData: IntakeData;
  firstProductName: string;
  setFirstProductName: (v: string) => void;
  firstProductPrice: string;
  setFirstProductPrice: (v: string) => void;
  template: string;
...
export default function Step4({
  ...
  const [localProductName, setLocalProductName] = useState(firstProductName || intakeData?.initial_products?.[0]?.name || '');
  const [localProductPrice, setLocalProductPrice] = useState(firstProductPrice || intakeData?.initial_products?.[0]?.price || '');

  useEffect(() => {
    setFirstProductName(localProductName);
  }, [localProductName, setFirstProductName]);

  useEffect(() => {
    setFirstProductPrice(localProductPrice);
  }, [localProductPrice, setFirstProductPrice]);
           ...
           <div className="flex gap-3">
             ...
             <div className="w-24">
               ...
               <input
                 ...
                 onChange={(e) => setLocalProductPrice(e.target.value)}
                 className="w-full p-3 rounded-[10px] border border-white/50 focus:border-[#0066FF] focus:ring-2 focus:ring-[#0066FF]/30 outline-none bg-white/60 backdrop-blur-sm text-gray-900 shadow-inner transition-all"
                 placeholder="0.00"
               />
             </div>
