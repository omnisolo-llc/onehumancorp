            ...
            <form className="mt-6 flex flex-col gap-5" onSubmit={verify}>
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold">
                ...
                <input
                  ...
                  autoFocus
                  className="glassmorphism min-h-[52px] rounded-lg px-4 text-center text-xl tracking-[0.4em]"
                  inputMode="numeric"
                  maxLength={6}
              ...
              <button className="min-h-[54px] rounded-lg bg-[#1D1D1F] p-4 font-bold text-white disabled:opacity-60" disabled={pending || code.length !== 6} type="submit">
                {pending ? "Verifying…" : "Verify email"}
              </button>
            </form>
          ) : (
            <form className="mt-6 flex flex-col gap-5" onSubmit={createAccount}>
              <p className="rounded-lg border border-green-300 bg-green-50 p-3 text-sm text-green-900" role="status">Email verified. Now choose your account credentials.</p>
              <label className="flex flex-col gap-2 text-sm font-semibold">
                Username
                <input autoComplete="username" className="glassmorphism min-h-[52px] rounded-lg px-4" maxLength={32} minLength={3} onChange={(event) => setUsername(event.target.value)} pattern="[A-Za-z0-9][A-Za-z0-9._-]{1,30}[A-Za-z0-9]" required value={username} />
              </label>
              <label className="flex flex-col gap-2 text-sm font-semibold">
                Workspace ID
                <input aria-describedby="workspace-help" autoComplete="organization" className="glassmorphism min-h-[52px] rounded-lg px-4" maxLength={48} minLength={3} onChange={(event) => setOrganizationId(event.target.value)} pattern="[A-Za-z0-9](?:[A-Za-z0-9-]{1,46}[A-Za-z0-9])?" required value={organizationId} />
              </label>
              ...
              <label className="flex flex-col gap-2 text-sm font-semibold">
                Password
                <input aria-describedby="password-help" autoComplete="new-password" className="glassmorphism min-h-[52px] rounded-lg px-4" maxLength={128} minLength={12} onChange={(event) => setPassword(event.target.value)} required type="password" value={password} />
              </label>
              ...
              <button className="min-h-[54px] rounded-lg bg-[#1D1D1F] p-4 font-bold text-white disabled:opacity-60" disabled={pending} type="submit">
