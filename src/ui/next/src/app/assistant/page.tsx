...
const fallbackTasks: AssistantTask[] = [
  {
    ...
    actions: [
      {
        id: "fallback-action-stop",
        label: "Stop",
        kind: "control",
        approvalRequired: false,
      },
      {
        id: "fallback-action-preview",
        label: "Open Preview",
        kind: "preview",
        approvalRequired: false,
      },
      {
        id: "fallback-action-run",
        label: "Run Locally",
        kind: "execute",
...
function PanelButton({
  ...
  return (
    <button
      ...
      className={cx(
        styles.panelButton,
        active ? styles.panelButtonActive : styles.panelButtonIdle,
      )}
    >
      {children}
    </button>
...
export default function AssistantPage() {
  ...
  const [capabilities, setCapabilities] =
    useState<AssistantCapabilities>(defaultCapabilities);
  ...
  const [taskStatusFilter, setTaskStatusFilter] = useState<
    "all" | AssistantTaskStatus
  >("all");
  const [taskDateFilter, setTaskDateFilter] = useState<
    "all" | "today" | "this_week" | "older"
  >("all");
  const [prompt, setPrompt] = useState(
    "Build a weekly research brief with charts",
  );
  ...
  const [constraints, setConstraints] = useState(
    "Ask before sharing or overwriting files",
  );
  ...
  useEffect(() => {
    ...
    async function loadTasks() {
      try {
        ...
        const loadedTasks: AssistantTask[] = data.tasks?.length
          ? data.tasks
          : fallbackTasks;
        if (!mounted) return;
  ...
  const workspaces = useMemo(
    () => Array.from(new Set(tasks.map((task) => task.workspace))),
    [tasks],
  );
  ...
  async function startTask() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || "Task could not be started");
      setTasks((current) => [
        data.task,
  ...
  async function runFeatureAction(action: string) {
    if (action === "upload_remote_file") {
      ...
      await runApiAction(
        `/api/assistant/tasks/${activeTask.id}`,
        "PATCH",
        {
    ...
    <div className={styles.shell} data-testid="assistant-shell">
          ...
          <div className={styles.headerTop}>
            <div>
              ...
              <p className={styles.subtitle}>
                Natural-language workstation for tasks, files, artifacts, remote
                control, automations, memory, skills, and connectors.
              </p>
            </div>
      ...
      <main className={styles.workstation} data-testid="assistant-workstation">
        <aside className={styles.panel} aria-label="Task rail">
          ...
          <div className={styles.taskTools}>
            ...
            <select
              ...
              onChange={(event) =>
                setTaskStatusFilter(
                  event.target.value as "all" | AssistantTaskStatus,
                )
              }
            ...
            <select
              ...
              onChange={(event) =>
                setTaskDateFilter(
                  event.target.value as "all" | "today" | "this_week" | "older",
                )
              }
              className={styles.select}
            >
          ...
          <div className={styles.taskList}>
            {visibleTasks.length === 0 && (
              <p className={styles.emptyText}>No matching tasks.</p>
            )}
            {visibleTasks.map((task) => (
              <button
                ...
                className={cx(
                  styles.taskCard,
                  activeTask.id === task.id && styles.taskCardActive,
                )}
              >
                <div className={styles.metaRow}>
                  <span className={styles.overline}>{task.workspace}</span>
                  <span
                    className={cx(styles.statusBadge, statusClass(task.status))}
                  >
                    {task.status}
                  </span>
                </div>
        ...
        <section className={styles.centerColumn}>
          <section className={styles.panel}>
            ...
            <div className={styles.featureGridThree}>
              {[
                [
                  "Collapse Sidebar",
                  "Hide the task rail for a cleaner conversation view.",
                ],
                [
                  "History",
                  "Jump through previous messages and execution checkpoints.",
                ],
                [
                  "Show Details Panel",
                  "Open artifacts, files, changes, and browser preview beside the chat.",
                ],
                [
                  "File & Image Upload",
                  "Paste screenshots, drag files, or upload context into the thread.",
                ],
                [
                  "Execution Progress",
                  "Show stage descriptions, intermediate steps, and result summaries.",
                ],
                [
                  "Interrupt & Resume",
                  "Stop a running task, add instructions, and continue with context.",
                ],
              ].map(([title, detail]) => (
          ...
          <section className={styles.panel}>
            ...
            <div className={styles.featureGridThree}>
              {[
                ...
                [
                  "Context Tools",
                  "@ references, files, screenshots, and details.",
                ],
                ["Mode Selector", "Switch between work and coding task modes."],
                ["Send Button", "Launch the task or press Enter."],
                [
                  "One-sentence Assignment",
                  "Start with a concise description and let Agent plan.",
                ],
                [
                  "Default Directory",
                  "Use the default task output folder when none is selected.",
                ],
                [
                  "Parallel Work",
                  "Create more tasks or follow-ups while execution continues.",
                ],
              ].map(([title, detail]) => (
                <div key={title} className={styles.resultItem}>
                  <div className={styles.resultTitle}>{title}</div>
            ...
            <div className={styles.fieldGrid}>
              ...
              <div className={styles.formGridFour}>
                ...
                <label className={styles.fieldLabel}>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setContextReferences(event.target.value)
                    }
                    className={styles.input}
                  />
                </label>
              ...
              <div className={styles.composerFooter}>
                <div className={styles.quickChips}>
                  {["@ files", "Screenshot", "Guarded", "Parallel tasks"].map(
                    (item) => (
                      <span key={item} className={styles.formatChip}>
                        {item}
                      </span>
                    ),
                  )}
                  {outputFormat === "Code App" && (
                    <>
                      ...
                      <button
                        type="button"
                        className={cx(styles.smallButton, styles.warningButton)}
                      >
                        Run Locally
                      </button>
                    </>
          ...
          {actionNotice && (
            <div className={styles.resultItem} role="status">
              {actionNotice}
            </div>
          )}
          <FeaturePanel
          ...
          <div className={styles.tabGrid}>
            {resultTabs.map((tab) => (
              <button
                ...
                className={cx(
                  styles.tabButton,
                  resultTab === tab && styles.tabButtonActive,
                )}
              >
                {tab}
              </button>
          ...
          <div className={styles.resultList}>
            <div className={styles.resultItem}>
              ...
              <div className={styles.cardDetail}>
                Generated files and web previews refresh when artifacts change.
              </div>
            </div>
            {[
              [
                "Selected Artifact Preview",
                "Artifact list and preview area stay side by side for review.",
              ],
              [
                "Spreadsheet Preview",
                "Inspect headers, data completeness, and formatting before downloading.",
              ],
              [
                "Document Preview",
                "Review headings, layout, and body text before delivery.",
              ],
              [
                "Web Preview Controls",
                "Track URL, refresh, and rendered page content for local apps.",
              ],
              [
                "All Files Tree",
                "Browse workspace files with open-file tabs and content panes.",
              ],
              [
                "Changes Detail Review",
                "Inspect modified files and change contents before accepting.",
              ],
            ].map(([title, detail]) => (
              <div key={title} className={styles.resultItem}>
                <div className={styles.resultTitle}>{title}</div>
          ...
          <div className={styles.resultActions}>
            {[
              ...
              <button
                key={action}
                type="button"
                onClick={() => runResultAction(action)}
    ...
    return (
      <div className={styles.resultList}>
        {task.changes.length === 0 && (
          <p className={styles.emptyText}>No file changes yet.</p>
        )}
        {task.changes.map((change) => (
          <div key={change.id} className={styles.resultItem}>
            <div className={styles.resultTitle}>{change.summary}</div>
            <div className={cx(styles.statusBadge, styles.warningButton)}>
    ...
    <div className={styles.resultList}>
      {task.artifacts.length === 0 && (
        <p className={styles.emptyText}>
          Preview appears after the first artifact.
        </p>
      )}
      {task.artifacts.map((artifact) => (
        <div key={artifact.id} className={styles.resultItem}>
...
function FeaturePanel({
  ...
  if (panel === "remote") {
    return (
      <section className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[...capabilities.remotePlatforms, "File/Image Upload"].map(
            (platform) => (
              <div key={platform} className={styles.featureCard}>
                <div className={styles.cardTitle}>{platform}</div>
                <div className={styles.cardDetail}>
                  {platform === "File/Image Upload"
                    ? "Remote bots can attach files and images to tasks."
                    : "Connected task intake"}
                </div>
              </div>
            ),
          )}
        ...
        <div className={styles.featureGridThree}>
          {[
            ...
            [
              ...
              "DingTalk long-lived remote control channel.",
            ],
            [
              "URL Callback",
              "DingTalk callback mode with AES key and token validation.",
            ],
            [
              "Pairing Code",
              "Bind Slack and chat channels by sending a one-time code.",
            ],
            [
              "QR Code Linking",
              "Bind WeChat ClawBot by scanning the local setup QR code.",
            ],
            [
              "Credential Fields",
              "Show the exact token, secret, intent, AES, and callback fields required by each platform.",
            ],
            [
              "Troubleshooting Catalog",
              "Surface platform-specific reconnect, token, permission, and callback fixes.",
            ],
          ].map(([title, detail]) => (
            <div
              key={title}
              className={cx(styles.featureCard, styles.featureCardAccent)}
            >
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("upload_remote_file")}
            className={styles.smallButton}
          >
            Upload Remote File
          </button>
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.featureGridTwo}>
          ...
          {[
            [
              "Execution history",
              "Runs, approvals, outputs, and notifications",
            ],
            [
              "One-time task",
              "Schedule a single future run without binding a permanent workspace.",
            ],
            [
              "List mode",
              "Scan active, paused, and completed automations in a compact list.",
            ],
          ].map(([title, detail]) => (
  ...
  if (panel === "memory") {
    return (
      <section className={styles.panel}>
        ...
        <div className={styles.resultList}>
          <div className={styles.resultItem}>
            Prefer concise technical summaries with citations.
          </div>
          <div className={styles.resultItem}>
            Ask before sending messages or modifying original files.
          </div>
        </div>
        <button type="button" className={styles.smallButton}>
          Import Memory
    ...
    return (
      <section className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            ...
            <div key={skill} className={styles.featureCard}>
              ...
              <div className={styles.cardDetail}>
                {skill.includes("Batch")
                  ? "Update all marketplace skills at once."
                  : skill.includes("Generated")
                    ? "skill.yml, README, and implementation files."
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("summon_expert")}
            className={styles.smallButton}
          >
            Summon Expert
          </button>
      ...
      <section aria-label="Connector panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            [
              "OAuth Flow",
              "Google Calendar, Drive, Gmail, and workspace connectors support OAuth setup.",
            ],
            [
              "Calendar Events",
              "Read meeting context and scheduling metadata for tasks.",
            ],
            [
              "Drive Context",
              "Select files and folders as assistant task references.",
            ],
          ].map(([title, detail]) => (
            <div
              key={title}
      ...
      <section aria-label="Data management panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            ...
            [
              "Copy Share Link",
              "Copy the current artifact share URL after review.",
            ],
            [
              "Download Shared File",
              "Prepare a downloadable file URL for the current artifact.",
            ],
            [
              "Cancel Sharing",
              "Remove a shared file from public access and revoke its link.",
            ],
            [
              "Unarchive Task",
            ...
            [
              "Pinned Tasks",
              "Keep important tasks at the top of the sidebar for quick access.",
            ],
            [
              "Workspace Management",
              "Search, filter, collapse, pin, archive, and organize tasks by workspace.",
            ],
            [
              "Feedback Product Team Route",
              "Send issues through Help & Feedback with screenshots and logs.",
            ],
            ["Collapse All", "Fold every workspace and task group."],
            ["Expand All", "Open every workspace and task group."],
            [
              "Hard Delete",
              "Permanently remove selected workspace or task records.",
            ],
          ].map(([title, detail]) => (
            <div key={title} className={styles.featureCard}>
              <div className={styles.cardTitle}>{title}</div>
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("copy_share_link")}
            className={styles.smallButton}
          >
            Copy Current Link
          </button>
          <button
            type="button"
            onClick={() => onAction("download_shared_file")}
            className={styles.smallButton}
          >
            Prepare Download
          </button>
          <button
            type="button"
            onClick={() => onAction("cancel_sharing")}
            className={styles.smallButton}
          >
            Revoke Share
          </button>
          <button
            type="button"
            onClick={() => onAction("unarchive_task")}
            className={styles.smallButton}
          >
            Restore Task
          </button>
          <button
            type="button"
            onClick={() => onAction("collapse_workspaces")}
            className={styles.smallButton}
          >
      ...
      <section aria-label="Explore panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            [
              "Community Templates",
              "Browse shared task patterns and try them against your own workspace.",
            ],
            [
              "Investor Update Agent",
              "Remix metrics, notes, and tasks into a private Agent agent.",
            ],
            [
              "Local File Cleanup Agent",
              "Adapt batch rename, conversion, and PDF merge workflows safely.",
            ],
            [
              "Research Deck Agent",
              "Turn public research workflows into cited decks and charts.",
            ],
            [
              "Remix as Agent Agent",
              "Copy a useful workflow into your own workspace with attribution.",
            ],
            ["Review & Share", "Share your remixed result only after review."],
            [
              "File Content Recognition",
              "Recognize and summarize uploaded local files.",
            ],
            [
              "Document Generation & Editing",
              "Draft and revise documents from task context.",
            ],
            [
              "Data Analysis & Visualization",
              "Analyze tables and produce charts or forecasts.",
            ],
            [
              "Social Media Content Creation",
              "Create content variants for Twitter/X, LinkedIn, YouTube, and Medium.",
            ],
            [
              "Automated Daily News Briefing",
              "Build recurring briefings from search and remote channels.",
            ],
            [
              "Remote Control via Slack",
              "Operate and confirm assistant tasks from Slack.",
            ],
            [
              "Google Calendar & Drive Integration",
              "Combine meeting and file context through Google OAuth.",
            ],
            [
              "Zero-Code Local Application Development",
              "Generate runnable local apps from natural language.",
            ],
            [
              "Creating Custom Skills",
              "Create skill.yml, README, and implementation files.",
            ],
            [
              "AI Self-Driven Workflows",
              "Let an agent plan, execute, inspect, and continue autonomously.",
            ],
            [
              "Make My Version",
              "Personalize an official practice case into your own Agent workflow.",
            ],
          ].map(([title, detail]) => (
            <div key={title} className={styles.featureCard}>
              <div className={styles.cardTitle}>{title}</div>
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("remix_template")}
            className={styles.smallButton}
          >
            Remix Template
          </button>
          <button
            type="button"
            onClick={() => onAction("share_exploration")}
            className={styles.smallButton}
          >
            Share Exploration
          </button>
      ...
      <section aria-label="Cloud runtime panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            [
              "Cloud Agent",
              "Run long tasks asynchronously with the same Agent task contract.",
            ],
            [
              "Background Session",
              "Keep research, generation, and file prep active while you leave the page.",
            ],
            [
              "Uploaded Files",
              "Attach local files, workspace files, and screenshots to cloud runs.",
            ],
            [
              "Pause/Resume",
              "Pause or resume background execution without deleting the task.",
            ],
            [
              "Task Search",
              "Find cloud and local tasks from the same workspace list.",
            ],
            [
              "Runtime Manifest",
              "Track isolation, files, model, and session state before actions.",
            ],
          ].map(([title, detail]) => (
            <div key={title} className={styles.featureCard}>
              <div className={styles.cardTitle}>{title}</div>
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("start_cloud_session")}
            className={styles.smallButton}
          >
            Start Cloud Session
          </button>
          <button
            type="button"
            onClick={() => onAction("pause_cloud_session")}
            className={cx(styles.smallButton, styles.warningButton)}
          >
            Pause Cloud Session
          </button>
  ...
  if (panel === "parity") {
    const summary = capabilities.paritySummary || {
      total: 50,
      implemented: 50,
      remaining: 0,
    };
        ...
        <div className={styles.featureGridThree}>
          {categories.map((category) => (
            <div key={category} className={styles.featureCard}>
              ...
              <div className={styles.cardDetail}>
                Implemented in Agent assistant surfaces and API contracts.
              </div>
            </div>
          ))}
        </div>
        <div className={styles.featureGridThree}>
          {highlights.map((highlight) => (
            ...
            >
              ...
              <div className={styles.cardDetail}>
                Gap closed and tracked in the parity registry.
              </div>
            </div>
          ))}
        </div>
      ...
      <section aria-label="Models and runtime panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {capabilities.modelProviders.map((provider) => (
            <div key={provider} className={styles.featureCard}>
              ...
              <div className={styles.cardDetail}>
                Selectable per task or automation.
              </div>
            </div>
          ))}
        </div>
        <div className={styles.featureGridThree}>
          {[
            [
              "Provider Presets",
              "Auto-fill model URL, list, and capability flags for standard providers.",
            ],
            [
              "Custom Model UI",
              "Configure API keys, endpoints, headers, and model parameters visually.",
            ],
            [
              "Model Capability Flags",
              `Track ${capabilities.modelCapabilities?.join(", ") || "tool calling, image input, reasoning, and local inference"}.`,
            ],
            [
              "Custom Protocol",
              "Send requests directly to a non-standard endpoint URL when advanced settings require it.",
            ],
            [
              "Runtime Detection",
              "Detect Python and Node.js availability before running local skills.",
            ],
            [
              "System Proxy",
              "Use system proxy settings for model and connector calls.",
            ],
          ].map(([title, detail]) => (
            <div
              key={title}
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("save_custom_model")}
            className={styles.smallButton}
          >
            Save Custom Model
          </button>
      ...
      <section aria-label="System and safety panel" className={styles.panel}>
        ...
        <div className={styles.featureGridThree}>
          {[
            [
              "Plugin Marketplace",
              "Install, update, try, and uninstall skill or suite packages with security checks.",
            ],
            [
              "Claw Setup",
              "Connect or disconnect mobile bot channels and confirm remote commands.",
            ],
            [
              "High-risk Confirmations",
              "Review external sends, destructive changes, and full-access operations.",
            ],
            [
              "UI Settings",
              "Adjust font size, language, generated-content markers, and filter handling.",
            ],
            [
              "Compact Mode",
              "Collapse decorative chat details and tool-call chrome for dense task work.",
            ],
            [
              "Auto-install Low-risk Skills",
              "Continue non-high-risk skill installs after security scan approval.",
            ],
            [
              "Prevent Sleep",
              "Keep remote control and automation sessions running without the machine sleeping.",
            ],
            [
              "Account Profile",
              "Show avatar, account details, and OAuth sign-in providers from the sidebar.",
            ],
            [
              "Version Information",
              "Expose the current Agent parity/version build in the sidebar footer.",
            ],
            [
              "Feedback & Logs",
              "Send product feedback and upload diagnostic logs for support.",
            ],
            [
              "Screenshot Attachment",
              "Attach a screenshot with feedback so support can inspect visual issues.",
            ],
            [
              "Desktop Platform Support",
              "macOS Apple Silicon, macOS Intel, Windows x64, and Windows ARM64 support matrix.",
            ],
            [
              "Online Requirement",
              "Account, connector, and remote-control features require online service access.",
            ],
            [
              "Multi-device Sync",
              "Account settings sync across signed-in desktop installations.",
            ],
            [
              "Log Folder Locations",
              "Open Log Folder and Open Log Directory entries expose diagnostics paths.",
            ],
            [
              "Windows Installation Guide",
              "Windows 10 1809 or later, Windows 11, x64, ARM64, and .exe installer flow.",
            ],
            [
              "macOS Installation Guide",
              "Apple Silicon and Intel installation through a .dmg package.",
            ],
            [
              "Universal Binary",
              "macOS package supports Apple Silicon and Intel processors.",
            ],
            [
              "Windows Defender SmartScreen",
              "Installation troubleshooting covers SmartScreen prompts and first-launch delays.",
            ],
            [
              "Privacy & Security Permission",
              "macOS remote-control setup points users to System Settings -> Privacy & Security.",
            ],
            [
              "Privacy Retention Matrix",
              "Inputs and outputs retain for 14 days; configuration stays on the local device.",
            ],
            [
              "Data Subject Rights",
              "Access, portability, correction, erasure, restriction, objection, and consent withdrawal.",
            ],
            [
              "AI Training Opt-out",
              "Inputs and outputs training opt-out route is agent_ai@tencent.com.",
            ],
            [
              "Billing Retention",
              "Payment and billing information retention is tracked for 24 months.",
            ],
          ].map(([title, detail]) => (
            <div key={title} className={styles.featureCard}>
              <div className={styles.cardTitle}>{title}</div>
        ...
        <div className={styles.actionRow}>
          <button
            type="button"
            onClick={() => onAction("install_plugin")}
            className={styles.smallButton}
          >
          ...
          <button
            type="button"
            onClick={() => onAction("create_approval")}
            className={cx(styles.smallButton, styles.warningButton)}
          >
          ...
          <button
            type="button"
            onClick={() => onAction("save_ui_settings")}
            className={styles.smallButton}
          >
            Save UI Settings
          </button>
      ...
      <div className={styles.featureGridThree}>
        {[
          ...
          [
            ...
            "Require explicit confirmation before fully opening permissions.",
          ],
          ...(
            capabilities.computerUseModes || ["Normal", "Auto", "Full Access"]
          ).map((mode) => [
            `${mode} computer use`,
            "Agent-style computer operation scope.",
          ]),
        ].map(([title, detail]) => (
          <div key={title} className={styles.featureCard}>
            <div className={styles.cardTitle}>{title}</div>
