  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("/api/onboarding/draft", {
        headers: { "X-Tenant-ID": savedTenantId, "X-User-ID": userId },
      })
      ...
      .then(([draftData, stateData]) => {
        ...
        if (
          data &&
          data.wizardState &&
          Object.keys(data.wizardState).length > 0
        ) {
          let localState: any = null;
          try {
            const localStr = localStorage.getItem("builder-storage");
            if (localStr) {
          ...
          const localStep =
            typeof localState?.wizardStep === "number"
              ? localState.wizardStep
              : 0;
          const localName =
            typeof localState?.businessName === "string"
              ? localState.businessName
              : "";

          const backendStep =
            data.wizardState.step !== undefined
              ? data.wizardState.step
              : data.wizardState.wizardStep !== undefined
                ? data.wizardState.wizardStep
                : 0;
          const backendName =
            typeof data.wizardState.businessName === "string"
              ? data.wizardState.businessName
              : "";

          if (
            backendStep > localStep ||
            (backendStep === localStep &&
              backendName.length >= localName.length)
          ) {
            if (data.wizardState.step !== undefined)
              setWizardStep(data.wizardState.step);
            if (data.wizardState.wizardStep !== undefined)
              setWizardStep(data.wizardState.wizardStep);
            if (data.wizardState.businessGoal !== undefined)
              setBusinessGoal(data.wizardState.businessGoal);
          }
        }
      })
    ...
    try {
      ...
      newBlocks.push({
        ...
        props: {
          ...
          offerDescription:
            "Get 20% off your next purchase when a friend buys from us!",
        },
      });
      ...
      if (draft2[0] && draft2[0].props)
        draft2[0].props.headline += " (Variant B)";

      const draft3 = JSON.parse(JSON.stringify(newBlocks));
      if (draft3[0] && draft3[0].props)
        draft3[0].props.headline += " (Variant C)";
      ...
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50 dark:bg-[#000] font-inter overflow-hidden">
        <div className="relative w-[375px] h-[812px] sm:h-[812px] min-h-[100dvh] sm:min-h-auto flex flex-col overflow-hidden sm:rounded-[16px] glassmorphism shadow-2xl">
          <div className="px-8 pt-12 pb-6 text-center">
            <h1 className="text-2xl font-extrabold font-outfit text-gray-900 mb-2">
              Pick your draft
            </h1>
            <p className="text-sm text-gray-500">
              The Architect generated 3 options for you.
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 space-y-6 pb-24">
            {drafts.map((d, idx) => (
              <button
                ...
                className={`w-full text-left glassmorphism backdrop-blur-md rounded-[16px] border-2 transition-all duration-[250ms] ease-[cubic-bezier(0.4,0,0.2,1)] overflow-hidden ${selectedDraftIndex === idx ? "border-[#0066FF] ring-2 ring-[#0066FF]/20 shadow-lg" : "border-white/50 dark:border-white/10 opacity-70 hover:opacity-100 hover:border-white/80"}`}
              >
                <div className="h-32 bg-white/50 dark:bg-black/30 flex items-center justify-center relative backdrop-blur-sm border-b border-white/40 dark:border-white/10">
                  <span className="font-outfit font-bold text-gray-400 dark:text-gray-500">
                    Draft {idx + 1}
                  </span>
                ...
                <div className="p-4 bg-white/60 dark:bg-black/40 backdrop-blur-md">
                  <p className="text-xs font-bold text-gray-500 dark:text-[#A1A1A6] uppercase tracking-wider mb-1">
                    Preview
                  </p>
                  <p className="text-sm text-[#1D1D1F] dark:text-[#F5F5F7] line-clamp-1 font-inter">
                    {d[0]?.props?.headline || "Storefront Preview"}
                  </p>
                </div>
              </button>
            ))}
      ...
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50 dark:bg-[#000] font-inter overflow-hidden">
        <div className="relative w-[375px] h-[812px] sm:h-[812px] min-h-[100dvh] sm:min-h-auto flex flex-col overflow-hidden sm:rounded-[16px] glassmorphism shadow-2xl">
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
            <div
              className="glassmorphism backdrop-blur-md rounded-[16px] border border-white/50 dark:border-white/10 shadow-sm p-8 w-full animate-fade-in"
              ...
              <div className="space-y-4">
                {[
                  ...
                  >
                    ...
                    <span className="text-lg font-bold font-outfit text-[#1D1D1F] dark:text-[#F5F5F7]">
                      {option.label}
                    </span>
                  </button>
                ))}
          ...
          <div className="px-8 pt-12 pb-4">
            <div className="flex justify-between mb-8">
              {[1, 2, 3].map((step) => (
                <div
                  key={step}
                  className={`h-1.5 flex-1 mx-1 rounded-full ${step <= wizardStep ? "bg-[#0066FF]" : "bg-gray-200 dark:bg-gray-700"}`}
                  style={{
          ...
          <div className="px-8 pb-8 flex flex-col flex-1 justify-start overflow-y-auto">
            {wizardStep === 1 && (
              ...
              >
                ...
                <p className="text-gray-500 dark:text-[#a1a1a6] text-sm mb-8 leading-relaxed">
                  Start with the basics. What's your business called, and what
                  do you do?
                </p>

                <label className="text-sm font-semibold text-gray-700 dark:text-[#a1a1a6] mb-2 block text-left">
                  Business Name
                </label>
                <input
                  type="text"
                ...
                {wizardStep1Error && (
                  <p className="text-[#FF3B30] text-sm mb-4 text-left">
                    {wizardStep1Error}
                  </p>
                )}
                ...
                <button
                  className={`w-full p-4 font-bold font-outfit text-lg transition-all duration-[250ms] ease-[cubic-bezier(0.4,0,0.2,1)] ${
                    ...
                    businessCategory.trim().length >= 5
            ...
            {wizardStep === 2 && (
              ...
              >
                ...
                <p className="text-gray-500 dark:text-[#a1a1a6] text-sm mb-8 leading-relaxed">
                  How should your store feel? Our AI agents will match this
                  tone.
                </p>

                <div className="grid gap-4 mb-8">
                  {["Professional", "Friendly", "Energetic", "Minimalist"].map(
                    (v) => (
                      <button
                        key={v}
                        onClick={() => setVibe(v)}
                        className={`p-4 border text-left transition-all duration-[250ms] ease-[cubic-bezier(0.4,0,0.2,1)] font-semibold backdrop-blur-md ${
                          vibe === v
                            ...
                            : "border-white/50 dark:border-white/10 text-gray-700 dark:text-gray-300 hover:border-white/80 dark:hover:border-white/20 glassmorphism"
                        }`}
                        style={{ borderRadius: "8px" }}
                      >
                        {v}
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!bio.trim()) {
                        setBio(
                          `I run a ${businessCategory} business called ${businessName}. We want a ${vibe.toLowerCase()} vibe.`,
                        );
                      }
                      setWizardStep(3);
                    }}
        ...
        <div className="relative w-[375px] h-[812px] sm:h-[812px] min-h-[100dvh] sm:min-h-auto flex flex-col overflow-hidden sm:rounded-[16px] glassmorphism shadow-2xl">
          <div className="px-8 pt-20 pb-4 text-center">
            ...
            <p className="text-sm text-gray-500 animate-pulse">
              Designing your custom storefront...
            </p>
          </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50 dark:bg-[#000] font-inter">
        <div className="relative w-[375px] h-[812px] sm:h-[812px] min-h-[100dvh] sm:min-h-auto flex flex-col items-center overflow-x-hidden overflow-y-auto hide-scrollbar sm:rounded-[16px] glassmorphism shadow-2xl px-6 pt-12 pb-8">
          ...
          <h1 className="text-3xl font-extrabold font-outfit text-gray-900 mb-2 tracking-tight">
            You're Live!
          </h1>
          <p className="text-gray-500 mb-8 text-sm max-w-[240px]">
            Your business is now open to the world. Scan the code to see it.
          </p>
          ...
          <div className="w-full glassmorphism backdrop-blur-md rounded-[16px] border border-white/50 dark:border-white/10 shadow-sm p-5 mb-4 text-left">
            <h2 className="text-lg font-bold font-outfit text-[#1D1D1F] dark:text-[#F5F5F7] mb-1">
              Sell Anywhere 💻
            </h2>
            <p className="text-xs text-gray-500 dark:text-[#A1A1A6] mb-4">
              Embed your OHC storefront on your existing website, blog, or
              partner pages.
            </p>
...
<div style="text-align: center; margin-top: 8px; font-family: sans-serif; font-size: 11px;">
  <a href="/onboarding?ref=${tenantId}" style="color: #646b78; text-decoration: none;">Powered by <b>OHC</b></a>
          ...
          <div className="w-full glassmorphism backdrop-blur-md rounded-[16px] border border-white/50 dark:border-white/10 shadow-sm p-5 mb-4 text-left">
            ...
            <p className="text-xs text-gray-500 dark:text-[#A1A1A6] mb-4">
              Share your new store with friends and family to get early sales.
            </p>
          ...
          <div className="w-full bg-blue-50/50 dark:bg-blue-900/20 backdrop-blur-md border border-[#0066FF]/30 dark:border-[#0066FF]/20 shadow-sm p-5 mb-6 text-left rounded-[16px]">
            <h2 className="text-lg font-bold font-outfit text-[#0066FF] dark:text-blue-300 mb-1">
              Generative Visibility Score (GEO)
            </h2>
            <p className="text-xs text-blue-700 dark:text-blue-200 mb-4">
              Improve how LLM crawlers like ChatGPT or Gemini see your business.
            </p>
            ...
            {geoScore === null ? (
              ...
              <div className="animate-fade-in">
                <div className="flex items-end gap-2 mb-3">
                  ...
                  <span className="text-sm font-medium text-blue-600 pb-1">
                    / 100
                  </span>
                </div>
                {geoRecs.length > 0 && (
      ...
      <div className="relative w-[375px] h-[812px] sm:h-[812px] min-h-[100dvh] sm:min-h-auto flex flex-col overflow-hidden sm:rounded-[16px] glassmorphism shadow-2xl">
          ...
          <div className="space-y-4 font-inter">
            {blocks[selectedBlockIndex || 0]?.type === "Hero" && (
              <>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const newBlocks = [...blocks];
                    newBlocks[selectedBlockIndex || 0].props.headline =
                      e.target.value;
                    setBlocks(newBlocks);
                  }}
                />
            ...
            {blocks[selectedBlockIndex || 0]?.type !== "Hero" && (
              <p className="text-sm text-gray-500 italic">
                ...
                {blocks[selectedBlockIndex || 0]?.type} coming soon...
              </p>
            )}
            <button
        ...
        <div className="absolute bottom-0 w-full p-4 glassmorphism border-t border-white/40 dark:border-white/10 z-50">
          <div className="flex gap-3 mb-2">
            <WithTooltip
              id="change-vibe-tooltip"
              defaultText="Change the theme and colors of your website."
            >
            ...
            {!isPremium && (
              <WithTooltip
                id="remove-branding-tooltip"
                defaultText="Upgrade to Premium to remove OHC branding."
              >
                <button
          ...
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-[60] flex flex-col justify-end">
            <div className="bg-white/90 dark:bg-[#16161a]/90 backdrop-blur-xl w-full rounded-t-[16px] p-6 shadow-2xl animate-slide-up pb-10 border-t border-white/40 dark:border-white/10">
              ...
              <p className="text-gray-500 dark:text-[#A1A1A6] mb-6 font-inter text-sm leading-relaxed">
                Unlock white-labeling, custom domains, and advanced analytics to
