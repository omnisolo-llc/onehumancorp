  ...
  useEffect(() => {
    ...
    .then(data => {
      if (data && data.wizardState) {
        if (data.wizardState.wizardStep) setWizardStep(data.wizardState.wizardStep);
        if (data.wizardState.businessName) setBusinessName(data.wizardState.businessName);
        if (data.wizardState.businessCategory) setBusinessCategory(data.wizardState.businessCategory);
        if (data.wizardState.vibe) setVibe(data.wizardState.vibe);
        if (data.wizardState.bio) setBio(data.wizardState.bio);
        if (data.wizardState.status) setStatus(data.wizardState.status);
        if (data.wizardState.liveUrl) setLiveUrl(data.wizardState.liveUrl);
        if (data.wizardState.blocks) setBlocks(data.wizardState.blocks);
      }
    })
    .catch(err => console.error('Failed to load wizard state', err));
  }, []);

  useEffect(() => {
    // Save state back to server when it changes
    if (status !== 'onboarding' || businessName !== '' || businessCategory !== '' || vibe !== '' || bio !== '' || blocks.length > 0) {
      const payload = {
        wizardState: { wizardStep, businessName, businessCategory, vibe, bio, status, liveUrl, blocks }
      };
      ...
      const timer = setTimeout(() => {
        ...
        }).catch(err => console.error('Failed to sync wizard state', err));
      }, 1000); // debounce 1s

      return () => clearTimeout(timer);
    }
  }, [wizardStep, businessName, businessCategory, vibe, bio, status, liveUrl, blocks, tenantId]);
        ...
        <div className="relative w-[375px] h-[812px] bg-white shadow-2xl overflow-hidden flex flex-col border-x border-gray-200">
           ...
           <div className="absolute bottom-0 w-full p-6 bg-white/90 backdrop-blur-md border-t border-gray-200">
              <button
                ...
                className="w-full bg-[#0071E3] text-white p-4 rounded-[8px] font-bold font-outfit shadow-lg active:scale-[0.98] transition-all"
              >
  ...
  if (status === "idle") {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-gray-50 font-inter">
        <div className="w-[375px] h-[812px] bg-white shadow-2xl overflow-hidden flex flex-col relative border-x border-gray-200 mac-glass-container"
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (businessCategory.trim().length < 3) {
                      setValidationError("Category must be at least 3 characters.");
                      return;
                    }
                    setValidationError("");
                    setWizardStep(2);
            ...
            {wizardStep === 2 && (
              <div className="animate-fade-in" style={{ animation: 'fadeIn 250ms cubic-bezier(0.4, 0, 0.2, 1)' }}>
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    className={`flex-1 p-4 font-bold font-outfit text-lg transition-all rounded-[8px] ${
                      vibe
                        ? "text-white shadow-md active:scale-[0.98] bg-[#0071E3]"
                        : "bg-gray-100 text-gray-400 cursor-not-allowed"
                    }`}
...
<div style="text-align: center; margin-top: 8px; font-family: sans-serif; font-size: 11px;">
          ...
          <div className="w-full bg-white rounded-[16px] border border-gray-200 shadow-sm p-5 mb-4 text-left">
            ...
            <div className="flex gap-3">
              <a
                ...
                className="flex-1 bg-[#34C759] text-white flex items-center justify-center gap-2 p-3 rounded-[8px] font-semibold text-sm shadow-sm hover:bg-[#20bd5a] transition-all"
              >
          ...
          <div className="w-full bg-blue-50 border border-blue-200 shadow-sm p-5 mb-6 text-left rounded-[16px]">
            ...
            {geoScore === null ? (
              <button
                ...
                className="w-full bg-[#0071E3] text-white font-semibold py-2 rounded-[8px] text-sm shadow-sm hover:bg-blue-700 transition-all"
              >
                Analyze Visibility
              </button>
            ) : (
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-50 font-inter">
      <div className="w-[375px] h-[812px] bg-white shadow-2xl flex flex-col relative border-x border-gray-200 overflow-hidden mac-glass-container" style={{ borderRadius: '16px' }}>
          ...
          <div className="space-y-4 font-inter">
            {blocks[selectedBlockIndex || 0]?.type === 'Hero' && (
              <>
                ...
                <input
                  type="text"
                  className="w-full p-4 bg-gray-50 rounded-[8px] border-none focus:ring-2 focus:ring-[#0071E3] transition-all"
        ...
        <div className="absolute bottom-0 w-full p-4 bg-white/90 backdrop-blur-md border-t border-gray-200 z-50">
          <div className="flex gap-3 mb-2">
            ...
            {!isPremium && (
              <button
                className="flex-1 py-2 text-sm font-medium text-[#0071E3] bg-blue-50 rounded-[8px] border border-blue-100"
          ...
          <WithTooltip id="launch-btn-tooltip" defaultText="Launch your storefront immediately to a live URL.">
            <button
              id="launch-btn"
              className="w-full bg-[#34C759] text-white p-4 rounded-[8px] font-bold shadow-lg hover:bg-[#20bd5a] active:scale-[0.98] transition-all flex justify-center items-center gap-2"
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .mac-glass-container { background: rgba(255, 255, 255, 0.65); backdrop-filter: blur(30px) saturate(210%); -webkit-backdrop-filter: blur(30px) saturate(210%); border: 1px solid rgba(255, 255, 255, 0.4); border-radius: 16px; }
        @media (prefers-color-scheme: dark) {
          ...
          .mac-glass-container { background: rgba(22, 22, 26, 0.7); backdrop-filter: blur(30px) saturate(210%); -webkit-backdrop-filter: blur(30px) saturate(210%); border: 1px solid rgba(255, 255, 255, 0.1); }
        }
      `}} />
    </div>
  );
