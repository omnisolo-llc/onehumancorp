...
export function SkeletonBlock() {
  return (
    <div className="w-full p-6 animate-pulse">
      <div className="h-40 bg-white/40 dark:bg-black/20 backdrop-blur-[20px] backdrop-saturate-200 rounded-[16px] mb-4 border border-white/50 dark:border-white/10" />
      <div className="h-4 w-3/4 bg-white/40 dark:bg-black/20 backdrop-blur-[20px] backdrop-saturate-200 rounded-[8px] mb-2 border border-white/50 dark:border-white/10" />
      <div className="h-4 w-1/2 bg-white/40 dark:bg-black/20 backdrop-blur-[20px] backdrop-saturate-200 rounded-[8px] border border-white/50 dark:border-white/10" />
    </div>
  );
}
...
export function ActionSheet({ isOpen, onClose, title, children }: { isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="absolute inset-0 z-[100] flex flex-col justify-end">
      ...
      <div className="bg-white/90 dark:bg-[#16161a]/90 backdrop-blur-[20px] backdrop-saturate-200 border-t border-white/40 dark:border-white/10 w-full rounded-t-[16px] p-6 shadow-2xl animate-slide-up relative z-10">
...
export function QRCode({ value }: { value: string }) {
  return (
    <div className="bg-white/40 dark:bg-black/20 backdrop-blur-[20px] backdrop-saturate-200 p-4 rounded-[16px] shadow-sm border border-white/50 dark:border-white/10 inline-block">
...
export function SmartBlock({ type, props }: { type: string; props: any }) {
  if (type === "Hero") {
    return (
      <div className="relative w-full overflow-hidden bg-white/20 dark:bg-black/20 min-w-[375px]">
        ...
        <div className="relative z-10 p-6 flex flex-col items-center justify-center min-h-[300px] text-center text-white m-4 rounded-[16px] backdrop-blur-[20px] backdrop-saturate-200 bg-white/20 border border-white/40 shadow-lg">
      ...
      <div className="p-6 bg-transparent font-inter min-w-[375px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {props.items.map((item: any, i: number) => (
            <div key={i} className="backdrop-blur-[20px] backdrop-saturate-200 bg-white/40 dark:bg-black/20 border border-white/50 dark:border-white/10 shadow-sm p-4 rounded-[16px] flex flex-col">
  ...
  if (type === "Booking") {
    return (
      <div className="p-6 bg-transparent font-inter min-w-[375px]">
        <div className="backdrop-blur-[20px] backdrop-saturate-200 bg-white/40 dark:bg-black/20 border border-white/50 dark:border-white/10 shadow-sm p-5 rounded-[16px] text-center">
  ...
  if (type === "Contact") {
    return (
      <div className="p-6 bg-white/40 dark:bg-black/40 backdrop-blur-[20px] backdrop-saturate-200 text-[#1D1D1F] dark:text-[#F5F5F7] font-inter text-center border-y border-white/50 dark:border-white/10">
