      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {status === "ready" && order && (
          <>
            ...
            <section className="app-card rounded-2xl border border-gray-200 bg-white/70 p-6 shadow-sm">
              <h2 className="text-lg font-bold font-outfit text-gray-900">Fulfillment</h2><p>Powered by Shippo</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <label className="text-sm font-medium">Weight (oz)<input aria-label="Package weight in ounces" type="number" placeholder="Weight in ounces" value={weight} onChange={(event) => setWeight(event.target.value)} className="mt-1 w-full rounded-lg border p-2" /></label>
              ...
              {rates.length > 0 && <div className="mt-4 space-y-2"><h3>Select a Service</h3>{rates.map((rate) => (
              ...
              {label && <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4"><h2>Label Purchased Successfully</h2><p>{label.carrier} tracking: <strong>{label.trackingNumber}</strong></p><p>Shipped</p><a href={label.url} target="_blank" rel="noopener noreferrer" className="text-indigo-700 underline">Print Label</a></div>}
            </section>
          </>
        )}
      </div>
