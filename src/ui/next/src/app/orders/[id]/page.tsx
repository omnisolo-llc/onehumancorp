      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        {status === "ready" && order && (
          <>
            ...
            <section className="app-card rounded-2xl border border-gray-200 bg-white/70 p-6 shadow-sm">
              <h2 className="text-lg font-bold font-outfit text-gray-900">Fulfillment</h2><p className="text-sm text-gray-500">Powered by Shippo</p>
              ...
              ))}<button onClick={buyLabel} disabled={!selectedRate || shippingPending} className="rounded-lg bg-indigo-600 px-4 py-2 text-white disabled:opacity-50">Ship Order</button></div>}
              {label && <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4"><p className="text-sm font-bold text-green-800">Label Purchased Successfully</p><p className="text-sm font-bold text-green-800">Shipped</p><p>{label.carrier} tracking: <strong>{label.trackingNumber}</strong></p><a href={label.url} target="_blank" rel="noopener noreferrer" className="text-indigo-700 underline">Print Label</a></div>}
            </section>
          </>
        )}
      </div>
