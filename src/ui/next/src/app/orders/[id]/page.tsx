        ...
        {status === "ready" && order && (
          <>
            ...
            <section className="app-card rounded-2xl border border-gray-200 bg-white/70 p-6 shadow-sm">
              ...
              <p className="text-xs text-gray-500">Powered by Shippo</p>
              ...
              ))}<button onClick={buyLabel} disabled={!selectedRate || shippingPending} className="mt-4 rounded-lg bg-indigo-600 px-4 py-2 text-white disabled:opacity-50">Buy Label & Print</button></div>}
              {label && <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4">
                <p className="text-green-800 font-bold">Label Purchased Successfully</p>
                <p>{label.carrier} tracking: <strong>{label.trackingNumber}</strong> - <span>Shipped</span></p>
                <a href={label.url} target="_blank" rel="noopener noreferrer" className="text-indigo-700 underline font-bold mt-2 inline-block">Print Label</a>
              </div>}
            </section>
          </>
        )}
