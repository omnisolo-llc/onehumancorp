        ...
        {status === "ready" && order && (
          <>
            ...
            <section className="app-card rounded-2xl border border-gray-200 bg-white/70 p-6 shadow-sm">
              ...
              {rates.length > 0 && <div className="mt-4 space-y-2"><p className="text-sm font-medium">Select a Service</p>{rates.map((rate) => (
              ...
              {label && <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-4"><p className="font-bold text-green-700">Label Purchased Successfully</p>
...
function Field({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-xs font-bold uppercase tracking-wide text-gray-500">{label}</dt><dd className="mt-1 text-sm font-medium text-gray-900">{value === "SHIPPED" ? "Shipped" : value}</dd></div>;
}
