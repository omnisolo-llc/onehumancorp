...
export const InstagramDMCard: React.FC<InstagramDMCardProps> = ({
  approval,
  onApprove,
  onEdit,
  onDismiss,
  ...
  const context = approval.context_payload || approval.proposed_action || {};
  const proposed = approval.proposed_action || approval.context_payload || {};
  const customerId = context.customer_id || context.sender_id || 'unknown';
  ...
  return (
    <div className="mb-4 p-4 rounded-[16px] bg-[rgba(255,255,255,0.65)] dark:bg-[rgba(22,22,26,0.7)] backdrop-blur-[30px] saturate-[210%] border border-[rgba(255,255,255,0.4)] dark:border-[rgba(255,255,255,0.1)] flex flex-col gap-3 shadow-sm" data-testid="instagram-dm-card">
      <div className="flex items-center gap-2 text-pink-600 font-semibold text-sm">
        ...
        <strong className="font-outfit tracking-wide">Instagram DM from @{customerId}</strong>
      </div>
      <div className="text-[15px] text-gray-500 dark:text-gray-400 font-medium break-words">
        Customer: <div className="triage-context inline break-words">{context.customer_message || context.original_message || context.description || 'No message content'}</div>
      </div>
      <div className="text-sm text-[#1D1D1F] dark:text-[#F5F5F7] bg-white/50 dark:bg-black/20 p-3 rounded-[8px] break-words shadow-sm">
        ...
        {isEditing ? (
          <textarea
            className="w-full min-h-[60px] p-2 rounded-[8px] border border-pink-300 dark:border-pink-700 bg-white dark:bg-gray-800 text-[#1D1D1F] dark:text-[#F5F5F7] text-sm focus:ring-2 focus:ring-pink-500 outline-none transition-all resize-none"
            rows={4}
            value={editContent}
            onChange={(e) => setEditContent && setEditContent(e.target.value)}
            data-testid="edit-instagram-dm-textarea"
            autoFocus
          />
        ) : (
          <div className="whitespace-pre-wrap">{proposed.draft_reply || proposed.generated_response || ''}</div>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full mt-2">
        {onApprove && (
          ...
          >
            {isEditing ? 'Save & Send' : 'Send Draft'}
          </button>
        )}
