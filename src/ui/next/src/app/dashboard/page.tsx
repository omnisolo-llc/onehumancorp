    ...
    >
      ...
      <main id="dashboard-screen" className="app-grid" style={{ gap: 16 }}>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h2 className="app-panel-title">Referral Program</h2>
              <p className="app-list-subtitle">Invite businesses and track rewards.</p>
            </div>
            <button
              onClick={() => setShowReferralModal(true)}
              className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-xl shadow-sm transition-all text-sm flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="mac-glass-container p-5 rounded-[16px] border border-white/40 dark:border-white/10">
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Team Invites Sent</div>
              <div className="text-2xl font-bold font-outfit text-indigo-900 dark:text-white">0</div>
            </div>
            <div className="mac-glass-container p-5 rounded-[16px] border border-white/40 dark:border-white/10">
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Active Referrals</div>
              <div className="text-2xl font-bold font-outfit text-gray-900 dark:text-white">0</div>
            </div>
            <div className="mac-glass-container p-5 rounded-[16px] border border-white/40 dark:border-white/10">
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Revenue from Referrals</div>
              <div className="text-2xl font-bold font-outfit text-gray-900 dark:text-white">$0</div>
            </div>
            <div className="mac-glass-container p-5 rounded-[16px] border border-white/40 dark:border-white/10">
              <div className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Pending Rewards</div>
              <div className="text-2xl font-bold font-outfit text-gray-900 dark:text-white">$0</div>
            </div>
          </div>
        </section>
      ...
      {showReferralModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-2xl max-w-md w-full border border-gray-200 dark:border-gray-800">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
              Share your unique link with other business owners. When they join and make their first sale, you both earn $50 in premium credits!
            </p>

            <div className="mb-6">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Your Unique Link</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  readOnly
                  value={`https://ohc.store/join?ref=${tenantId()}`}
                  className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg py-2 px-3 text-sm text-gray-700 dark:text-gray-300"
                />
                <button
                  onClick={() => navigator.clipboard.writeText(`https://ohc.store/join?ref=${tenantId()}`)}
                  className="bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 py-2 px-4 rounded-lg font-medium text-sm transition-colors"
                >
                  Copy
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
               <a href={`https://twitter.com/intent/tweet?text=Start%20your%20business%20on%20OHC!%20Use%20my%20link:%20https://ohc.store/join?ref=${tenantId()}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2 px-4 bg-[#1DA1F2]/10 hover:bg-[#1DA1F2]/20 text-[#1DA1F2] rounded-lg font-medium text-sm transition-colors">
                 <span>X (Twitter)</span>
               </a>
               <a href={`https://wa.me/?text=Start%20your%20business%20on%20OHC!%20Use%20my%20link:%20https://ohc.store/join?ref=${tenantId()}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2 px-4 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] rounded-lg font-medium text-sm transition-colors">
                 <span>WhatsApp</span>
               </a>
            </div>
          </div>
