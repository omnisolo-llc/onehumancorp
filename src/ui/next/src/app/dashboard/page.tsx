...
export default function Dashboard() {
  ...
  const [metrics, setMetrics] = useState({ sales: 0, customers: 0, orders: 0 });
  ...
  useEffect(() => {
    ...
    async function fetchMetrics() {
        try {
            const tenant = typeof window !== 'undefined' ? localStorage.getItem('tenant_id') || 'e2e-tenant' : 'e2e-tenant';
            const res = await fetch('/api/v1/dashboard/sales', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + (typeof window !== 'undefined' ? localStorage.getItem('token') || 'test-token' : 'test-token') },
    ...
    const connectSwarmMesh = () => {
        try {
            ...
            ws.onmessage = (event) => {
                try {
                    // Try to parse base64 proto message
         ...
         <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="p-5 shadow-sm flex flex-col justify-between" style={{ background: 'rgba(255, 255, 255, 0.65)', backdropFilter: 'blur(30px) saturate(210%)', border: '1px solid rgba(255, 255, 255, 0.4)', borderRadius: '16px' }}>
                    ...
                    <div className="text-3xl font-bold font-outfit" style={{ color: '#1D1D1F' }}>${metrics.sales.toFixed(2)}</div>
                </div>

                <div className="p-5 shadow-sm flex flex-col justify-between" style={{ background: 'rgba(255, 255, 255, 0.65)', backdropFilter: 'blur(30px) saturate(210%)', border: '1px solid rgba(255, 255, 255, 0.4)', borderRadius: '16px' }}>
                    <div className="text-sm font-medium mb-1" style={{ color: '#86868B' }}>Active Customers</div>
                    <div className="text-3xl font-bold font-outfit" style={{ color: '#1D1D1F' }}>{metrics.customers}</div>
                </div>

                <div className="p-5 shadow-sm flex flex-col justify-between" style={{ background: 'rgba(255, 255, 255, 0.65)', backdropFilter: 'blur(30px) saturate(210%)', border: '1px solid rgba(255, 255, 255, 0.4)', borderRadius: '16px' }}>
                    <div className="text-sm font-medium mb-1" style={{ color: '#86868B' }}>Pending Orders</div>
                    <div className="text-3xl font-bold font-outfit" style={{ color: '#1D1D1F' }}>{metrics.orders}</div>
                </div>
