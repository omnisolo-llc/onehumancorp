...
const Card = ({ children, className }: React.HTMLAttributes<HTMLDivElement>) => <div className={`rounded-xl border bg-white text-gray-950 shadow-sm ${className}`}>{children}</div>;
...
const CardTitle = ({ children, className }: React.HTMLAttributes<HTMLHeadingElement>) => <h3 className={`font-semibold leading-none tracking-tight ${className}`}>{children}</h3>;
const CardContent = ({ children, className }: React.HTMLAttributes<HTMLDivElement>) => <div className={`p-6 pt-0 ${className}`}>{children}</div>;
const CardFooter = ({ children, className }: React.HTMLAttributes<HTMLDivElement>) => <div className={`flex items-center p-6 pt-0 ${className}`}>{children}</div>;
const Button = ({ children, variant, size, className, disabled, onClick }: React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'outline' | 'ghost'; size?: 'sm' }) => {
...
const Textarea = ({ value, onChange, className }: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => <textarea value={value} onChange={onChange} className={`flex min-h-[60px] w-full rounded-md border border-gray-200 bg-transparent px-3 py-2 text-sm shadow-sm placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-950 disabled:cursor-not-allowed disabled:opacity-50 ${className}`} />;
