          ...
          <>
            ...
            {approvals.map((approval) => (
              ...
              >
                <ActionCard
                  id={approval.id}
                  department={approval.department.replace('_', ' ')}
                  description={approval.description}
                  status="pending"
                  featureType={
                    approval.payload?.context?.smart_pricing ? "smart_pricing" :
                    approval.payload?.context?.weekly_health_report ? "health_report" :
                    approval.payload?.remaining_stock !== undefined ? "restock" : undefined
                  }
                  suggestedPrice={approval.payload?.context?.suggested_price}
                  scope={
                    approval.payload?.context?.abandoned_carts_count !== undefined
                      ? `Abandoned Carts: ${approval.payload.context.abandoned_carts_count}, Potential Revenue: ${Number(approval.payload.context.potential_revenue).toFixed(2)}`
                      : approval.payload?.context?.weekly_health_report
                        ? `Revenue (Week): ${approval.payload.context.weekly_revenue} - "${approval.payload.context.message || "Your revenue is up 12% from last week. I've drafted a quick update to send to your co-founders."}"`
                        : approval.payload?.remaining_stock !== undefined
                          ? `Product: ${approval.payload.product_id} | Remaining: ${approval.payload.remaining_stock} | Alert: ${approval.payload.message}`
                          : approval.payload?.context?.smart_pricing
                            ? `Current Price: ${approval.payload.context.current_price}`
                            : undefined
                  }
                  onApprove={(id) => handleDecision(id, true)}
                  onEdit={() => {}}
                  onDiscard={(id) => handleDecision(id, false)}
                />
              </div>
            ))}
          </>
