...
export function UnifiedAgentFeed() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (refreshRes.ok) {
            ...
            setItems(data.pending_approvals as any);
        }
  ...
  if (error) {
    return (
      <div className="w-full mb-6 p-4 glassmorphism rounded-[16px] border border-[#FF3B30]/50 bg-red-500/10 text-red-500 text-center">
        {error}
      </div>
    );
  }
