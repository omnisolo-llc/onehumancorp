...
export function TriageFeed() {
  ...
  if (items.length === 0 && !loading && !error) {
     return null; // Don't show anything if there are no triage items to avoid clutter
  }
      ...
      <div className="app-grid two">
        <section className="app-panel">
          ...
          <div id="triage-list" className="app-list">
            ...
            {loading && items.length === 0 && <div className="app-empty">Loading triage items...</div>}
            {!error && items.length === 0 && !loading ? (
              <div className="app-empty">No items need your attention right now. Great job!</div>
            ) : items.map((item) => (
              <button
                key={item.id}
                type="button"
