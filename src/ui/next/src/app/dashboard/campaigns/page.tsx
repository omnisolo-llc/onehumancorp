      ...
      <section className="app-grid two">
            ...
            {workflows.map((workflow) => (
              <div key={workflow.key} className="app-list-item items-start">
                ...
                <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                  <button
                    type="button"
                    className={selected === workflow.key ? "app-button primary" : "app-button"}
                    disabled={submitting}
                    onClick={() => runWorkflow(workflow)}
                  >
                    {submitting && selected === workflow.key ? "Working..." : workflow.methodLabel}
                  </button>
                  <Link href={workflow.href} className="app-button">
                    {workflow.linkLabel}
                  </Link>
                </div>
              </div>
        ...
        <div className="app-panel">
          ...
          <div className="app-panel-body">
            ...
            {draft ? (
              <pre className="min-h-[220px] whitespace-pre-wrap rounded-[8px] border border-gray-200 glassmorphism p-4 text-sm text-gray-800 shadow-sm dark:border-white/10 dark:bg-black/30 dark:text-gray-100">
                {draft}
              </pre>
            ) : (
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Link href={activeWorkflow.href} className="app-button primary">
