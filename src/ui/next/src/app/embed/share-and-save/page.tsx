        ...
        <div className={`w-full min-h-full font-inter ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-transparent text-gray-900'} flex flex-col justify-between`}>
            <div className="p-4 sm:p-6 text-center flex-1 flex flex-col justify-center">
                ...
                <p className={`text-xs sm:text-sm mb-4 sm:mb-5 leading-relaxed ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                    Share verification is unavailable. Sharing will not apply the configured {discountType === '$' ? '$' : ''}{discountAmount}{discountType === '%' ? '%' : ''} reward.
                </p>

                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <button disabled className="flex items-center justify-center gap-1.5 sm:gap-2 py-1.5 sm:py-2 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-gray-400 text-white text-xs sm:text-sm font-medium shadow-sm">
                    ...
                    <button disabled className="flex items-center justify-center gap-1.5 sm:gap-2 py-1.5 sm:py-2 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-gray-500 text-white text-xs sm:text-sm font-medium shadow-sm">
