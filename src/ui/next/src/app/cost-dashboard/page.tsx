        ...
        <section id="my-plan-section" className="p-6 md:p-8 shadow-lg bg-white/60 backdrop-blur-2xl saturate-200 border border-white/40 rounded-2xl md:rounded-[24px] hover:shadow-xl transition-shadow duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="p-4 rounded-xl bg-white/50 border border-white/50">
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">{myPlanData?.ai_actions_used || 0} <span className="text-sm text-gray-500 font-normal">{myPlanData?.ai_actions_limit === null ? '/ Unlimited' : (myPlanData?.ai_actions_limit ? `/ ${myPlanData.ai_actions_limit}` : '')}</span></p>
              </div>
              <div className="p-4 rounded-xl bg-white/50 border border-white/50">
                  <h3 className="text-sm font-medium text-gray-500">Storage Used</h3>
                  <p className="text-2xl font-bold text-gray-900 mt-1">{((myPlanData?.storage_used_bytes || 0) / (1024 * 1024)).toFixed(1)} MB <span className="text-sm text-gray-500 font-normal">{myPlanData?.storage_limit_bytes === null ? '/ Unlimited' : (myPlanData?.storage_limit_bytes ? `/ ${(myPlanData.storage_limit_bytes / (1024 * 1024)).toFixed(0)} MB` : '')}</span></p>
              </div>
              <div className="p-4 rounded-xl bg-white/50 border border-white/50">
