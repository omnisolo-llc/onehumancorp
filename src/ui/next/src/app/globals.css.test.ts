...
describe('Global CSS Standards', () => {
  it('enforces blur(20px) saturate(200%) in global styles instead of old blur(30px) tokens', () => {
    ...
    try {
       ...
       if (globalsPath) {
           ...
           expect(content).toContain('blur(20px) saturate(200%)');
           expect(content).not.toContain('blur(30px) saturate(210%)');
           expect(content).not.toContain('blur(30px)');
           expect(content).not.toContain('blur(40px)');
           expect(content).not.toContain('saturate(210%)');
       }
    } catch (e) {
       // Ignore if not found
    }
