...
const getSourceIcon = (source: string) => {
  ...
  if (s.includes("payment") || s.includes("stripe") || s.includes("invoice")) return "💳";
...
export default function TodayFeedPage() {
  ...
  async function loadItems() {
    ...
    try {
      ...
      setError(e?.message || "Failed to load today's items");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <AppShell
      title="Today"
      subtitle="Your actionable daily feed. Triage, review, and send."
      statusItems={[
        {
          label: "Active",
      ...
      <div className="flex flex-col gap-4 w-full max-w-full pb-20">
        ...
        {loading ? (
          <div className="p-6 space-y-4">
            <div className="h-24 bg-gray-200 dark:bg-gray-700 rounded-2xl animate-pulse w-full"></div>
            <div className="h-24 bg-gray-200 dark:bg-gray-700 rounded-2xl animate-pulse w-full"></div>
          </div>
        ) : !error && items.length === 0 ? (
          <div
            className="app-empty flex flex-col items-center justify-center py-12"
            data-testid="today-feed-empty"
          >
            ...
            <div className="text-sm text-gray-500 mt-2 text-center">
              Your AI assistant has organized everything. Enjoy the rest of your day!
            </div>
          </div>
        ) : (
          items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                data-testid={`today-card-${item.id}`}
                className="w-full glassmorphism bg-[rgba(255,255,255,0.65)] dark:bg-[rgba(22,22,26,0.7)] backdrop-blur-[30px] backdrop-saturate-[210%] border border-[rgba(255,255,255,0.4)] dark:border-[rgba(255,255,255,0.1)] rounded-[24px] shadow-sm flex flex-col mb-4 overflow-hidden transition-all duration-300"
              >
                {/* Header Context */}
                <div
                  ...
                  data-testid={`today-card-header-${item.id}`}
                >
                  ...
                  <div className="text-[15px] font-medium text-gray-900 dark:text-white leading-snug break-words line-clamp-3">
                  ...
                  <div className="animate-in slide-in-from-top-2 duration-200 fade-in">
                    {item.action_type && (
                      <div className="p-5 bg-[#0066FF]/10 dark:bg-[#0066FF]/20 backdrop-blur-[30px] saturate-[210%] flex flex-col gap-2">
                        ...
                        <div className="proposed-action border border-[#0066FF]/20 dark:border-[#0066FF]/30 bg-white/50 dark:bg-black/30 backdrop-blur-[30px] saturate-[210%] p-4 rounded-xl text-[13px] leading-relaxed text-gray-900 dark:text-white whitespace-pre-wrap break-words">
                    ...
                    {editingId === item.id ? (
                      <div className="p-5 flex flex-col gap-3 border-t border-white/20 dark:border-white/10 bg-white/40 dark:bg-black/20 backdrop-blur-[30px] saturate-[210%]">
                        <textarea
                          ...
                          data-testid={`today-edit-textarea-${item.id}`}
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full pt-2">
                          <button
                            ...
                            className="w-full flex-1 min-h-[44px] min-w-[44px] px-4 rounded-xl bg-[#0066FF] text-white font-medium hover:bg-[#0052CC] transition-all duration-200 shadow-md flex items-center justify-center disabled:opacity-50"
                            data-testid={`today-save-btn-${item.id}`}
                          >
                      ...
                      <div className="p-5 pt-3 flex flex-col sm:flex-row gap-3 w-full border-t border-white/20 dark:border-white/10 bg-white/40 dark:bg-black/20 backdrop-blur-[30px] saturate-[210%]">
                        {item.action_type ? (
                          <>
                            <button
                              disabled={isProcessing}
                              className="w-full flex-1 min-h-[44px] min-w-[44px] px-4 rounded-xl border border-[#0066FF]/30 dark:border-[#0066FF]/50 bg-[#0066FF]/10 dark:bg-[#0066FF]/20 text-[#0066FF] dark:text-[#3388FF] font-medium hover:bg-[#0066FF]/20 transition-all duration-200 shadow-sm flex items-center justify-center disabled:opacity-50"
                              data-testid={`today-review-btn-${item.id}`}
                              onClick={() => {
                            ...
                            <button
                              disabled={isProcessing}
                              className="w-full flex-1 min-h-[44px] min-w-[44px] px-4 rounded-xl bg-[#0066FF] text-white font-medium hover:bg-[#0052CC] transition-all duration-200 shadow-md flex items-center justify-center disabled:opacity-50"
                              data-testid={`today-approve-${item.id}`}
