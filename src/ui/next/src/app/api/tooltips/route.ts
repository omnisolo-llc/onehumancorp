...
let tooltipsRegistry: Record<string, string> = {
...
export async function GET() {
  return NextResponse.json(tooltipsRegistry);
}

export async function POST(req: Request) {
  try {
    const newTooltips = await req.json();
    tooltipsRegistry = { ...tooltipsRegistry, ...newTooltips };
    return NextResponse.json({ success: true, registry: tooltipsRegistry });
  } catch (error) {
