...
describe("POST /api/v1/booking/request", () => {
  ...
  it("delegates identity and backend I/O to the authenticated transport", async () => {
    ...
    const request = new Request(`https://app.example.test/api/v1/booking/request?tenant_id=forged`, {
      method: "POST",
    ...
    expect(proxyBackendRequest).toHaveBeenCalledWith(request, "/api/v1/booking/request");
