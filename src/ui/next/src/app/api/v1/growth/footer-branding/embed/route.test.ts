...
describe("Footer Branding Embed API", () => {
  it("returns javascript code with default parameters", async () => {
    const req = new NextRequest(
      new URL("http://localhost/api/v1/growth/footer-branding/embed"),
    );
  ...
  it("returns javascript code with custom parameters", async () => {
    const req = new NextRequest(
      new URL(
        "http://localhost/api/v1/growth/footer-branding/embed?tenant=test-tenant&style=footer&text=Made%20with%20Love&theme=dark",
      ),
    );
    const res = await GET(req);
