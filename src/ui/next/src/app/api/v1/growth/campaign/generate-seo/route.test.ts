...
describe('POST /api/v1/growth/campaign/generate-seo', () => {
    beforeEach(() => {
        global.fetch = vi.fn();
    });
    ...
    it('returns 200 with message on successful backend fetch', async () => {
        ...
        const req = new Request('http://localhost/api/v1/growth/campaign/generate-seo', {
            method: 'POST',
    ...
    it('returns fallback message on backend failure', async () => {
        ...
        expect(res.status).toBe(200);
        expect(data.message).toContain('SEO Audit Report for My Store');
    ...
    it('returns fallback message on fetch error', async () => {
        ...
        expect(res.status).toBe(200);
        expect(data.message).toContain('SEO Audit Report');
        expect(data.message).toContain('Powered by OHC');
