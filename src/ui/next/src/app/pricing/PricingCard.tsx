...
export const PricingCard: React.FC<PricingCardProps> = ({
  ...
  return (
    <div className={`p-6 flex flex-col justify-between ${isRecommended ? 'relative shadow-xl' : 'shadow-lg'} app-card ohc-growth-card glass-card backdrop-blur-2xl bg-white/30 border border-white/30 rounded-2xl hover:-translate-y-1 hover:shadow-2xl hover:bg-white/40 transition-all duration-300 w-full`}>
      {isRecommended && (
        <div className="absolute top-0 right-0 bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-bl-xl rounded-tr-2xl z-10">Recommended</div>
      )}
      <div>
      ...
      {(() => {
        ...
        return (
          <button onClick={() => onUpgrade(tierName, isAnnual)} className={`w-full min-h-[44px] px-4 py-2 text-white rounded-xl font-medium transition-colors shadow-sm flex items-center justify-center ${isRecommended ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-900 hover:bg-black'}`}>
