...
export class SyncManager {
  ...
  public async sync(retryCount = 0) {
    ...
    try {
      ...
      const generalGenMutations = generalMutations.filter(m => m.type !== 'UPDATE_ORDER_STATUS' && m.type !== 'TOGGLE_SOLD_OUT' && m.type !== 'update_quote' && m.type !== 'approve_quote' && m.type !== 'DRAWER_OP' && m.type !== 'START_TERMINAL_SESSION' && m.type !== 'END_TERMINAL_SESSION' && m.type !== 'CASH_SALE');
