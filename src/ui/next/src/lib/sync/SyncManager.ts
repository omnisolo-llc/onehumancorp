    ...
    try {
      ...
      for (const action of triageActions) {
        try {
          ...
          if (!res.ok) {
            console.warn(`Triage Action Sync failed with status ${res.status}`);
            if (res.status >= 500) allOk = false;
          }
        } catch (err) {
          console.warn("Triage Action Sync error:", err);
          allOk = false;
        }
      }
      ...
      for (const action of advisoryActions) {
        try {
          ...
          if (!res.ok) {
            console.warn(`Advisory Action Sync failed with status ${res.status}`);
            if (res.status >= 500) allOk = false;
          }
        } catch (err) {
          console.warn("Advisory Action Sync error:", err);
          allOk = false;
        }
      }
      ...
      for (const action of fieldOpsActions) {
        try {
          ...
          if (!res.ok) {
            console.warn(`Field Ops Status Sync failed with status ${res.status}`);
            if (res.status >= 500) allOk = false;
          }
        } catch (err) {
          console.warn("Field Ops Status Sync error:", err);
          allOk = false;
        }
      }
