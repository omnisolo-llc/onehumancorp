...
describe("authentication runtime configuration", () => {
  ...
  it.each([
    ...
    [{ ...production, BACKEND_URL: "http://8.8.8.8:8080", OHC_WEB_CANONICAL_ORIGIN: "https://app.example.com" }, "backend origin must use HTTPS or loopback HTTP"],
    [{ ...production, BACKEND_URL: "https://api.example.com/path" }, "backend origin must not contain"],
    [{ OHC_WEB_LOCAL_DEV: "true", OHC_WEB_CANONICAL_ORIGIN: "http://8.8.8.8:3000", BACKEND_URL: "http://127.0.0.1:8080" }, "local development canonical origin must be loopback or a private LAN IP"],
    [{ OHC_WEB_LOCAL_DEV: "true", OHC_WEB_CANONICAL_ORIGIN: "http://devbox.local:3000", BACKEND_URL: "http://127.0.0.1:8080" }, "local development canonical origin must be loopback or a private LAN IP"],
