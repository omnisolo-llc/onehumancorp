...
export default function HelpCenter() {
    ...
    return (
        <div className="help-center-container" style={{
                            ...
                            <li style={{ marginBottom: '8px' }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        backgroundColor: activeTopic === null ? '#f1f5f9' : 'transparent',
                                        color: activeTopic === null ? '#0f172a' : '#475569',
                                ...
                                <li key={topic} style={{ marginBottom: '8px' }}>
                                    <button
                                        ...
                                        style={{
                                            ...
                                            backgroundColor: activeTopic === topic ? '#f1f5f9' : 'transparent',
                                            color: activeTopic === topic ? '#0f172a' : '#475569',
                                            transition: 'all 0.2s ease'
                                        }}
                                    >
                                        {topic}
                ...
                <main style={{ flex: '3 1 600px' }}>
                    {filtered.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '48px', background: 'rgba(255,255,255,0.5)', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
                            ...
                            <p style={{ color: '#94a3b8' }}>Try adjusting your search terms or selecting a different topic.</p>
                            <button
                                onClick={() => {setSearchTerm(''); setActiveTopic(null);}}
                                style={{ marginTop: '16px', padding: '8px 16px', background: '#f1f5f9', border: 'none', borderRadius: '6px', cursor: 'pointer', color: '#0f172a' }}
                            >
                                Clear Filters
                            </button>
                        </div>
                            ...
                            {filtered.map(a => (
                                <article key={a.id} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
                                        <span style={{ fontSize: '0.8rem', background: '#e2e8f0', color: '#475569', padding: '4px 8px', borderRadius: '4px', fontWeight: '500' }}>
                                            {a.topic}
                                        </span>
                                    </div>
                                    ...
                                    <div style={{ marginTop: '20px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                        {a.keywords.map(kw => (
                                            <span key={kw} style={{ fontSize: '0.75rem', color: '#64748b', background: '#f8fafc', border: '1px solid #f1f5f9', padding: '2px 6px', borderRadius: '4px' }}>
                                                #{kw}
                                            </span>
                                        ))}
                                    </div>
