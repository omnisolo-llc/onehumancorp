        ...
        <div className="floating-help-chat" style={{
            ...
            {!isOpen && (
                <button onClick={() => setIsOpen(true)} style={{
                    borderRadius: '50%', width: '50px', height: '50px', background: '#0070f3', color: '#fff', border: 'none',
                    backdropFilter: 'blur(20px) saturate(200%)', cursor: 'pointer'
                }}>Ask</button>
            )}

            {isOpen && (
                <div style={{
                    width: '300px', height: '400px', background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(20px) saturate(200%)',
                    borderRadius: '12px', border: '1px solid rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', padding: '16px',
                    ...
                    <form onSubmit={sendChat} style={{ display: 'flex', marginTop: '10px' }}>
                        <input name="chat" value={msg} onChange={e => setMsg(e.target.value)} style={{ flex: 1, padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }} placeholder="How do I setup my store?" />
