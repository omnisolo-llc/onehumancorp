...
export function RateLimitWarningProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <RateLimitWarningContext.Provider value={{ warningMessage, showWarning, hideWarning }}>
      ...
      {warningMessage && (
        <div
          className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-[rgba(255,255,255,0.03)] border border-white/40 dark:border-white/10 text-amber-800 dark:text-amber-100 px-6 py-4 rounded-xl shadow-lg z-[9999] flex items-start gap-3 max-w-md w-full backdrop-blur-[20px] backdrop-saturate-[200%]"
          role="alert"
          aria-live="polite"
        >
