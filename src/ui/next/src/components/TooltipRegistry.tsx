  ...
  return (
    <TooltipContext.Provider value={{ activeTooltip, setActiveTooltip, tooltipRect, setTooltipRect, tooltipText, setTooltipText, getTooltip: (id: string) => tooltips[id] }}>
      ...
      {activeTooltip && tooltipRect && (
        <div
          className="fixed z-[100] bg-black/70 text-white text-sm font-inter p-3 rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.24)] pointer-events-none w-64 max-w-[calc(100vw-32px)] mx-4 text-center leading-relaxed backdrop-blur-2xl saturate-150 border border-white/20 animate-fade-in-up"
          style={{
        ...
        >
          {tooltipText}
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 border-solid border-t-black/70 border-t-8 border-x-transparent border-x-8 border-b-0"></div>
        </div>
      )}
