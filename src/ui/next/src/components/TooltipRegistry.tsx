  ...
  return (
    <TooltipContext.Provider value={{ activeTooltip, setActiveTooltip, tooltipRect, setTooltipRect, tooltipText, setTooltipText, getTooltip: (id: string) => tooltips[id] }}>
      ...
      {activeTooltip && tooltipRect && (
        <div
          className="fixed z-[100] bg-white/80 text-gray-900 text-sm font-outfit font-medium p-3 rounded-xl shadow-2xl pointer-events-none w-64 text-center leading-relaxed backdrop-filter backdrop-blur-[20px] saturate-200 border border-white/50 animate-fade-in-up"
          style={{
        ...
        >
          {tooltipText}
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 border-solid border-t-white/80 border-t-8 border-x-transparent border-x-8 border-b-0 backdrop-filter backdrop-blur-[20px] saturate-200"></div>
        </div>
      )}
