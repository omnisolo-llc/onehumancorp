...
export function HelpChat() {
  ...
  return (
    <div className="help-chat-wrapper pointer-events-none">
      ...
      {/* Chat Interface */}
      {isOpen && (
        <div id="ai-chat-interface" role="dialog" aria-labelledby="ai-chat-header-title" aria-modal="false" className="fixed bottom-24 right-6 z-[10000] w-full max-w-[350px] pointer-events-auto bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-3xl saturate-200 border border-white/50 dark:border-white/10 rounded-[24px] shadow-xl flex flex-col overflow-hidden animate-slide-up-chat text-gray-900 dark:text-gray-100">
          {/* Header */}
          <div
            id="ai-chat-header"
          ...
          <div role="log" aria-live="polite" aria-atomic="false" className="flex-1 p-5 overflow-y-auto h-[350px] bg-gradient-to-b from-white/40 to-transparent dark:from-white/5 flex flex-col gap-5 font-inter text-sm custom-scrollbar">
            {messages.map((msg) => (
              ...
              >
                <div
                  className={`px-4 py-3 rounded-2xl max-w-[85%] leading-relaxed shadow-sm saturate-200 ${
                    msg.sender === "user"
                      ? "bg-blue-600/90 backdrop-blur-3xl saturate-200 text-white rounded-br-sm border border-white/20"
                      : "bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-3xl saturate-200 border border-white/50 dark:border-white/10 text-gray-900 dark:text-gray-100 rounded-bl-sm prose prose-sm prose-blue dark:prose-invert max-w-none prose-p:my-1 prose-ul:my-1 prose-li:my-0.5"
                  }`}
          ...
          <form
            onSubmit={handleSend}
            className="p-3 bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-3xl saturate-200 border-t border-white/50 dark:border-white/10 flex gap-2 items-center"
          >
