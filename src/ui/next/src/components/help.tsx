...
export function HelpWidget() {
  ...
  return (
    <>
      ...
      {open && (
        <div id="help-widget-container" data-ui-overlay="true" className="fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-32px)] sm:w-[380px] h-[75vh] sm:h-[550px] max-h-[700px] glassmorphism/70 backdrop-blur-[30px] saturate-200 rounded-3xl shadow-2xl flex flex-col overflow-hidden z-[90] border border-white/50 transition-all font-inter">
          <div className="flex border-b border-white/30 glassmorphism/40 backdrop-blur-md overflow-x-auto scrollbar-hide">
            {helpTabs.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`flex-1 min-w-[80px] min-h-[44px] px-3 py-3 text-sm font-bold transition-all whitespace-nowrap ${
                  tab === t.id ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-600 hover:text-gray-900 hover:glassmorphism/20"
                }`}
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
            {tab === "center" && (
              <div>
                ...
                <input type="text" placeholder="Search for help..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-4 border border-white/50 rounded-2xl mb-6 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm glassmorphism/60 backdrop-blur-[20px] saturate-200 min-h-[44px]" />
                <div className="space-y-3 mb-8">
                  {filteredArticles.map((a, idx) => (
                    <div key={idx} className="glassmorphism/70 backdrop-blur-[20px] saturate-200 p-5 rounded-2xl shadow-sm border border-white/50 cursor-pointer hover:border-blue-300 hover:shadow-md transition-all">
                      {a.link ? (
            ...
            {tab === "chat" && (
              <div className="flex flex-col h-full glassmorphism/30 backdrop-blur-[20px] saturate-200 rounded-xl p-2">
                ...
                <form onSubmit={handleChatSubmit} className="mt-4 flex gap-2 pt-3 border-t border-white/50">
                  <input
                    ...
                    className="flex-1 p-3 border border-white/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 glassmorphism/70 backdrop-blur-md shadow-sm min-h-[44px]"
                  />
                ...
                <div className="grid grid-cols-2 gap-4">
                  {videos.map((v) => (
                    <div key={v.id} onClick={() => setActiveVideo(v)} className="aspect-[9/16] bg-gray-200 rounded-2xl flex items-center justify-center relative overflow-hidden group cursor-pointer shadow-sm border border-white/30">
                      ...
                      <div className="w-10 h-10 glassmorphism/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg z-10 group-hover:scale-110 transition-transform">
