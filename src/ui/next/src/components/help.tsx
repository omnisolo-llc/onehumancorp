...
export function Tooltip({ id, defaultText, children }: { id?: string; defaultText: string; children: ReactNode }) {
  ...
  return (
    ...
    >
      {children}
      {visible && (
        <div className="absolute z-50 bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-max max-w-xs px-3 py-2 text-sm text-white bg-gray-900 rounded-[8px] shadow-lg pointer-events-none text-center leading-tight">
          {displayText}
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
            {tab === "center" && (
              <div>
                ...
                <input type="text" placeholder="Search for help..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full p-3 border border-gray-200 rounded-[16px] mb-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                <div className="space-y-2 mb-4">
                  {filteredArticles.map((a, idx) => (
                    <div key={idx} className="bg-white p-4 rounded-[16px] shadow-sm border border-gray-100 cursor-pointer hover:border-blue-300">
                      {a.link ? (
                ...
                <div className="space-y-2">
                  <button onClick={() => startWalkthrough([{ targetId: "bio-input", message: "Enter your business description." }, { targetId: "generate-btn", message: "Click to generate!" }])} className="w-full text-left bg-blue-50 p-3 rounded-[16px] shadow-sm border border-blue-100 hover:bg-blue-100 transition-colors">
                    <span className="font-bold text-blue-800 text-sm block">Tour: Set up your store</span>
                  </button>
                  <button onClick={() => startWalkthrough([{ targetId: "team-activity-tooltip", message: "View the real-time actions performed by your AI workforce." }])} className="w-full text-left bg-blue-50 p-3 rounded-[16px] shadow-sm border border-blue-100 hover:bg-blue-100 transition-colors">
                    <span className="font-bold text-blue-800 text-sm block">Tour: Accept your first payment</span>
                  </button>
                  <button onClick={() => startWalkthrough([{ targetId: "generate-btn", message: "Activate your AI agent." }])} className="w-full text-left bg-blue-50 p-3 rounded-[16px] shadow-sm border border-blue-100 hover:bg-blue-100 transition-colors">
              ...
              <div className="flex flex-col h-full">
                ...
                <div className="mt-4 flex gap-2 pt-2 border-t border-gray-100">
                  <input
                    ...
                    className="flex-1 p-3 border border-gray-200 rounded-[16px] text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <button className="bg-blue-600 text-white p-3 rounded-[16px] hover:bg-blue-700 active:scale-95 transition-all">
            ...
            {tab === "videos" && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {videos.map((v) => (
                    <div key={v.id} className="aspect-[9/16] bg-gray-200 rounded-[16px] flex items-center justify-center relative overflow-hidden group cursor-pointer">
            ...
            {tab === "whatsnew" && (
              <div>
                ...
                <div className="w-full aspect-video bg-gray-200 rounded-[16px] mb-4 relative overflow-hidden border border-gray-100 shadow-sm flex items-center justify-center">
