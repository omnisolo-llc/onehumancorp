...
export function HelpWidget() {
  ...
  return (
    <>
      ...
      {open && (
        <div id="help-widget-container" className="fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-32px)] sm:w-[380px] h-[75vh] sm:h-[550px] max-h-[700px] bg-white/70 backdrop-blur-[20px] saturate-200 rounded-3xl shadow-2xl flex flex-col overflow-hidden z-[90] border border-white/50 transition-all font-inter">
          <div className="flex border-b border-white/30 bg-white/40 backdrop-blur-[20px] saturate-200 overflow-x-auto scrollbar-hide">
            {helpTabs.map((t) => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
              ...
              <div className="flex flex-col h-full bg-white/30 backdrop-blur-[20px] saturate-200 rounded-xl p-2">
                ...
                <form onSubmit={handleChatSubmit} className="mt-4 flex gap-2 pt-3 border-t border-white/50">
                  <input
                    ...
                    className="flex-1 p-3 border border-white/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white/70 backdrop-blur-[20px] saturate-200 shadow-sm min-h-[44px]"
                  />
                  <button type="submit" disabled={!chatInput.trim()} className="bg-blue-600/90 backdrop-blur-[20px] saturate-200 text-white p-3 rounded-xl hover:bg-blue-700/90 shadow-sm active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed min-w-[44px] min-h-[44px] flex items-center justify-center" aria-label="Send message">
      ...
      {activeVideo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/80 backdrop-blur-[20px] saturate-200 p-4 animate-fade-in">
          <div className="bg-black backdrop-blur-[20px] rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-white/20 w-full max-w-[375px] mx-auto aspect-[9/16] relative animate-pop-in">
            {/* Header */}
            <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/90 to-transparent z-10 flex justify-between items-start pt-6">
              ...
              <button onClick={() => setActiveVideo(null)} className="text-white/80 hover:text-white bg-white/20 hover:bg-white/30 backdrop-blur-[20px] saturate-200 border border-white/20 rounded-full p-2 transition-all min-h-[44px] min-w-[44px] flex items-center justify-center flex-shrink-0" aria-label="Close video">
