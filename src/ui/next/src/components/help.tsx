            ...
            {tab === "center" && (
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={() => { setOpen(false); fetch("/api/walkthrough/store-setup").then(res => res.json()).then(data => data && data.length > 0 ? startWalkthrough(data) : startWalkthrough([{ targetId: "bio-input-tooltip", title: "Welcome", content: "Learn how to easily set up your store and accept your first payment." }, { targetId: "bio-input-tooltip", title: "What do you sell?", content: "Tell us what you sell so we can create the perfect storefront for you." }, { targetId: "generate-btn-tooltip", title: "Generate Store", content: "Click here and watch our AI build your store from scratch." }])); }} className="w-full text-left bg-white/65 dark:bg-[#16161a]/70 backdrop-blur-[30px] saturate-[210%] p-4 rounded-2xl shadow-sm border border-blue-100 hover:bg-blue-100/90 hover:shadow-md transition-all min-h-[44px]">
