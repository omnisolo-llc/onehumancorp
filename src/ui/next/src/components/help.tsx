            ...
            {tab === "center" && (
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={() => { setOpen(false); fetch("/api/v1/walkthrough/store-setup").then(res => res.json()).then(data => data && data.length > 0 ? startWalkthrough(data) : startWalkthrough([{ targetId: "help-widget-container", title: "Set up your store", content: "Learn how to easily set up your store and accept your first payment." }, { targetId: "help-widget-container", title: "Add Products", content: "Tell us what you sell so we can create the perfect storefront for you." }, { targetId: "help-widget-container", title: "Launch", content: "Click here and watch our AI build your store from scratch." }])); }} className="w-full text-left bg-[rgba(255,255,255,0.65)] dark:bg-[rgba(22,22,26,0.7)] backdrop-blur-[30px] saturate-[210%] p-4 rounded-2xl shadow-sm border border-blue-100 hover:bg-blue-100/90 hover:shadow-md transition-all min-h-[44px]">
