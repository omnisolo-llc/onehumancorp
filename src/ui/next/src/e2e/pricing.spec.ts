...
test.describe('Pricing Page Loop', () => {
  ...
  test('Pricing page toggles between monthly and annual pricing', async ({ page }) => {
    ...
    const proPrice = page.locator('.omnisolo-growth-card').filter({ has: page.locator('h3', { hasText: 'Pro' }) }).first();
    const businessPrice = page.locator('.omnisolo-growth-card').filter({ has: page.locator('h3', { hasText: 'Business' }) }).first();
