...
test.describe('Dashboard Triage Action Feed Edit UI', () => {
  ...
  test('should allow editing a draft from the unified dashboard feed', async ({ page }) => {
    ...
    const seedData = [Object.fromEntries([['source', 'Instagram DM'], ['priority', 'high'], ['context', 'Message: Customer asked about vegan cakes.'], ['action_type', 'Draft Reply'], ['action_payload', 'Yes, we have vegan options.'], ['customer_id', 'cust_test_1']])];

    for (const data of seedData) {
      await page.request.post(`/api/triage/create?tenant_id=${encodeURIComponent(tenantId)}`, Object.fromEntries([["data", data]]));
    }
