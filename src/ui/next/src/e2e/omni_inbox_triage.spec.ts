...
test.describe('Omni Inbox Agentic Triage', () => {
  test('displays unread leads summary and allows inventory deduction approval', async ({ page }) => {
    ...
    await (page as any).route('**/api/ui/inbox/messages*', async (route) => {
      const json = [
        {
          id: 'msg_triage_1',
          source: 'Instagram DM',
    ...
    await (page as any).route('**/api/agents/approvals*', async (route) => {
      const json = {
        pending_approvals: [
          {
            id: 'app_triage_1',
    ...
    await (page as any).route('**/api/agents/approvals/app_triage_1', async (route) => {
