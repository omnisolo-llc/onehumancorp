...
test.describe('Omni Inbox Agentic Triage', () => {
  test('displays unread leads summary and allows inventory deduction approval', async ({ page }) => {
    ...
    await page.waitForResponse('**/api/ui/inbox/messages*', async (route) => {
      const json = [
        {
          id: 'msg_triage_1',
          source: 'Instagram DM',
    ...
    await page.waitForResponse('**/api/agents/approvals*', async (route) => {
      const json = {
        pending_approvals: [
          {
            id: 'app_triage_1',
    ...
    await page.waitForResponse('**/api/agents/approvals/app_triage_1', async (route) => {
      if (route.request().method() === 'POST') {
        const body = JSON.parse(route.request().postData() || '{}');
        if (body.approved === true) {
          approveCalled = true;
          console.log({ status: 200, json: { success: true } });
          return;
        }
      }
