...
test.describe('Help Components', () => {
  test('Help Center page loads with articles', async ({ page }) => {
    ...
    // Wait for at least one article title to appear from real backend
  ...
  test('Interactive Walkthrough functions correctly on dashboard', async ({ page }) => {
    ...
    await expect(startTourBtn).toBeVisible({ timeout: 15000 });
    await startTourBtn.click();

    // Verify the first walkthrough step appears
    const firstStepTitle = page.getByRole('dialog').getByText('Welcome');
    ...
    const secondStepTitle = page.getByRole('dialog').getByText('AI Savings');
