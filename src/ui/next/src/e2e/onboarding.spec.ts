...
test.describe("OnboardingWizard CUJ", () => {
  ...
  test("Maya the Baker can complete the onboarding flow", async ({ page }) => {
    ...
    await expect(
      page.getByText("What's the name of your business?"),
    ).toBeVisible();
    ...
    await page
      .getByPlaceholder(/I bake custom vegan cakes/i)
      .fill("I bake custom vegan cakes for weddings and parties.");
    await page.getByRole("button", { name: "Next" }).click();
    ...
    await page
      .getByPlaceholder(/Local families, Tech startups/i)
      .fill("Everyone");
    await page.getByRole("button", { name: "Next" }).click();
  ...
  test("Carlos the Handyman sets up his repair business", async ({ page }) => {
    ...
    await page
      .getByPlaceholder(/I bake custom vegan cakes/i)
      .fill("Plumbing and general repairs");
    await page.getByRole("button", { name: "Next" }).click();
  ...
  test("Leo the Music Tutor configures online bookings", async ({ page }) => {
    ...
    await page
      .getByPlaceholder(/Maya's Custom Cake/i)
      .fill("Leo Guitar Lessons");
    await page.getByRole("button", { name: "Next" }).click();

    await page
      .getByPlaceholder(/I bake custom vegan cakes/i)
      .fill("Guitar tutoring online");
    await page.getByRole("button", { name: "Next" }).click();
  ...
  test("Fatima the Food Cart Operator on a slower network", async ({
    page,
  }) => {
    await page.goto("/onboarding");
    await expect(page.getByText("10-Minute Setup Wizard")).toBeVisible();
    ...
    await page
      .getByPlaceholder(/Maya's Custom Cake/i)
      .fill("Fatima Halal Food");
    await page.getByRole("button", { name: "Next" }).click();

    await page
      .getByPlaceholder(/I bake custom vegan cakes/i)
      .fill("Halal food cart pickup orders");
    await page.getByRole("button", { name: "Next" }).click();
  ...
  test("User can save a draft and restore it across sessions", async ({
    page,
    context,
  }) => {
    ...
    await page
      .getByPlaceholder(/Maya's Custom Cake/i)
      .fill("My Restored Business");
    await page.getByRole("button", { name: "Save Draft" }).click();
    await expect(page.getByText("Draft Saved!")).toBeVisible();

    // Ensure localStorage is populated via zustand persist
    const lsStore = await page.evaluate(() =>
      window.localStorage.getItem("onboarding-storage-v4"),
    );
    ...
    await context.route("**/api/onboarding/draft", async (route, request) => {
      if (request.method() === "GET") {
        route.fulfill({
          ...
          json: {
            wizardState: {
              step: 1,
              chatStep: 1,
              businessName: "My Restored Business",
            },
          },
        });
      } else {
        route.fulfill({
          status: 200,
          json: { success: true, organization_id: "test-tenant-123" },
        });
      }
    });
    await page.reload();
    ...
    await expect(page.locator("input").first()).toHaveValue(
      "My Restored Business",
      { timeout: 15000 },
    );
  });

  test("Validation errors prevent launching without complete admin info", async ({
    page,
  }) => {
    await page.goto("/onboarding");
    await expect(page.getByText("10-Minute Setup Wizard")).toBeVisible();
  ...
  test("Submitting empty inputs displays validation errors with visual indicators", async ({
    page,
  }) => {
    await page.goto("/onboarding");
    await expect(page.getByText("10-Minute Setup Wizard")).toBeVisible();
    ...
    await expect(
      page.getByText("Business Name must be at least 3 characters."),
    ).toBeVisible();
    ...
    const whatYouSellInput = page.getByPlaceholder(
    ...
    const audienceInput = page.getByPlaceholder(
      /Local families, Tech startups/i,
    );
    await expect(
      page.getByText("Please tell us your target audience."),
    ).toBeVisible();
    await expect(audienceInput).toHaveClass(/border-\[#FF3B30\]/);
  });

  test("User can use Instant Build to launch storefront quickly", async ({
    page,
  }) => {
    await page.goto("/onboarding");
    await expect(page.getByText("10-Minute Setup Wizard")).toBeVisible();
    ...
    await bioInput.fill(
      "I am Maya, I run a local bakery making custom vegan cakes in Portland, OR.",
    );
  ...
  test("should complete the zero-click onboarding flow", async ({ page }) => {
    await page.goto("/onboarding");

    // Step 0: Click "Zero-Click Build"
    await page.getByRole("button", { name: "Zero-Click Build" }).click();

    // Step -3: Zero-Click input screen
    const textArea = page.getByPlaceholder(/e.g., I'm a baker/);
    await expect(textArea).toBeVisible();
    await textArea.fill(
      "I'm a baker specializing in custom vegan cakes in Austin.",
    );

    // Click "Build My Business"
    await page.getByRole("button", { name: "Build My Business" }).click();
    ...
    const openAssistantBtn = page.getByRole("link", { name: "Open Assistant" });
    await expect(openAssistantBtn).toBeVisible();
  });
});
test("should complete the zero-click onboarding flow successfully", async ({
  page,
  context,
}) => {
  // Note: To prevent test timeout we mock the generate response since there is no backend
  await context.route("**/api/onboarding/generate", async (route) => {
    route.fulfill({
      status: 200,
      json: { success: true, organization_id: "test-tenant-123" },
    });
  ...
  // Mock API requests after generation
  await context.route("**/api/onboarding/launch", async (route) => {
    route.fulfill({ status: 200, json: { status: "launched" } });
  });

  await page.getByRole("button", { name: "Build My Business" }).click();

  // Verify it reached the final screen
  await expect(page.getByText("Building Your Business...")).toBeVisible();
  await expect(page.getByText("You're Live!")).toBeVisible({ timeout: 5000 });
});
