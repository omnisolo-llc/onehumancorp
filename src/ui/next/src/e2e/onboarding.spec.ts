...
test.describe("OnboardingWizard CUJ", () => {
  ...
  test("Maya the Baker can complete the onboarding flow", async ({ page }) => {
    ...
    await expect(
      page.locator('input[value="I bake custom vegan cakes f..."]'),
    ).toBeVisible();
  ...
  test("Carlos the Handyman sets up his repair business", async ({ page }) => {
    ...
    await expect(
      page.locator('input[value="Plumbing and general repairs"]'),
    ).toBeVisible();
    await page.getByRole("button", { name: "Continue" }).click();
  ...
  test("Leo the Music Tutor configures online bookings", async ({ page }) => {
    ...
    await expect(
      page.locator('input[value="Guitar tutoring online"]'),
    ).toBeVisible();
  ...
  test("Fatima the Food Cart Operator on a slower network", async ({
    ...
    await expect(
      page.locator('input[value="Halal food cart pickup orders"]'),
    ).toBeVisible();
    await page.getByRole("button", { name: "Continue" }).click();
  ...
  test("User can save a draft and restore it across sessions", async ({
    ...
    await expect(
      page.locator('input[value="My Restored Business"]'),
    ).toBeVisible();
  });
...
test("Instant Build handles network failures gracefully without mock data", async ({
  page,
  context,
}) => {
  await page.goto("/onboarding");
  ...
  // Fill the form
  await page
    .getByPlaceholder(/e.g. I run a local bakery/i)
    .fill("Failing business info");

  // Intercept the API route to fail
  await context.route("/api/onboarding/intake", (route) =>
    route.abort("failed"),
  );
  ...
  await expect(
    page.getByText(/Failed to launch. Please try again./i),
  ).toBeVisible();
...
test("Step-by-step intake handles backend processing errors correctly", async ({
  page,
  context,
}) => {
  await page.goto("/onboarding");
...
test("Store launch correctly fails when start API is down", async ({
  page,
  context,
}) => {
  await page.goto("/onboarding");
  ...
  await context.route("/api/onboarding/start", (route) =>
