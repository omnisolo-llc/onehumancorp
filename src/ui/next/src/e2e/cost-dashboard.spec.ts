...
test.describe('Cost Dashboard Loop', () => {
  test('Cost dashboard loads and displays data', async ({ page }) => {
    ...
    await expect(page.locator('span', { hasText: 'Prompt Caching Savings' })).toBeVisible();
    await expect(page.locator('span', { hasText: 'Storage Compression Savings' })).toBeVisible();
