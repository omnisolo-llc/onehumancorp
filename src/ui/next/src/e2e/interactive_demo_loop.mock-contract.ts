...
test.describe('Interactive Demo Generator Growth Loop', () => {
    test('generator links from somewhere or is accessible directly, generates an embed with a viral footer', async ({ page, adminUser, loginAs }) => {
        ...
        await expect(page.locator('.bg-white.border.border-gray-200')).toContainText('⚡ Powered by OmniSolo');
        ...
        expect(embedHtml).toContain('⚡ Powered by OmniSolo');
        ...
        await expect(page.locator('.bg-white.border.border-gray-200')).not.toContainText('⚡ Powered by OmniSolo');

        // Watermark should be gone in code output
        const newEmbedHtml = await codeOutput.inputValue();
        expect(newEmbedHtml).not.toContain('⚡ Powered by OmniSolo');
    });
});
