...
describe('AgentFeedCard', () => {
    ...
    it('calls onEdit with draft_id and modified text when Save button is clicked', () => {
        const mockApprove = vi.fn();
        const mockEdit = vi.fn();
        ...
        // Find the textarea in the editing state
        ...
        fireEvent.change(textarea, { target: { value: 'New customized response!' } });

        // Click Save
        const saveButton = screen.getByTestId('feed-save-btn');
        fireEvent.click(saveButton);

        expect(mockEdit).toHaveBeenCalledWith('draft-123', 'New customized response!');
