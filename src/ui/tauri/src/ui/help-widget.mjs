...
// --- Global Tooltip & Walkthrough Logic ---
    ...
    function showTooltip(e, text) {
        ...
        let tt = e.target;
        let tTarget = null;
        while (tt && tt !== document) {
            if ((tt.hasAttribute && tt.hasAttribute('data-tooltip')) || (tt.id && window.OMNISOLO_TOOLTIPS && window.OMNISOLO_TOOLTIPS[tt.id])) {
                tTarget = tt;
                break;
            }
            tt = tt.parentNode;
        }
        const targetRect = tTarget ? tTarget.getBoundingClientRect() : e.target.getBoundingClientRect();
