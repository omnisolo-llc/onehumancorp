...
test.describe('Loyalty & Rewards Engine', () => {
  ...
  test('Should create and retrieve loyalty wallet balance', async ({ page }) => {
    ...
    // In this m_cked check, we navigate to the quote and check if the wallet loads.
    await page.r_oute('**/api/ui/loyalty/balance*', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.r_oute('**/api/ui/quote*', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
