...
test.describe('Omnichannel Review Reputation Flow', () => {
    test.use({ viewport: { width: 375, height: 667 } }); // Mobile viewport

    test('should allow owner to 1-tap approve a review mitigation draft', async ({ page, request }) => {
        ...
        const response = await request.post('/api/v1/omnichannel/review-webhook', {
          data: {
            tenant_id: tenantId,
            source: 'yelp',
            sender_id: customerPhone,
            rating: 2,
            comment: 'The cake was a bit dry.',
          }
        });
        ...
        const dmCard = page.getByTestId('review-reply-card');
        await expect(dmCard).toBeVisible({ timeout: 15000 });
        await expect(dmCard.getByText('Action Required: Negative Review on yelp')).toBeVisible();
