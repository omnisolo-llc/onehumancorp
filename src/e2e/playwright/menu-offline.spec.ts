...
test.describe('Menu Management Offline Sync', () => {
  ...
  test.beforeEach(async ({ page, context }) => {
    ...
    await page.goto('http://127.0.0.1:3000/menu-management');
    await page.evaluate(() => localStorage.clear());
    await page.evaluate(() => localStorage.setItem('tenant_id', 'test-tenant'));
    await page.reload();
  ...
  test('performs optimistic UI updates and syncs queue when offline', async ({ page, context }) => {
    // Wait for the menu to be populated from the initial online fetch
