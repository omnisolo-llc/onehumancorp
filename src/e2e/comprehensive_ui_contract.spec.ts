...
function normalizeInternalHref(href: string): string | null {
  ...
  try {
    const url = new URL(href, 'http://test.base');
    if (url.origin !== 'http://test.base') return null;
  ...
  test('visible external and protocol links use expected destinations', async ({ page }) => {
    ...
    for (const route of appRoutes) {
      ...
      for (const link of hrefs) {
        ...
        const url = new URL(link.href, 'http://test.base');
        if (url.origin === 'http://test.base') continue;
