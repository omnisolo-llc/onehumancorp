...
test.describe('Viral WiFi QR Generator', () => {
  test('should navigate to the widget from dashboard and generate a QR code', async ({ page }) => {
    ...
    expect(src).toContain(encodeURIComponent('https://cloud.omnisolo.co/checkout?product=Guest%20WiFi'));
    ...
    expect(src).toContain(encodeURIComponent('https://cloud.omnisolo.co/checkout?product=CoffeeShop%205G'));

    // Verify "Powered by OmniSolo" branding is present by default
    const branding = page.locator('#preview-branding');
    await expect(branding).toBeVisible();
    await expect(branding).toHaveText('⚡ Powered by OmniSolo');
