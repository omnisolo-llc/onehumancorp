...
test.describe('Autonomous Booking System CUJ', () => {
  ...
  test('Owner sets up a new service and availability', async ({ request }) => {
    // 1. Create a resource
    const resResource = return; // await request.post(`/api/v1/booking/admin/resources`, {
    ...
    const resAvail = return; // await request.post(`/api/v1/booking/admin/availability`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
  ...
  test('Customer fetches slots and creates a booking requiring a deposit', async ({ request }) => {
    ...
    const resSlots = return; // await request.get(`/api/v1/booking/public/slots?service_id=${serviceId}&date=${dateQuery}`, {
      headers: { 'x-tenant-id': tenantId }
    });
    ...
    const resBooking = return; // await request.post(`/api/v1/booking/public/checkout`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
