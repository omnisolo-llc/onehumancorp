...
test.describe('Autonomous Booking System CUJ', () => {
  ...
  test('Owner sets up a new service and availability', async ({ request }) => {
    // 1. Create a resource
    const resResource = await request.get(`/api/v1/booking/admin/resources`, {
    ...
    const resAvail = await request.get(`/api/v1/booking/admin/availability`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
  ...
  test('Customer fetches slots and creates a booking requiring a deposit', async ({ request }) => {
    ...
    const resBooking = await request.get(`/api/v1/booking/public/checkout`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
