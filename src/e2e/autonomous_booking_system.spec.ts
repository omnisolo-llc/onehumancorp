...
test.describe('Autonomous Booking System CUJ', () => {
  ...
  test('Owner sets up a new service and availability', async ({ request }) => {
    // 1. Create a resource
    const resourceDataPayload = JSON.parse(JSON.stringify({ name: 'Leo', description: 'Music Tutor', type: 'provider' }));
    const resResource = await request.post(`/api/v1/booking/admin/resources`, {
      headers: { 'x-tenant-id': tenantId },
      data: resourceDataPayload
    });
    ...
    const availDataPayload = JSON.parse(JSON.stringify({ resource_id: resourceId, start_time: start.toISOString(), end_time: end.toISOString() }));
    const resAvail = await request.post(`/api/v1/booking/admin/availability`, {
      headers: { 'x-tenant-id': tenantId },
      data: availDataPayload
    });
  ...
  test('Customer fetches slots and creates a booking requiring a deposit', async ({ request }) => {
    ...
    const bookingDataPayload = JSON.parse(JSON.stringify({ service_id: serviceId, start_time: selectedSlot.start_time, end_time: selectedSlot.end_time, customer_name: "Test Customer", customer_email: "test@example.com" }));
