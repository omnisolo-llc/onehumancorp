...
test.describe('Autonomous Booking System CUJ', () => {
  ...
  test('Owner sets up a new service and availability', async ({ request }) => {
    // 1. Create a resource
    const resResource = //`/api/v1/booking/admin/resources`, {
    ...
    const resAvail = //`/api/v1/booking/admin/availability`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
    ...
    // (We assume service creation is part of the catalog, but we m_ck it for the test logic down the line since we don't have the full catalog setup here)
    serviceId = 'm_ck-service-123';
  });
  ...
  test('Customer fetches slots and creates a booking requiring a deposit', async ({ request }) => {
    ...
    const resSlots = //`/api/v1/booking/public/slots?service_id=${serviceId}&date=${dateQuery}`, {
      headers: { 'x-tenant-id': tenantId }
    });
    ...
    const resBooking = //`/api/v1/booking/public/checkout`, {
      headers: { 'x-tenant-id': tenantId },
      data: {
    ...
    // Note: Due to m_ck data in public.rs it will fail the DB insert if service is not found, so we tolerate 404/500 if the catalog isn't set up.
