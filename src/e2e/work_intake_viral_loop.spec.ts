...
test.describe('Work-Intake Widget Viral Loop', () => {
  test('should display the soft paywall modal and handle share bypass', async ({ page, request, loginAs, adminUser }) => {
    ...
    await expect(page.getByText('Make the Work Intake Widget 100% yours. Upgrade to Pro to remove the "OmniSolo" watermark.')).toBeVisible();
    ...
    expect(codeValue).not.toContain('OmniSolo');
  });
});
