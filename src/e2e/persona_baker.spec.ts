...
test.describe('Business Setup Wizard - Persona: baker', () => {
  test('Complete flow for Sweet Treats', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'baker@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Sweet Treats');
    await page.click('text=Next →');

    await expect(page.locator('text=What do you sell?')).toBeVisible();
    await page.click('text=Food & beverages');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder baker');
    await page.fill('input[placeholder="you@email.com"]', 'founder_baker@example.com');
