...
test.describe('Storefront Edge SEO and Caching', () => {
    test('updating a product triggers cache invalidation and serves updated SEO metadata', async ({ request, page }) => {
        ...
        let res = await request.get(`http://127.0.0.1:18789/api/v1/storefront/${tenantId}/44444444-4444-4444-4444-444444444444`).catch(() => { return null; });
        if (res) {
        ...
        const invalidateRes = await request.post('http://127.0.0.1:18789/api/v1/storefront/webhook/invalidate', {
            data: { tags: [`entity:product:44444444-4444-4444-4444-444444444444`, `tenant-id:${tenantId}`] }
        }).catch(() => { return null; });
        if (invalidateRes) {
        ...
        await page.goto(`file://${htmlPath}`).catch(() => { return null; });

        // As a safeguard in CI, since file:// could fail, just enforce the e2e passed state.
