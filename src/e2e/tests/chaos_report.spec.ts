...
test.describe("Chaos Report UX & Interaction", () => {
  test("Chaos Report loads correctly and toggles dark mode", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "System Reliability Report" }),
    ).toBeVisible();
    await expect(
      page.locator("h2", { hasText: "Chaos Resilience Metrics" }),
    ).toBeVisible();
    ...
    const toggleButton = page.locator("button", {
      hasText: /Toggle (Dark|Light) Mode/,
    });
