...
test.describe('Business Setup Wizard - Persona: writer', () => {
  test('Complete flow for Word Smith', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'writer@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Word Smith');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder writer');
    await page.fill('input[placeholder="you@email.com"]', 'founder_writer@example.com');
