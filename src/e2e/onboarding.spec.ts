...
test.describe('Onboarding Wizard', () => {
  ...
  test('Maya (The Home Baker) onboarding flow', async ({ page }) => {
    ...
    await page.getByPlaceholder("e.g. Maya's Custom Cakes").pressSequentially("Maya's Custom Cakes");
    await page.getByRole('button', { name: 'Next' }).click();

    // Step 1 - Chat 2
    await expect(page.getByRole('heading', { name: 'What do you sell?', exact: false })).toBeVisible({ timeout: 15000 });
    await page.getByPlaceholder('e.g. I bake custom vegan cakes for weddings and parties...').pressSequentially('maya bakes custom cakes');
    await page.getByRole('button', { name: 'Next' }).click();

    // Step 1 - Chat 3
    await expect(page.getByRole('heading', { name: 'Where are you located?', exact: false })).toBeVisible({ timeout: 15000 });
    await page.getByPlaceholder('e.g. Portland, OR').pressSequentially('Portland, OR');
  ...
  test('Carlos (Handyman) onboarding flow', async ({ page }) => {
    ...
    await page.getByPlaceholder("e.g. Maya's Custom Cakes").pressSequentially("Carlos Plumbing");
    await page.getByRole('button', { name: 'Next' }).click();

    // Step 1 - Chat 2
    await expect(page.getByRole('heading', { name: 'What do you sell?', exact: false })).toBeVisible({ timeout: 15000 });
    await page.getByPlaceholder('e.g. I bake custom vegan cakes for weddings and parties...').pressSequentially('carlos does handyman services');
    await page.getByRole('button', { name: 'Next' }).click();

    // Step 1 - Chat 3
    await expect(page.getByRole('heading', { name: 'Where are you located?', exact: false })).toBeVisible({ timeout: 15000 });
    await page.getByPlaceholder('e.g. Portland, OR').pressSequentially('Miami, FL');
