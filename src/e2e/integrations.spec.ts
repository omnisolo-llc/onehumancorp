...
test.describe('Integrations Page', () => {
  ...
  test('opens integrations from dashboard quick actions', async ({ page }) => {
    ...
    await expect(page.locator('#meta_graph-integration').getByRole('button', { name: 'Configure' })).toBeVisible();
  });

  test('configure action routes to inbox', async ({ page }) => {
    await page.getByRole('button', { name: 'Back to Dashboard' }).click();
    await page.getByRole('button', { name: 'Integrations' }).click();
    await page.locator('#meta_graph-integration').getByRole('button', { name: 'Configure' }).click();
