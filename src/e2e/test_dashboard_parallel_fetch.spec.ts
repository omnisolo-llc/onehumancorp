...
test.describe('Dashboard Parallel Fetch', () => {
  test('displays all parallel-fetched components', async ({ page }) => {
    ...
    const ordersContainer = page.locator('.app-panel:has-text("Recent Orders")').first();
  ...
  test('Exit Intent Generator link is visible under Growth & Virality', async ({ page }) => {
    await page.goto('/dashboard');
    await expect(page.locator('text=Growth & Virality')).toBeVisible();
    await expect(page.locator('text=Exit-Intent Generator')).toBeVisible();
    await expect(page.locator('text=Capture customers before they leave with an AI-powered discount pop-up.')).toBeVisible();
  });
});
