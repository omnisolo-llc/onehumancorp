...
test('setup onboarding mobile-first inputs and logic', async ({ page }) => {
  ...
  const tauriUiDir = process.env.RUNFILES_DIR ? require('path').join(process.env.RUNFILES_DIR, '_main', 'src', 'ui', 'tauri', 'src', 'ui') : require('path').join(process.cwd(), 'src', 'ui', 'tauri', 'src', 'ui');
