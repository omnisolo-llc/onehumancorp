...
test.describe('Business Setup Wizard - Persona: makeup_artist', () => {
  test('Complete flow for Glamour Girl', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'makeup_artist@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Glamour Girl');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder makeup_artist');
    await page.fill('input[placeholder="you@email.com"]', 'founder_makeup_artist@example.com');
