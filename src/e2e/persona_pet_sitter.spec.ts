...
test.describe('Business Setup Wizard - Persona: pet_sitter', () => {
  test('Complete flow for Pet Pals', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'pet_sitter@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Pet Pals');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder pet_sitter');
    await page.fill('input[placeholder="you@email.com"]', 'founder_pet_sitter@example.com');
