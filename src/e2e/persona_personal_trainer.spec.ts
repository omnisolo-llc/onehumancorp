...
test.describe('Business Setup Wizard - Persona: personal_trainer', () => {
  test('Complete flow for Fit Fam', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'personal_trainer@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Fit Fam');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder personal_trainer');
    await page.fill('input[placeholder="you@email.com"]', 'founder_personal_trainer@example.com');
