...
export function currentAppSmoke(label: string) {
  test(`current embedded app smoke: ${label}`, async ({ page, request }) => {
    ...
    await page.goto('/unboxing-inserts');
    await expect(page.getByRole('heading', { name: 'Printable Inserts' }).first()).toBeVisible();
    await expect(page.getByRole('button', { name: 'Print Insert' }).first()).toBeVisible();
