...
test.describe('Tool Integrations UI Premium Dashbaord', () => {
  ...
  test('displays social media integration card', async ({ page }) => {
    ...
    await expect(page.getByText('Unified Social Media Inbox and Cross-Posting across all major social networks.')).toBeVisible();
  ...
  test('displays online booking integration card', async ({ page }) => {
    ...
    await expect(page.getByText('Zero-Config Booking & Calendar Sync to prevent double-booking.')).toBeVisible();
