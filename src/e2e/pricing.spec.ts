...
test.describe('Pricing Page', () => {
  ...
  test('should display "What does this cost?" wizard flow correctly', async ({ page }) => {
    ...
    // Wait for Dashboard
    await expect(page.locator('text=Dashboard').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();

    // Navigate to Billing / Pricing Wizard
    await page.click('button:has-text("Billing")');

    // Check initial step (Usage)
    await expect(page.locator('text=/Your Current Usage/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
    await expect(page.locator('text=/Projected Cost this Month/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
    ...
    await expect(page.locator('text="Start Free"').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  ...
  test('should display pricing page', async ({ page }) => {
    ...
    await expect(page.locator('text=/pricing|plan/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });

  test('should show plan comparison', async ({ page }) => {
    await page.goto('/pricing');
    await expect(page.locator('text=/plan|comparison/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });

  test('should display free plan', async ({ page }) => {
    await page.goto('/pricing');
    await expect(page.locator('text=/free|starter/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });

  test('should display pro plan', async ({ page }) => {
    await page.goto('/pricing');
    await expect(page.locator('text=/pro|professional/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });

  test('should display enterprise plan', async ({ page }) => {
    await page.goto('/pricing');
    await expect(page.locator('text=/enterprise|business/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });

  test('should show plan prices', async ({ page }) => {
    await page.goto('/pricing');
    const price = page.locator('text=/\\$\\d+/').filter({ visible: true }).filter({ visible: true }).first();
    await expect(price).toBeVisible();
  });

  test('should highlight recommended plan', async ({ page }) => {
    await page.goto('/pricing');
    const recommended = page.locator('text=/recommended|popular|best/i').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should show feature list', async ({ page }) => {
    ...
    await expect(features.filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });
  ...
  test('should select pro plan', async ({ page }) => {
    await page.goto('/pricing');
    const proButton = page.locator('button:has-text("Pro"), button:has-text("Choose")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should start free plan', async ({ page }) => {
    await page.goto('/pricing');
    const freeButton = page.locator('button:has-text("Free"), button:has-text("Start")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should contact sales for enterprise', async ({ page }) => {
    await page.goto('/pricing');
    const contactButton = page.locator('button:has-text("Contact"), button:has-text("Sales")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should show annual billing discount', async ({ page }) => {
    await page.goto('/pricing');
    const annualToggle = page.locator('text=/annual|monthly/i').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should display FAQ section', async ({ page }) => {
    await page.goto('/pricing');
    const faqSection = page.locator('text=/faq|questions|help/i').filter({ visible: true }).filter({ visible: true }).first();
    await expect(faqSection).toBeVisible();
  });

  test('should expand FAQ item', async ({ page }) => {
    await page.goto('/pricing');
    const faqItem = page.locator('[class*="faq"], [class*="question"]').filter({ visible: true }).filter({ visible: true }).first();
...
test.describe('My Plan Page', () => {
  ...
  test('should display over storage quota warning on My Plan dashboard', async ({ page }) => {
    ...
    await page.locator('text=/Upload Photo/i').filter({ visible: true }).filter({ visible: true }).first().click();
    ...
    await expect(page.locator('text=/Storage Used:/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });
  test('should display current plan', async ({ page }) => {
    await page.goto('/my-plan');
    await expect(page.locator('text=/my.*plan|current.*plan/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });
  ...
  test('should show billing history', async ({ page }) => {
    await page.goto('/my-plan');
    const historyBtn = page.locator('button:has-text("History"), button:has-text("Invoices")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should upgrade plan', async ({ page }) => {
    await page.goto('/my-plan');
    const upgradeBtn = page.locator('button:has-text("Upgrade"), button:has-text("Change Plan")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should cancel subscription', async ({ page }) => {
    await page.goto('/my-plan');
    const cancelBtn = page.locator('button:has-text("Cancel"), button:has-text("Unsubscribe")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should update payment method', async ({ page }) => {
    await page.goto('/my-plan');
    const paymentBtn = page.locator('button:has-text("Payment"), button:has-text("Update")').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should download invoice', async ({ page }) => {
    await page.goto('/my-plan');
    const downloadBtn = page.locator('button:has-text("Download"), [class*="download"]').filter({ visible: true }).filter({ visible: true }).first();
  ...
  test('should open cost transparency dashboard', async ({ page }) => {
    ...
    await expect(page.locator('text=/my.*plan|current.*plan/i').filter({ visible: true }).filter({ visible: true }).first().filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();

    // Verify Cost Details button and click it
    const detailsBtn = page.locator('button:has-text("View Cost Details")').filter({ visible: true }).filter({ visible: true }).first();
    await expect(detailsBtn).toBeVisible();
    await detailsBtn.click();

    // Assert Cost Dashboard appears
    await expect(page.locator('text=/Cost & AI Usage/i').filter({ visible: true }).filter({ visible: true }).first()).toBeVisible();
  });
});
