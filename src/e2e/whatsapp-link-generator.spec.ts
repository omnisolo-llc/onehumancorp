...
test.describe('WhatsApp Link Generator Growth Loop', () => {
    test('renders generator, handles input, shows link, and verifies branding', async ({ page }) => {
        ...
        // Check the "Powered by OmniSolo" footer is present on the page
        const footerLink = page.locator('footer a', { hasText: '⚡ Powered by OmniSolo' });
        ...
        await expect(previewText).toContainText('⚡ Powered by OmniSolo');
        ...
        const removeBrandingToggle = page.locator('label', { hasText: 'Remove "Powered by OmniSolo" Badge (Pro)' });
