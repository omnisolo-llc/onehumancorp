...
test.describe('Social Share Cards Direct Intents - Owner Journey', () => {
    test('owner navigates from dashboard to share cards, configures, and verifies social share buttons', async ({ page }) => {
        ...
        // Check for the "Powered by OmniSolo" watermark text in the preview
        const poweredBy = page.locator('span', { hasText: 'Powered by OmniSolo' });
        ...
        // Ensure "Powered by OmniSolo" is embedded in the encoded text, if applicable,
        // or check that the URL contains the encoded store link logic.
        expect(twitterHref).toContain('text=');
        expect(twitterHref).toContain(encodeURIComponent('Powered by OmniSolo'));
        ...
        expect(fbHref).toContain(encodeURIComponent('Powered by OmniSolo'));
        ...
        expect(waHref).toContain(encodeURIComponent('Powered by OmniSolo'));
