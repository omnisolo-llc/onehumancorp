...
test.describe('Onboarding Wizard - Cross Device Resilience', () => {
  test('persists state correctly across separate sessions', async ({ browser }) => {
    ...
    await expect(page1.locator('text="Tell us about your business"')).toBeVisible({ timeout: 15000 });
    ...
    // Wait for the review step so we know it successfully started processing
    await expect(page1.getByRole('heading', { name: "Review Details" })).toBeVisible({ timeout: 5000 });
    ...
    await expect(page2.locator('text="Review Details"')).toBeVisible({ timeout: 15000 });
    ...
    await expect(page2.locator('input[value="Maya\'s Cross-Device Bakery"]')).toBeVisible();
