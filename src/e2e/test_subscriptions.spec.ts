...
test.describe('AI-Driven Subscription & Membership Lifecycle Management', () => {
  test('Subscription offer generation UI handles natural language parsing via m_ck', async ({ page, adminUser, loginAs }) => {
    ...
    // Route the API call to return a m_cked response since we don't have the LLM running consistently in E2E
    await page.r_oute('**/api/v1/subscription/parse', async (route) => {
        await route.fulfill({
            status: 200,
