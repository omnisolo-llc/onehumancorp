...
test.describe('AI-Driven Subscription & Membership Lifecycle Management', () => {
  test('Subscription offer generation UI handles natural language parsing via mock', async ({ page, adminUser, loginAs }) => {
    // Use test fixture seed parameter to configure test state
    await page.goto('/ui/subscription-offer-generator.html?test_fixture_seed=subscription_parse');
    ...
    /* await page.evaluate(() => {
        window.sessionStorage.setItem('e2e_mock_subscription_parse', JSON.stringify({
