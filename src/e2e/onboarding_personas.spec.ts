...
test.describe('OHC Onboarding Persona Critical User Journeys', () => {
  ...
  for (const persona of personas) {
    test(`CUJ: ${persona.name} Onboarding Flow`, async ({ page }) => {
      ...
      // Step 2: Review Details (AI should have filled these)
      ...
      // Verify dashboard and agent role (mocked logic normally seeds these)
      await page.getByRole('link', { name: /Go to Dashboard/i }).click();
      await expect(page.getByRole('heading', { name: 'Dashboard' })).toBeVisible({ timeout: 15000 });

      // Navigate to Agents to see the generated workforce
