...
test.describe('Business Setup Wizard - Persona: carpenter', () => {
  test('Complete flow for Wood Work', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'carpenter@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Wood Work');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder carpenter');
    await page.fill('input[placeholder="you@email.com"]', 'founder_carpenter@example.com');
