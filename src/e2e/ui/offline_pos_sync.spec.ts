...
test.describe('Offline Mobile Sync & Tap-to-Pay Architecture', () => {
  ...
  test('should trigger Customer Success Agent draft on offline payment failure', async ({ page, context }) => {
    // Navigate to terminal
    await page.goto('/pos.html');
    ...
    // Modify the payload in local storage to force a payment failure
    await page.evaluate(() => {
        const queue = JSON.parse(localStorage.getItem('ohc_offline_queue') || '[]');
        if (queue.length > 0) {
            queue[0].payload = JSON.stringify({ payment_status: "failed" });
            localStorage.setItem('ohc_offline_queue', JSON.stringify(queue));
        }
    });
    ...
    // Wait for the action card generated by Customer Success Agent
    await expect(page.getByText(/Hi, your card at .* couldn't be processed later/)).toBeVisible({ timeout: 15000 });
  });
