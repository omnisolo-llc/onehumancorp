...
test.describe('Agentic Work Triage Feed', () => {
  test('Owner can review and approve AI-drafted replies', async ({ page, loginAs, adminUser }) => {
    ...
    const response = await page.request.post(`/api/dev/simulate-triage-item?tenant_id=e2e-tenant`);
    ...
    await expect(card).toContainText('Hi! Yes, we have 2 vegan chocolate cakes left for this weekend. Would you like me to hold one for you? [Link to $20 deposit]');
