...
test('Conversational Growth Loop CUJ', async ({ page, loginAs, adminUser }) => {
  ...
  await expect(page.locator('.message.agent').last()).toContainText(/Powered by OmniSolo/i);
  ...
  await expect(page.locator('.action-card').last()).toContainText(/Powered by OmniSolo/i);
});
