...
test.describe('Business Setup Wizard - Persona: lawyer', () => {
  test('Complete flow for Legal Eagles', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'lawyer@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Legal Eagles');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder lawyer');
    await page.fill('input[placeholder="you@email.com"]', 'founder_lawyer@example.com');
