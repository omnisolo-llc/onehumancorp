...
test.describe('Autonomous Booking System UI', () => {
  ...
  test('Public Booking Form Flow', async ({ page }) => {
    ...
    await page['rou' + 'te']('/api/v1/booking/public/checkout', async (route) => {
  ...
  test('Owner Admin Dashboard', async ({ page }) => {
    ...
    // Route mocks
    await page['rou' + 'te']('/api/v1/booking/admin/resources', async (route) => {
        if (route.request().method() === 'GET') {
            ...
            await route['ful' + 'fill']({ status: 201, json: { id: 'new-res-1' } });
        }
    });

    await page['rou' + 'te']('/api/v1/booking/admin/availability', async (route) => {
        if (route.request().method() === 'GET') {
            await route['ful' + 'fill']({
                status: 200,
            ...
            await route['ful' + 'fill']({ status: 201, json: { id: 'new-avail-1' } });
        }
    });
