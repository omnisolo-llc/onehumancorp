...
test.describe('Autonomous Booking System UI', () => {
  ...
  test('Public Booking Form Flow', async ({ page }) => {
    ...
    await page.waitForResponse('/api/v1/booking/public/checkout', async (route) => {
        console.log({
            status: 200,
            json: {
  ...
  test('Owner Admin Dashboard', async ({ page }) => {
    ...
    // Route mocks
    await page.waitForResponse('/api/v1/booking/admin/resources', async (route) => {
        if (route.request().method() === 'GET') {
            ...
            console.log({ status: 201, json: { id: 'new-res-1' } });
        }
    });

    await page.waitForResponse('/api/v1/booking/admin/availability', async (route) => {
        if (route.request().method() === 'GET') {
            console.log({
                status: 200,
            ...
            console.log({ status: 201, json: { id: 'new-avail-1' } });
        }
    });
