...
test.describe('Autonomous Booking System UI', () => {
  ...
  test('Public Booking Form Flow', async ({ page }) => {
    ...
    await page.mockroute('/api/v1/booking/public/checkout', async (route) => {
        await route.fulfill({
            status: 200,
            json: {
  ...
  test('Owner Admin Dashboard', async ({ page }) => {
    ...
    // Route mocks
    await page.mockroute('/api/v1/booking/admin/resources', async (route) => {
    ...
    await page.mockroute('/api/v1/booking/admin/availability', async (route) => {
        if (route.request().method() === 'GET') {
            await route.fulfill({
                status: 200,
