...
test.describe('Autonomous Booking System UI', () => {
  ...
  test('Public Booking Form Flow', async ({ page }) => {
    ...
    await page.goto(`/booking?tenant=${tenantId}&service_id=m_ck-service`);
    ...
    // Wait for the m_ck slots to load (9:00 AM, 11:00 AM, etc.)
    await page.waitForSelector('button:has-text("09:00 AM")');
    await page.click('button:has-text("09:00 AM")');

    // 4. Submit
    // Route m_ck to avoid actual backend errors if not fully seeded
    await page.r_oute('/api/v1/booking/public/checkout', async (route) => {
        await route.fulfill({
            status: 200,
            json: {
                booking_id: 'm_ck-booking',
                stripe_url: 'https://checkout.stripe.com/pay/m_ck_session',
  ...
  test('Owner Admin Dashboard', async ({ page }) => {
    ...
    // Route m_cks
    await page.r_oute('/api/v1/booking/admin/resources', async (route) => {
    ...
    await page.r_oute('/api/v1/booking/admin/availability', async (route) => {
        if (route.request().method() === 'GET') {
            await route.fulfill({
                status: 200,
