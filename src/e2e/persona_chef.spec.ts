...
test.describe('Business Setup Wizard - Persona: chef', () => {
  test('Complete flow for Chef's Kiss', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'chef@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Chef's Kiss');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder chef');
    await page.fill('input[placeholder="you@email.com"]', 'founder_chef@example.com');
