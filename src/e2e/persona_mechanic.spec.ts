...
test.describe('Business Setup Wizard - Persona: mechanic', () => {
  test('Complete flow for Auto Fix', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'mechanic@example.com');
    ...
    await page.click('text=Local Business');
    await page.click('text=Next →');

    await expect(page.locator('text=What is your business called?')).toBeVisible();
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Auto Fix');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder mechanic');
    await page.fill('input[placeholder="you@email.com"]', 'founder_mechanic@example.com');
