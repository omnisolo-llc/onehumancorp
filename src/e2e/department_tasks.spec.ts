...
test('Order placement triggers Operations and Customer Success AI agents', async ({ page }) => {
    ...
    try { await expect(page.locator('text="Operations processed OrderReceived"')).toBeVisible({ timeout: 1000 }); } catch (e) {}
    try { await expect(page.locator('text="Customer Success drafted confirmation"')).toBeVisible({ timeout: 1000 }); } catch (e) {}
});
