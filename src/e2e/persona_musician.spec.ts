...
test.describe('Business Setup Wizard - Persona: musician', () => {
  test('Complete flow for Music Maker', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'musician@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Music Maker');
    await page.click('text=Next →');

    await expect(page.locator('text=What do you sell?')).toBeVisible();
    await page.click('text=Digital downloads');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder musician');
    await page.fill('input[placeholder="you@email.com"]', 'founder_musician@example.com');
