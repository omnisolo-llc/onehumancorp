import { test, expect } from './fixtures';
import { currentAppSmoke } from './current_app_smoke';

test('growth_referral_widget_milestone', async ({ page, request, loginAs, adminUser }) => {
  await loginAs(page, adminUser);
  await currentAppSmoke(page, request, 'growth_referral_widget_milestone');
});

test.describe('Growth Referral Widget Milestone UI', () => {
  test('should display 10th order and 100th order milestone alert and card on Team Page', async ({ page, loginAs, unlimitedAdminUser }) => {
    // Login
    await loginAs(page, unlimitedAdminUser);

    // Navigate to /team where the widget is embedded
    await page.goto('/team');
    await page.waitForLoadState('networkidle');

    // Verify 100th milestone banner exists since we seeded it
    await expect(page.getByRole('heading', { name: /100th Order! Share your success/i })).toBeVisible();

    // Verify WhatsApp share button exists
    await expect(page.getByRole('link', { name: /Share to WhatsApp/i })).toBeVisible();

    // Verify the milestone card image is loaded
    const milestoneImage = page.locator('img[alt="100th Order Milestone"]');
    await expect(milestoneImage).toBeVisible();
    await expect(milestoneImage).toHaveAttribute('src', /milestone_id=100th_order/);
  });
});
