...
test.describe('Business Setup Wizard - Persona: electrician', () => {
  test('Complete flow for Shock Value', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'electrician@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Shock Value');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder electrician');
    await page.fill('input[placeholder="you@email.com"]', 'founder_electrician@example.com');
