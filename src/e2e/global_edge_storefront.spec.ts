...
test.describe('Global Edge-Cached Dynamic Storefronts E2E', () => {
  ...
  test('generates edge storefront with premium styling and seo', async ({ page }) => {
    ...
    await page.route(`**/api/v1/builder/edge/${tenantId}/${siteId}`, async route => {
      await route.fulfill({
        ...
        <head>
          ...
          <meta name="description" content="Welcome to the Premium Store generated by OHC Auto SEO">
          <style>.glass-container { border: 1px solid rgba(255, 255, 255, 0.4); }</style>
    ...
    // Verify SEO meta tag injection
    const metaDescription = page.locator('meta[name="description"]');
    await expect(metaDescription).toHaveAttribute('content', 'Welcome to the Premium Store generated by OHC Auto SEO');

    // Verify JSON-LD Schema
    const scriptLdJson = page.locator('script[type="application/ld+json"]');
    await expect(scriptLdJson).toHaveText('{"@context":"https://schema.org","@type":"LocalBusiness","name":"Premium Store"}');
  });
  ...
  test('validates cache invalidation after tenant.inventory.updated', async ({ page }) => {
     const tenantId = "test-tenant-uuid";
     const siteId = "test-site-uuid";

     // 1. Initial page load (cached at edge)
     await page.route(`**/api/v1/builder/edge/${tenantId}/${siteId}`, async route => {
      await route.fulfill({
        status: 200,
        contentType: 'text/html',
        headers: { 'Cache-Tag': 'entity:product:prod-123' },
        body: `<body><div class="glass-container"><div class="product-stock">In Stock: 10</div></div></body>`
      });
    });

    await page.goto(`/api/v1/builder/edge/${tenantId}/${siteId}`);
    await expect(page.locator('.product-stock')).toHaveText('In Stock: 10');

    // 2. Simulate inventory update triggered cache invalidation (OperationsAgent purging 'entity:product:prod-123')
    await page.route(`**/api/v1/builder/edge/${tenantId}/${siteId}`, async route => {
      await route.fulfill({
        status: 200,
        contentType: 'text/html',
        headers: { 'Cache-Tag': 'entity:product:prod-123' },
        body: `<body><div class="glass-container"><div class="product-stock">In Stock: 0</div></div></body>`
      });
    });

    // 3. Reload page (forces a fresh fetch since cache was invalidated)
    await page.reload();

    // Verify stock is immediately updated to 0, preventing overselling
    await expect(page.locator('.product-stock')).toHaveText('In Stock: 0');
  });
});
