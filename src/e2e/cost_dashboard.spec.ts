...
test.describe('Cost Dashboard "My Plan" functionality', () => {
  test('Cost Dashboard renders the "My Plan" fields completely', async ({ page, adminUser, loginAs }) => {
    ...
    await expect(page.locator('h1', { hasText: 'My Plan' }).first()).toBeVisible();
    await expect(page.locator('h2', { hasText: 'Plan:' }).first()).toBeVisible();
    ...
    await expect(page.locator('h2', { hasText: 'Estimated Next Bill:' }).first()).toBeVisible();
    await expect(page.locator('button', { hasText: 'View Upgrade Plans' }).first()).toBeVisible();
  ...
  test('Cost Dashboard renders limits correctly for Pro tenants', async ({ unlimitedAdminUser, loginAs, browser }) => {
    ...
    // In plan/page.tsx, it renders formatStorage(planData?.storage_limit_bytes)
    await expect(proPage.locator('span', { hasText: /50(\.0)? GB/ }).first()).toBeVisible();
  ...
  test('Cost Dashboard displays AI actions used this month correctly without limits', async ({ unlimitedAdminUser, loginAs, browser }) => {
    ...
    await expect(proPage.locator('span', { hasText: '/ Unlimited' }).first()).toBeVisible();
  ...
  test('Cost Dashboard renders the cost transparency section completely', async ({ page, adminUser, loginAs }) => {
    ...
    await expect(page.locator('span', { hasText: 'Total Costs' }).first()).toBeVisible();
