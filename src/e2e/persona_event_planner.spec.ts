...
test.describe('Business Setup Wizard - Persona: event_planner', () => {
  test('Complete flow for Party Pro', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'event_planner@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Party Pro');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder event_planner');
    await page.fill('input[placeholder="you@email.com"]', 'founder_event_planner@example.com');
