...
test.describe('Business Setup Wizard E2E - Plumber Persona', () => {
  test.use({ viewport: { width: 375, height: 667 } });

  test('Complete setup wizard flow for Plumber', async ({ page }) => {
    ...
    await page.fill('#biz-name', "FixIt Fast");
    ...
    await page.fill('input[placeholder="Your Full Name"]', 'Plumber User');
    await page.fill('input[placeholder="Email Address"]', 'Plumber@ohc.app');
    await page.fill('input[placeholder="Password"]', 'securepassword123');
    await page.click('button:has-text("Next")');

    await expect(page.locator('#step-7')).toBeVisible();
    await expect(page.locator('#review-name')).toHaveText("FixIt Fast");
