...
test.describe('Viral Testimonial Wall Generator Widget', () => {
  test('should load the widget, generate embed code, and copy to clipboard', async ({ page, context }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Viral Testimonial Wall Generator' })).toBeVisible();

    // Fill the generator details
    await page.locator('#wall-title').fill('Wall of Love');
    await page.locator('#wall-theme').selectOption('dark');
    ...
    await expect(embedCode).toContainText('Wall of Love');
    await expect(embedCode).toContainText('#0f172a'); // Dark theme background

    // Verify "Powered by OHC" branding is visible by default
    await expect(embedCode).toContainText('Powered by OHC');
    await expect(embedCode).toContainText('source=viral_testimonial_wall');
    ...
    try {
        ...
        expect(clipboardText).toContain('Wall of Love');
        expect(clipboardText).toContain('#0f172a');
