...
test('shared task list loads', async ({ page }) => {
  ...
  await expect(page.getByText('Inventory Reorder Strategy')).toBeVisible({ timeout: 30000 });
});
...
test('teammate mesh nodes are visible', async ({ page }) => {
  ...
  await expect(page.locator('#kairos-nerves').getByText('Brain', { exact: true })).toBeVisible({ timeout: 30000 });
...
test('autodream memory stats', async ({ page }) => {
  ...
  await expect(page.getByText('842.5 MB')).toBeVisible({ timeout: 30000 });
});
