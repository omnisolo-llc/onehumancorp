...
test.describe('Business Setup Wizard - Persona: cleaner', () => {
  test('Complete flow for Sparkle Clean', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'cleaner@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Sparkle Clean');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder cleaner');
    await page.fill('input[placeholder="you@email.com"]', 'founder_cleaner@example.com');
