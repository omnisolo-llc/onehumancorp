...
test.describe('Priya Onboarding Flow (Boutique)', () => {
  test('Completes the Idea to Live setup', async ({ page }) => {
    const id = `priya-onboarding-${Date.now()}`;
    ...
    await page.getByPlaceholder(/e.g. Maya's Custom Cakes/i).fill('Priya Boutique');
    await page.getByRole('button', { name: 'Next' }).click();

    await expect(page.getByRole('heading', { name: 'What do you sell?' })).toBeVisible();
    await page.getByPlaceholder(/e.g. I bake custom vegan cakes/i).fill('Clothing and accessories');
    await page.getByRole('button', { name: 'Next' }).click();

    await expect(page.getByRole('heading', { name: 'Where are you located?' })).toBeVisible();
    await page.getByPlaceholder(/e.g. Portland, OR/i).fill('New York, NY');
