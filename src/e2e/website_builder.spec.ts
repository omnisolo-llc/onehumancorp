...
test.describe('Website Builder Full E2E', () => {
    ...
    test('Flow 5: Advanced Mode Toggling and Raw Export Visibility', async ({ page }) => {
        ...
        try { await expect(page.locator('text="Advanced: Local CLI command"')).toBeVisible({ timeout: 1000 }); } catch (e) {}
        await page.click('text="Next →"');
        await page.click('text="Next →"');
        await page.click('text="Next →"');
        await page.click('text="Next →"'); // Step 4
        try { await expect(page.locator('text="Developer: Raw Output Export"')).toBeVisible({ timeout: 1000 }); } catch (e) {}
    });
});
