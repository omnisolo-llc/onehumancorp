...
test.describe("Website Builder Full E2E", () => {
  ...
  test("generates storefront via AI without mocks", async ({ page }) => {
    ...
    await expect(page.locator("#builder-preview-container")).toBeVisible({
      timeout: 20000,
    });
  });

  test("publishes storefront with generated payload without mocks", async ({
    page,
  }) => {
    await page.goto("/storefront-builder");
    ...
    const requestPromise = page.waitForRequest(
      (request) =>
        request.url().includes("/api/v1/builder/publish_draft") &&
        request.method() === "POST",
    );
  ...
  test("nextjs builder workflow handles empty inputs correctly", async ({
    page,
  }) => {
    ...
    // Check initial state
    await expect(
      page.getByRole("heading", { name: "Welcome to OHC Smart Builder" }),
    ).toBeVisible();

    const textarea = page.locator(
      'textarea[placeholder*="e.g. I run a mobile dog grooming service"]',
    );
    ...
    await textarea.press("Enter");
    await expect(
      page.getByText("Agents are building your store..."),
    ).toBeVisible({ timeout: 15000 });
  });

  test("nextjs builder workflow generates and publishes successfully to the real database", async ({
    page,
  }) => {
    await page.goto("/builder");
