...
test.describe('Cost Dashboard & Plan Limits UI', () => {
  test('should display the cost dashboard and check expected sections', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Cost Transparency' }).first()).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.locator('a.back-link', { hasText: '← Back to My Plan' })).toBeVisible();
  });
