...
test.describe('Business Setup Wizard E2E - Artist Persona', () => {
  test.use({ viewport: { width: 375, height: 667 } });

  test('Complete setup wizard flow for Artist', async ({ page }) => {
    ...
    await page.fill('#biz-name', "Digital Canvas");
    await page.click('button:has-text("Next")');

    await expect(page.locator('#step-4')).toBeVisible();
    await page.click('div.card-btn:has-text("Digital downloads")');
    await page.click('button:has-text("Next")');
    ...
    await page.fill('input[placeholder="Your Full Name"]', 'Artist User');
    await page.fill('input[placeholder="Email Address"]', 'Artist@ohc.app');
    await page.fill('input[placeholder="Password"]', 'securepassword123');
    await page.click('button:has-text("Next")');

    await expect(page.locator('#step-7')).toBeVisible();
    await expect(page.locator('#review-name')).toHaveText("Digital Canvas");
