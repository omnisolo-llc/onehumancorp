...
test.describe('Business Setup Wizard - Persona: coach', () => {
  test('Complete flow for Life Coach', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'coach@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Life Coach');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder coach');
    await page.fill('input[placeholder="you@email.com"]', 'founder_coach@example.com');
