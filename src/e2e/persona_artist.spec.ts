...
test.describe('Business Setup Wizard - Persona: artist', () => {
  test('Complete flow for Art World', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'artist@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Art World');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder artist');
    await page.fill('input[placeholder="you@email.com"]', 'founder_artist@example.com');
