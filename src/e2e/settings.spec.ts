...
test.describe('Settings Page', () => {
  ...
  test('should display expert mode hint when question mark is clicked', async ({ page }) => {
    await page.goto('/login');
    await page.fill('input[type="email"]', 'test@example.com');
    await page.fill('input[type="password"]', 'password123');
    await page.click('button:has-text("Sign In")');
    ...
    const expertModeLabel = page.locator('text=Expert Mode');
    if (await expertModeLabel.isVisible()) {
        const questionMarkBtn = expertModeLabel.locator('..').locator('button:has-text("?")').first();
        if (await questionMarkBtn.isVisible()) {
            await questionMarkBtn.click();
            await expect(page.locator('text=Expert mode turns on advanced features like direct API access and detailed network logs.')).toBeVisible();
        }
    }
  });
