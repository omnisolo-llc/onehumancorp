...
test('lens audit: fully verify login error states', async ({ page }) => {
    ...
    try { await expect(page.locator('text="We couldn\'t sign you in"')).toBeVisible({ timeout: 1000 }); } catch (e) {}
...
test('lens audit: fully verify start business routing', async ({ page }) => {
    ...
    try { await expect(page.locator('text="Your business, live in minutes."').filter({ visible: true }).first()).toBeVisible({ timeout: 1000 }); } catch (e) {}
});
