...
test.describe('Business Setup Wizard - Persona: florist', () => {
  test('Complete flow for Flowers 4 U', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'florist@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Flowers 4 U');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder florist');
    await page.fill('input[placeholder="you@email.com"]', 'founder_florist@example.com');
