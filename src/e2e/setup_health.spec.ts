...
test.describe('Hybrid Setup Health Check Endpoint', () => {
  test('returns ready for provisioned standalone mode', async ({ request }) => {
    ...
    const response = await request.get((process.env.BASE_URL || 'http://127.0.0.1:18789') + '/api/onboarding/setup-health?mode=standalone');
  ...
  test('returns error for unprovisioned cloud mode', async ({ request }) => {
    ...
    const response = await request.get((process.env.BASE_URL || 'http://127.0.0.1:18789') + '/api/onboarding/setup-health?mode=cloud');
    expect(response.status()).toBe(200);
    const body = await response.json();
