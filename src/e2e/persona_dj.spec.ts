...
test.describe('Business Setup Wizard - Persona: dj', () => {
  test('Complete flow for DJ Beats', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'dj@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'DJ Beats');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder dj');
    await page.fill('input[placeholder="you@email.com"]', 'founder_dj@example.com');
