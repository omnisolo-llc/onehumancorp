...
test.describe('Business Setup Wizard E2E - Mechanic Persona', () => {
  test.use({ viewport: { width: 375, height: 667 } });

  test('Complete setup wizard flow for Mechanic', async ({ page }) => {
    ...
    await page.fill('#biz-name', "Auto Repair");
    ...
    await page.fill('input[placeholder="Your Full Name"]', 'Mechanic User');
    await page.fill('input[placeholder="Email Address"]', 'Mechanic@ohc.app');
    await page.fill('input[placeholder="Password"]', 'securepassword123');
    await page.click('button:has-text("Next")');

    await expect(page.locator('#step-7')).toBeVisible();
    await expect(page.locator('#review-name')).toHaveText("Auto Repair");
