...
test.describe('Business Setup Wizard - Persona: photographer', () => {
  test('Complete flow for Photo Pro', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'photographer@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Photo Pro');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder photographer');
    await page.fill('input[placeholder="you@email.com"]', 'founder_photographer@example.com');
