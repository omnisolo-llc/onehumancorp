...
test('flash-sale-generator', async ({ page, request, loginAs, adminUser }) => {
  await loginAs(page, adminUser);
  await currentAppSmoke(page, request, 'flash-sale-generator');
});

test.describe('Growth & Virality: Flash Sale Generator', () => {
  test('User can create and preview a flash sale widget', async ({ page, loginAs, adminUser }) => {
    // Navigate to dashboard first
    await loginAs(page, adminUser);
    await page.goto('/dashboard');

    // Wait for the Dashboard to load fully
    await expect(page.locator('h1:has-text("Welcome")')).toBeVisible({ timeout: 15000 }).catch(() => {});
    ...
    await expect(page.locator('span:has-text("FLASH20")')).toBeVisible();
    ...
    await page.fill('input[placeholder="e.g. 20"]', '50');
