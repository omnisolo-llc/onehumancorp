...
test.describe('openrouter hire-agent flow', () => {
  test('hiring an agent starts a real business swarm with specialist agents', async ({ request }) => {
    test.setTimeout(360_000);

    test.skip(!process.env.OPENROUTER_API_KEY, 'requires a real OPENROUTER_API_KEY in the environment or .env');
    expect(process.env.OPENROUTER_API_KEY, 'requires a real OPENROUTER_API_KEY in the environment or .env').toBeTruthy();
    expect(process.env.OHC_LLM_PROVIDER || 'openrouter').toBe('openrouter');
    expect(process.env.OHC_LLM_MODEL || process.env.OPENROUTER_MODEL || 'openai/gpt-4o-mini').toBe('openai/gpt-4o-mini');
    ...
    expect(workflow!.task).toContain('openai/gpt-4o-mini');
    expect(workflow!.command).toContain('ohc_business_swarm');
    expect(workflow!.command).toContain('openai/gpt-4o-mini');
    ...
    expect(commands).toContain('openai/gpt-4o-mini');
  });
});
