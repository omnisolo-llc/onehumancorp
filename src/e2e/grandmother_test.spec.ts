...
test.describe('Grandmother Test - Plain Language Check', () => {
  test('should display login page with form', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Email address').filter({ visible: true }).first()).toBeVisible();
  ...
  test('should display agents page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'My AI Assistants' })).toBeVisible();
  });

  test('should display business setup', async ({ page }) => {
    await page.goto('/business-setup');
    await expect(page.locator('text=Store Designer')).toBeVisible();
  });
});