...
test.describe('Business Setup Wizard E2E - Designer Persona', () => {
  test.use({ viewport: { width: 375, height: 667 } });

  test('Complete setup wizard flow for Designer', async ({ page }) => {
    ...
    await page.fill('#biz-name', "Web Design Pro");
    ...
    await page.fill('input[placeholder="Your Full Name"]', 'Designer User');
    await page.fill('input[placeholder="Email Address"]', 'Designer@ohc.app');
    await page.fill('input[placeholder="Password"]', 'securepassword123');
    await page.click('button:has-text("Next")');

    await expect(page.locator('#step-7')).toBeVisible();
    await expect(page.locator('#review-name')).toHaveText("Web Design Pro");
