    ...
    await request.post("/api/e2e/setup", {
      data: {
        query: `
          ...
          VALUES
            ('e2e-feed-test-1', 'e2e-tenant', 'operations', '{"description": "3 new orders to fulfill"}'::jsonb, '{"feature_type": "fulfillment_batch", "message": "Batch process 3 orders?"}'::jsonb, 'PENDING_APPROVAL', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('e2e-feed-test-2', 'e2e-tenant', 'marketing', '{"description": "Draft promo email?"}'::jsonb, '{"context": {"weekly_health_report": true}, "message": "Send promo?"}'::jsonb, 'PENDING_APPROVAL', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
