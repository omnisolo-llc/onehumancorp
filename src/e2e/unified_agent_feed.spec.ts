...
test.describe("Unified Agent Feed Mobile UX", () => {
  ...
  test("Renders and actions can be tapped on 375px mobile screen", async ({
    ...
    // 1. Seed some distinct approvals representing different departments using the NEW schema
    await request.post("/api/e2e/setup", {
      data: {
        query: `
          INSERT INTO agent_feed_items (id, tenant_id, event_source, context_payload, proposed_action, lifecycle_state, created_at, updated_at)
          VALUES
            ('e2e-feed-test-1', 'e2e-tenant', 'operations', null, '{"message": "3 new orders to fulfill", "action_type": "fulfillment_batch"}'::jsonb, 'PENDING_APPROVAL', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('e2e-feed-test-2', 'e2e-tenant', 'marketing', null, '{"message": "Draft promo email?", "context": {"weekly_health_report": true}}'::jsonb, 'PENDING_APPROVAL', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
