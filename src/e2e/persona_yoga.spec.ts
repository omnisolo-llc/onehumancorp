...
test.describe('Business Setup Wizard - Persona: yoga', () => {
  test('Complete flow for Yoga Zen', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'yoga@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Yoga Zen');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder yoga');
    await page.fill('input[placeholder="you@email.com"]', 'founder_yoga@example.com');
