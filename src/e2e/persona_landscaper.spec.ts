...
test.describe('Business Setup Wizard - Persona: landscaper', () => {
  test('Complete flow for Green Grass', async ({ page }) => {
    await page.goto('/login');
    await page.click('button:has-text("Don\'t have an account? Sign Up")');
    await page.fill('input[placeholder="Email or Username"]', 'landscaper@example.com');
    ...
    await page.fill('input[placeholder="e.g. Maya\'s Cakes"]', 'Green Grass');
    await page.click('text=Next →');
    ...
    await page.fill('input[placeholder="e.g. Maya Smith"]', 'Founder landscaper');
    await page.fill('input[placeholder="you@email.com"]', 'founder_landscaper@example.com');
