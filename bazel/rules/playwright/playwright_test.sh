...
for i in {1..3}; do
  if docker pull pgvector/pgvector:pg16-bookworm >/dev/null 2>&1; then
...
if [ "$PULL_PG_SUCCESS" = true ]; then
  ...
  if [ "$PULL_VK_SUCCESS" = true ]; then
    if docker rm -f "$POSTGRES_NAME" >/dev/null 2>&1 || true; docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1::5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc pgvector/pgvector:pg16-bookworm; then
