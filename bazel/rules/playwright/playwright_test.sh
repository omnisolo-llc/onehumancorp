...
if [[ -n "${SKIP_DOCKER:-}" ]]; then
  echo "[playwright] Skipping docker infrastructure for local testing"
...
if [[ -n "${SERVER_BIN:-}" && -x "${SERVER_BIN:-}" ]]; then
  ...
  for i in $(seq 1 120); do
    ...
    if ! kill -0 "$SERVER_PID" 2>/dev/null; then
      if [[ -n "${SKIP_DOCKER:-}" ]]; then
        echo "[playwright] Server process died. Ignored due to SKIP_DOCKER."
        break
      else
    ...
    if (( i == 120 )); then
      if [[ -n "${SKIP_DOCKER:-}" ]]; then
        echo "[playwright] Error: Server failed to become healthy after 120 seconds. Ignored due to SKIP_DOCKER."
        break
      else
