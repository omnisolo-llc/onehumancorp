...
trap cleanup EXIT
    bash bazel/rules/playwright/scripts/setup_local_pgvector.sh || true

echo "[playwright] Starting E2E infrastructure..."
docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1::5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc local-pgvector:16-alpine
