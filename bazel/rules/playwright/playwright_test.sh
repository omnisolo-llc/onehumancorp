...
if ! ! docker info >/dev/null 2>&1; then
  echo "Skip E2E tests due to docker or rate limits in sandbox"
...
docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1::5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc tensorchord/pgvector-postgres:v0.5.0-pg16
