...
docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1::5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc mirror.gcr.io/pgvector/pgvector:pg16
docker run -d --name "$VALKEY_NAME" -p 127.0.0.1::6379 mirror.gcr.io/valkey/valkey:8-alpine
