...
for i in {1..3}; do docker pull timescale/timescaledb:latest-pg16 >/dev/null 2>&1 && break || sleep 2; done
for i in {1..3}; do docker pull valkey/valkey:8-alpine >/dev/null 2>&1 && break || sleep 2; done

docker rm -f "$POSTGRES_NAME" >/dev/null 2>&1 || true; docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1::5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc timescale/timescaledb:latest-pg16
...
postgres_exec "CREATE EXTENSION IF NOT EXISTS vector;" "create vector extension"
