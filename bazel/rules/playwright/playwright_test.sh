...
for i in {1..3}; do
  if docker pull mirror.gcr.io/ankane/pgvector:v0.5.1 >/dev/null 2>&1; then
...
if [ "$PULL_PG_SUCCESS" = true ]; then
  ...
  for i in {1..3}; do
  if docker pull mirror.gcr.io/library/redis:7 >/dev/null 2>&1; then
  ...
  if [ "$PULL_VK_SUCCESS" = true ]; then
    if docker rm -f "$POSTGRES_NAME" >/dev/null 2>&1 || true; docker run -d --name "$POSTGRES_NAME" -p 127.0.0.1:0:5432 -e POSTGRES_USER=ohc -e POSTGRES_PASSWORD=ohc -e POSTGRES_DB=ohc mirror.gcr.io/ankane/pgvector:v0.5.1; then
      docker run -d --name "$VALKEY_NAME" -p 127.0.0.1:0:6379 mirror.gcr.io/library/redis:7
