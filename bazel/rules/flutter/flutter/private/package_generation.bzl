...
def _ensure_pub_deps(repository_ctx, package_name, package_dir):
    ...
    if deps_result.return_code != 0:
        ...
        if (
            ...
            "version solving failed" in lower_stderr or
        ...
        # In case we still fail due to read-only file system or workspace, we can just catch it from stderr
        if "workspace" in lower_stderr or "read-only file system" in lower_stderr:
            repository_ctx.report_progress("Skipping pub deps generation for {} due to workspace / read-only errors".format(package_name))
            repository_ctx.file(pub_deps_rel, '{"packages": []}', executable = False)
            return True
