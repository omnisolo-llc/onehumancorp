...
def _ensure_pub_deps(repository_ctx, package_name, package_dir):
    ...
    if deps_result.return_code != 0:
        ...
        if (
            ...
            return False
        repository_ctx.report_progress("Failed to run `{tool} pub deps --json` for package '{pkg}' (dir: {dir}).\nstdout: {stdout}\nstderr: {stderr}".format(
            tool = tool,
            pkg = package_name,
            dir = package_dir,
    ...
    for idx in range(len(output)):
        ...
        if ch == "{" or ch == "[":
            ...
            break
    if json_start == -1:
        repository_ctx.report_progress("`{tool} pub deps --json` for package '{pkg}' did not produce JSON output.\nstdout: {stdout}\nstderr: {stderr}".format(
            tool = tool,
            pkg = package_name,
            stdout = deps_result.stdout,
