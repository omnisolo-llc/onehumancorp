...
def _ensure_pub_deps(repository_ctx, package_name, package_dir):
    ...
    if deps_result.return_code != 0:
        ...
        repository_ctx.report_progress("pub deps failed for {}. Outputting dummy pub_deps.json".format(package_name))
        repository_ctx.file(pub_deps_rel, '{"packages": []}\n')
        return False
    ...
    for idx in range(len(output)):
        ...
        if ch == "{" or ch == "[":
            ...
            break
    if json_start == -1:
        repository_ctx.report_progress("pub deps failed to produce json for {}. Outputting dummy pub_deps.json".format(package_name))
        repository_ctx.file(pub_deps_rel, '{"packages": []}\n')
        return False
