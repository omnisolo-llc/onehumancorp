...
func main() {
	...
	if !strings.Contains(newContent, `"mesh.go",`) {
		newContent = strings.Replace(newContent, `"tasks.go",`, "\"tasks.go\",\n        \"mesh.go\",\n        \"queue.go\",", 1)
	}

	if !strings.Contains(newContent, `"mesh_v2_test.go",`) {
		newContent = strings.Replace(newContent, `"mesh_test.go",`, "\"mesh_test.go\",\n        \"mesh_v2_test.go\",\n        \"queue_test.go\",\n        \"queue_v2_test.go\",", 1)
	}
