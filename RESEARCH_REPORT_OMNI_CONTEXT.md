...
Technically feasible by extending the current `agent_missions` table injection logic (in `srcs/server/orchestration/sip.go` or `tasks.go`). The orchestrator simply scans the root for `AGENTS.md` and appends its contents directly to the `payload` TEXT blob under a `[SYSTEM GROUNDING]` namespace.
