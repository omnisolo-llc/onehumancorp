...
test.describe('Business Setup Wizard', () => {
  ...
  test('should handle instant build flow', async ({ page }) => {
    // Start Guided Setup to enter the wizard
    await page.click('text=Guided Setup →');

    // There should be a "Skip to Instant Build" or "Tell us about your business" option/step
    // Based on the code, step 11 is the Instant Build step. We need to trigger it.
    // The UI should have a button or link to enter instant build from step 0.
    // Looking at setup_wizard.slint, on step 0:
    // `TouchArea { clicked => { is_instant_build = true; step = 11; save_state(); } }`
    // The text on that card is typically "Instant Build" or "Build it for me"
    const instantBuildCard = page.locator('text=/Instant Build|Build it for me|30 seconds/i').first();
    await expect(instantBuildCard).toBeVisible();
    await instantBuildCard.click();

    // Now on step 11, we should see "Tell us about your business"
    await expect(page.locator('text="Tell us about your business"')).toBeVisible();

    // Fill in the bio
    await page.fill('input[placeholder*="e.g. I run a local bakery"]', 'I run an AI product shop.');

    // Click Generate Storefront
    await page.click('text="Generate Storefront →"');

    // It should mock the API call and skip to step 9 (Review & Launch)
    await expect(page.locator('text=/Ready to launch!|Launch My Business/i')).toBeVisible({ timeout: 10000 });

    // Assert the properties were set correctly on the review screen
    await expect(page.locator('text="Business: AI Store"')).toBeVisible();
    await expect(page.locator('text="Type: Online Store"')).toBeVisible();
    await expect(page.locator('text="Template: Classic"')).toBeVisible();
    await expect(page.locator('text="First Product: AI Product"')).toBeVisible();
