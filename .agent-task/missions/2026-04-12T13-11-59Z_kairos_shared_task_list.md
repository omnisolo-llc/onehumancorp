...
# Title: OHC Hybrid OS - Shared Task List Backend (Orchestration Phase 1)
...
1. **Shared Task List Schema (`srcs/server/db/migrations/036_kairos_shared_tasks.sql`)**:
   ```sql
...
Hello Implementer! Please create the SQL migration `srcs/server/db/migrations/036_kairos_shared_tasks.sql` as defined in the Design Doc. Remember to add the file path to the `embedsrcs` list in `srcs/server/db/BUILD.bazel`. Also create `srcs/server/orchestration/tasks.go` with the appropriate struct mirroring the schema. Don't forget tests.
