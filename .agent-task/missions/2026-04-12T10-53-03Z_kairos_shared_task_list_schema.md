...
2. **Sub-Agent Orchestration Queue (`srcs/server/orchestration/tasks_db.go`)**:
   ...
   - **Cloud Mode (`!to.dbProvider.IsSQLite()`)**: Use `SELECT * FROM shared_tasks_v2 WHERE status = 'PENDING' FOR UPDATE SKIP LOCKED`.
...
5. Create comprehensive test coverage in `tasks_db_test.go`, maintaining >90% coverage.
