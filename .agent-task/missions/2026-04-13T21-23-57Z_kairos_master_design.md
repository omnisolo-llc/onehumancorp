...
Please author the comprehensive master design document by following these specific instructions:

1.  **Create/Overwrite `docs/KAIROS_AI_OS_IMPLEMENTATION_MASTER.md`**:
    -   Consolidate the architecture for the Shared Task List, Teammate Mesh, and autoDream pipeline into a single, cohesive narrative. Include the database schemas, API contracts, and the Mermaid.js sequence diagram provided above.
    -   Enclose the entire document's contents within the following HTML wrapper to ensure aesthetic compliance:
        `<div markdown="1" style="backdrop-filter: blur(20px) saturate(200%); font-family: 'Outfit', 'Inter', sans-serif; background: rgba(255, 255, 255, 0.03); color: #fff; padding: 20px; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.1);">`
        `<!-- Content Here -->`
        `</div>`
...
2.  **Verify**:
    ...
    -   Validate the HTML encapsulation using `grep`.
    -   Run `./check_links.sh` to ensure documentation integrity.
