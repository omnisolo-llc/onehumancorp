...
3. **Metric Functions**: Implement RecordPostgresLockContention(ctx context.Context, operation string) and RecordLLMNetworkLatency(ctx context.Context, model string, latency float64). Ensure graceful error handling instead of panics during initialization. Also ensure they check for BufferMetricFunc != nil.
