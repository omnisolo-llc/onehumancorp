...
- **Transport Layer (`srcs/server/orchestration/hub.go` or similar):**
...
3. In `srcs/server/orchestration/`, integrate `CentrifugeNode` where appropriate. Define the `MeshTransport` interface.
4. Implement `RedisMeshTransport` (using `github.com/redis/rueidis` for Redis Pub/Sub) and `MemoryMeshTransport`.
5. Update task management logic to utilize the new `MeshTransport` for broadcasting events via the `CentrifugeNode` hub.
