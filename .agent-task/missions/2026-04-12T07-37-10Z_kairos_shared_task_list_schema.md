...
1. Shared Task List Schema (`srcs/server/db/migrations/20260410_kairos_tasks.sql`):
   ```sql
...
Hello Implementer! Please update the SQL migration `srcs/server/db/migrations/20260410_kairos_tasks.sql` as defined in the Design Doc. Remember to add the file path to the `embedsrcs` list in `srcs/server/db/BUILD.bazel`. Also ensure `srcs/server/orchestration/tasks.go` has the appropriate struct mirroring the schema. Don't forget tests.
