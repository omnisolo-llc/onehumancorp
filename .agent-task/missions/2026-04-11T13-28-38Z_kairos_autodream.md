...
1. Create the SQL migration for the `autodream_memories` table in `srcs/server/db/migrations/` (e.g., `033_autodream_memories_refactor.sql`). Include `CREATE EXTENSION IF NOT EXISTS vector;` for PostgreSQL. Provide a SQLite equivalent (e.g., storing embedding as a JSON text blob) for Standalone degradation. Add to `embedsrcs` in `srcs/server/db/BUILD.bazel`.
