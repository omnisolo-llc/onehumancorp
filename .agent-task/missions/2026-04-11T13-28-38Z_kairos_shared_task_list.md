...
- `shared_tasks` must have fields like `dependencies` (JSONB) to compress dependent task IDs.
...
1. Create the SQL migration file for `shared_tasks` in `srcs/server/db/migrations/`. Name it appropriately (e.g., `032_shared_tasks_refactor.sql` or similar, ensuring there are no conflicts).
2. Add the migration to `embedsrcs` in `srcs/server/db/BUILD.bazel`.
3. Verify the data access layer in `srcs/server/orchestration/tasks_db.go`.
