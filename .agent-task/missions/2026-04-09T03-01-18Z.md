...
4. **Testing**: Write unit tests in `srcs/server/orchestration/sipdb_test.go`. Mock the authenticated context using `context.WithValue(ctx, auth.ClaimsContextKeyForTest, claims)`. Extract tenant claims properly, verifying `claims != nil` and `claims.OrganizationID != ""`.
5. **Pre-Commit Verifications**: Run tests locally with `bazelisk test //srcs/server/... --test_output=errors`.
