...
2. Check `srcs/server/orchestration/` and `srcs/server/dashboard/server.go` for Teammate Mesh endpoints.
3. Ensure endpoints `/api/mesh/broadcast`, `/api/mesh/direct`, and `/api/mesh/mailbox` use robust implementations based on the mode.
4. If Redis is configured, subscribe to channels and broadcast messages across cluster nodes. If not, use an internal Go channel dispatcher.
5. Integrate with `CentrifugeNode` for downstream WebSocket connections.
6. Apply SPIFFE/SPIRE authentication logic for agent-to-agent and backend-to-backend communication.
7. Write rigorous tests (>95% coverage) to verify both Redis and local-fallback behaviors.
8. Verify all changes using Bazel.
