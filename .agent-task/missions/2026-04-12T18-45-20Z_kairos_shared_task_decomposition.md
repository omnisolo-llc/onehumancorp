...
- Save resulting subtasks into `sub_agent_jobs`.
- Maintain State Machine dependency DAG in `state_machine_transitions`.

### 2. Teammate Mesh Coordination
- Broadcast `TaskDecomposed` event to the `Teammate Mesh`.
...
3. Hook into Teammate Mesh for broadcasting.
