...
status: BLOCKED
blockers:
  - The requested paths (srcs/server/orchestration, etc.) are outside the Principal Interoperability Engineer & Link domain bounds (srcs/server/interop/, srcs/server/lib/integrations/).
---
# Title: KAIROS Master Orchestration Interfaces
...
3. **AutoDream Pipeline**: Build the background worker in `srcs/server/orchestration/autodream.go`. It should scan `.agent-task/memory/*.yml`, embed using a suitable LLM client, and insert into `autodream_memories`. Verify using `db.NewTestProvider(t)` mock DB.
