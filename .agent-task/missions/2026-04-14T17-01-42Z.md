# Title: Implement KAIROS Orchestration Phases 1-3
...
Implement a `MeshTransport` interface in `srcs/server/orchestration/mesh/mesh.go`:
```go
...
Implement `AutoDreamWorker` in `srcs/server/orchestration/autodream_worker.go` to pull from `.agent-task/memory/` and embed using Minimax/OpenAI, storing into `autodream_memories`.
...
2. In `srcs/server/orchestration/mesh/mesh.go`, define the `MeshTransport` interface with `Publish` and `Subscribe`. Implement `MemoryMeshTransport` and `RedisMeshTransport`.
3. In `srcs/server/orchestration/autodream_worker.go`, scaffold the `AutoDreamWorker` struct with a `ConsolidateMemories()` method.
4. Update `srcs/server/db/provider.go` to add a `GetSharedTask()` method to the `Provider` interface.
5. Provide tests in `srcs/server/orchestration/mesh/mesh_test.go` and `srcs/server/orchestration/autodream_worker_test.go`. Ensure >90% test coverage.
