...
*Note:* `dependencies` uses a JSONB array, avoiding relational `task_dependencies` tables to optimize storage costs as per memory guidelines. `TIMESTAMP WITH TIME ZONE` is used for Postgres, while SQLite will handle it appropriately if we translate it in migrations.
...
sequenceDiagram
    ...
    alt Task Found
        ...
        Agent->>DB: UPDATE shared_tasks SET status = 'ASSIGNED', agent_id = 'worker-1' WHERE id = 123
        Agent->>DB: COMMIT
