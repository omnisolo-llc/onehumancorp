...
Realtime communication layer for agent coordination using `mesh:tasks`, `mesh:coordination`, and `mesh:ultraplan`.

### 3. AutoDream Data Pipelines (Memory Consolidation)
Asynchronous background pipeline to summarize task outcomes and store in `autodream_memories`.
