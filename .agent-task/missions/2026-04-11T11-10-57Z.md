...
- The `shared_tasks` table is critical for representing a global queue of decomposed features. Wait, `shared_tasks` already exists and was created in `013_shared_tasks.sql` and `021_kairos_orchestration.sql`, but the implementer must create the data access layer `tasks_db.go` which actually claims tasks safely. Wait, `tasks.go` and `task_orchestrator.go` also exist. The exact objective of the prompt was to create `srcs/server/orchestration/tasks_db.go`. However, since `TaskOrchestrator` is already used in `task_orchestrator.go`, the `tasks_db.go` will use `TasksDB` struct instead to avoid conflicts.
...
3. Create the data access layer in `srcs/server/orchestration/tasks_db.go`. Use the `TasksDB` struct instead of `TaskOrchestrator` to avoid redeclaration conflicts with `task_orchestrator.go`.
4. Implement a `ClaimTask` method inside `TasksDB`.
...
5. Create unit tests in `srcs/server/orchestration/tasks_db_test.go`. If simulating authentication claims in tests, use `context.WithValue(ctx, auth.ClaimsContextKeyForTest, claims)`.
