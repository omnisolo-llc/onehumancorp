...
**KAIROS AI OS Core Implementation**
...
1. Add a new migration to `srcs/server/db/migrations/20260415120000_shared_tasks_decomposition_schema.sql` that defines the `shared_tasks_decomposition` table.
2. Update `srcs/server/db/BUILD.bazel` to include the new migration file in the `embedsrcs` list.
3. Add the struct representing the `shared_tasks_decomposition` table to `srcs/server/orchestration/task_orchestrator.go` (type SharedTaskDecomposition struct).
